module RomanNumerals
  ...
  # Public: Instance method converts a decimal to Roman Numerals
  #
  # Returns a String representing Roman Numerals
  def to_roman
    # take care of the simple numbers
    return ROMAN[self] if ROMAN[self]
    # go get the roman numerals
    raise ArgumentError, "Argument #{self} is too large for RomanNumerals" if self > 3000 
    get_roman(self)
  end
  
  private
    # Internal: Recurse over the Integer, divide by known values which match
    #           Roman Numerals, continue with any remainder
    #
    # num: Integer
    #
