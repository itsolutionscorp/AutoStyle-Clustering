class ::Integer
  def to_roman
    param_hash = { num:self, string: "", patterns:{ 1=>"I", 5=>"V", 10=>"X", 50=>"L", 100=>"C", 500=>"D", 1000=>"M" }, seq:[1000, 500, 100, 50, 10, 5, 1] }

    is_rank_jump = lambda { |string, patterns, seq, index|
    string.chars.pop == patterns[seq[index - 1]] }

    substitute_4_sybmols = lambda { 
        |num, seq, patterns, index, string|
    ...
    if ((acc[:num] / acc[:seq][index]) > 0)
      then
      acc[:string] = substitute_4_sybmols.call(acc[:num], acc[:seq], acc[:patterns], index, acc[:string])
