...
class Verse
  ...
  def initialize(n)
    ...
    @animal = @animals.last or fail ArgumentError, 'verses start at 1'
  end
  ...
  def chaser_line(chaser, chased)
    # Handle the line where the elaboration does not mention the subject.
    unless elaboration(chased)[chased.to_s]
      chased = elaboration(chased).sub(/\w+/, "#{chased} that").chop
    end
