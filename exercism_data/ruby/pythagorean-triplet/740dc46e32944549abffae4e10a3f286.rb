class Triplet
  ...
  def self.where(min_factor: 1, max_factor: 100, sum: nil)
    test_triplets(factors(min_factor,max_factor)) do |factors,triplets|
  ...
  def self.test_triplets(factors,&block)
    factors.product(factors, factors).map(&:sort).uniq.each_with_object do |factors, triplets|
      yield(factors,triplets)
    end
  end
