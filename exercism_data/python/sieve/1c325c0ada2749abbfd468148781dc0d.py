...
def prime_numbers(upto):
    ...
    if 5 <= upto:
        yield 5
        # See the comment below for the start (5 ** 2) and step (5 * 2)
        # arguments.
    ...
    for number in xrange(6, multiple_of_6_lte(isqrt(upto)) + 6, 6):
        ...
        # because number is a multiple of 6.  number + 1, number + 5 are
        # neither multiples of 2 nor multiples of 3.
        num = number + 1
        if is_prime[num]:
            yield num
            # 2 * num, 3 * num, ..., (num - 1) * num have already been marked
            # as composite numbers because they are multiples of 2, 3, ...,
            # num - 1.
            #
            # num ** 2 + num, num ** 2 + num * 3, ... are even (i.e. multiples
            # of 2) because num is odd.
