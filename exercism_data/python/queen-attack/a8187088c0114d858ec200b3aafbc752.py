...
class NotOnBoardError(ValueError): pass
class AlreadyOnBoardError(ValueError): pass
class PositionTakenError(ValueError): pass
class InvalidPositionError(ValueError): pass
...
class Board(object):
    ...
    def validate_position(self, position):
        if not Board.is_valid_position(position):
            raise InvalidPositionError("(%d, %d): invalid board position" % position)
    ...
    def place(self, piece, position):
        ...
        piece.assert_not_on_board()
        if self.get_piece_at(position):
            raise PositionTakenError(
                "(%d, %d): field is already taken by another piece" % position)
    ...
    def get_position_for(self, piece):
        try:
            ...
            raise NotOnBoardError("The piece is not on the board")
...
class Piece(object):
    ...
    def assert_on_board(self):
        if not self.board:
            raise NotOnBoardError("The piece is not on a board")

    def assert_not_on_board(self):
        if self.board:
            raise AlreadyOnBoardError("The piece is already on a board")
        ...
        for direction in self.directions:
            ...
            for _ in range(self.max_steps):
                if not self.nav.go(1, direction):
                   break
                target_piece = self.nav.piece_at_position
                if target_piece:
...
class BoardNavigator(object):
    ...
    def is_at_empty_field(self):
        return not self.piece_at_position

    @property
    def piece_at_position(self):
        return self.piece.board.get_piece_at(self.position) 
