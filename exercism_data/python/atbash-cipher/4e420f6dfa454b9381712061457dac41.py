...
_encode_map = maketrans(_plain_alphabet, _cipher_alphabet)
_decode_map = maketrans(_cipher_alphabet, _plain_alphabet)
_lower_map = maketrans(uppercase, lowercase)

def encode(message):
    normalized = _normalize(message)
    ciphertext = normalized.translate(_encode_map)
    return _separate_words(ciphertext, 5)

def decode(message):
    normalized = _normalize(message)
    return normalized.translate(_decode_map)
