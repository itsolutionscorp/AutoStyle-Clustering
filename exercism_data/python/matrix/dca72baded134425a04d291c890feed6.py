# Dirk Herrmann's solution version 3 for exercism exercise "Matrix"
...
def parseMatrixRow(line):
   tokens = line.split()
   return [parseMatrixElement(token) for token in tokens]

def parseMatrix(string):
   lines = string.splitlines()
   return [parseMatrixRow(line) for line in lines]
