class Mines(object):
    ...
    def check_valid_board(self,input_board):
        ...
        if not all((lambda line: len(line) == len(input_board[0]))(line) for line in input_board):
        ...
        if not all((lambda side: side[0] == '|' and side[-1] == '|')(line) for line in input_board[1:-1]):
            raise ValueError('Must use valid border {0}.'.format(sides))
        if not all((lambda char: char in ' *')(char) for line in input_board[1:-1] for char in line[1:-1]):
