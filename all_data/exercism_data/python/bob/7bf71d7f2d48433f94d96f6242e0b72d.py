...
def hey(what):
    ...
    beats default."""
    if not what.strip():  # If what is just empty or whitespace
    ...
    if what[-1] == '?':
        ...
        state = STRINGS['question']
    else:
        state = STRINGS['default']

    is_allcaps = True  # iff there are no lowercase characters
    has_alpha = False  # iff there are no alpha characters
    for char in what:
        if char.isalpha():
            has_alpha = True
            if not char.isupper():
