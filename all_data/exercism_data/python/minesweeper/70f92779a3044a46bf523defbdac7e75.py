...
def test_board(game):
    ...
    for x in range(len(game)):
        for y in range(len(game[x])):
            if (x == 0 or x == len(game)) and (game[x][y] not in '+-'):
                raise ValueError("Bad Board H.border")
            if (y == 0 or y == len(game[x])) and (game[x][y] not in '+|'):
                raise ValueError("Bad Board V.border")
            if game[x][y] not in '+-|* ':
