...
def decode(message):
    if not isinstance(message,basestring):
        return None
    return message.lower().translate(DECODING_TABLE,string.punctuation+string.whitespace)

def encode(message):
    if not isinstance(message,basestring):
        return None
    message = message.lower().translate(ENCODING_TABLE,string.punctuation+string.whitespace)
