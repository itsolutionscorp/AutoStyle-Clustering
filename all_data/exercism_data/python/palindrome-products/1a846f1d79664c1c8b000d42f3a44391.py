def smallest_palindrome(max_factor, min_factor = 1):
    def generate_x(): return xrange(min_factor, max_factor+1)
    def generate_y(x): return xrange(x, max_factor + 1)
    return _find_palindrome(generate_x, generate_y, min)

def largest_palindrome(max_factor, min_factor = 1):
    def generate_x(): return xrange(max_factor, min_factor-1, -1)
    def generate_y(x): return xrange(max_factor, x-1, -1)
    return _find_palindrome(generate_x, generate_y, max)

def _find_palindrome(generate_x, generate_y, pick):
    palindrome = None
    for x in generate_x():
        for y in generate_y(x):
            product = x * y
            if palindrome and pick(palindrome[0], product) != product:
                break
