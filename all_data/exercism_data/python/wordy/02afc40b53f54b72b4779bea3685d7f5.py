...
def calculate(mathq):
    ...
    if 'raised to the' in mathq:
        for postfix in ['st', 'nd', 'rd', 'th']:
            mathq = mathq.replace('%s power' % postfix, '')
...
def main():
    ...
    matq = "What is 5 raised to the 1st power?"
    assert calculate(matq) == 5
    matq = "What is 5 raised to the 2nd power?"
    assert calculate(matq) == 25
    matq = "What is 5 raised to the 3rd power?"
    assert calculate(matq) == 125
    matq = "What is 5 raised to the 0th power?"
    assert calculate(matq) == 1
