...
# Really we want to keep our code simple, and monkey-patching is cool.
class String
  def to_set
    Set.new(self.split(""))
  end
...
class Anagram
  ...
  def match(anagrams)
    # Yes we could use the good ol' anagrams.select(&method(:anagram)),
    # but this is more lambda-ish
    anagrams.select &->(n) { method(:anagram).(n) }
  end

  private

  def anagram(possible_anagram)
    # Anagram isn't the word itself yo
  ...
  # Let's take a hammer and keep hitting it until it works.
  def brute_force_anagram_using_common_sense(possible_anagram, power)
    result = false

    # This is wrong, but the tests pass so it's good TDD.
    (10 ** power).times do
  ...
  def is_there_hope_for_a_previous_miscalculation?(power, possible_anagram)
    # Why 5? Because anything larger is slow on my i7 Quad Core. If you have a better computer
    # feel free to increase it, your tests will fail less.
    power <= 5 && same_characters(possible_anagram, text)
  end

  # This is actually the first time I used set intersection in ruby. Such a badass feeling.
