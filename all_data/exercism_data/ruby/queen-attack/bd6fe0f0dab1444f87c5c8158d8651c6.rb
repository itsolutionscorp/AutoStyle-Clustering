require 'matrix'

class Queens
  attr_accessor :white, :black

  def initialize positions = nil
    @white = positions[:white] unless positions.nil?
    @white ||= [0, 3]
    @black = positions[:black] unless positions.nil?
    @black ||= [7, 3]
    permited_positions?
  end

  def to_s
    Board.new.set_queens_positions(white, black).to_s
  end

  def attack?
    same_row? || same_column? || diagonal?
  end

  private

  def permited_positions?
    raise ArgumentError if white == black
  end

  def same_row?
    white.first == black.first
  end

  def same_column?
    white.last == black.last
  end

  def diagonal?
    (white.first - black.first).abs ==
      (black.last - white.last).abs
  end
end

class Board
  def initialize
    @matrix = Matrix.build(8, 8) {"_ "}.to_a
  end

  def set_queens_positions white, black
    @matrix[white.first][white.last] = "W "
    @matrix[black.first][black.last] = "B "
    self
  end

  def to_s
    @matrix.map { |row| row.join.chop + "\n" }.join.chomp
  end
end
