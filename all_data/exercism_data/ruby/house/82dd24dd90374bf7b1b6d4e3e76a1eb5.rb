class House
  ...
  def verse(count)
    STARTER + prepare_events(count).map.with_index do |event, idx|
      separator = last_event?(idx, count) ? "that" : "the"
      " #{separator} #{event}"
    end.join + ENDING
  end
  ...
  private
    ...
    def prepare_events(count)
      EVENTS.take(count).reverse
    end

    def last_event?(current_event, events_count)
      (current_event - 1) == events_count
    end
