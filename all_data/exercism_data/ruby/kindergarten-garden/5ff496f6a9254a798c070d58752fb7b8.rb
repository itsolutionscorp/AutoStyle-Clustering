class Garden
  ...
  def initialize(plant_string, diagram=0)
  ...
  def patricia
    @diagram = [:violets, :clover, :radishes, :violets]
  end

  def roger
    @diagram = [:radishes, :radishes, :grass, :clover]
  end

  def samantha
    @diagram = [:grass, :violets, :clover, :grass]
  end

  def xander
    @diagram = [:radishes, :grass, :clover, :violets]
  end

  # def bob_and_charlie_per_garden
  # end

end
#very last test only one not passing
