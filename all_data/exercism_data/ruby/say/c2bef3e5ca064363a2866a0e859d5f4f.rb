class Say
  ...
  def in_english
    NUMBERS[number] || say_numbers(split_number)
  end
  ...
  def split_number
    number.to_s.chars.reverse.each_slice(3).map { |triplet| triplet.reverse.join.to_i }
  end
  ...
  def speak_two_decimal num, index
    return NUMBERS[num[1].to_i] + "teen" if number < 20
  ...
  def speak_three_decimal num, index
    string = " " + speak_two_decimal(num[1..-1], index) unless num[1..-1].to_i == 0
    string ||= ''
    NUMBERS[num[0].to_i] + " hundred" + string
  end
end
