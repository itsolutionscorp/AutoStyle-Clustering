class Phone
  ...
  NUMBER_LENGTH = AREA_CODE_LENGTH + CENTRAL_OFFICE_CODE_LENGTH + SUBSCRIBER_NUMBER_LENGTH
  COUNTRY_CODED_LENGTH = NUMBER_LENGTH + COUNTRY_CODE_LENGTH

  BAD_NUMBER = "0" * NUMBER_LENGTH
  ...
  def number
    if normalized_number.length == NUMBER_LENGTH
      normalized_number
    else
      BAD_NUMBER
    end
