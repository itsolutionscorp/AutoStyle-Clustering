class PhoneNumber
  ...
  INVALID_NUMBER_PATTERS = [/[a-zA-Z]/, /[^1][0-9]{10}/, /[0-9]{12,}/, /^[0-9]{9}$/ ]
  def clean(number)
    cleaned_number =  number.gsub(/[()-. ]/,'')
    cleaned_number = "0"*10 if INVALID_NUMBER_PATTERS.any?{|invalid_patten| invalid_patten.match(cleaned_number)}
