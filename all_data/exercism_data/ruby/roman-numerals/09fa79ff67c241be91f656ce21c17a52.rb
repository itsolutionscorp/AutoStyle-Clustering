...
module Romans
  class RomanComponents
    ...
    def gen_canonical_componentf(sym, numval)
      ...
      case sym
      when :THOUSANDS
        return CANONICAL_DATA[sym] * numval
      when :HUNDREDS,:TENS,:ONES
        case numval
        when 1..3 # I, II, III
          return CANONICAL_DATA[sym][0] * numval
        when 4    # IV
          return CANONICAL_DATA[sym][0] + CANONICAL_DATA[sym][1]
        when 5    # V
          return CANONICAL_DATA[sym][1]
        when 6..8 # VI, VII, VIII
          return CANONICAL_DATA[sym][1] + (CANONICAL_DATA[sym][0] * (numval-5) )
        when 9    # IX
    ...
    def to_canonical_form
      ...
      zipped_sym_val.each do |argpiece|
        canonical_form << gen_canonical_componentf(argpiece[0],argpiece[1])
      end
      canonical_form
    end
