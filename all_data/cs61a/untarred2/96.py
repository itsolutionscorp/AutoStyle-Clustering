...
def make_word_master(goal_word):
    ...
    def check_word (guess_word):
        if get_string (guess_word) == get_string (goal_word):
            return win_message
...
def filter_subsets(word, score, possible_subsets):
    ...
    """
    filtered_list = []
    for word_list in possible_subsets:
        if compatible (word, score, word_list) and word_list != get_list (word):
...
def make_deductions(possible_subsets, letters):
    ...
    for letter in letters:
        present_count, not_present_count = 0, 0
