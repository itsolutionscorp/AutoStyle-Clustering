...
def subsets(lst, n):
    ...
    end_indices = list(range(len(lst)))[-n:]
    while len(lst) not in indices:
        this_subset = []
        for i in indices:
            this_subset += [lst[i]]
        subsets_lst += [this_subset]
        new_indices = [i for i in range(n)]
        for i in range(len(indices)):
            if indices[i] == end_indices[i]:
                if has_incremented:
                    new_indices[i] = new_indices[i - 1] + 1
                else:
                    new_indices[i - 1] = new_indices[i - 1] + 1
                    new_indices[i] = new_indices[i - 1] + 1
...
def filter_subsets(word, score, possible_subsets):
    ...
    [['a', 'b', 'e', 'l', 's'], ['b', 'l', 'e', 's', 't']]
    """
    return [possible_subsets[i] for i in range(len(possible_subsets))\
...
def make_deductions(possible_subsets, letters):
    ...
    for letter in letters:
        ...
        for subset in possible_subsets:
            if letter in subset:
                in_num_subsets += 1
        if in_num_subsets == len(possible_subsets):
            present += [letter]
        elif in_num_subsets == 0:
            not_present += [letter]
