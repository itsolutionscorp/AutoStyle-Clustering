...
def subsets(lst, n):
    ...
    def insert_at_beginning(x, lst):
        final_lst = []
        for stuff in lst:
            final_lst += [x + stuff]
        return final_lst

    return subsets(lst[1:], n) + insert_at_beginning([lst[0]], subsets(lst[1:], n-1))
...
def make_deductions(possible_subsets, letters):
    ...
    for char in letters:
        not_present_count = 0
        present_count = 0
        for subs in possible_subsets:
            if char in subs:
                present_count += 1
            else:
                not_present_count += 1
        if present_count == len(possible_subsets):
            present += [char]
        if not_present_count == len(possible_subsets):
            not_present += [char]
...
def print_deductions(possible_subsets):
    ...
    if not_present:
        print('The following letters must not be present:', not_pzresent)
