...
def subsets(lst, n):
    ...
    """
    # item = lst[0]
    # [item] + subsets(lst[1:],n-1)
    # use 3: for elem in subsets(lst[1:],n-1)
    # dont use 3 subsets(lst[1:],n)

    #subset takes in a list and some n, and it computes how many subsests
    #of  that list (of length n) there are
    #subset must return a list of lists
    ...
    partial = subsets(rest,n-1)
    for i in range(len(partial)):
        partial[i] = [first] + partial[i]
    return partial + subsets(rest,n)
...
def compatible(guess, score, letter_list):
    ...
    False
    """
    goal_word = make_word_from_list(letter_list)
    if num_common_letters(goal_word, guess) == score:
        return True
    else:
        return False
...
def filter_subsets(word, score, possible_subsets):
    ...
    for i in range(len(possible_subsets)):
        if num_common_letters(word,make_word_from_list(possible_subsets[i])) == score:
            final.append(possible_subsets[i])
    return final
...
def make_deductions(possible_subsets, letters):
    ...
    preallocate = [None]*len(letters)
    for k in range(len(letters)):
        preallocate[k] = 0

    for i in range(len(letters)): 
        for j in range(len(possible_subsets)):
            for k in range(len(possible_subsets[j])):
                if letters[i] == possible_subsets[j][k]:
                    preallocate[i] += 1
    for i in range(len(letters)):
        if preallocate[i] == len(possible_subsets):
            present.append(letters[i])
        elif preallocate[i] == 0:
            not_present.append(letters[i])
