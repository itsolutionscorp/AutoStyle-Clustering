...
def subsets(lst, n):
    ...
    if len(lst) == n:
        ...
        return add_element(lst[0], subsets(lst[1:], n-1)) + subsets(lst[1:], n)
...
def filter_subsets(word, score, possible_subsets):
    ...
    for item in possible_subsets:
        if compatible(word, score, item) == True:
            answer_list += [item]
    return answer_list
