...
def make_word_from_list(lst):
    ...
    def new_func(lst):
        if lst == []:
            ...
            return lst[0] + new_func(lst[1:])
    return lambda f: f(new_func(lst))
...
def get_list(word):
    ...
    """
    "*** YOUR CODE HERE ***"
    word = word(lambda x: x)
    def word_recursion(word):
        if word == '':
            return []
        else:
            return [word[0]] + word_recursion(word[1:])
    return word_recursion(word)
...
def subsets(lst, n):
    ...
    if lst == []:
        ...
        for i in range(len(lst) - n + 1):
            ...
            final_lst += [([lst[i]] + k) for k in subset]
        return final_lst
...
def filter_subsets(word, score, possible_subsets):
    ...
    """
    "*** YOUR CODE HERE ***"
    return [item for item in possible_subsets if compatible(word, score, item)]
...
def make_deductions(possible_subsets, letters):
    ...
    not_present = []
    "*** YOUR CODE HERE ***"
    present, not_present = letters, letters
    for word in possible_subsets:
        for letter in letters:
            if letter not in word:
                present = [item for item in present if item != letter]
            else:
                not_present = [item for item in not_present if item != letter]
