...
def combinations(first, next, n):
    ...
    if len(next) > 0:
        for i in range(len(next)):
            sets += combinations(first + [next[i]], next[(i + 1):], n - 1)
    return sets
...
def compatible(guess, score, letter_list):
    ...
    False
    """
    subs1 = subsets(letter_list, score)
    subs2 = subsets(guess, score)
    same_score = 0
    for s1 in subs1:
        for s2 in subs2:
            common = num_common_letters(s1, s2)
            if num_common_letters(s1, s2) == score:
                same_score += 1
    return same_score == 1
...
def filter_subsets(word, score, possible_subsets):
    ...
    for subset in possible_subsets:
        if num_common_letters(subset, word) == score:
            sets += [subset]
    return sets
...
def make_deductions(possible_subsets, letters):
    ...
    for i in range(0, len(letters)):
        ...
        elif letter_count[i] == num_subsets:
