...
def subsets(lst, n):
    ...
    if lst == []:
          return []  
    recursive_result = subsets(lst[1:],n-1)
    new_list = insert_every(lst[0],recursive_result)
    return new_list + subsets(lst[1:],n)
...
def compatible(guess, score, letter_list):
    ...
    """
    "*** YOUR CODE HERE ***"
    this_score = num_common_letters(make_word_from_list(letter_list), guess)
