...
def make_word_master(goal_word):
    ...
    """
    def foo(guess):
        if len(guess) < len(goal_word) or len(guess) > len(goal_word):
...
def subsets(lst, n):
    ...
    if n == 1:
        ...
        for index in range(len(lst) - n + 1):
            for item in subsets(lst[index + 1:], n - 1):
                new_list.append([lst[index]] + item)
        return new_list
...
def filter_subsets(word, score, possible_subsets):
    ...
    [['a', 'b', 'e', 'l', 's'], ['b', 'l', 'e', 's', 't']]
    """
    # takes in a word and sees if that word is compatible with each item in the possible_subsets
    possible_subsets_list = []
    for item in possible_subsets:
        if compatible(word, score, item):
            possible_subsets_list.append(item)
    return possible_subsets_list
    ...
    for item in letters:
        for element in possible_subsets:
            if item not in element:
                counter2 += 1 
            if counter2 == len(possible_subsets):
