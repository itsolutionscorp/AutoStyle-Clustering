...
def make_word_master(goal_word):
    ...
    def word_master(guess_word):
        ...
        if len(goal_string) != len(guess_string):
            return bad_num_letters
        elif not is_valid_guess(guess_string):
            return not_a_word
...
def subsets(lst, n):
    ...
    [2, 3, 4]
    """
    if n == 0 or len(lst) < n: # Base: lists size n or cannot make list size n
        return [[]]
    elif len(lst) == n: # Base: can make list exactly of size n
        return [lst]
    else:
        first, rest = lst[0], lst[1:]
        return subsets(rest, n) + insert_every(first, subsets(rest, n - 1))

def insert_every(elem, lst): # Iterative version for better runtime
    new_lst = []
    for el in lst:
        new_lst += [[elem] + el]
    return new_lst
...
def make_deductions(possible_subsets, letters):
    ...
    for subset in possible_subsets:
        ...
        for elem in subset:
            if elem not in all_present:
                all_present += elem
        # Check which letters must be present
        while i < len(present):
            if present[i] not in subset:
