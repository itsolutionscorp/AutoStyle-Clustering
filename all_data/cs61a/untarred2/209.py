...
def insert_every(elem,lst):
    """ Inserts elem into the front of every element in 
    lst (a list of lists). This method was taken from the Guerilla
    section handout. """
    return [[elem] + k for k in lst]
...
def compatible(guess, score, letter_list):
    ...
    actualScore = num_common_letters(goal_word, guess)
...
def filter_subsets(word, score, possible_subsets):
    ...
    """
    filteredSubsets = []
    for subset_word in possible_subsets:
        if compatible(word, score, subset_word):
            filteredSubsets.append(subset_word)
