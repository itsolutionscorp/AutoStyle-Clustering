...
def num_common_letters(goal_word, guess):
    ...
    #     if goal_word_list[letter] in guess_word_list:
    #         total+=1
    #     letter+=1
    # return total
...
def subsets(lst, n):
    ...
    [2, 3, 4]
    """
    def insert_every(lst,elem):
        copy_list=lst[:]
        for i in range(len(copy_list)):
            copy_list[i]=copy_list[i]+[elem]
        return copy_list
    def helper(lst,n,num_so_far):
        if lst==[]:
            return num_so_far
        elif n==0:
            return []
        else:
            return helper(lst[1:],n+1,insert_every(num_so_far,lst[0]))+helper(lst[1:],n,num_so_far)
    return [i for i in helper(lst,n,[[]]) if len(i)==n]
...
def filter_subsets(word, score, possible_subsets):
    ...
    for elem in possible_subsets:
        if num_common_letters(elem,word)==score:
            result=result+[elem]
    return result
    ...
    for elem in letters:
        for i in possible_subsets:
            if elem in i:
                ...
                is_not_in+=1
        if is_in==len(possible_subsets):
            present=present+[elem]
        elif is_not_in==len(possible_subsets):
            not_present=not_present+[elem]
        is_in=0
        is_not_in=0
