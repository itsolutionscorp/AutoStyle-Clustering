...
def make_word_master(goal_word):
    ...
    def inner(guess):
        guess_list = get_list(guess) 
        if len(goal_list) != len(guess_list):
            return bad_num_letters
        elif goal_list == guess_list:
            return win_message
        elif is_valid_guess(guess) == False:
            return not_a_word 
        else:
            return num_common_letters(goal_list, guess_list)
    return inner
...
def subsets(lst, n):
    ...
    if n == 0:
        ...
        wo_first = subsets(rest, n)
        w_first_before = subsets(rest, n-1)
        w_first = [[first] + x for x in w_first_before]
        return w_first + wo_first
...
def compatible(guess, score, letter_list):
    ...
    "*** YOUR CODE HERE ***"
    guess_list = get_list(guess)
    return num_common_letters(guess_list, letter_list) == score
...
def filter_subsets(word, score, possible_subsets):
    ...
    for subsets in possible_subsets:
        if compatible(word, score, subsets):
            result += [subsets]
        result = result
    return result 
...
def make_deductions(possible_subsets, letters):
    ...
    "*** YOUR CODE HERE ***"
    for subsets in possible_subsets:
         present += [ i for i in letters if i in subsets and i not in present]
         not_present += [ i for i in letters if i not in subsets and i not in not_present]
    intersection = [ i for i in present if i in not_present]
    p1 = [ i for i in present if i not in intersection]
    np1 = [ i for i in not_present if i not in intersection]
    return p1, np1
