...
def make_word_master(goal_word):
    ...
    def func(a_guess_word):
        if  is_valid_guess(a_guess_word) == False:
            return not_a_word
        elif len(get_list(a_guess_word)) != len(get_list(goal_word)):
            return bad_num_letters
        elif goal_word == a_guess_word:
            return win_message
        else:
            return num_common_letters(goal_word,a_guess_word)
    return func
...
def compatible(guess, score, letter_list):
    ...
    False
    """
    if len(get_list(guess))==len(get_list(letter_list)):
    
        if score==num_common_letters(guess,letter_list):
            return True 
        else:
            return False 
...
def filter_subsets(word, score, possible_subsets):
    ...
    for item in possible_subsets:
        if compatible(get_list(word),score,get_list(item))==True:
            result.append(item)
