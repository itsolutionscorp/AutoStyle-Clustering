...
def make_word_from_string(s):
    ...
    for letter in s:
        lst += [letter]
    return lambda msg: s if msg else lst 
...
def compatible(guess, score, letter_list):
    ...
    out = False
    length = len(get_string(guess))
    for subset in subsets(letter_list, length):
