...
def make_word_master(goal_word):
    ...
    def word_master(guess):
        if guess != len(goal_word):
            return bad_num_letters
        elif guess == len(goal_word):
            return win_message
        else:
...
def subsets(lst, n):
    ...
    if lst==[] and n!=0:
        return[]
    elif type(lst)==list and n==0:
        return [[]]
    elif lst==[] and n==0:
        return [[]]
    else:
        return [(n+lst[0])]+subsets(n-1,lst[1:])
...
def compatible(guess, score, letter_list):
    ...
    score= 
    g=get_list(guess)
    if len(g)!=len(letter_list):
        return False
    elif len(g)==len(letter_list):
