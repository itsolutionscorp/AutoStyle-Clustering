...
def compatible(guess, score, letter_list):
    ...
    """
    "*** YOUR CODE HERE ***"
    matching_letters = num_common_letters(guess, make_word_from_string(letter_list))
    if matching_letters == score:
        return True
    else:
        return False
...
def filter_subsets(word, score, possible_subsets):
    ...
    for elem in possible_subsets:
        if compatible(word, score, elem) == True:
            final_subsets = final_subsets + [elem]
    ...
    for elem in letters:
        for i in range(len(possible_subsets)):
            if elem in possible_subsets[i]:
                counted = counted + 1
        if counted == len(possible_subsets):
