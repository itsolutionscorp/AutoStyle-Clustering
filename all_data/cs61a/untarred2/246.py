...
def make_word_master(goal_word):
    ...
    def helper(guess):
        i = 0
        while i < 26:
            if guess == make_word_from_string(letters[i]*len(guess)):
                return not_a_word
            i+=1
...
def subsets(lst, n):
    ...
    i = 0
    while i < len(lst):
        helper += lst[i:] + lst[0:i]
        i+=1
    for j in range(0, len(helper) - n):
        helper1 = helper[j:j+n]
        helper1.sort()
        if helper1 not in mylst:
            mylst.append(helper1)
    return mylst
