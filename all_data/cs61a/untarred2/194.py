...
def make_word_master(goal_word):
    ...
    "*** YOUR CODE HERE ***"
    def help(string):
        if is_valid_guess(string)==False:
            return not_a_word
        if len(string) != len(goal_word):
            return bad_num_letters
        if string==goal_word:
            return win_message
        else:
            return num_common_letters(goal_word, string)
    return help
...
def compatible(guess, score, letter_list):
    ...
    a=get_list(guess)
    same=0
    b=num_common_letters(guess,make_word_from_list(letter_list))
    while i<len(a):
        k=0
        while k<i:
            if a[i]==a[k]:
