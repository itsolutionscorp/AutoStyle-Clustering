...
def num_common_letters(goal_word, guess):
    ...
    """
    "*** YOUR CODE HERE ***"
    #make a third list of common letters, then add to it each time it's a match make sure you
    #check common letters list before you add to the list. return length of common letters list, len[commonletters]
    num_common_letters = []
    i = 0
...
def subsets(lst, n):
    ...
    """
    "*** YOUR CODE HERE ***"
    def elements(first_elements, remaining_elements): #some way of enumerating lists beginning with a certain element.
        lst1 = []
        for elem in remaining_elements:
            lst1 += [first_elements + [elem]]
        return lst1
    ...
    if n > len(lst):
        ...
        first_elements = lst[:n-1]
        remaining_elements = lst[n-1:]
        return elements(first_elements, remaining_elements) + subsets(lst[1:], n) + subsets([lst[0]]+lst[2:],n)
...
def filter_subsets(word, score, possible_subsets):
    ...
    new_list = []
    i = 0
    while i < len(possible_subsets):
        if compatible(word, score, possible_subsets[i]):
            new_list += [possible_subsets[i]]
        i += 1
...
def make_deductions(possible_subsets, letters):
    ...
    i = 0
    while i < len(letters):
        j, in_subset_counter, not_in_subset_counter = 0, 0, 0
        while j < len(possible_subsets):
            if letters[i] in possible_subsets[j]:
                in_subset_counter += 1
                #print(possible_subsets[j])
            elif letters[i] not in possible_subsets[j]:
                not_in_subset_counter += 1
                #print("in")
            j += 1

        if in_subset_counter == len(possible_subsets): #if letters[i] in all possible subsets
            present += letters[i]
        if not_in_subset_counter == len(possible_subsets):
