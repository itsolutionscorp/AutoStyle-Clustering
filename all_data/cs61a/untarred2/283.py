...
def make_word_from_list(lst):
    ...
    while count < len(lst):
         ...
         count += 1
    return word
    # I have now represented the Word ADT as a string
...
def subsets(lst, n):
    ...
    if lst == []:
        ...
        for elem in subsets(lst[1:], n - 1):
            all_lst += [[first] + elem]
        return all_lst + subsets(lst[1:], n)
...
def filter_subsets(word, score, possible_subsets):
    ...
    for lst in possible_subsets:
        if compatible(make_word_from_list(lst), score, word) == True:
            subsets_to_return += [lst]
...
def make_deductions(possible_subsets, letters):
    ...
    for letter in letters:
        present_in_all = True
        not_present_in_all = True
        for lst in possible_subsets:
            if letter not in lst:
                present_in_all = False
            if letter in lst:
                not_present_in_all = False
        if present_in_all:
            present += [letter]
        if not_present_in_all:
            not_present += [letter]
