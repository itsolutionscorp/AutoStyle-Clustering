...
def make_word_master(goal_word):
    ...
    "*** YOUR CODE HERE ***"
    def guesser(guess):
        if len(mwfs(goal_word)) != len(mwfs(guess)):
...
def subsets(lst, n):
    ...
    # lst_subsets = []
    # def helper(lst, n, first, rest, lst_subsets):
    # first, rest = lst[0], lst[1:]
    #     if n == 0:
    #         return [[]]
    #     elif lst == []:
    ...
    # return subset(rest, n)
    # return subset(rest, n - 1)
    # return helper(lst, n, first, rest, lst_subsets)
    ...
    # current_subset = []
    # k = 0
    # def dispatch(lst, n, current_subset, lst_subsets, k):
    #     if len(current_subset) == n:
    #         lst_subsets = lst_subsets + [current_subset]
    #         current_subset = []
    #         # k += 1
    #         # if k <= n:
    #         return dispatch(lst, n, current_subset, lst_subsets, k)
    #     for el in lst:
    #         current_subset = current_subset + [el]
    #         return dispatch(lst[1:], n, current_subset, lst_subsets, k)
    #     return lst_subsets       
    # return dispatch(lst, n, current_subset, lst_subsets, k)
...
""""
    ...
    def dispatch(lst, n, current_subset, lst_subsets):
        # if len(current_subset) == n:
        #     lst_subsets = lst_subsets + [current_subset]
        #     current_subset = []
            # return dispatch(lst[1:], n, current_subset, lst_subsets)
            #lst_subsets
        for el in lst:
            # current_subset = current_subset + [el]
            if len(current_subset) < n:
                current_suset = current_suset + [el]
            ...
            #     lst_subsets = lst_subsets + [current_subset]
                # current_subset = []
                return lst_subsets
                #dispatch(lst, n, current_subset, lst_subsets)  #rotate??
            else: #can i splice particular elements>??
                #dispatch(rotate_lst(lst, 1), n, current_subset, lst_subsets) +//
                
                return dispatch(lst[1:], n, current_subset, lst_subsets)
        return lst_subsets       
    return dispatch(lst, n, current_subset, lst_subsets)
"""""


#borrowed from a section
...
def filter_subsets(word, score, possible_subsets):
    ...
    for subset in possible_subsets:
        if num_common_letters(word, subset) == score:
            rel_s += [subset]
    return rel_s
...
def make_deductions(possible_subsets, letters):
    ...
    for lets in letters:
        k = 0
        j = 0
        for sets in possible_subsets:
            if lets in sets:
                k += 1
            if lets not in sets:
                j += 1
        if k == len(possible_subsets):
            present += [lets]
        if j == len(possible_subsets):
            not_present += [lets]
    #         if lets not in sets:
    #             not_present += [lets]
    #             break 
