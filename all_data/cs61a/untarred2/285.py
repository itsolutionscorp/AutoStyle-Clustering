...
def subsets(lst, n):
    ...
    if n == 0:
        ...
        return [[first]+x for x in without_first] + subsets(rest,n)
...
def compatible(guess, score, letter_list):
    ...
    if score == num_common_letters(mwfl(letter_list),guess):
        return True
    else:
        return False
...
def make_deductions(possible_subsets, letters):
    ...
    for elem in letters:
        bool_seq = []
        for sub in possible_subsets:
            bool_seq += [elem in sub]
        if len([x for x in bool_seq if x == True]) == len(bool_seq):
            present += [elem]
        if len([x for x in bool_seq if x == False]) == len(bool_seq):
            not_present += [elem]
