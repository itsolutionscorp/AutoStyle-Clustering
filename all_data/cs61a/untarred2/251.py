...
def subsets(lst, n):
    ...
    [2, 3, 4]
    """
    def helper(lst,slst,n):
        if n==0:
            return [slst]
        elif lst==[]:
            return []
        return helper(lst[1:],slst+[lst[0]],n-1)+helper(lst[1:],slst,n) 
    subset=helper(lst,[],n)
    return subset
...
def make_deductions(possible_subsets, letters):
    ...
    for i in letters:
        while n<len(possible_subsets):
            for j in possible_subsets[n]:
                if i==j:
                    n+=1
                if n==len(possible_subsets) and i==j:
                    present.append(i)
    for subset in possible_subsets:
            for y in subset:
                for x in not_present:
                    if x==y:
                        not_present.remove(x)
                        break
