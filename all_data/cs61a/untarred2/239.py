    ...
    def check_guess(guess):
        if is_valid_guess(guess):
            if get_list(goal_word) == get_list(guess):
                ...
                return num_common_letters(goal_word, guess)  #Include the case of steal
        else:
            return not_a_word
    return check_guess
...
def subsets(lst, n):
    ...
    if n == 0 or n > len(lst) :
        return []
    if n == 1:
        return [[i] for i in lst]    # if n == len(lst) return lst --> useless base case
    return [ ([lst[0]] + sub) for sub in subsets(lst[1:], n - 1)] + subsets(lst[1:], n)
...
def filter_subsets(word, score, possible_subsets):
    ...
    for sub in possible_subsets:
        if compatible(word, score, sub):
            s += [sub]    # put [] outside the sub
    return s
    # how to write it with list comprehension
    ...
    for letter in letters:
        ...
        for sub in possible_subsets:
            if letter in sub:
                ...
                counter2 += 1
        if counter1 == len(possible_subsets):
            present += [letter]
        elif counter2 == len(possible_subsets):
            not_present += [letter]
