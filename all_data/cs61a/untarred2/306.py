...
def subsets(lst, n):
    ...
    for i in range(n):
        b.append(0)
    def subset(a,b,arange,n,N):
        for i in range(arange,n-1,-1):
            b[n-1]=i-1
            if n>1:
                subset(a,b,i-1,n-1,N)
            else:
                c=[]
                for j in range(N):
                    c.append(a[b[j]])
                answers.append(c)
    subset(lst,b,len(lst),n,n)
    return answers                         
...
def compatible(guess, score, letter_list):
    ...
    False
    """
    n=len(get_list(guess))
    possible_words=subsets(letter_list,n)
    for word in possible_words:
        if num_common_letters(make_word_from_list(word),guess)==score:
            return True
    return False
...
def filter_subsets(word, score, possible_subsets):
    ...
    for subset in possible_subsets:
        if compatible(make_word_from_list(subset),score,get_list(word))==True:
