...
def make_word_master(goal_word):
    ...
    """
    "*** YOUR CODE HERE ***"
    def check_words(guess):
        if is_valid_guess(guess) and is_valid_goal_word(goal_word): 
            if guess == goal_word:
                return win_message
...
def subsets(lst, n):
    ...
    return subsets(lst[1:], n) + append_to_all(lst[0], subsets(lst[1:], n - 1))

def append_to_all(el, lst):
    return [[el] + sub_lst for sub_lst in lst]
...
def make_deductions(possible_subsets, letters):
    ...
    "*** YOUR CODE HERE ***"
    for elem in letters:
        if num_common_letters(possible_subsets, elem) == len(possible_subsets):
            present = present + [elem]
        elif num_common_letters(possible_subsets, elem) == 0:
