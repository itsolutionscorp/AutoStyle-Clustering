...
def get_list(word):
    ...
    """
    "*** YOUR CODE HERE ***"
    return [word[0][i] for i in range(len(word[0]))]
...
def subsets(lst, n):
    ...
    smaller_list = subsets(lst[1:], n)
    smaller_n = subsets(lst[1:], n - 1)
    smaller_n = [[lst[0]] + x for x in smaller_n]
    return smaller_list + smaller_n
...
def make_deductions(possible_subsets, letters):
    ...
    for letter in letters:
        if len(filter_subsets(letter, 1, possible_subsets)) == len(possible_subsets):
            present += [letter]
        elif len(filter_subsets(letter, 1, possible_subsets)) == 0:
            not_present += [letter]
