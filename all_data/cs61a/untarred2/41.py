...
def make_word_from_string(s): #return a string representation of word
    """Creates an instance of the word ADT from the string s.
    """
    "*** YOUR CODE HERE ***"
    return s #s already string, return s
    #lst = [el for el in s]
    #return lambda: [s, lst]

def make_word_from_list(lst): #return a string representation of word
    ...
    for el in lst:
        s  += el
    return s #convert list lst to string s
    #return lambda: [s, lst]

def get_string(word): #input a word represented as str
    ...
    """
    "*** YOUR CODE HERE ***"
    return word #word already string, return word
    #return word()[0]

def get_list(word): #input a word represented as str
    ...
    for el in word:
        lst += [el]
    return lst #convert string word to list lst
    #return word()[1]
...
def make_word_master(goal_word): #input 2 word represented as strings
    ...
    """
    "*** YOUR CODE HERE ***"
    def helper(guess_word):
        if len(get_string(guess_word)) != len(get_string(goal_word)):
            return bad_num_letters
        elif not is_valid_guess(guess_word):
            return not_a_word
        elif get_string(guess_word) == get_string(goal_word):
            return win_message
        else:
            return num_common_letters(guess_word, goal_word)
    return helper
...
def subsets(lst, n):
    ...
    if n == 0:
        ...
        with_first = [[lst[0]]+el for el in subsets(lst[1:], n-1)] #insert lst[0]
...
def compatible(guess, score, letter_list):
    ...
    """
    "*** YOUR CODE HERE ***"
    if len(get_list(guess)) == len(letter_list):
...
def filter_subsets(word, score, possible_subsets):
    ...
    for subset in possible_subsets:
        if compatible(word, score, subset) == True:
            result += [subset]
    return result
    """
    return [subset for subset in possible_subsets if compatible(word, score, subset)]
...
def make_deductions(possible_subsets, letters):
    ...
    for el in letters:
        ...
        for subset in possible_subsets:
            if el in subset:
                el_not_present = False
            if el not in subset:
                el_present = False
        if el_present == True:
            present += [el]
        elif el_not_present == True:
            not_present += [el]
    return present, not_present
    # def helper_in(el, deep_lst):
    #     if deep_lst == []:
    #         return True
    #     elif el not in deep_lst[0]:
    #         return False
    #     elif el in deep_lst[0]:
    #         return helper_in(el, deep_lst[1:])
    # def helper_not_in(el, deep_lst):
    #     if deep_lst == []:
    #         return True
    #     elif el in deep_lst[0]:
    #         return False
    #     elif el not in deep_lst[0]:
    #         return helper_not_in(el, deep_lst[1:])
    # for el in letters:
    #     if helper_in(el, possible_subsets):
    #         present += [el]
    #     elif helper_not_in(el, possible_subsets):
    #         not_present += [el]
    # return present, not_present
    for el in letters:
        el_present = True
