...
def make_word_master(goal_word): 
    ...
    """
    "*** YOUR CODE HERE ***"
    #domain: one 'word' string
    #range: varrying messages or function
    #goal_word==mwfs('string')=='string'


    def matcher(guess): #guess and #goal are both words from 
    #make word from string function
        if is_valid_guess(get_string(guess))==True and is_valid_goal_word(get_string(goal_word))==True:
            if get_string(guess)==get_string(goal_word):
                return win_message
            if len(get_string(guess))!=len(get_string(goal_word)):
...
def subsets(lst, n):
    ...
    """
    "*** YOUR CODE HERE ***"
    #before print, should look like [[0,1,2],[0,1,3]...[2,3,4]]
...
def insert_every(k,lst):
    if lst==[]:
        return []
    return [[k]+lst[0]]+insert_every(k,lst[1:])
...
def compatible(guess, score, letter_list):
    ...
    """
    "*** YOUR CODE HERE ***"
    word=get_string(make_word_from_list(letter_list))
    return num_common_letters(word, guess)==score
...
def filter_subsets(word, score, possible_subsets):
    ...
    """
    "*** YOUR CODE HERE ***"
    #possible_subsets = a python list of subsets which is a list of letters
    lister=[]

    if possible_subsets != []:
        for i in range(len(possible_subsets)):
            if compatible(word, score, possible_subsets[i]):
                lister+=[possible_subsets[i]]
...
def make_deductions(possible_subsets, letters):
    ...
    #subsets=sets of letters
    for i in range(len(letters)):
        b=0
        a=0
        for k in range(len(possible_subsets)):
            if letters[i] in possible_subsets[k]:
                a+=1
            if letters[i] not in possible_subsets[k]:
                b+=1
        if a==len(possible_subsets):
            present+=[letters[i]]
        if b==len(possible_subsets):
