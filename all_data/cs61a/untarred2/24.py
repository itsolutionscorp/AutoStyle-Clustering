...
def make_word_master(goal_word):
    ...
    def word_master(guess):
        ...
        guess_string = get_string(guess)
        if len(goal_word_string) != len(guess_string):
            return bad_num_letters
        if goal_word_string == guess_string:
            return win_message
...
def subsets(lst, n):
    ...
    """
    "*** YOUR CODE HERE ***"
    def get_sub(_lst, _current):
        if len(_current)  > n:
            return []
        if len(_current) + len(_lst) == n:
            return [(_current + _lst)]
        if len(_lst) == 0:
            return []
        return get_sub(_lst[1::], _current) + get_sub(_lst[1::], _current + _lst[0:1:])
    return get_sub(lst, [])
