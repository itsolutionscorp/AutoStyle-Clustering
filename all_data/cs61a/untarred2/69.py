...
def get_string(word):
    ...
    'world'
    """
    assert word == str or list, 'Error: word must be a string or a character list'
    if type(word) == str: 
        return word
...
def num_common_letters(goal_word, guess):
    ...
    assert len(goal) == len(guess_word), 'Error: guess must be of same length as goal word'
    assert type(guess_word) == type(goal), 'Error: guess must be of same type as goal word'
...
def make_word_master(goal_word):
    ...
    def rohin_function(guess):
        goal = get_list(goal_word)
        word_guess = get_list(guess)

        if len(goal) != len(word_guess): 
            return bad_num_letters
        if type(goal) != type(word_guess): 
            return not_a_word
        if is_valid_guess(word_guess) == False: 
            return not_a_word
        if goal == word_guess: 
            return win_message

        return num_common_letters(goal, word_guess)
    return rohin_function
