...
def make_word_master(goal_word):
    ...
    def word_master(guess):
            ...
            if (len(guess)!= len(goal_word)):
                    ...
                    if (num_common_letters(goal_word, guess)!=len(guess) or not(goal_word==guess)):
...
"""VERY IMPORTANT: The following two functions, which are coded out, 
are based on insert_first and powerset, which we saw in other parts of the class (Guerrilla sections). However, I decided not to use this code
as that did not seem to be the intent or purpose of the excercise (the function subsets, was indented to return a list of only the subsets, 
    provided by all_subs, that were of length n. Thus, although the code works with all_subs and insert, I decided to code those functions out. All 
    other functions work and are based on the assumption that subsets work.)

def insert(element, lst):
  if lst == []:
    return []
  else:
    return [[element] +lst[0]] + insert(element, lst[1:])
...
def all_subs(lst):
    if lst == []:
            ...
            without_first = all_subs(rest)
            return without_first +  insert(first, without_first)
"""
"""THe following function will not work without the two coded out functions above"""
def subsets(lst, n):
    ...
    [2, 3, 4]
    """
    all_sub=all_subs(lst)
    all_right_sub=[]
    for sub in all_sub:
        if (len(sub)==n):
            all_right_sub+=[sub]
    return all_right_sub
...
def filter_subsets(word, score, possible_subsets):
    ...
    for sub in possible_subsets:
        if (compatible(word, score, sub)==True):
                filtered+=[sub]
