...
def make_word_master(goal_word):
    ...
    def h(guess):
        ...
        elif is_valid_guess(guess) != True:
            return not_a_word
...
def filter_subsets(word, score, possible_subsets):
    ...
    for p in possible_subsets:
        if compatible(word_list, score, p):
            total += [p]
    return total 
...
def make_deductions(possible_subsets, letters):
    ...
    present = []
    not_present = []
    letters = get_list(letters)
    temp = []
    for s in possible_subsets:
        for elem in s:
            for i in letters:
                if i in elem and i not in present:
                    present += i
                elif i not in elem and i not in not_present:
                    not_present += i
    for s in possible_subsets:
        for p in present:
            if p not in s:
