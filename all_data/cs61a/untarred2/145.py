...
def make_word_master(goal_word):
    ...
    "*** YOUR CODE HERE ***"
    def guess(word):
        if len(get_string(word)) != len(get_string(goal_word)):
            return bad_num_letters
        elif get_string(word) == get_string(goal_word):
            return win_message
        elif is_valid_guess(word):
...
def subsets(lst, n):
    ...
    """
    "*** YOUR CODE HERE ***"
    def combinations(lst, n , s):
        if n == 0:
            return [s]
        elif len(lst) == 0:
            return []
        return combinations(lst[1:], n - 1, s + [lst[0]] ) + combinations(lst[1:], n, s)
...
def compatible(guess, score, letter_list):
    ...
    """
    "*** YOUR CODE HERE ***"
    if num_common_letters(get_string(letter_list), guess) == score:
        return True
    else:
        return False
...
def filter_subsets(word, score, possible_subsets):
    ...
    for i in range(len(possible_subsets)):
        if compatible(get_string(possible_subsets[i]), score, get_string(word)):
            s += [possible_subsets[i]]
    return s
