...
def make_word_master(goal_word):
    ...
    True
    """
    def like_make_work_master(guess):
...
def compatible(guess, score, letter_list):
    ...
    False
    """
    return make_word_master(make_word_from_list(letter_list))(make_word_from_list(guess)) == score
...
def filter_subsets(word, score, possible_subsets):
    ...
    for i in range(len(possible_subsets)):
        if compatible(word,score,possible_subsets[i]):
            compatible_word_list += [possible_subsets[i]]
...
def make_deductions(possible_subsets, letters):
    ...
    for item in letters:
        ...
        is_not_in_all = True
        for i in range(len(possible_subsets)):
            if item in possible_subsets[i]:
                is_not_in_all = False
            if item not in possible_subsets[i]:
                is_in_all = False
        if is_in_all == True :
            present += [item]
        if is_not_in_all == True:
            not_present += [item]
