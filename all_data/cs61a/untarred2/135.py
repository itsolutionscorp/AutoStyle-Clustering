...
def make_word_from_list(lst):
    ...
    for i in range(len(lst)):
        instance2 += str(lst[i])
    return [instance2, lst] 
    
#change lst into a string 
...
def subsets(lst, n):
    ...
    for el in subsets(rest, n - 1): 
        final_lst += [[first] + el]
    return subsets(rest, n) + final_lst 
...
def compatible(guess, score, letter_list):
    ...
    if score == num_common_letters(guess, make_word_from_list(letter_list)):
        return True 
    else: 
        return False 
