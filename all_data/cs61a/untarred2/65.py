...
def num_common_letters(goal_word, guess):
    ...
    """
    "*** YOUR CODE HERE ***"
    #if an element in list of goal_word is repeated in list of goal_word + guess, add to counter
...
def subsets(lst, n):
    ...
    if lst == [] and n == 0:
        ...
        return subsets(lst[1:], n) + insert(lst[0], subsets(lst[1:], n-1))
...
def filter_subsets(word, score, possible_subsets):
    ...
    for subset in possible_subsets:
        if compatible(word, score, ) == True:
            new_list += subset(possible_subsets, n)
            return new_list
