...
def make_word_master(goal_word):
    ...
    def word_master(guess):
        guess_s, goal_s = get_string(guess), get_string(goal_word)
        if len(guess_s) != len(goal_s):
...
def subsets(lst, n):
    ...
    if lst == [] or n == 0:
        ...
        return [item for item in without_first + insert_every(first, subsets(rest, n-1)) if len(item) == n]

def insert_every(elem, lst):
    new_lst = []
    for sublist in lst:
        new_lst += [[elem] + sublist]
    return new_lst
...
def make_deductions(possible_subsets, letters):
    ...
    for letter in letters:
        ...
        if in_all:
            ...
            temp_not_pres += [letter]
    for letter in temp_not_pres:
        in_one = False
        for subset in possible_subsets:
            if letter in subset:
                in_one = True
