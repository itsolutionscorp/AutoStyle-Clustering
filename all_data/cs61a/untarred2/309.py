...
def get_list(word):
    ...
    if word == '':
        return char_list 
    while len(char_list) < len(word): 
        char_list = char_list + [word[char_num]]
        char_num += 1 
    return char_list
...
def compatible(guess, score, letter_list):
    ...
    False
    """
    x = num_common_letters(make_word_from_list(letter_list), guess) 
    if x == score:
        return True 
    else:
        return False
...
def filter_subsets(word, score, possible_subsets):
    ...
    for sublist in possible_subsets: 
        if compatible(word, score, sublist) == True:
            goal_list = goal_list + [sublist]
    return goal_list 
