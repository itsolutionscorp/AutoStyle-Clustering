...
def make_word_master(goal_word):
    ...
    True
    """
    def guess_word_acceptor(guess):
        if not is_valid_guess(guess):
            return not_a_word
        elif len(get_list(guess)) == len(get_list(goal_word)):
            if guess == goal_word:
                return win_message
            else:
...
def subsets(lst, n):
    ...
    [2, 3, 4]
    """
    def insert_elem(elem, lst):
        if lst == []:
            return []
        elif type(lst[0]) == type([]):
            return [[elem] + lst[0]] + insert_elem(elem, lst[1:])
        else:
            return [[elem] + [lst[0]]]+ insert_elem(elem, lst[1:])
    def subset_mechanism(lst,n):
        if lst == []:
            return [[]]
        if n == 0:
            return []
        subset_list = subset_mechanism(lst[1:],n)
        return subset_list + insert_elem(lst[0],subset_list)
    subset_list_end = subset_mechanism(lst,n)
    possible_subsets = [elem for elem in subset_list_end if type(elem) == type([]) and len(elem) == n]
...
def filter_subsets(word, score, possible_subsets):
    ...
    [['a', 'b', 'e', 'l', 's'], ['b', 'l', 'e', 's', 't']]
    """
    valid_subsets = [subset for subset in possible_subsets if compatible(subset, score, word)]
...
def make_deductions(possible_subsets, letters):
    ...
    for subset in possible_subsets:
        for character in subset:
            n = 0
            while n < len(possible_subsets):
                if not(is_in_list(character,possible_subsets[n])) and is_in_list(character, present):
                    present.remove(character)
                n += 1 
            if is_in_list(character,not_present):
                not_present.remove(character)
        x+=1
    for character in not_present:
        if is_in_list(character, present):
            present.remove(character)
