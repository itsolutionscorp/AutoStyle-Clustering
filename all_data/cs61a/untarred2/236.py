...
def subsets(lst, n):
    ...
    if len(lst) < n or n == 0: 
        ...
        has_first = subsets(lst[1:], n-1)
        for subset in has_first:
            subset += [lst[0]]
        return has_first + subsets(lst[1:], n)
...
def filter_subsets(word, score, possible_subsets):
    ...
    for word in possible_subsets:
        return filter_subsets(word, score, subsets(possible_subsets))
...
def make_deductions(possible_subsets, letters):
    ...
    # if possible_subsets in letters:
    #     return present 
    # if possible_subsets not in letters: 
    #     return not_present
