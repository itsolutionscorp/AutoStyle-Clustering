...
def compatible(guess, score, letter_list):
    ...
    False
    """
    return num_common_letters(guess, make_word_from_string(letter_list)) == score
...
def filter_subsets(word, score, possible_subsets):
    ...
    [['a', 'b', 'e', 'l', 's'], ['b', 'l', 'e', 's', 't']]
    """
    return list(filter(lambda s: compatible(word, score, s), possible_subsets))
...
def make_deductions(possible_subsets, letters):
    ...
    for l in letters:
        ...
        for s in possible_subsets:
            if l in s: seen = True
            elif l not in s: notseen = True
        if seen and not notseen: present.append(l)
        if not seen and notseen: not_present.append(l)
