...
def make_word_from_list(lst):
    ...
    """
    return make_word_from_string(lst)
...
def subsets(lst, n):
    ...
    [1, 3, 4]
    [2, 3, 4]
    >>> four_subsets = subsets(list(range(6)), 4)
    >>> four_subsets.sort()
    ...
    if len(lst) == 0:
        return []
    subs = [x + [lst[0]] for x in subsets(lst[1:], n - 1)]
    return [sorted(x) for x in subs + subsets(lst[1:], n)]
