...
def make_word_master(goal_word):
    ...
    def helper(guess):
        ...
        if is_valid_guess(guess) is False:
            return not_a_word
        if goal_word == guess:
            return win_message
        else:
...
def subsets(lst, n):
    ...
    if n == 0:
        ...
        return [lst]
            ...
            return [[lst[0]] + s for s in subsets(lst[1:], n - 1)] + subsets(lst[1:], n)
...
def make_deductions(possible_subsets, letters):
    ...
    for letter in letters:
        if letter not in single_lst:
            not_present += letter
    for elem in letters:
        if single_lst.count(elem) == len(possible_subsets):
            present += elem
