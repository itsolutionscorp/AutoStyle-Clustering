...
def make_word_master(goal_word):
    ...
    True
    """
    def function_that_does_what_word_master_does(guess):
    ...
    return function_that_does_what_word_master_does
...
def compatible(guess, score, letter_list):
    ...
    for element in letter_list:
        if element == element in guess:
            total_number_of_common_letters += 1
    if total_number_of_common_letters == score:
        return True
    else:
        return False
...
def make_deductions(possible_subsets, letters):
    ...
    present = []
    not_present = []
    number_of_times_present = 0
    number_of_times_notpresent = 0
    for element in letters:
        for subsets in possible_subsets:
            if element in subsets:
                number_of_times_present += 1
            elif element not in subsets:
                number_of_times_notpresent += 1
        if number_of_times_present == len(possible_subsets):
            present += [element]
        elif number_of_times_notpresent == len(possible_subsets):
            not_present += [element]
        number_of_times_present = 0
        number_of_times_notpresent = 0
