...
def make_word_from_string(s):
    ...
    """
    "*** YOUR CODE HERE ***"
    return [s, [element for element in s]]
...
def subsets(lst, n):
    ...
    def insert_every(i,lst):
        if lst == []:
            return []
        else:
            return [[i] + lst[0]] + insert_every(i, lst[1:])
    ...
    if len(lst) < n:
        ...
        keep_first_item = insert_every(lst[0], subsets(lst[1:], n - 1)) 
        not_keep_first_item = subsets(lst[1:], n)
        
        return keep_first_item + not_keep_first_item
...
def compatible(guess, score, letter_list):
    ...
    for j in letter_list:
        for i in get_list(guess):
            if j == i:
                if not j in already_counted:
                    already_counted += [j]
                    common_letters += 1
    if common_letters == score:
        return True
    else:
        return False
...
def filter_subsets(word, score, possible_subsets):
    ...
    if len(possible_subsets) == 0:
        return []
    elif compatible(word, score, possible_subsets[0]):
