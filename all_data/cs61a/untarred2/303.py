...
def num_common_letters(goal_word, guess):
    ...
    """
    "*** YOUR CODE HERE ***"
    # for i in make_word_from_string(goal_word):
    #     if i not in (make_word_from_string(goal_word)+1):
    #         return True

    # for or make_word_from_list(goal_word):
    #     if i not in (make_word_from_list(goal_word)+1):
    #         return True 

    # len(make_word_from_string(goal_word))==len(make_word_from_string(guess)) /
    # or len(make_word_from_list(goal_word))==len(make_word_from_list(guess))
...
def make_word_master(goal_word):
    ...
    # get_string(goal_word)=unique(is_valid_guess)
    def foo(guess):
...
def subsets(lst, n):
    ...
    "*** YOUR CODE HERE ***"
    # import itertools
    # for i in itertools.product(lst,repeat=3):
    #     print (i)
...
def filter_subsets(word, score, possible_subsets):
    ...
    for sub in possible_subsets:
        if num_common_letters(word,sub)==score:
            new_list+=[sub]
    return new_list            
