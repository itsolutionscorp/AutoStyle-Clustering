...
def subsets(lst, n):
    ...
    for elem in subsets(lst[1:],n-1):
        x += [[lst[0]] + elem]  #incrementing
    
    return x + subsets(lst[1:],n)
...
def compatible(guess, score, letter_list):
    ...
    False
    """
    for elem in subsets(letter_list,len(letter_list)):
        if num_common_letters(make_word_from_list(elem),guess) == score:
            return True
    return False
...
def filter_subsets(word, score, possible_subsets):
    ...
    [['a', 'b', 'e', 'l', 's'], ['b', 'l', 'e', 's', 't']]
    """
    final_subsets = possible_subsets
    x = []
    for elem in final_subsets:
...
def make_deductions(possible_subsets, letters):
    ...
    new_lst = []
    num_subsets = len(possible_subsets)
    for subset in possible_subsets:
        new_lst += subset
    for elem in letters:
        if new_lst.count(elem) == num_subsets:
            present += elem
        elif new_lst.count(elem) == 0:
            not_present += elem
