    ...
    if possible_subsets == []:
        ...
        for item in possible_subsets:
            if compatible(word, score, item) == True:
                working_subsets.append(item)
        return working_subsets
...
def make_deductions(possible_subsets, letters):
    ...
    for item in letters:
        in_lst = 0
        not_in_lst = 0
        for subsublst in possible_subsets:
            for element in subsublst:
                if item == element:
                    in_lst += 1
                elif item != element:
                    not_in_lst += 1
        if in_lst == len(possible_subsets):
            present += [item]
        elif not_in_lst == len(possible_subsets)*len(subsublst):
            not_present += [item]
