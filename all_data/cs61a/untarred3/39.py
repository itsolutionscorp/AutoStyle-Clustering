    ...
    if lst == []:
        ...
        def insert_every (lst, elem):
            if lst == []:
                ...
                first, rest = lst[0], lst[1:]
                return [elem] + [first] + insert_every(rest, elem)

        only_rest = [first] + subsets(rest, n)
        only_n = subsets(lst, n-1)
        return [only_rest] + [insert_every(only_n, first)]
...
def filter_subsets(word, score, possible_subsets):
    ...
    for subset in possible_subsets:
        if compatible(guess, score, subset):
            result += subset
    return result
