...
def subsets(lst, n):
    ...
    return subsets(lst[1:], n) + insert_every(first, subsets(lst[1:], n-1))
...
def compatible(guess, score, letter_list):
    ...
    False
    """
    if make_word_master(letter_list) == score:
        return True
    else:
        return False
...
def make_deductions(possible_subsets, letters):
    ...
    for element in letters:
        ...
        present = []
        not_present = []
        for y in possible_subsets:
            if element == y:
                new_is_in == True
            else:
                new_is_in == False
            old_is_in = new_is_in
            if old_is_in != new_is_in:
                neither =+ element
    ...
    return present, not_present
            ...
            #check element is in y
            #if yes, add element to is_in_subset
            #if no, keep at not_present
            #if it goes from not_present to present, put in neither
            #return
