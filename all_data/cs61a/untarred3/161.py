...
def word(s):
    """Takes in a list of characters and constructs a word.
    """
    lst = list(s)
    return [s, lst]
...
def subsets(lst, n):
    ...
    def powerset(lst):
        if lst == []:
            ...
            recursive_result = powerset(lst[1:])
            new_elements = powerset(lst[1:])
            counter = 0
            for elem in recursive_result:
                new_elements[counter] = [lst[0]] + recursive_result[counter]
                counter += 1
            return recursive_result + new_elements
    res = powerset(lst)
    return_lst = []
    for elem in res:
        if len(elem) == n:
...
def compatible(guess, score, letter_list):
    ...
    """
    "*** YOUR CODE HERE ***"
    other_word = make_word_from_list(letter_list)
    if num_common_letters(other_word, guess) == score:
        return True
    return False
...
def make_deductions(possible_subsets, letters):
    ...
    for char in range(len(letters)):
        for set in possible_subsets:
            for letter in set:
                if letters[char] == letter:
                    letter_count[char] += 1
    for num in range(len(letter_count)):
        if letter_count[num] == 0:
            not_present += [letters[num]]
        if letter_count[num] == len(possible_subsets[0]):
            present += [letters[num]]
