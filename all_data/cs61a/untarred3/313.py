...
def get_string(word):
    ...
    "*** YOUR CODE HERE ***"
    result = ""
    for string_piece in word:
        result = result + string_piece
    return result
...
def subsets(lst, n):
    ...
    if n == 0:
        ...
        return [[lst[0]] + i for i in subsets(lst[1:], n-1)] + subsets(lst[1:], n)
...
def compatible(guess, score, letter_list):
    ...
    """
    "*** YOUR CODE HERE ***"
    if make_word_master(make_word_from_list(letter_list))(guess) == score:
        return True
    return False
...
def make_deductions(possible_subsets, letters):
    ...
    for letter in letters:
        letter_in_all_subsets = True
        letter_in_no_subsets = False
        for subset in possible_subsets:
            if letter not in subset:
                letter_in_all_subsets = False
            elif letter in subset: 
                letter_in_no_subsets = True
        if letter_in_no_subsets and letter_in_all_subsets:
            present += letter
        elif not letter_in_all_subsets and not letter_in_no_subsets:
            not_present += letter
