...
def make_word_master(goal_word):
    ...
    def helper(guess):
        ...
        if is_valid_guess(guess) == False:
            return not_a_word
        if goal_lst == guess_lst:
            return win_message
        elif len(goal_lst) != len(guess_lst):
...
def filter_subsets(word, score, possible_subsets):
    ...
    "*** YOUR CODE HERE ***"
    result_sets = []
    for subst in possible_subsets:
        if compatible(word, score, subst):
            result_sets += [subst]
    return result_sets
...
def make_deductions(possible_subsets, letters):
    ...
    for target in letters:
        if filter_subsets(target, 1, possible_subsets) == subsets:
            present += target
        elif filter_subsets(target, 1, possible_subsets) == []:
            not_present += target
