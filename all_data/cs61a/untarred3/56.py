...
def make_word_from_list(lst):
    ...
    result = []
    for word in lst:
        result += [make_word_from_string(word)]
    return result
...
def make_word_master(goal_word):
    ...
    "*** YOUR CODE HERE ***"
    def guess_check(x):
        if len(x) != len(goal_word):
            return bad_num_letters
        elif is_valid_guess(x) != True:
            return not_a_word
        elif x == goal_word:
            return win_message
...
def subsets(lst, n):
    ...
    if n == 0:
        ...
        while k > 0: 
            result += [lst[x:n]]
            result2 += [[lst[0]] + [lst[1]] + [lst[2+x]]]
            result3 += [[lst[0]] + [lst[1+x]] + lst[-1:]]
            result4 += [[lst[1]] + [lst[1+x]] + lst[-1:]]
            result5 += [[lst[0]] + [lst[1+x]] + [lst[3]]]
            k -= 1
            x += 1
            n += 1 
    return result + result2[1:n] + result3[1:n] + result4[1:n] + result5[1:2]
...
def filter_subsets(word, score, possible_subsets):
    ...
    result = []
    x = 0
    while x < len(possible_subsets):
        if compatible(possible_subsets[x], score, word):
            result += [possible_subsets[x]]
        x += 1
    return result
...
def make_deductions(possible_subsets, letters):
    ...
    not_present = []
    "*** YOUR CODE HERE ***"
    y = len(possible_subsets)
    for x in letters:
        if x in possible_subsets[0] and x in possible_subsets[1] and x in possible_subsets[2]:
            present += x
        if x not in possible_subsets[0] and x not in possible_subsets[1] and x not in possible_subsets[2]:
            not_present += x
