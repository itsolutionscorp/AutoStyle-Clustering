...
def make_word_master(goal_word):
    ...
    def guess_function(guess):
        y = get_list(guess)
        x = get_list(goal_word)
        if is_valid_guess(y) == False:
            return not_a_word
        elif len(x) != len(y):
            return bad_num_letters
        elif x != y:
            return num_common_letters(x, y)
        else:
            return win_message
...
def subsets(lst, n):
    ...
    """ Use a while loop to run through from 0 to n-1 for possibilities for lst[0].

    2 recursive calls
    power set

    base case subset([],4) => []   if lst == empty
    base case ([1,2,3], 0) => []   if range = 0
    base case subset([], 0) => []   if range = 0 and lst = empty
...
"""
    ...
    if n == 0:
        ...
        first, rest = lst[0], lst[1:]
        return combine([first] , subsets(rest, n-1) ) + subsets(rest,n) 
...
def filter_subsets(word, score, possible_subsets):
    ...
    """
    "*** YOUR CODE HERE ***"
    #score number letters common with possibel subset
    
    x,i = [],0
    while i < len(possible_subsets):
        if num_common_letters(word,possible_subsets[i]) == score:
            x += [possible_subsets[i]]
        i += 1
    return x
...
def make_deductions(possible_subsets, letters):
    ...
    for elem in letters:
        for subset in possible_subsets:
            if elem in subset:
                a += elem 
            elif elem not in subset:
                b += elem 
    
    for apples in a:
        if a.count(apples) == len(possible_subsets):
            present += apples
    for bananas in b:
        if b.count(bananas) == len(possible_subsets):
            not_present += bananas
