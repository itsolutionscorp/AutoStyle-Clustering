...
def get_string(word):
    ...
    """
    "*** YOUR CODE HERE ***"
    if word==make_word_from_string(word):
        return word
    else:
...
def subsets(lst, n):
    ...
    if n==0:
        ...
        for i in range(len(lst)):
            if n==1:
                ...
                for j in subsets(lst[i+1:len(lst)], n-1):
                    newlist+=[[lst[i]]+[j]]
        return newlist
    ...
    #subset([1,2,3],3)
     #   it should retrun [[1,2,3]]
    # subset([1,2,3], 2)-->[[1,2][1,3][2,3]]
    #lst --> [1,2,3], n-->2
    #how to make it smaller?
    #first, rest=lst[0], lst[1:]
    #then, consider n-1
    #subset(rest(lst),n) --> you will get [[2,3]]
    #subset(rest(lst),n-1))--> you will get [[2],[3]]
    #subset(res(lst), n-1) add 1 to all elements --> [[1,2],[1,3]]
...
def make_deductions(possible_subsets, letters):
    ...
    for i in range(len(letters)):
        counts=0
        for elem in possible_subsets:
            if letters[i] in elem:
                counts+=1
        if counts==len(possible_subsets):
            present+=[letters[i]]
        elif counts < 1:
