...
def insert_every(x,lst):
    if lst ==[]:
        return []
    return [[x]+lst[0]] + insert_every(x,lst[1:])
...
def subsets(lst, n):
    ...
    def make_list(lst,n):
        ...
        sublist = []
        while i<len(lst):
            possible = lst[:n-1] + [lst[i]]
            sublist += [possible]
            i+=1
        k = 0
        list_without = lst[:]
        list_without.pop(1)
        return sublist + make_list(lst[1:],n) + make_list(list_without,n)
    
    final_list = []
    for lit in make_list(lst,n):
        if lit not in final_list:
            final_list += [lit]

    return final_list
    
    #Another version - pass doctest, but can't play, reach maximum recursion 
    """    
    if n== -1 :
        return []
    if len(lst) == n:
    ...
    def make_list(lst):
        if lst ==[]:
            ...
            return make_list(lst[1:]) + insert_every(lst[0], make_list(lst[1:]))

    all_possible_list = make_list(lst)

    final_list = []
    for lit in all_possible_list:
        if len(lit) ==n:
            final_list += [lit]
...
def make_deductions(possible_subsets, letters):
    ...
    for lett in letters:
        for sets in possible_subsets:
            if lett in sets:
                num_present +=1
        if num_present == len(possible_subsets):
            present +=[lett]
        if num_present == 0:
            not_present += [lett]
        num_present = 0
