...
def make_word_master(goal_word):
    ...
    True
    """
    def word_master_function(guess_word):
 
        goalL = get_list(goal_word)
        guessL = get_list(guess_word)

        if len(guessL) < 4 or len(guessL) > 7 or len(guessL) != len(goalL):
            return bad_num_letters

        if is_valid_guess(guess_word) == False:
            return not_a_word

        matches = num_common_letters(goal_word, guess_word)
        if matches != len(goalL):
            return matches
