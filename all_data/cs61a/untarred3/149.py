...
def subsets(lst, n):
    ...
    def insert_every(ins, lst): # insert_every(9, [[1], [2]) -> [[1,9], [2,9]]
        if lst==[]:
            return []
        return [[ins]+lst[0]] + insert_every(ins, lst[1:])
...
def compatible(guess, score, letter_list):
    ...
    False
    """
    common = num_common_letters(make_word_from_list(letter_list), guess)
    return common == score
...
def make_deductions(possible_subsets, letters):
    ...
    present = []
    not_present = []
    # I'm sorry this is so convoluted :-(
