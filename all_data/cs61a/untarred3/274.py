...
def make_word_master(goal_word):
    ...
    "*** YOUR CODE HERE ***"
    def play(guess):
        goal_char = get_list(goal_word)
        guess_char = get_list(guess)
        if goal_word == guess:
            return win_message
        elif len(guess_char) != len(goal_char):
...
def subsets(lst, n):
    ...
    if n == 0:
        ...
        smaller_n = subsets(rest, n- 1)
        new = []
        for i in recur:
            new += [[first] + i]
        same_n = subsets(rest, n)
    return same_n + smaller_n
...
def compatible(guess, score, letter_list):
    ...
    """
    "*** YOUR CODE HERE ***"
    common = num_common_letters(guess, get_string(letter_list))
    if common == score:
        return True
    else:
        return False
    ...
    for letter in letters:
        ...
        for subset in possible_subsets:
            if letter not in subset:
                ...
                no_subsets = False
        if all_subsets == False and no_subsets == True:
            not_present += [letter]
        elif all_subsets == True and no_subsets == False:
            present += [letter]
