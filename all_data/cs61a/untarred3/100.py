...
class PairError(Exception):     # I copy-pasted the fxns from hw3 to implement
    pass                        # linked lists in order to represent
                                # word as a singly linked list 
def cons(a, b):                 # with first returning a str and
    def answer(m):              # rest returning a lst
        if m == 'car':
            return a
        elif m == 'cdr':
            return b
...
def make_word_from_string(s):
    ...
    return link(string_as_list[0], (string_as_list[1:]))
...
def make_word_from_list(lst):
    ...
    return make_word_from_string(list_as_string)
...
def get_string(word):
    ...
    'world'
    """
    word_as_list = [first(word)] + rest(word)
...
def subsets(lst, n):
    ...
    if n == 0: 
        ...
        lst1 = [lst[0:1] + i for i in subsets(lst[1:], n-1)] 
        lst2 = subsets(lst[1:], n)
        return lst1 + lst2
...
def compatible(guess, score, letter_list):
    ...
    False
    """
    matching = num_common_letters(make_word_from_list(letter_list), guess)
    if matching == score:
        return True
    else: 
        return False
...
def filter_subsets(word, score, possible_subsets):
    ...
    for i in range(len(possible_subsets)):
        if compatible(word, score, possible_subsets[i]) == True:
            filtered += [possible_subsets[i]]
    return filtered
...
def make_deductions(possible_subsets, letters):
    ...
    for i in range(len(letters)):
        ...
        if temp.count(letters[i]) == len(possible_subsets):
            present += letters[i]
