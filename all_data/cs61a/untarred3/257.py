# CS 61A Spring 2014
# Name: Thomas Wu
# Login: cs61a-kh@cory.eecs.berkeley.edu
...
def make_word_master(goal_word):
    ...
    """
    "*** YOUR CODE HERE ***"
    goal_word=get_list(goal_word) #make sure it's of list type
    def guess_word(guess):
        guess=get_list(guess) #make sure it's of list type
        if not is_valid_guess(guess): # check for valid word
            return not_a_word
        elif len(guess)!=len(goal_word): # check for same length
            return bad_num_letters
        elif guess==goal_word: #check if win!
            return win_message
        else: # check how many letters are in common
...
def subsets(lst, n):
    ...
    def insert_every(elem,lst):
        for i in range(len(lst)):
            lst[i]=[elem]+lst[i]
        return lst
    def powerset(lst,n): #this implementation returns only subsets of length n
        if n<=0:
            return [[]]
        if len(lst)==n:
            return [lst]
            ...
            without_first = powerset(rest,n-1)
            return powerset(rest,n) + insert_every(first, without_first)
    return powerset(lst,n)
...
def compatible(guess, score, letter_list):
    ...
    """
    "*** YOUR CODE HERE ***"
    if score == num_common_letters(get_list(letter_list), get_list(guess)):
        return True
    else:
        return False
...
def filter_subsets(word, score, possible_subsets):
    ...
    for i in range(len(possible_subsets)):
        if compatible(word, score, possible_subsets[i]):
            filtered_subsets+=[possible_subsets[i]]
...
def make_deductions(possible_subsets, letters):
    ...
    for elem in letters:
        is_in_all=0
        for i in range(len(possible_subsets)):
            is_in_all+=possible_subsets[i].count(elem)
        if is_in_all==len(possible_subsets):
            present+=[elem]
        elif is_in_all==0:
            not_present+=[elem]
