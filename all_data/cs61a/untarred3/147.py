...
def subsets(lst, n):
    ...
    if n==0:
        ...
        return helper(lst[0],subsets(lst[1:],n-1)) + subsets(lst[1:],n)

def helper(first_element, rest_lst):
    new=[]
    for x in rest_lst:
        new+= [[first_element] + x]
    return new
...
def make_deductions(possible_subsets, letters):
    ...
    for x in letters:
        ...
        for i in possible_subsets:
            if x in i:
                new_total+=1
        if  len(possible_subsets)==new_total:
            present+=[x]
        elif new_total==0:
            not_present+=[x]
