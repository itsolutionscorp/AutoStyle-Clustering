...
def make_word_from_string(s):
    ...
    """
    result_str = s
    return result_str  #representation of both of them are going to be String
...
def make_word_from_list(lst):
    ...
    result_lst = ''
    while i < length_lst:
        result_lst = result_lst + get_st[i]
        i += 1
    return result_lst
...
def get_string(word):
    ...
    'world'
    """
    return word   #return value going to be string
...
def get_list(word):
    ...
    while i < length_word:
        result_gl += [get_lst[i]]
        i += 1
    return result_gl   #return value going to be list
...
def make_word_master(goal_word):
    ...
    True
    """
    def word_master(word):
        master_list = []
        if len(word) != len(goal_word):
            return bad_num_letters
        elif is_valid_guess(word) == False:
             return not_a_word
        else:
            master_list = master_list + [goal_word] + [word]
            if master_list[0] == master_list[1]:
                return win_message
            else:
                return num_common_letters(master_list[0], master_list[1])
    return word_master
...
def subsets(lst, n):
    ...
    if n == 0:
        ...
        for elem in subsets(lst[1:], n-1):
            total_sub = total_sub + [[first_lst] + elem]
        return total_sub + subsets(lst[1:], n)
...
def compatible(guess, score, letter_list):
    ...
    False
    """
    letter_string = make_word_from_list(letter_list)
    if score == num_common_letters(make_word_from_string(guess), letter_string):
        return True
    else:
        return False
...
def filter_subsets(word, score, possible_subsets):
    ...
    while i < len(possible_subsets):
        if compatible(word, score, make_word_from_list(possible_subsets[i])) == True:
            result_filter = result_filter + [possible_subsets[i]]
        i += 1
...
def make_deductions(possible_subsets, letters):
    ...
    not_present = []
    a = 0
    expected = len(possible_subsets)
    while a < len(letters):
        actual = 0
        for lst in possible_subsets:
            if letters[a] in lst:
                actual += 1      
        if actual == expected:
            present = present + [letters[a]]
        elif actual == 0:
            not_present = not_present + [letters[a]]
        a += 1
