...
def subsets(lst, n):
    ...
    def get_index(m, n):
        if n == 0:
            ...
            return [lst]
        output = []
        for lst in get_index(m - 1, n - 1):
            output += [lst + [i] for i in range(lst[-1] + 1, m)]
        return output    
    if n > m:
        return []
    return [[lst[i] for i in index_set] for index_set in get_index(len(lst), n)]
