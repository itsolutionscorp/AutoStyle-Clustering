...
def num_common_letters(goal_word, guess):
    ...
    0
    >>> num_common_letters(mwfs('steal'), mwfl(['l', 'e', 'a', 's', 't']))
    5
    """
...
def make_word_master(goal_word):
    ...
    True
    """
    def make_word_helper(guess):
        if is_valid_guess(guess):
            guess_lst = get_list(guess)
            goal_lst = get_list(goal_word)
            if len(guess_lst) != len(goal_lst):
                return bad_num_letters
            else:
                if guess_lst == goal_lst:
                    return win_message
                else:
                    return num_common_letters(goal_word, guess)
        else:
            return not_a_word
    return make_word_helper   
...
def subsets(lst, n):
    ...
    if lst == []:
        ...
        without_first = subsets(rest, n-1)
        without_first2 = subsets(rest, n)
        return without_first2 + insert_every(first, without_first)
...
def filter_subsets(word, score, possible_subsets):
    ...
    """
    new_list = []
    for element in possible_subsets:
        guess = make_word_from_list(element)
        letter_list = get_list(word)
        if compatible(guess, score, letter_list):
            new_list += [element]
    return new_list
...
def make_deductions(possible_subsets, letters):
    ...
    for element in letters:
        missing = False
        not_missing = True
        for lst_element in possible_subsets:
            if element not in lst_element:
                not_missing = False 
            if element in lst_element:
                missing = True
        if missing == False:
            not_present += [element]
        elif not_missing == True:
            present += [element]
