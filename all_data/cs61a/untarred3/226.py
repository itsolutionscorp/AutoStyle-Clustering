...
def subsets(lst, n):
    ...
    >>> three_subsets = subsets(list(range(3)), 3)
    ...
    if lst==[] and n==0:        #subsets([],0)=[[]]
        return [[]]
    if n==0:                    #subsets([1,2,3],0)=[[]]
        return [[]]
    if lst==[]:
        return []               #subsets([],4)=[]
        
    # subs=subsets(lst[1:],n-1)
    # for sub in subs:
        # sub=[lst[:1]]+sub
    
    return subsets(lst[1:],n) + [lst[:1]+sub for sub in subsets(lst[1:],n-1)]
...
def make_deductions(possible_subsets, letters):
    ...
    for l in letters:
        remove_from_present, remove_from_not_present=False,False    
        for set in possible_subsets:
            if l not in set:
                remove_from_present=True
            elif l in set:
                remove_from_not_present=True
        if not remove_from_present:
            present+=[l]
        elif not remove_from_not_present:
            not_present+=[l]
