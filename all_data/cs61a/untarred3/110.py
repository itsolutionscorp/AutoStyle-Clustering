...
def make_deductions(possible_subsets, letters):
    ...
    while x <= (len(letters) - 1):
        ...
        for eachsubset in possible_subsets:
            for eachletter in eachsubset:
                if item == eachletter:
                    return True
                else: 
                    return False
            if possible_subsets.count(True) == len(possible_subsets):
                present += [item]
            elif possible_subsets.count(False) == len(possible_subsets):
                not_present += [item]
            else:
                return None
        x += 1
