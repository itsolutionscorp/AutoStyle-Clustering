...
def make_word_master(goal_word):
    ...
    def word_master(guess):
        ...
        elif num_common_letters(letters, guess) <= 1:
            return not_a_word
        elif num_common_letters(goal_word, guess) <= len(goal_word) and goal_word != guess:
...
def subsets(lst, n):
    ...
    """
    "*** YOUR CODE HERE ***"
    def add_first(first, sub_list):
        sub_new_list = []
        for i in range(len(sub_list)):
            sub_new_list = sub_new_list + [[first] + sub_list[i]]
        return sub_new_list
    ...
    if len(lst) < n:
        ...
        return subsets(lst[1:], n) + add_first(lst[0], subsets(lst[1:], n-1))
...
def compatible(guess, score, letter_list):
    ...
    """
    "*** YOUR CODE HERE ***"
    return make_word_master(make_word_from_list(letter_list))(guess) == score
...
def filter_subsets(word, score, possible_subsets):
    ...
    for i in range(len(possible_subsets)):
        if compatible(word, score, possible_subsets[i]):
            f_subsets = f_subsets + [possible_subsets[i]]
    return f_subsets
...
def make_deductions(possible_subsets, letters):
    ...
    for i in range(len(letters)):
        ...
        flag_not_present = True
        for j in range(len(possible_subsets)):
            if letter in possible_subsets[j]:
