...
def compatible(guess, score, letter_list):
    ...
    """
    "*** YOUR CODE HERE ***"
    if num_common_letters(make_word_from_string(letter_list), make_word_from_string(guess)) != score:
        return False
    return True
...
def filter_subsets(word, score, possible_subsets):
    ...
    for i in range(len(possible_subsets)):
        if num_common_letters(make_word_from_string(word), possible_subsets[i]) == score:
            temp_list += [possible_subsets[i]]
    return temp_list
...
def make_deductions(possible_subsets, letters):
    ...
    for i in letters:
        for j in range(len(possible_subsets)):
            if i not in possible_subsets[j]:
                temp_present += i
            if i in possible_subsets[j]:
                temp_not_present += i
    for k in letters:
        if k not in temp_present:
            present += k
        if k not in temp_not_present:
            not_present += k
