...
def make_word_master(goal_word):
    ...
    def f(guess):
        if len(get_list(guess)) != len(get_list(goal_word)):
            ...
            return win_message
        elif get_list(guess).count(get_list(guess)[0]) > 3:
            return not_a_word 
        else:
            return num_common_letters(goal_word, guess)
    return f

def deep_list_insert(lst, a):
        if lst == []:
            return []
        else:
            lst[0].insert(0, a)
            return [lst[0]] + deep_list_insert(lst[1:], a)
...
def subsets(lst, n):
    ...
    if n == 0:
        ...
        return [lst[-n:]] + subsets(lst[1:], n) + deep_list_insert(subsets(lst[1:], x), lst[0])
     # inserts a into the first of every sublist
# only 1 helper function ^^^ this one
...
def compatible(guess, score, letter_list):
    ...
    False
    """
    guess_combos = subsets(sorted(get_list(guess)), score)
    list_combos = subsets(sorted(letter_list), score)
    def helper(guess_combos, score, list_combos):
        if guess_combos == []:
            return False
        elif guess_combos[0] in list_combos:
            return True
        else:
            return helper(guess_combos[1:], score, list_combos)
    return helper(guess_combos, score, list_combos)
...
def filter_subsets(word, score, possible_subsets):
    ...
    if possible_subsets == []:
        return []
    elif possible_subsets[0] == get_list(word):
        return filter_subsets(word, score, possible_subsets[1:])
    elif compatible(make_word_from_list(possible_subsets[0]), score, get_list(word)):
        return [possible_subsets[0]] + filter_subsets(word, score, possible_subsets[1:])
    else:
        return filter_subsets(word, score, possible_subsets[1:])


def in_all(letter, sublists):
    if sublists == []:
        return True
    elif letter in sublists[0]:
        return in_all(letter, sublists[1:])
    else:
        return False
def not_in_all(letter, sublists):
    if sublists == []:
        return True
    elif letter not in sublists[0]:
        return not_in_all(letter, sublists[1:])
    else:
        return False
...
def make_deductions(possible_subsets, letters):
    ...
    there = []
    not_there = []
    possible = possible_subsets
    for letter in letters:
        if in_all(letter, possible_subsets):
            there += [letter]
    for letter in letters:
        if not_in_all(letter, possible_subsets):
            not_there += [letter]
