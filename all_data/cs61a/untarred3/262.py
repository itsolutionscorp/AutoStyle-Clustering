...
def subsets(lst, n):
    ...
    if lst == []:
        ...
        new_list = subsets(lst[1:], n - 1)
        return subsets(lst[1:], n) + insert_every(lst[0], new_list)
...
def filter_subsets(word, score, possible_subsets):
    ...
    for letters in possible_subsets:
        if compatible(word, score, letters): 
            subset_list.append(letters) 
    return subset_list 
