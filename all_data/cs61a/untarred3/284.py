...
def compatible(guess, score, letter_list):
    ...
    False
    """
    guess=make_word_from_string(guess)
...
def filter_subsets(word, score, possible_subsets):
    ...
    """
    a,result=0,[]
    while a<len(possible_subsets):
        if compatible(word,score,possible_subsets[a])==True:
            result=result+[possible_subsets[a]]
        a+=1
    return result
