        ...
        if len(guess) == len(goal):
            if is_valid_guess(guess):
                if goal == guess:
                    ...
                    return num_common_letters(goal, guess)
            else:
                return not_a_word
        else:
...
def subsets(lst, n):
    ...
    [2, 3, 4]
    """
    lst0 = get_list(make_word_from_string(lst))

    def combo_maker(element, lst1):
        length = n
        if lst1 == []:
            return []
        #if len([element] + lst1[0]) == length:
            #return [[element] + lst1[0]] + combo_maker(element, lst1[1:])
        else:
            #return [] + combo_maker(element, lst1[1:])
            ##I tried to filter through this before I returned the combinations, but
            #it doesn't seem possible with recursion.
            return [[element] + lst1[0]] + combo_maker(element, lst1[1:])
            
  

    def produce_everything(lst2):
        ...
        if lst2 == []:
            ...
            first = lst2[0]
            rest = lst2[1:]
            no_first = produce_everything(rest)
            return no_first + combo_maker(first, no_first)
    combinations = []
    for elem in produce_everything(lst0):
        if len(elem) == n:
            combinations = combinations + [elem]
...
def compatible(guess, score, letter_list):
    ...
    if num_common_letters(word, guess) == score:
        return True
    return False
...
def make_deductions(possible_subsets, letters):
    ...
    for letter in letters:
        ...
        for subset in possible_subsets:
            if compatible(letter, 1, subset) == False:
                counter +=1
        if counter == 0:
            present = present + [letter]
        if counter == len(possible_subsets):
            not_present = not_present + [letter]
