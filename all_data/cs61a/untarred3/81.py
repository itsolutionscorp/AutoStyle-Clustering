...
def make_word_from_string(s):
    ...
    """
    def word(sel):
        if sel == 'string': return s
        elif sel == 'lst': return [s[i] for i in range(len(s))]
        else: return 'error only takes argument string or lst'
    return word
...
def make_word_master(goal_word):
    ...
    True
    """
    def word_master(guess_word):
        goal_str, guess_str = get_string(goal_word), get_string(guess_word)
        if goal_str == guess_str: return win_message
        elif len(goal_str) != len(guess_str): return bad_num_letters
        elif not is_valid_guess(guess_word): return not_a_word
        else: return num_common_letters(goal_word, guess_word)
    return word_master
...
def compatible(guess, score, letter_list):
    ...
    num_duplicates = sum([guess_list.count(g) > 1 for g in guess_list])
    if len(guess_list) != len(letter_list): return bad_num_letters
    else: return score == (sum([ g in letter_list for g in guess_list ]) - num_duplicates)
...
def filter_subsets(word, score, possible_subsets):
    ...
    [['a', 'b', 'e', 'l', 's'], ['b', 'l', 'e', 's', 't']]
    """
    return [sub for sub in possible_subsets if compatible(make_word_from_list(sub),score,get_list(word))]
...
def make_deductions(possible_subsets, letters):
    ...
    ['d', 'f']
    """
    present = [ letter for letter in letters if all(letter in lst for lst in possible_subsets) ]
    not_present = [ letter for letter in letters if all(letter not in lst for lst in possible_subsets) ]
