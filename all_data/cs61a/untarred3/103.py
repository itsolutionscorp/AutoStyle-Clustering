...
def make_word_master(goal_word):
    ...
    """
    "*** YOUR CODE HERE ***"
    word = make_word_from_string('type test')
    if type(goal_word) == type(''):
        goal_word = make_word_from_string(goal_word)
    elif type(goal_word) != type(word):
        return 'Not valid "word" data type.'
...
def subsets(lst, n):
    ...
    if n <= 0 or n > len(lst):
        ...
        if n == 1:
            for elem in lst:
                subset_list.append([elem])
        else:
            working_lst = list(lst)
            for elem in lst:
                working_lst.remove(elem)
                for subset in subsets(working_lst,n-1):
                    elem_subset = [elem]
                    elem_subset.extend(subset)
                    subset_list.append(elem_subset)
        return subset_list
...
def filter_subsets(word, score, possible_subsets):
    ...
    "*** YOUR CODE HERE ***"
    filtered = []
    for letter_set in possible_subsets:
        if compatible(word, score, letter_set):
            filtered.append(letter_set)
    return filtered
