...
def subsets(lst, n):
    ...
    new_list = []
    sub1, sub2 = subsets(rest, n - 1), subsets(rest, n)
    for subset in sub1:
        new_list += [[first] + subset]
    return new_list + sub2

    #length of subset must equal 3
...
def filter_subsets(word, score, possible_subsets):
    ...
    for subset in possible_subsets:
        if num_common_letters(make_word_from_list(subset), word) == score:
            subset_score += [subset]
    return subset_score
