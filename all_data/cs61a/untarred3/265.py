...
def make_word_master(goal_word):
    ...
    def word_master(guess):
        if (len(guess[0]) < 4 or len(guess[0]) > 7)\
        or (len(goal_word[0]) != len(guess[0])):
            return bad_num_letters
        elif not is_valid_guess(guess):
            return not_a_word
        elif num_common_letters(goal_word, guess) == len(goal_word[0]):
            if goal_word[0] == guess [0]:
                return win_message
            else:
...
def subsets(lst, n):
    ...
    def add_el(x, lst_to_edit):
        y = []

        for el in lst_to_edit:
            y += [[x]+el]

        return y

    return add_el(lst[0], subsets(lst[1:], n-1)) + subsets(lst[1:],n)
...
def compatible(guess, score, letter_list):
    ...
    s1 = make_word_from_list(letter_list)
    
    if num_common_letters(guess,s1) == score:
        return True
    else:
        return False
...
def make_deductions(possible_subsets, letters):
    ...
    for el in letters:
        for c in possible_subsets:
            for l in c:
                if el == l:
                    counter += 1
        if counter == 0:
            not_present += el
        elif counter == len(possible_subsets):
            present += el
        counter = 0
