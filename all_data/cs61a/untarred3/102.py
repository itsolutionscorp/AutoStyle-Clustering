...
def subsets(lst, n):
    ...
    return subsets(lst[1:], n) + helper(lst[0], subsets(lst[1:], n - 1))

def helper(elem, lst):
    #Returns a list of list that contains elem.
...
def compatible(guess, score, letter_list):
    ...
    False
    """
    letter_list = make_word_from_list(letter_list)
...
def make_deductions(possible_subsets, letters):
    ...
    "*** YOUR CODE HERE ***"
    for char in letters:
        if check(possible_subsets, char):
            present += [char]
        if uncheck(possible_subsets, char):
            not_present += [char]
    return present, not_present

def check(possible_subsets, char):
    #checks if a character is in all possible subsets of possible_subsets
    for cur_sub in possible_subsets:
        if char not in cur_sub:
            return False
    return True

def uncheck(possible_subsets, char):
    #checks if a character is not in all possible subsets of possible_subsets
    for cur_sub in possible_subsets:
        if char in cur_sub:
            return False
    return True
