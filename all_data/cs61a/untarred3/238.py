...
def subsets(lst, n):
    ...
    if lst == []:
        ...
        rec_result += subsets(lst[1:], n - 1)
        for e in rec_result:
            new_list += [lst[0]] + rec_result
        new_list += subsets(lst[1:], n)
    return new_list
...
def filter_subsets(word, score, possible_subsets):
    ...
    """
    new_list = []
    for items in possible_subsets:
        if compatible(word, score, items) == True:
            new_list += [items] 
    return new_list
