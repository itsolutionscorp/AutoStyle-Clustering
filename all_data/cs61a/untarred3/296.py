...
def subsets(lst, n):
    ...
    ret = []
    def helper(curr, left, left_index):
        if left == 0:
            ret.append(curr)
        else:
            for i, elem in enumerate(lst):
                ...
                if i > left_index:
                    ...
                    helper(new_list, left - 1, i)
    for i, elem in enumerate(lst):
        new_lst = []
        new_lst.append(elem)
        helper(new_lst, n-1, i)
    return ret
...
def compatible(guess, score, letter_list):
    ...
    """
    "*** YOUR CODE HERE ***"
    compatible_words = num_common_letters(make_word_from_list(letter_list), guess)
    if compatible_words == score:
        return True
    else:
        return False
...
def make_deductions(possible_subsets, letters):
    ...
    present = []
    not_present = []
    for x in range(len(possible_subsets)):
        if type(possible_subsets[x]) == []:
            for y in range(len(letters)):
                if possible_subsets[x][y] in letters:
                    present += [possible_subsets[x][y]]
        else:
            not_present += [possible_subsets[x]] 
