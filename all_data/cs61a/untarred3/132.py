...
def make_word_master(goal_word):
    ...
    """
    "*** YOUR CODE HERE ***"
    def guess_word_master(guess_word):
...
def subsets(lst, n):
    ...
    if n == 0:
        ...
        rejectList = subsets(lst[1:], n) 
        takelst = subsets(lst[1:], n-1) 
        finallst = []
        for elem in takelst:
            finallst += [[lst[0]] + elem]
    return finallst + rejectList
...
def compatible(guess, score, letter_list):
    ...
    letter_string = make_word_from_string(letter_list)
    if num_common_letters(letter_string, guess) == score:
        return True
    else:
        return False
...
def filter_subsets(word, score, possible_subsets):
    ...
    for lst in possible_subsets:
        if num_common_letters(word, get_string(lst)) == score:
            newlist += [lst]
    return newlist
