...
def num_common_letters(goal_word, guess):
    ...
    letters_checked=[]  #this is unecessary if goal_word alwys gets input
                        #before the guess but with it, regardless of 
                        #whether or not goal_word has no repeats
                        #it will still  yield the desired result
    letters_in_common=0
...
def make_deductions(possible_subsets, letters):
    ...
    for letter in letters:
        ...
        in_every=True
        for subset in possible_subsets:
            if not_in_any and (letter in subset):   
                not_in_any=False
            if in_every and not(letter in subset):
                in_every=False
        if in_every:
            present+=[letter]
        if not_in_any:
