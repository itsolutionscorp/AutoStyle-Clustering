...
def make_word_master(goal_word):
    ...
    def word_master(guess):
        ...
        if is_valid_guess(string_guess) == False:
            return not_a_word
        elif len(string_guess) != len(string_goal_word):
            return bad_num_letters
        elif string_goal_word == string_guess:
            return win_message
        else:
            return num_common_letters(string_goal_word,string_guess)
    return word_master

def insert_every(num, lst_of_lst):
    """
    ...
    if lst_of_lst == []:
        return []
    first, rest = lst_of_lst[0], lst_of_lst[1:]
    return [[num]+ first] + insert_every(num, rest)
...
def subsets(lst, n):
    ...
    def power_sets(lst):
        ...
        without_first = power_sets(rest)
        return without_first + insert_every(first,without_first)
    for subsets in power_sets(lst):
        if len(subsets) == n:
            all_subsets += [subsets]
    return all_subsets
