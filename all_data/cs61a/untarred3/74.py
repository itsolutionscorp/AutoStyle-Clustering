...
def subsets(lst, n):
    ...
    for i in range(len(lst)):
        pow_set = [inner_list + [lst[i]] for inner_list in pow_set] + pow_set
    return [subset for subset in pow_set if len(subset) == n]
...
def filter_subsets(word, score, possible_subsets):
    ...
    [['a', 'b', 'e', 'l', 's'], ['b', 'l', 'e', 's', 't']]
    """
    return [lst for lst in possible_subsets if compatible(word, score, lst)]
...
def make_deductions(possible_subsets, letters):
    ...
    for letter in letters:
        if [x for x in possible_subsets if letter in x] == possible_subsets:
            present = present + [letter]
        if [x for x in possible_subsets if letter not in x] == possible_subsets:
