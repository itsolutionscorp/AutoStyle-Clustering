...
def insert(elem, lst):
    if lst==[]:
        ...
        return [[elem]+lst[0]]+insert(elem, lst[1:])
...
def subsets(lst, n):
    ...
    if lst==[]:
        return []
    return insert(lst[0], subsets(lst[1:], n-1))+subsets(lst[1:], n)
...
def filter_subsets(word, score, possible_subsets):
    ...
    if compatible(word, score, possible_subsets[0]):
        return final_lst+[possible_subsets[0]]+filter_subsets(word, score, possible_subsets[1:])
    else:
        return final_lst+filter_subsets(word, score, possible_subsets[1:])
...
def make_deductions(possible_subsets, letters):
    ...
    temp1_list=[]
    count=0
    a=possible_subsets[0]
    b=possible_subsets[0]

    
    while count<(len(possible_subsets)-1):
        b=possible_subsets[count+1]
    ...
    for element_s in letters:
        if element_s in temp1_list:
            present.append(element_s)
    temp1_list=[]
    
    for elem in range(len(possible_subsets)):
        temp1_list+=possible_subsets[elem]
        
    temp_list=list(set(temp1_list))
    temp_list=sorted(temp_list)
            
    for element_d in letters:
        if element_d not in temp_list:
            not_present.append(element_d)
