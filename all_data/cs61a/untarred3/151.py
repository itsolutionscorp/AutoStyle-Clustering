...
def make_word_from_list(lst):
    ...
    Takes in a list and outputs a string.
    """
    # i, result = 0, ""
...
def make_word_master(goal_word):
    ...
    def word_master(guess):
        goal, guess1 = get_list(goal_word), get_list(guess)
        if len(goal) != len(guess1):
...
def subsets(lst, n):
    ...
    if n == 0:
        ...
        total1 = subsets(lst[1:], n - 1) 
        total2 = subsets(lst[1:], n)
        for subset in total1:     
            total += [[lst[0]] + subset]
        return total + total2
...
def filter_subsets(word, score, possible_subsets):
    ...
    for subset in possible_subsets:
        if compatible(make_word_from_list(subset), score, get_list(word)):
            total = total + [subset] 
    return total 
