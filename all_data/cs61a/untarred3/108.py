...
def subsets(lst, n):
    ...
    """
    new = []
    for element in list(combinations(lst, n)):
...
def filter_subsets(word, score, possible_subsets):
    ...
    for element in possible_subsets:
        if compatible(word, score, element) == True:
            work += [element]
    return work
...
def make_deductions(possible_subsets, letters):
    ...
    present = []
    not_present = []
    subs = len(possible_subsets)
    counter = 0
    full_list = []
    while counter < subs:
        full_list += possible_subsets[counter]
        counter += 1
    for element in letters:
        if element not in full_list:
            not_present += [element]
        if full_list.count(element) == subs:
            present += [element]
