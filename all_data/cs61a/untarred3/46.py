...
def make_word_master(goal_word):
    ...
    True
    """
    def guess_time(word1):
        if is_valid_guess(word1) == False:
            return not_a_word
        if word1 == goal_word:
            return win_message
        return num_common_letters(goal_word, word1)
    return guess_time
...
def subsets(lst, n):
    ...
    if n == 0: 
        ...
        return [] 
    def add_elem_every(first_term, leftovers):
        for h in range(len(leftovers)):
            leftovers[h] = [first_term] + leftovers[h]
        return leftovers
    return add_elem_every(lst[0], subsets(lst[1:], n-1)) + subsets(lst[1:], n)
...
def make_deductions(possible_subsets, letters):
    ...
    for every_letter in letters: 
        for each_subset in possible_subsets:
            for letter in each_subset:
                if every_letter == letter:
                    possible += every_letter
                    if possible.count(every_letter) == len(possible_subsets):
                        present += [every_letter]
            if every_letter not in each_subset:
                not_possible += every_letter
                if not_possible.count(every_letter) == len(possible_subsets):
                    not_present += [every_letter]
