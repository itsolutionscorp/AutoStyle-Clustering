...
def make_word_from_list(lst):
    ...
    if (len(lst) == 1):
        ...
        y=lst[0]
        return y + make_word_from_list(lst[1:])
...
def make_word_master(goal_word):
    ...
    def messages(guess):
        ...
        guess_temp_word = get_string(guess)
        goal_temp_word = get_string(goal_word)
        guess_string = get_list(guess)
        goal_string= get_list(goal_word)
        if is_valid_guess(guess) == False:
            return not_a_word
        elif (len(guess_string)  == 0) or (len(guess_string) != len(goal_string)):
