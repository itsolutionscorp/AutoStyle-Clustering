...
def make_word_master(goal_word):
    ...
    """
    "*** YOUR CODE HERE ***"
    length = len(get_list(goal_word))
    goal_string = get_string(goal_word)

    def func1(guess):
        if not(is_valid_guess(guess)):
            return not_a_word
        elif length != len(get_list(guess)):
            return bad_num_letters
        elif get_string(guess) == goal_string:
            return win_message
        else:
...
def subsets(lst, n):
    ...
    fac = math.factorial
    master_lst = lst 
    number_comb = fac(len(master_lst))/fac(n)/(fac(len(master_lst)-n))
    py_lst = []
       
    def make_subset(indeces,master_lst):
        sub_set = []
        for i in indeces:
            #if i == n-1
            sub_set = sub_set + [master_lst[i]]  
        return sub_set

    indeces = list(range(n)) #[0,1,2]
    count=1
    while count <= number_comb:
        py_lst = py_lst + [make_subset(indeces,master_lst)]
        indeces[n-1] = indeces[n-1]+1 
        
        while len(master_lst) in indeces:
            x = indeces.index(len(master_lst))
            indeces[x-1] = indeces[x-1]+1
            for i in range(x,n):
...
def compatible(guess, score, letter_list):
    ...
    """
    "*** YOUR CODE HERE ***"
    n = num_common_letters(make_word_from_list(letter_list),guess)
    if n == score:
        return True
    else:
        return False
...
def filter_subsets(word, score, possible_subsets):
    ...
    "*** YOUR CODE HERE ***"
    update = []
    #word1 = make_word_from_list(word)
    for subset in possible_subsets:
        if compatible(word, score, subset):
            if subset != get_list(word):
                update = update + [subset]
    return update
