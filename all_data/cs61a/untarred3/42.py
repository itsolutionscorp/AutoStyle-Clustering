...
def make_word_master(goal_word):
    ...
    """
    "*** YOUR CODE HERE ***"
    def guess_master(word):
        if get_string(goal_word) == get_string(word):
            return win_message
        if is_valid_guess(word) == False:
            return not_a_word
        l = len(get_list(goal_word))
        if l != len(get_list(word)):
            return bad_num_letters
        cor_num = num_common_letters(goal_word, word)
        return cor_num
    return guess_master
...
def compatible(guess, score, letter_list):
    ...
    """
    "*** YOUR CODE HERE ***"
    tmp_word = make_word_from_list(letter_list)
    if num_common_letters(tmp_word, guess) == score:
        return True
    else:
        return False
...
def make_deductions(possible_subsets, letters):
    ...
    for i in range(len(possible_subsets)):
        for j in range(len_l):
            if (letters[j] in possible_subsets[i]) == True:
                if (letters[j] in not_present) == True:
                    not_present.remove(letters[j])
            else:
                if (letters[j] in present) == True:
                    present.remove(letters[j])
...
def play_game(word=""):
    ...
    len_of_word = len(get_string(word))

    print('Playing with a', len_of_word, 'letter word.')
    word_master = make_word_master(word)
    possible_subsets = subsets(letters, len_of_word)
    result = ''
