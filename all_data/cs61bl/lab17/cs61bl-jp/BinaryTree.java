...
public class BinaryTree<T> implements Iterable<T> {
    ...
    public BinaryTree(ArrayList<T> preorder, ArrayList<T> inorder) {
    	myRoot = bHelper(myRoot, preorder, inorder);
    }
    
    private TreeNode bHelper(TreeNode t, ArrayList<T> preorder, ArrayList<T> inorder) {
    	if (preorder == null || inorder == null || preorder.size() == 0 || inorder.size() == 0) {
    		return null;
    	}
		t = new TreeNode(preorder.get(0));
		int indexIn = inorder.indexOf(t.myItem);
		ArrayList<T> right = new ArrayList<T>();
		ArrayList<T> left = new ArrayList<T>();
		if (indexIn == inorder.size() - 1) {
			right = null;
		...
		inorder.remove(t.myItem);
		preorder.remove(t.myItem);
		t.myLeft = bHelper(t.myLeft, preorder, left);
		t.myRight = bHelper(t.myRight, preorder, right);
		return t;
    }
    ...
    protected class TreeNode {
        ...
        public int determineSize() {
        	if (myLeft == null && myRight == null) {
        		...
        		return 1 + myRight.determineSize();
        	} else if (myRight == null) {
        		return 1 + myLeft.determineSize();
        	} else {
        		return 1 + myLeft.determineSize() + myRight.determineSize();
        	}
        }
