...
public class BinaryTree<T> implements Iterable<T> {
	...
	private TreeNode helper(List<T> preOrder, List<T> inOrder) {
		...
		List<T> rightPreOrder = preOrder.subList(leftInOrder.size() + 1, preOrder.size());

		return new TreeNode(myitem, helper(leftPreOrder, leftInOrder), helper(rightPreOrder, rightInOrder));
	...
	public static void main(String[] args) {
		...
		System.out.println(searchTree.kthLargest(4));
	...
	protected class TreeNode {
		...
		public Comparable kthLargest(int k) {
			System.out.println(size(myRight));
			if(size(myRight)>k && myRight!=null){
				return myRight.kthLargest(k);
			}else if (size(myRight)==k){
				return (Comparable) myItem;
			}else if(size(myRight)+1<=k && myLeft!=null){
			       return myLeft.kthLargest(k);
			}else{
				return (Comparable) myItem;	
			}
		}
