...
public class SimpleDate extends Date implements Comparable<SimpleDate> {
	...
	public SimpleDate (int month, int dayOfMonth) {
		super(0, month, dayOfMonth);
	}
	
	public int compareTo(SimpleDate muhDate) {
		return dayOfYear() - muhDate.dayOfYear();
	}
	...
	public Date nextDate() {
    	if (month() == 12 && dayOfMonth() == 31) {
    		return new SimpleDate(1, 1);
    	} else if (dayOfMonth() == monthLengths[month() - 1]) {
    		return new SimpleDate(month() + 1, 1);
    	} else {
    		return new SimpleDate(month(), 1);
    	}
	}
