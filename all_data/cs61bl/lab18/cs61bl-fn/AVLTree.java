public class AVLTree {
	...
	public void checkRotate() {
		if (!isAlmostBalanced()) {
			if (myRoot.balanceFactor() == -2) {
				myRoot = myRoot.rotateLeft(myRoot);
			} else {
				myRoot = myRoot.rotateRight(myRoot);
			}
		}
	}
	...
	private class NonEmptyTreeNode extends TreeNode {
		...
		public void insert(int item) {
			if (item() == item) {
				...
				if (!right().equals(theEmptyTreeNode)){
					right().insert(item);
				} else {
					setRight(new NonEmptyTreeNode(item));
				}
			} else {
				if (!left().equals(theEmptyTreeNode)){
					left().insert(item);
				} else {
		...
		public void checkBranchRotate() {
			if (!left().isAlmostBalanced()) {
				if (left().balanceFactor() == -2) {
					setLeft(rotateLeft(left()));
				} else {
					setLeft(rotateRight(left()));
				}
			}
			if (!right().isAlmostBalanced()) {
				if (right().balanceFactor() == -2) {
					setRight(rotateLeft(right()));
				} else {
					setRight(rotateRight(right()));
				}
			}
		}
