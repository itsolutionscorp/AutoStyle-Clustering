...
public class AVLTree {
	...
	private class NonEmptyTreeNode extends TreeNode {
		...
		public void insert(int item) {
			...
			//call to rotate left or rotate right
			// take care of cases, rotate left or right, then check isAlmostBalanced, if false rotate a second time the other way
			if(!isAlmostBalanced()) {
				if (balanceFactor() > 1) {
					rotateRight();						
					if (!isAlmostBalanced()) {
		...
		public void insertHelper(int item) {
			if (left().isEmpty() && item < item()) {
				setLeft(new NonEmptyTreeNode(item, this));
			} else if (this.myRight.isEmpty() && item > item()) {
				setRight(new NonEmptyTreeNode(item, this));
			} else if (item > myItem) {
				right().insertHelper(item);
			} else if (item < myItem) {
				left().insertHelper(myItem);
			}
		}
		...
		public TreeNode rotateLeft() {
			TreeNode holder = right().left();
			right().setLeft(right().parent());
			setParent(right());
			right().parent().setRight(holder);
			right().setParent(null);
			return right();			
		}
		
		public TreeNode rotateRight(){
			TreeNode holder = left().right();
			left().setRight(left().parent());
			setParent(left());
			left().parent().setLeft(holder);
			left().setParent(null);
			return left();
		}
