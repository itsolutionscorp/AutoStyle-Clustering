...
public class AVLTree {
	...
	private class NonEmptyTreeNode extends TreeNode {
		...
		public void insert(int item) {
			// TODO Your code here!
			if (myItem>item) {
				if (myLeft.equals(theEmptyTreeNode)) {
					NonEmptyTreeNode result = new NonEmptyTreeNode(item);
					this.setLeft(result);
					result.setParent(this);
					for (TreeNode temp = this;temp!=theEmptyTreeNode;temp=temp.parent()){
				...
				} else myLeft.insert(item);
			} else {
				if (myRight.equals(theEmptyTreeNode)) {
					NonEmptyTreeNode result = new NonEmptyTreeNode(item);
				...
				} else myRight.insert(item);
			}
		}
		@Override
		public void rotateRight() {
			TreeNode temp = myLeft.right();
			TreeNode originalParent = myParent;
			this.setParent(myLeft);
			myLeft.setRight(this);
			myLeft.setParent(originalParent);
			if (originalParent!=theEmptyTreeNode) {
				originalParent.setRight(myLeft);
			}
			this.setLeft(temp);
			temp.setParent(this);
			if (myRoot==this) {
		...
		public void rotateLeft() {
			TreeNode temp = myRight.left();
			TreeNode originalParent = myParent;
			this.setParent(this.myRight);
			myRight.setLeft(this);
			myRight.setParent(originalParent);
			if (originalParent!=theEmptyTreeNode) {
				originalParent.setRight(myRight);
			}
			this.setRight(temp);
			temp.setParent(this);
			if (myRoot==this) {
