...
public class AVLTree {
	...
	private class NonEmptyTreeNode extends TreeNode {
		...
		@Override
		public void insert(int item) {
			TreeNode rightRemember = this.right();
			TreeNode leftRemember = this.left();
			if (item < myItem) {
				if (myLeft.isEmpty()) {
			...
			TreeNode p;
			if (!this.parent().isEmpty() && !this.parent().equals(rightRemember) && !this.parent().equals(leftRemember)) {
				p = this.parent();
				if (!p.isAlmostBalanced()) {
					if (p.right().height() > p.left().height()) {
						if (item > p.right().item()){
							p.right().rotateRight();
						}
						p.rotateLeft();
					} else {
		...
		public void rotateLeft() {
			if (this.myParent.isEmpty()){
				myRoot = this.myRight;
			} else if (this.isAlmostBalanced()){
				this.myRight.setParent(this.parent());
				...
				this.myParent.setRight(this.myRight);
			}
			this.setParent(this.myRight);
			this.myRight.setParent(this.myParent.parent());
			this.setRight(this.myRight.left());
			this.myRight.setParent(this);
			this.myParent.setLeft(this);
		}
		...
		public void rotateRight() {
			if (this.myParent.isEmpty()){
				...
				this.myLeft.setParent(this.parent());
				...
				this.myParent.setLeft(this.myLeft);
			}
			this.setParent(this.myLeft);
			this.myLeft.setParent(null);
			this.setLeft(this.myLeft.right());
			this.myLeft.setParent(this);
			this.myParent.setRight(this);
		}
