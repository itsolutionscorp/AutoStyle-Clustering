...
public class AVLTree {
	...
	private abstract class TreeNode {
		...
		public abstract void testBalanced();
	...
	private class NonEmptyTreeNode extends TreeNode {
		...
		public void insert(int item) {
			if (item > myItem) {
				...
				System.out.println("Cannot insert existing item");
			}
		}
		...
		public void testBalanced() {
			if (!isAlmostBalanced()){
				if (!left().isAlmostBalanced()){
					...
					myRight.testBalanced();
				} else if (balanceFactor() > 1){
					rotateRight();
				} else if (balanceFactor() < -1){
					rotateLeft();
				}
			} else if (myParent != theEmptyTreeNode){
				myParent.testBalanced();
			}
		...
		public void rotateLeft() {
			if (myRight != theEmptyTreeNode){
				TreeNode newLeft = new NonEmptyTreeNode(item(), this);
				newLeft.setLeft(myLeft);
				newLeft.setRight(myRight.left());
				TreeNode newRight = myRight.right();
		...
		public void rotateRight() {
			if (myLeft != theEmptyTreeNode){
				TreeNode newRight = new NonEmptyTreeNode(item(), this);
				newRight.setRight(myRight);
				newRight.setLeft(myLeft.right());
				TreeNode newLeft = myLeft.left();
