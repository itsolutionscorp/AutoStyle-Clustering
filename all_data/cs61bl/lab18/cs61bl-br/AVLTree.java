...
public class AVLTree {
	...
	private class NonEmptyTreeNode extends TreeNode {
		...
		public void insert(int item) {
			...
			if (this.parent().balanceFactor() >= 2) {
				if (this.parent().left().balanceFactor() == -1) {
					((NonEmptyTreeNode) this.right()).rotateLeft();
					((NonEmptyTreeNode) this.parent()).rotateRight();
				} else
					this.rotateRight();
				// ((NonEmptyTreeNode) parent()).rotateRight();
			}
			if (this.parent().balanceFactor() <= -2) {
				if (this.parent().right().balanceFactor() == 1) {
					((NonEmptyTreeNode) this.left()).rotateRight();
					((NonEmptyTreeNode) this.parent()).rotateLeft();
				} else
					this.rotateLeft();
				// ((NonEmptyTreeNode) parent()).rotateLeft();
			}
		}

		private void rotateRight() {
			TreeNode c = this.myParent;
			System.out.println("r");
			if (c == myRoot) {
				myRoot = this;
				this.setParent(theEmptyTreeNode);
			} else {
				this.setParent(c.parent());

				if (this.myItem > c.parent().item())
					c.parent().setRight(this);
				if (this.myItem < c.parent().item())
					c.parent().setLeft(this);
			}
			c.setParent(this);
			c.setLeft(this.myRight);
			this.setRight(c);
		}

		private void rotateLeft() {
			System.out.println("l");
			TreeNode a = this.myParent;
			if (a == myRoot) {
				myRoot = this;
				this.setParent(theEmptyTreeNode);
			} else {
				this.setParent(a.parent());
				if (this.myItem > a.parent().item())
					a.parent().setRight(this);
				if (this.myItem < a.parent().item())
					a.parent().setLeft(this);
			}
			a.setParent(this);
			a.setRight(this.myLeft);
			this.setLeft(a);
		}
