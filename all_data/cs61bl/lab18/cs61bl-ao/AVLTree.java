...
public class AVLTree {
	...
	private abstract class TreeNode {
		...
		public abstract TreeNode balanceHelper();
	...
	private class NonEmptyTreeNode extends TreeNode {
		...
		public void insert(int item) {
			// TODO Your code here!
			myRoot = myRoot.insertHelper(item);
		}

		@Override
		public TreeNode insertHelper(int item) {
			if (item < this.item()) {
				this.setLeft(this.left().insertHelper(item));
			} else if (item > this.item()) {
				this.setRight(this.right().insertHelper(item));
			}
			return this.balanceHelper();
		}
		
		@Override
		public TreeNode balanceHelper() {
			if (this.balanceFactor() > 1) {
				if (this.left().left().height() >= this.left().right().height()) {
					return rotateWithLeftChild(this);
				} else  return doubleWithLeftChild(this);
			} else if (this.balanceFactor() < -1){
				if (this.right().right().height() >= this.right().left().height()) {
					return rotateWithRightChild(this);
				} else  return doubleWithRightChild(this);
			} else return this;
		}
		...
		private TreeNode doubleWithRightChild(TreeNode t) {
			...
			t.setRight(rotateWithLeftChild(t.right()));
		...
		private TreeNode doubleWithLeftChild(TreeNode t) {
			// TODO Auto-generated method stub
			t.setLeft(rotateWithRightChild(t.left()));
