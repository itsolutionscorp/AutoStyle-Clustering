...
public class AVLTree {
	...
	private class NonEmptyTreeNode extends TreeNode {
		...
		@Override
		public void isOK() {
			if (myParent.isAlmostBalanced() == false) {
				if (myParent.balanceFactor() < 0) {
					myParent.rotateLeft();
				} else {
					myParent.rotateRight();
				}
			}
			
			if (myParent.parent().isAlmostBalanced() == false) {
				if (myParent.parent().balanceFactor() < 0) {
					myParent.parent().rotateLeft();
				} else {
					myParent.parent().rotateRight();
				}
			}
		}
		...
		public void rotateRight() {
			...
			TreeNode current = myLeft;
			myLeft = myLeft.right();
			if (current.right().isEmpty() == false) {
				current.right().setParent(this);
			}
			current.setParent(myParent);
			if (myParent.isEmpty()) {
				myRoot = current;
			} else if (myParent.left() == this) {
				myParent.setLeft(current);
			} else if(myParent.right() == this){
				myParent.setRight(current);
			}
		...
		public void rotateLeft() {
			// TODO Auto-generated method stub
			TreeNode current = myRight;
			myRight = myRight.left();
			if (!current.left().isEmpty()) {
				current.left().setParent(this);
			}
			current.setParent(myParent);
			if (myParent.isEmpty()) {
				myRoot = current;
