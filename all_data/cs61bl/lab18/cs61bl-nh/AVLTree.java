public class AVLTree {
    ...
    public void insert(int item) {
        myRoot.insert(item);
        while (myRoot.parent() != theEmptyTreeNode) {
    ...
    private class NonEmptyTreeNode extends TreeNode {
        ...
        public void insert(int item) {
            ...
            if(myItem > item){
                if(myLeft != theEmptyTreeNode){
                    myLeft.insert(item);
                }else
                    setLeft(new NonEmptyTreeNode(item, this));
                    //currNode = this.myLeft;
            }
            else if(myItem < item){
                 
                if(myRight != theEmptyTreeNode){
                    myRight.insert(item);
                }else
            ...
            //Balancing via rotating
            //while(currNode != theEmptyTreeNode){
                //System.out.println(currNode.item());
                if(!currNode.isAlmostBalanced()){
                    System.out.println("Not Balanced!");
                    if(currNode.balanceFactor()==2){
                        System.out.println("Left branch is longer");
                        //System.out.println(currNode.right().balanceFactor());
                        if(currNode.left().balanceFactor()==1){
                            myRoot = rotateRight(currNode);
                            //break;
                        }else if(currNode.left().balanceFactor()==-1){
                            rotateLeft(currNode.right());
                            myRoot = rotateRight(currNode);
                            //break;
                        }
                    }else if(currNode.balanceFactor()==-2){
                        if(currNode.right().balanceFactor()==-1){
                            myRoot = rotateLeft(currNode);
                            //break;
                        }else if(currNode.right().balanceFactor()==1){
        ...
        public TreeNode rotateRight(TreeNode me){
            System.out.println("Right rotating");
            //Make the Left Node the new root
            TreeNode myParent = me.parent();
            TreeNode myLeft = me.left();
            TreeNode myLeftRight = myLeft.right();
            //Left Node becomes new root
             
            myLeft.setParent(myParent);
             
            myParent.setLeft(myLeft);
            //this takes ownership of myLeft's right child as its left child
            me.setLeft(myLeftRight);
            myLeft.setRight(myLeftRight.right());//Modify this
            //myLeft takes ownership of me as it's right child
            myLeft.setRight(me);
            me.setParent(myLeft);
            return myLeft;
        }
         
        public TreeNode rotateLeft(TreeNode me){
            System.out.println("Left rotating");
            //Make the right node the new root
            TreeNode myParent = me.parent();
            TreeNode myRight = me.right();
            TreeNode myRightLeft = myRight.left();
            //Right Node becomes the new root
            myRight.setParent(myParent);
            myParent.setRight(myRight);
            //this takes ownership of myRight's left child as its right child
            me.setRight(myRightLeft);
            myRight.setLeft(myRightLeft.left());
            //myRight takes ownership of me as it's left child
            myRight.setLeft(me);
            me.setParent(myRight);
