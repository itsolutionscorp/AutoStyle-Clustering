...
public class AVLTree {
	...
	private class NonEmptyTreeNode extends TreeNode {
		...
		public boolean isBent() {
			if (myRight.isEmpty() && !myLeft.isEmpty() && myLeft.left().isEmpty()) {
				return true;
			}
			else if (myLeft.isEmpty() && !myRight.isEmpty() && myRight.right().isEmpty()) {
				return true;
			}
			return false;
		}
		...
		public void rotateRight() {
			if (isLeft()) {
				if (isBent()) {
					myLeft.right().setLeft(myLeft);
					myLeft.setParent(myLeft.right());
					this.setLeft(myLeft.right());
					this.myLeft.setParent(this);
					myLeft.left().setRight(new EmptyTreeNode());
				}
				TreeNode myNewRoot = left();
				setLeft(left().right());
				//myLeft = myLeft.right();
				myNewRoot.setRight(this);
				TreeNode myNewParent = myParent;
				setParent(myNewRoot);
				myNewRoot.setParent(myNewParent);
				myNewParent.setLeft(myNewRoot);
				this.myLeft.setParent(this);
			}
			else {
				if (isBent()) {
					myRight.left().setRight(myRight);
					myRight.setParent(myRight.left());
					this.setRight(myRight.left());
					this.myRight.setParent(this);
					myRight.right().setLeft(new EmptyTreeNode());
				}
				TreeNode myNewRoot = left();
				setLeft(left().right());
				//myLeft = myLeft.right();
				...
				setParent(myNewRoot);
				myNewRoot.setParent(myNewParent);
				myNewParent.setRight(myNewRoot);
				this.myLeft.setParent(this);
			}
		}
		...
		public void rotateLeft() {
			if (!isLeft()) {
				...
				TreeNode myNewRoot = right();
				setRight(right().left());
				myNewRoot.setLeft(this);
				TreeNode myNewParent = myParent;
				setParent(myNewRoot);
				myNewRoot.setParent(myNewParent);
				myNewParent.setRight(myNewRoot);
