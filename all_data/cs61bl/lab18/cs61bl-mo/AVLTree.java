...
public class AVLTree {
	...
	private class NonEmptyTreeNode extends TreeNode {
		...
		public void insert(int item) {
			// TODO Your code here!
			if (myRight.isEmpty() && myItem < item) {
				myRight = new NonEmptyTreeNode(item, this);
			}
			else if (myLeft.isEmpty() && myItem > item) {
			...
			if (!this.isAlmostBalanced()) {
				if (this.balanceFactor() < 0) {
					if (myRight.balanceFactor() > 0) {
						//System.out.println("right:" + myRight);
						this.myRight = myRight.left().rotateRight();
						myRoot = myRight.rotateLeft();	
					}
					else if (myRight.balanceFactor() < 0){
						myRoot = myRight.rotateLeft();
					}
					...
					if (myLeft.balanceFactor() < 0) {
						this.myLeft = myLeft.right().rotateLeft();
						myRoot = myLeft.rotateRight();
					} 
					else if (myLeft.balanceFactor() > 0) {
						myRoot = myLeft.rotateRight();
					}
				}
			}
		...
		public TreeNode rotateLeft() {
			...
			myParent.setParent(this);
			this.setLeft(myParent);
			return this;
		}
	...
	public static void main(String[] args) {
		...
		System.out.println(t5.isAlmostBalanced());
		
		AVLTree t6 = new AVLTree(5);
		t6.insert(3);
		t6.insert(6);
		t6.insert(2);
		t6.insert(4);
		...
		System.out.println(t6.isAlmostBalanced());
	}
}