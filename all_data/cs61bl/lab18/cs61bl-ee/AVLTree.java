...
public class AVLTree {
	...
	private class NonEmptyTreeNode extends TreeNode {
		...
		public void rotateLeft(TreeNode t) {
			boolean rotateRoot = false;
			if (t.parent().isEmpty()) {
				rotateRoot = true;
			}
			TreeNode tmp = t.right().left();
			TreeNode thisRight = t.right();
			t.parent().setRight(thisRight);

			thisRight.setParent(t.parent());

			t.setParent(thisRight);
			thisRight.setLeft(t);
			t.setRight(tmp);
			if (rotateRoot) {
				myRoot = thisRight;
		...
		public void rotateRight(TreeNode t) {
			...
			TreeNode tmp = t.left().right();
			TreeNode thisLeft = t.left();
			t.parent().setLeft(thisLeft);

			thisLeft.setParent(t.parent());

			t.setParent(thisLeft);
			thisLeft.setRight(t);
			t.setLeft(tmp);
			if (rotateRoot) {
				myRoot = thisLeft;
