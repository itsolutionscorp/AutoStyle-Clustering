...
public class AVLTree {
	...
	private class NonEmptyTreeNode extends TreeNode {
		...
		public void insert(int item) {
			// TODO Your code here
			insertHelper(item, this);
		}
		
		public void insertHelper(int item, TreeNode t) {
			if (item > t.item()) {
				if (t.right().isEmpty()) {
					t.setRight(new NonEmptyTreeNode(item, t));
					t.balance();
				} else {
					insertHelper(item, t.right());
				}
			} else {
				if (t.left().isEmpty()) {
					t.setLeft(new NonEmptyTreeNode(item, t));
				} else {
					insertHelper(item, t.left());
					t.balance();
				}
			}
		}
		...
		public void rotateRight() {
			System.out.println("rotate right " + myItem);
			TreeNode replace = left();
			setLeft(replace.right());
			replace.setParent(myParent);
			myParent.setLeft(replace);
			setParent(replace);
			replace.setRight(this);
			if (!parent().isEmpty()) {
			System.out.println("parent of " + myItem + " is " + parent().item());}
			if (!replace.left().isEmpty() && replace != null) {
			System.out.println("left of " + replace.item() + " is " + replace.left().item());}
			if (!replace.right().isEmpty() && replace != null) {
			System.out.println("right of " + replace.item() + " is " + replace.right().item());}
			if (!replace.parent().isEmpty() && replace != null) {
				System.out.println("parent of " + replace.item() + " is " + replace.parent().item());}
		}
		
		public void rotateLeft() {
			System.out.println("rotate left " + myItem);
			TreeNode replace = right();
			setRight(replace.left());
			replace.setParent(myParent);
			myParent.setRight(replace);
			setParent(replace);
			replace.setLeft(this);
			if (!parent().isEmpty()) {
			System.out.println("parent of " + myItem + " is " + parent().item());}
			if (!replace.left().isEmpty() && replace != null) {
			System.out.println("left of " + replace.item() + " is " + replace.left().item());}
	...
	public static void main(String[] args) {
		...
		System.out.println(t1.isAlmostBalanced());
		for (int i = 1; i < 6; i++) {
			System.out.println(t1.contains(i));}
		...
		for (int i = 1; i < 7; i++) {
			System.out.println(t2.contains(i));}
		t2.insert(7);
		System.out.println(t2);
		for (int i = 1; i < 8; i++) {
		System.out.println(t2.contains(i));}
