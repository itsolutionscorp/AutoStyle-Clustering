...
public class AVLTree {
	...
	public void insert(int item) {
		myRoot.insert(item);
		if (myRoot.isAlmostBalanced()) return;
		if (myRoot.balanceFactor() < -1) { // right is heavier
			if (myRoot.right().balanceFactor() > 0) { // right's left is heavier
				myRoot.right().setRight(myRoot.right().rotateRight());
				myRoot.rotateLeft();
			}
			else { // right's right is heavier
				myRoot.rotateLeft();
			}
		} 
		else { // left is heavier
			if (myRoot.left().balanceFactor() > 0) { // left's left is heavier
				myRoot.rotateRight();
			}
			else { // left's right is heavier
				myRoot.left().setLeft(myRoot.left().rotateLeft());
				myRoot.rotateRight();
			}
		}
	}
	...
	private class NonEmptyTreeNode extends TreeNode {
		...
		public TreeNode rotateLeft(){
			TreeNode rtn = myRight;
			TreeNode temp1 = rtn.left();
			TreeNode temp2 = this;
			rtn.setLeft(temp2);
		...
		public void insert(int item) {
			// TODO Your code here!
			if (this.isEmpty()) new NonEmptyTreeNode(item);
			if (item > myItem) myRight.insert(item);
