...
public class AVLTree {
	...
	private abstract class TreeNode {
		 ...
		 */
		public abstract void rigthRotation();
	...
	private class NonEmptyTreeNode extends TreeNode {
		...
		@Override
		public  void rigthRotation(){
			TreeNode temp = myLeft;
			myLeft = myLeft.left();
			if (!temp.right().isEmpty()){
				temp.right().setParent(this);
			}
		...
		public  void leftRotation(){
			TreeNode temp = myRight;
			myRight = myRight.left();
			if (!temp.left().isEmpty()){
				temp.left().setParent(this);
			}
			temp.setParent(myParent);
			if (myParent.isEmpty()){
				myRoot = temp;
		...
		@Override
		public void check(){
			if (!myParent.isAlmostBalanced()){
				if (myParent.balanceFactor() > 0){
					myParent.rigthRotation();
				} else {
					myParent.leftRotation();
				}
			}
			if (!myParent.parent().isAlmostBalanced()){
				if (myParent.parent().balanceFactor() > 0){
					myParent.parent().rigthRotation();
				} else {
					myParent.parent().leftRotation();
				}
			}
		}
