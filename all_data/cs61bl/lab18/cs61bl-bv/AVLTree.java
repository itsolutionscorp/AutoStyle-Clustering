...
public class AVLTree {
	...
	private class NonEmptyTreeNode extends TreeNode {
		...
		public void insert(int item) {
			...
			if (!isAlmostBalanced()) {
				if (balanceFactor() < -1) { //Right branch unbalanced
					rotateLeft();
				} else { //Left branch unbalanced
					rotateRight();
				}
			}
		}
		...
		public void rotateRight() {
			int oldRoot = item();
			int newRoot = left().item();
			setItem(newRoot);
			left().setItem(oldRoot);
			setRight(left());
			left().left().setParent(this);
			setLeft(left().left());
			right().setLeft(theEmptyTreeNode);
		}
		
		public void rotateLeft() {
			int oldRoot = item();
			int newRoot = right().item();
			setItem(newRoot);
			right().setItem(oldRoot);
			setLeft(right());
			right().right().setParent(this);
			setRight(right().right());
			left().setRight(theEmptyTreeNode);
		}
	...
	private class EmptyTreeNode extends TreeNode {
		...
		public void setItem(int i) {
			throw new UnsupportedOperationException(
					"Can't set an item of an empty node!");
		}
	}
