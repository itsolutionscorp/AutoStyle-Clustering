...
public class AVLTree {
	...
	private class NonEmptyTreeNode extends TreeNode {
		...
		public TreeNode insert(TreeNode t, int num){
			if(t.isEmpty()){
				return new NonEmptyTreeNode(num);
			}
			//go to right
			if(t.item()<num){
				t.setRight(insert(t.right(),num));
			}else{
				//go to left
				t.setLeft(insert(t.left(),num));
			}
			//check for balance
			if(!t.isAlmostBalanced()){
				if(t.balanceFactor()<-1){
					//rotate right
					if(t.right().balanceFactor()>0){//cause the child not balance
		 ...
		 public  TreeNode rotateRight(TreeNode n) {
			 ...
			 TreeNode copy =n.right();
			 copy.setParent(n.parent());
			 n.setRight(copy.left());
			 copy.left().setParent(n);
			 copy.setLeft(n);
			 n.setParent(copy);
		 ...
		 public TreeNode rotateLeft(TreeNode n) {
			 ...
			 TreeNode copy =n.left();
			 copy.setParent(n.parent());
			 n.setLeft(copy.right());
			 copy.right().setParent(n);
			 copy.setRight(n);
			 n.setParent(copy);
