public class AVLTree {
	...
	private class NonEmptyTreeNode extends TreeNode {
		...
		public void insert(int item) {
			// TODO Your code here!
			if (this.myLeft.equals(theEmptyTreeNode)
					&& this.myRight.equals(theEmptyTreeNode)) {
				if (item > this.myItem) {
			...
			} else if (this.myRight.equals(theEmptyTreeNode)
					&& item > this.myItem) {
				this.setRight(new NonEmptyTreeNode(item));
				return;
			} else if (this.myLeft.equals(theEmptyTreeNode)
					&& item < this.myItem) {
		...
		public void rotateRight() {
			...
			TreeNode oldLeft = this.left();
			TreeNode leftsRight = this.myLeft.right();
			oldParent.setLeft(oldLeft);
			oldLeft.setParent(oldParent);
			this.setParent(oldLeft);
			oldLeft.setRight(this);
			this.setLeft(leftsRight);
			myRoot = oldLeft;
		}

		public void rotateLeft() {
			TreeNode oldParent = this.parent();
			TreeNode oldRight = this.right();
			TreeNode rightsLeft = this.myRight.left();
			oldParent.setRight(oldRight);
			oldRight.setParent(oldParent);
			this.setParent(oldRight);
			oldRight.setLeft(this);
			this.setRight(rightsLeft);
			myRoot = oldRight;
		}
	...
	public static void main(String[] args) {
		...
		System.out.println(t1.contains(3));
