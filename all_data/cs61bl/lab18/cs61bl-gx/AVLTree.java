...
//		public abstract TreeNode insertHelper(TreeNode t, int item);
		/**
	...
	private class NonEmptyTreeNode extends TreeNode {
		...
		@Override
		public void insert(int item) {
			myRoot = insertHelper(myRoot, item);
			if (!isAlmostBalanced()) {
				if (right().height() > left().height()) {
					...
					if (left().right().height() > right().left().height()) {
						doubleRight();
					} else {
						rotateRight();
					}
		...
		public TreeNode insertHelper(TreeNode t, int item) {
			if (t instanceof EmptyTreeNode) {
				return new NonEmptyTreeNode(item);
			}
			if (item < t.item()) {
				t.setLeft(insertHelper(t.left(), item));
			} else if (item > t.item()) {
				t.setRight(insertHelper(t.right(), item));
			}
			return t;
		}
		private void doubleLeft() {
			int item = myRoot.right().item();
			TreeNode DrightSide = myRoot.right().right();
			TreeNode FjumpedNode = myRoot.right().left().right();
			myRoot.setRight(right().left());
			myRoot.right().setRight(new NonEmptyTreeNode(item, right()));
			myRoot.right().right().setLeft(FjumpedNode);
			myRoot.right().right().setRight(DrightSide);
			rotateLeft();
		}
		private void doubleRight() {
			int item = myRoot.left().item();
			TreeNode DleftSide = left().left();
			TreeNode GjumpedNode = left().right().left();
			myRoot.setLeft(left().right());
			myRoot.left().setLeft(new NonEmptyTreeNode(item, left()));
			myRoot.left().left().setRight(GjumpedNode);
			myRoot.left().left().setLeft(DleftSide);
			rotateRight();
		}
		private void rotateLeft() {
			int item = myRoot.item();
			TreeNode leftSide = myRoot.left();
			TreeNode jumpedNode = myRoot.right().left();
			myRoot = right();
			myRoot.setLeft(new NonEmptyTreeNode(item, myRoot));
			myRoot.left().setLeft(leftSide);
			myRoot.left().setRight(jumpedNode);
			
		}
		
		private void rotateRight() {
			int item = myRoot.item();
			TreeNode rightSide = myRoot.right();
			TreeNode jumpedNode = myRoot.left().right();
			myRoot = left();
			myRoot.setRight(new NonEmptyTreeNode(item, myRoot));
			myRoot.right().setRight(rightSide);
			myRoot.right().setLeft(jumpedNode);
		}
