...
public class AVLTree {
	...
	private abstract class TreeNode {
		...
		public abstract TreeNode Rotate(TreeNode t);
	...
	private class NonEmptyTreeNode extends TreeNode {
		...
		public void insert(int item) {
			insertHelper(item);			
			while (!myRoot.isAlmostBalanced()){
				myRoot = Rotate(myRoot);;
			}
		...
		public TreeNode Rotate(TreeNode t){
			...
			if (t.balanceFactor()>=2){
				if (t.left().balanceFactor() >= 1) {
					rtn = t.left();
					t.setLeft(t.left().left());
					rtn.setRight(t);

				}
				if (t.left().balanceFactor() <= -1){
					rtn = t.left();
					swap.setLeft(t.left().right());
					rtn.setRight(swap);
				}
				return rtn;
			}
			if (t.balanceFactor()<=-2){
				if (t.right().balanceFactor() <= -1){
					rtn = t.right();
					swap.setRight(t.right().left());
					rtn.setLeft(swap);
				}
				if (t.right().balanceFactor() >= 1){
					rtn = t;
					rtn.right().setLeft(t.left());
					rtn.setLeft(t.right().left());
				}
				return rtn;
			}
			if (t.left().isEmpty()){
				rtn.setLeft(new EmptyTreeNode());
			} else {
				rtn.setLeft(Rotate(t.left()));
			}
			if (t.right().isEmpty()){
				rtn.setRight(new EmptyTreeNode());
			} else {
				rtn.setRight(Rotate(t.right()));
			}
			return rtn;
		}
