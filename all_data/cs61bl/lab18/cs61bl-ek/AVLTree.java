...
public class AVLTree {
	...
	public void insert(int item) {
		myRoot.insert(item);
		TreeNode node = myRoot.inserted();
		
		while (node != theEmptyTreeNode) {
			if (!node.isAlmostBalanced() && node.balanceFactor() > 0) {
				rotateRight(node);
			}
			else if (!node.isAlmostBalanced() && node.balanceFactor() < 0) {
				rotateLeft(node);
			}
			node = node.parent();
		}
...
//		node = oldNode.left();
//		oldNode.setLeft(node.right());
//		node.setRight(oldNode);
		
		TreeNode oldRoot = myRoot;
		myRoot = node.left();
		node.setLeft(myRoot.right());
		myRoot.setRight(oldRoot);
...
//		TreeNode oldNode = node;
//		node = oldNode.right();
//		oldNode.setRight(node.left());
//		node.setLeft(oldNode);

		TreeNode oldRoot = myRoot;
		myRoot = node.right();
		node.setRight(myRoot.left());
		myRoot.setLeft(oldRoot);
	}
	...
	private abstract class TreeNode {
		...
		public abstract TreeNode inserted();
		...
		public abstract void insertHelper(TreeNode justInserted);
	...
	private class NonEmptyTreeNode extends TreeNode {
		...
		@Override
		public void insert(int item) {
			justInserted = new NonEmptyTreeNode(item);
			insertHelper(justInserted);	
			
		}		

		public void insertHelper(TreeNode justInserted) {
			if (justInserted.item() < myItem) {
				if (left().isEmpty()) {
					justInserted.setParent(this);
					setLeft(justInserted);
				}
				else {
					left().insertHelper(justInserted);
				}
			}
			else {
				if (right().isEmpty()) {
					justInserted.setParent(this);
					setRight(justInserted);
				}
				else {
					right().insertHelper(justInserted);
				}
			}
		}

		public TreeNode inserted() {
			return justInserted;
		}
	...
	private class EmptyTreeNode extends TreeNode {
		...
		@Override
		public void insertHelper(TreeNode t) {	
		}
