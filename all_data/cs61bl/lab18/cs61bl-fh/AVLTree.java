	...
	private class NonEmptyTreeNode extends TreeNode {
		...
		public void insert(int item) {
			...
			if (item == myItem) {
				...
				if (myLeft.isEmpty()) {
					myLeft = new NonEmptyTreeNode(item, this); // TreeNodes have
																// parent
																// pointers
																// now!!!
				} else {
			...
			if (!this.isAlmostBalanced()) {
				if (this.balanceFactor() < -1) { // right is too big => rotate
													// left
					this.rotateLeft();
				} else if (this.balanceFactor() > 1) { // left is too big =>
														// rotate right
					this.rotateRight();
				}
			}
		...
		public void rotateRight() {
			TreeNode A = this; // handle null check first, this makes it look
								// like the diagrams also
			TreeNode B = myLeft; // won't need to worry about B being empty
									// because it is heavier than A's right, so
									// it must have height of at least 1. Do
									// need to worry about child though
			TreeNode parent = myParent;
			TreeNode child = myLeft.right(); // B will never be empty because we
												// need to rotate right => child
												// will never be null
			...
			if (B.left().height() < B.right().height()) {
				...
				if (parent == null) {
				} else if (parent.left() == A) { // want == because must
													// actually be this
					// object, not a copy
					parent.setLeft(B);
				} else {
		...
		public void rotateLeft() {
			TreeNode A = this; // handle null check first, this makes it look
			// like the diagrams also
			TreeNode B = myRight; // won't need to worry about B being empty
			// because it is heavier than A's right, so
			// it must have height of at least 1. Do
			// need to worry about child though
			TreeNode parent = myParent;
			TreeNode child = myRight.left(); // B will never be empty because we
			// need to rotate right => child
			// will never be null
