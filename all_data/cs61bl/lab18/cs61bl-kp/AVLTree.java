...
public class AVLTree {
	...
	private class NonEmptyTreeNode extends TreeNode {
		...
		public void insert(int item) {
			...
			if (item < this.item()) {
				if (this.myLeft.isEmpty()) {
					this.setLeft(new NonEmptyTreeNode(item));
					this.left().setParent(this);
				} else {
					this.myLeft.insert(item);
...
//						}
						if (!this.right().isEmpty() && this.parent().balanceFactor() == 2 && this.left().balanceFactor() != 2) {
							return;
						}
						rotateRight(this);
					}
				}
			} else if (item > this.myItem) {
				if (this.myRight.isEmpty()) {
					this.setRight(new NonEmptyTreeNode(item));
					this.right().setParent(this);
				} else {
					this.myRight.insert(item);
					int bf = this.balanceFactor();
//					System.out.print("bf "); System.out.println(bf);
					if (bf == 0 || bf == -1 || bf == 1) {
						return;
					}
					if (bf == -2) {
...
//						}
						if (!this.left().isEmpty() && this.parent().balanceFactor() == -2 && this.right().balanceFactor() != 2) {
							return;
						}
						rotateLeft(this);
					}
		...
		public void rotateLeft(TreeNode d) {
//			System.out.print(d.item());
			System.out.println("  left");
			TreeNode f = d.parent();
			boolean fleft = false;
			boolean emptypar = false;
			if (f.isEmpty()) {
//				System.out.println("aprempty");
				emptypar = true;
			}
			if (!emptypar && d.parent().item() > d.item()) {
				fleft = true;
			}
			TreeNode dstore = d;
//			System.out.print("dstore "); System.out.println(dstore.item());
			TreeNode bstore = d.right();
//			System.out.print("bstore "); System.out.println(bstore.item());
			if (!bstore.left().isEmpty()) {
				dstore.setRight(bstore.left());
				bstore.left().setParent(dstore);
			} else {
				dstore.setRight(new EmptyTreeNode());
			}
			bstore.setLeft(dstore);
			dstore.setParent(bstore);
			...
			if (!emptypar) {
				...
				bstore.setParent(new EmptyTreeNode());
			}
//			System.out.println(this);
		}

		public void rotateRight(TreeNode b) {
			...
			if (!emptypar && b.parent().item() < b.item()) {
				fright = true;
			}
			TreeNode bstore = b;
			TreeNode dstore = b.left();
			bstore.setLeft(dstore.right());
			dstore.right().setParent(bstore);
			dstore.setLeft(bstore);
			bstore.setParent(dstore);
			...
			if (!emptypar) {
				...
				dstore.setParent(new EmptyTreeNode());
			}
			if (b.item() == 10) {
				System.out.print("the case for i want");
			System.out.println(this);}
		}
	...
	public static void main(String[] args) {
		...
		t1.insert(4);
		System.out.println("tree for after inserting 4");
		System.out.println(t1);

		// Should cause another rotation.
		System.out.println("Inserting 5 call in main string");
		t1.insert(5);
		System.out.println(t1);
		System.out.println(t1.isAlmostBalanced());
		t1.insert(6);
