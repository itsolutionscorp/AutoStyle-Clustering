...
public class AVLTree {
	...
	private class NonEmptyTreeNode extends TreeNode {
		...
		public void insert(int item) {
			...
			if (!isAlmostBalanced()) {
				if (balanceFactor() == 2) { // myLeft > myRight
					rotateRight();
				}
				if (balanceFactor() == -2) { // myRight > myLeft
					rotateLeft();
				}
			}
		}
		...
		@Override
		// Updates myRight of given. Reassigns new right node's babies.
		public void rotateRight() {		
			TreeNode newRoot = new NonEmptyTreeNode(myLeft.item(), parent());
			newRoot.setRight(this);
			newRoot.setLeft(myLeft.left());
			setLeft(myLeft.right());
			if (!myParent.isEmpty()) {
				myParent.setLeft(newRoot);
				myRoot.setLeft(newRoot);
			}
			if (myParent.isEmpty()) {
				myRoot = newRoot;
			}
		...
		@Override
		// Updates myLeft of given. Reassigns new left node's babies.
		public void rotateLeft() {
			TreeNode newRoot = new NonEmptyTreeNode(myRight.item(), parent());
			newRoot.setLeft(this);
			newRoot.setRight(myRight.right());			
			setRight(myRight.left());
			if (!myParent.isEmpty()) {
				myParent.setRight(newRoot);
				myRoot.setRight(newRoot);
			}
			if (myParent.isEmpty()) {
				myRoot = newRoot;
			}
