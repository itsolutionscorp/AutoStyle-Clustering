public class AVLTree {
	...
	private class NonEmptyTreeNode extends TreeNode {
		...
		public void insert(int item) {
			// TODO Your code here!
			if (item > myItem && right().isEmpty()) {
				setRight(new NonEmptyTreeNode(item, this));
			} else if (item < myItem && left().isEmpty()) {
		...
		public void balance() {
			if (right().height() > left().height()
					&& right().isAlmostBalanced()) {
				right().rotateLeft();

			} else if (left().height() > right().height()
					&& left().isAlmostBalanced()) {
				left().rotateRight();
			} else if (left().height() == 0 && right().height() > 1) {
				right().rotateLeft();
			} else if (right().height() == 0 && left().height() > 1) {
				left().rotateRight();
			} else if (left().height() > right().height()) {
				left().balance();
		...
		public void rotateLeft() {
			if (parent().parent().isEmpty()) {
				TreeNode holdRight = right();
				TreeNode holdLeft = left();
				TreeNode holdParent = parent();
				myRoot = this;
				myRoot.setLeft(holdParent);
				myRoot.setRight(holdRight);
				myRoot.left().setParent(this);	
				myRoot.left().setRight(holdLeft);	
				myRoot.right().setParent(this);
				myRoot.left().right().setParent(left());
