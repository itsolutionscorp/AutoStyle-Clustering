public class AVLTree {
	...
	private class NonEmptyTreeNode extends TreeNode {
		...
		public void insert(int item) {
			...
			TreeNode placeHolderParent = myRoot;
			...
			if (item < placeHolderParent.item()) {
				placeHolder = new NonEmptyTreeNode(item, placeHolderParent);
				placeHolderParent.setLeft(placeHolder);

			} else {
				placeHolder = new NonEmptyTreeNode(item, placeHolderParent);
				placeHolderParent.setRight(placeHolder);

			}
			while (!placeHolder.isEmpty()) {
				if (!placeHolder.isAlmostBalanced()) {
					if (placeHolder.balanceFactor() < 0) {
						placeHolder = ((NonEmptyTreeNode) placeHolder)
								.rotateRight();
					} else {
						placeHolder = ((NonEmptyTreeNode) placeHolder)
								.rotateLeft();
		...
		public TreeNode rotateRight() {
			...
			if (a.balanceFactor() < 0) {
				...
				if (!c.isEmpty()) {
					...
					a.setParent(theEmptyTreeNode);
					myRoot = a;
				}
				return a;
			} else {
				setRight(b.left());
				b.right().setParent(this);
