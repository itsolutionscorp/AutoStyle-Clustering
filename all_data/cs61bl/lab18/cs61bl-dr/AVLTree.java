public class AVLTree {
	...
	private abstract class TreeNode {
		...
		/**
		 * Adds an item to the TreeNode (this).
		 * 
		 * @param n
		 ...
		 * 
		 * @return
		 * 		The highest root/oldest parent of this TreeNode.
		 */
		public abstract TreeNode ancestor();
	...
	private class NonEmptyTreeNode extends TreeNode {
		...
		public void insert(int item) {			
			add(item);			
			for (TreeNode curr = this; !curr.isEmpty();) {
				TreeNode parent = curr.parent();
				if (!curr.isAlmostBalanced()) {
		...
		public void rotateLeft() {	
			...
			myRight.setLeft(root);			
			myRight.setParent(parent);
			if (isLeft(root, parent)) myParent.setLeft(rightNode);
			else myParent.setRight(rightNode);
			setRight(rightLeftChild);
			setParent(rightNode);
			rightLeftChild.setParent(root);
		}

		public void rotateRight() {
			TreeNode leftNode = left();
			TreeNode leftRightChild = left().right();
			TreeNode parent = parent();
			TreeNode root = this;
					
			myLeft.setRight(root);			
			myLeft.setParent(parent);
			if (!isLeft(root, parent)) myParent.setRight(leftNode);
			else myParent.setLeft(leftNode);
			setLeft(leftRightChild);
			setParent(leftNode);
			leftRightChild.setParent(root);
		}
		
		public boolean isLeft(TreeNode kid, TreeNode parent){
			return parent.left().equals(kid);
		}
		...
		public void add(int n) {
			if (item() < n) {
				if (myRight.isEmpty())
					setRight(new NonEmptyTreeNode(n, this));
				else
					myRight.add(n);
			} else {
				if (myLeft.isEmpty())
					setLeft(new NonEmptyTreeNode(n, this));
				else myLeft.add(n);
			}
		}
