...
public class AVLTree {
	...
	private class NonEmptyTreeNode extends TreeNode {
		...
		public void insert(int item) {
			...
			if (item < myItem) {
				if (myLeft.isEmpty()) {
					TreeNode left = new NonEmptyTreeNode(item, this);
					setLeft(left);
				} else {
					myLeft.insert(item);
				}
				...
				if (myRight.isEmpty()) {
					TreeNode right = new NonEmptyTreeNode(item, this);
					setRight(right);
				} else {
					myRight.insert(item);
				}
...
//			//actual insertion check and re-balance
			if (balanceFactor() < -1) {
		...
		public void rotateRight() {
			TreeNode parentSaver = parent();
			TreeNode save = left();
			setLeft(save.right());
			save.setRight(this);
			setParent(save);
			save.setParent(parentSaver);
			save.parent().setLeft(save);
		}
		
		@Override
		public void rotateLeft() {
//			System.out.println("first parents");
//			System.out.println(parent());
			...
			setParent(save);
			save.setParent(parentSaver);
			save.parent().setRight(save);
//			System.out.println("confirm parent");
