...
public class AVLTree {
	...
	private abstract class TreeNode {
		...
		public abstract TreeNode get(int item);
		/**
	...
	private class NonEmptyTreeNode extends TreeNode {
		...
		public void insert(int item) {
			...
			if(!this.isAlmostBalanced()) {
				// NonEmptyTreeNode toBeBalanced = (NonEmptyTreeNode) this.findUnbalanced();
		...
		public void rotateRight(){
			NonEmptyTreeNode child = (NonEmptyTreeNode) this.left();
			child.rotateLeft();
			int item = this.item();
			this.setItem(child.item());
			child.setItem(item);
			this.setLeft(child.left());
		...
		public TreeNode insert(TreeNode t, int item) {
			if(t.isEmpty()) {
				return new NonEmptyTreeNode(item);
			}
			else if(t.item() < item) {
				t.setRight(insert(t.right(), item));
				return t;
			}
			else {
				t.setLeft(insert(t.left(), item));
				return t;
			}
		}
		...
		public TreeNode get(int item) {
			...
			if(this.item() == item) {
				...
				return this.right().get(item);
			}
			else {
				return this.left().get(item);
			}
		}
