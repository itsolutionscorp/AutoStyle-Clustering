public class AVLTree {
	...
	private abstract class TreeNode {
		...
		/**
		 * Chooses which types/order of rotations to do. Compare nodes using
		 * item.
		 */
		public abstract void rotate(int item);

		/**
		 * Performs a left rotation
		 */
		public abstract TreeNode leftRotate(TreeNode t);
		 ...
		 * @return
		 */
		public abstract TreeNode rightRotate(TreeNode t);

		public abstract TreeNode rotateLR(TreeNode t);

		public abstract TreeNode rotateRL(TreeNode t);
	...
	private class NonEmptyTreeNode extends TreeNode {
		...
		public void insert(int item) {
			// TODO Your code here!
			TreeNode temp = new NonEmptyTreeNode(item);
			// STEP 1: put the item at appropriate place
			if (item() < item) { // search through right tree
				if (right().isEmpty()) {
					setRight(temp); // create new nonempty
									// treenode if the
									// right is empty
				} else {
					right().insert(item); // if right is not empty, recursively
											// call insert until the correct
											// leaf is found

				}

			} else { // search through left tree
				if (left().isEmpty()) {
					setLeft(temp); // create new nonempty
									// treenode if the
									// left is empty
				} else {
					left().insert(item); // if left is not empty, recursively
											// call insert until the correct
											// leaf is found
				}
			}

			// STEP 2: check if needs to rotate
			if (!isAlmostBalanced()) {
				rotate(item); // compare by checking items & balanceFactor();
			}
		}

		@Override
		public void rotate(int item) {
			if (balanceFactor() == -2) {
				if (item > myRoot.right().item()) { // far right
					// if(!myRoot.left().isAlmostBalanced()){

					myRoot = leftRotate(myRoot);
				} else {
					myRoot = rotateRL(myRoot);
				}
			} else if (balanceFactor() == 2) {
				if (item < myRoot.left().item()) { // far left
					// if(!myRoot.right().isAlmostBalanced()){
					myRoot = rightRotate(myRoot);
				} else {
					myRoot = rotateLR(myRoot);
				}
		...
		public TreeNode leftRotate(TreeNode t) {
			TreeNode newHead = t.right();
			t.setRight(newHead.left());
			newHead.setLeft(t);
			newHead.setParent(t.parent());
			return newHead;
		}

		@Override
		public TreeNode rightRotate(TreeNode t) {
			TreeNode newHead = t.left();
			t.setLeft(newHead.right());
			newHead.setRight(t);
			newHead.setParent(t.parent());
			return newHead;
		}
		...
		@Override
		public TreeNode rotateLR(TreeNode t) {
			TreeNode temp = leftRotate(t.left());
			t.setLeft(temp);
			temp.setParent(t.parent());
			return rightRotate(t);
		}

		@Override
		public TreeNode rotateRL(TreeNode t) {
			TreeNode temp = rightRotate(t.right());
			t.setRight(temp);
			temp.setParent(t.parent());
			return leftRotate(t);
		}
