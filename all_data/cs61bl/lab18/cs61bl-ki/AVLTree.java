...
public class AVLTree {
	...
	private abstract class TreeNode {
		...
		public abstract TreeNode insertHelper(int item);
		public abstract void balance();
		public abstract void balanceHelper();
		public abstract TreeNode reRoot();
		/*
	...
	private class NonEmptyTreeNode extends TreeNode {
		...
		@Override
		public void insert(int item) {
			TreeNode inserted = insertHelper(item);
			inserted.balanceHelper();
		}
		
		public void balance(){
			if (balanceFactor()>1){
				if (myLeft.balanceFactor() < 0){
					setLeft(myLeft.rotateLeft());
				} else {
					rotateRight();
				}
			} else if(balanceFactor()<-1){
				if (myRight.balanceFactor()>0){
					setRight(myRight.rotateRight());
				} else {
					rotateLeft();
				}
			}
			// check if need to rotate left or right
			// 
		}
		@Override
		public void balanceHelper() {
			if (isAlmostBalanced()){
				parent().balanceHelper();
			} else {
				balance();
			}
		...
		public TreeNode rotateLeft(){
			TreeNode nodeRight = myRight;
			parent().setRight(nodeRight);
			setRight(nodeRight.left());
			nodeRight.setLeft(this);
			nodeRight.setParent(this.parent());
			setParent(nodeRight);
			return nodeRight;
		}
		
		public TreeNode rotateRight(){
			TreeNode nodeLeft = myLeft;
			parent().setLeft(nodeLeft);
			setLeft(nodeLeft.right());
			nodeLeft.setRight(this);	
			nodeLeft.setParent(this.parent());
			setParent(nodeLeft);
			return nodeLeft;
		}

		public TreeNode insertHelper(int item){
			if (myItem > item){
				if (myLeft.isEmpty()){
					NonEmptyTreeNode toInsert = new NonEmptyTreeNode(item);
					toInsert.setParent(this);
					setLeft(toInsert);
					return toInsert;
				} else {
					return myLeft.insertHelper(item);
				}
			} else {
				if (myRight.isEmpty()){
					NonEmptyTreeNode toInsert = new NonEmptyTreeNode(item);
					...
					return myRight.insertHelper(item);
				}
			}
		...
		public TreeNode reRoot(){
			if (parent() instanceof EmptyTreeNode){
				return this;
			} else {
				return parent().reRoot();
			}
		}
...
//
		...
		AVLTree t3 = new AVLTree(4);
		t3.insert(3);
		t3.insert(2);
		t3.insert(1);
		System.out.println(t3);
		System.out.println(t3.isAlmostBalanced());
		AVLTree t4 = new AVLTree(10);
		t4.insert(8);
		t4.insert(6);
		t4.insert(4);
		t4.insert(2);
