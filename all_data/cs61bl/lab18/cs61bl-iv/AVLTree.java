...
public class AVLTree {
	...
	private class NonEmptyTreeNode extends TreeNode {
		...
		@Override
		public void insert(int item) {
			TreeNode inserted = theEmptyTreeNode;
			if (item < myItem) {
				if (!myLeft.isEmpty()) {
					left().insert(item);
				} else {
					inserted = new NonEmptyTreeNode(item);
					setLeft(inserted);
		...
		public TreeNode rotateRight() {
			...
			setParent(tempL);
			setLeft(tempL.right()); // move the child from new root
			left().setParent(this);
			tempL.setRight(this); // move the right node up
			tempL.setParent(parent);
			if (!parent.isEmpty() && parent.item() > myItem) {
				parent.setLeft(tempL);
			} else if (!parent.isEmpty() && parent.item() < myItem) {
		...
		public TreeNode rotateLeft() {
			...
			TreeNode tempR = right();
			TreeNode parent = parent();

			setParent(tempR);
			setRight(tempR.left()); // move the child from new root
			right().setParent(this);
			tempR.setLeft(this); // move the right node up
			tempR.setParent(parent);
			if (!parent.isEmpty() && parent.item() > myItem) {
