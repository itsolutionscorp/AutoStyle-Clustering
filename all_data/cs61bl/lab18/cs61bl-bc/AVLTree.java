...
public class AVLTree {
	...
	private class NonEmptyTreeNode extends TreeNode {
		...
		public void rebalance(){
			if(!isAlmostBalanced()){
				if(!myLeft.isEmpty())
					((NonEmptyTreeNode)myLeft).rebalance();
				if(!myRight.isEmpty())
					((NonEmptyTreeNode)myRight).rebalance();
		...
		public void rotateRight(){
			NonEmptyTreeNode tempRight = null;
			if(!myLeft.right().isEmpty()){
				tempRight = new NonEmptyTreeNode(myLeft.right().item(),new EmptyTreeNode());
			}
			myLeft.setRight(this);
			myLeft.setParent(new EmptyTreeNode());
			if(tempRight != null){
				tempRight.setParent(this);
				this.setLeft(tempRight);
			}
			else{
				this.setLeft(new EmptyTreeNode());
			}	
			this.setParent(myLeft);
		...
		public void rotateLeft(){
			NonEmptyTreeNode tempLeft = null;
			if(!myRight.left().isEmpty()){
				tempLeft = new NonEmptyTreeNode(myRight.left().item(), new EmptyTreeNode());
			}
			myRight.setLeft(this);
			myRight.setParent(new EmptyTreeNode());
			if(tempLeft !=  null){
				tempLeft.setParent(this);
				this.setRight(tempLeft);
			}
			else{
				this.setRight(new EmptyTreeNode());
			}				
			this.setParent(myRight);
			if(this.equals(myRoot)){
				myRoot = this.myParent;
			}
			//
