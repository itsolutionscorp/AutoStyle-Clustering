...
public class AVLTree {
	...
	private class NonEmptyTreeNode extends TreeNode {
		...
		@Override
		public void insert(int item) {
			TreeNode unbalancedNode = this;
			TreeNode unbalancedNodeChild;
			TreeNode unbalancedGrandChild;
			boolean xOnLeft = false;
			boolean yOnLeft = false;
			if (item <= myItem){
				if (myLeft.isEmpty()){
					myLeft = new NonEmptyTreeNode(item, this);
					while(unbalancedNode.isAlmostBalanced() && !unbalancedNode.parent().isEmpty()){
						unbalancedNode = unbalancedNode.parent();
					}
					if (unbalancedNode.isAlmostBalanced())
						return;
					if (myLeft.item() <= unbalancedNode.item()){
						unbalancedNodeChild = unbalancedNode.left();
						yOnLeft = true;
					} else
						unbalancedNodeChild = unbalancedNode.right();
					if (myLeft.item () <= unbalancedNodeChild.item()) {
						unbalancedGrandChild = unbalancedNodeChild.left();
						xOnLeft = true;
					} else
						unbalancedGrandChild = unbalancedNodeChild.right();
					if (xOnLeft && yOnLeft){
						unbalancedNodeChild.rotateRight();
					} else if (yOnLeft && !xOnLeft) {
						unbalancedGrandChild.rotateLeft();
						unbalancedGrandChild.rotateRight();
					} else if (!yOnLeft && !xOnLeft) {
						unbalancedNodeChild.rotateLeft();
					} else {
						unbalancedGrandChild.rotateRight();
						unbalancedGrandChild.rotateLeft();
					}
					...
					((NonEmptyTreeNode) myLeft).insert(item);
			}
			else {
				if (myRight.isEmpty()){
					...
					if (unbalancedNode.isAlmostBalanced())
						return;
					if (myRight.item() <= unbalancedNode.item()){
						unbalancedNodeChild = unbalancedNode.left();
						yOnLeft = true;
					} else
						unbalancedNodeChild = unbalancedNode.right();
					if (myRight.item () <= unbalancedNodeChild.item()) {
						unbalancedGrandChild = unbalancedNodeChild.left();
						xOnLeft = true;
					} else
						unbalancedGrandChild = unbalancedNodeChild.right();
					...
					((NonEmptyTreeNode) myRight).insert(item);
			}
		}
		
		public void rotateRight() {
			TreeNode temp = myParent.parent();
			myParent.setLeft(myRight);
			myRight.setParent(myParent);
			setRight(myParent);
			myRight.setParent(this);
			setParent(temp);
			if (!temp.isEmpty())
				if (myItem <= temp.item())
					temp.setLeft(this);
