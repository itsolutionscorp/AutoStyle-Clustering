...
public class AVLTree {
	...
	private class NonEmptyTreeNode extends TreeNode {
		...
		public void rotateRight() {
			NonEmptyTreeNode left = (NonEmptyTreeNode)this.myLeft.left();
			NonEmptyTreeNode root = (NonEmptyTreeNode) this.myLeft;
			NonEmptyTreeNode right = new NonEmptyTreeNode(this.myItem, root);
			right.myLeft = this.myRight;
			this.myLeft = left;
			this.myRight = right;
		}
		
		public void rotateLeft() {
			NonEmptyTreeNode right = (NonEmptyTreeNode)this.myRight.right();
			NonEmptyTreeNode root = (NonEmptyTreeNode)this.myRight;
			NonEmptyTreeNode left = new NonEmptyTreeNode(this.myItem, root);
			left.myRight = this.myRight.left();
			this.myLeft = left;
			this.myRight = right;
		}
		...
		public void insert(int item) {
			...
			if (this.item() > item) {
				if (this.right() != null)
				this.myLeft = new NonEmptyTreeNode(item, this.left());
			} else
				this.myRight = new NonEmptyTreeNode(item, this.right());
			if (this.insertCheck()) {
				return;
			} else
				this.insertCheck();
		...
		public boolean insertCheck() {
			if (this.isAlmostBalanced()) {
				return true;
			} else if (this.left().isAlmostBalanced()) {
				this.rotateRight();
				this.insertCheck();
			} else if (this.right().isAlmostBalanced()) {
				this.rotateLeft();
				this.rotateRight();
				this.insertCheck();
			}
