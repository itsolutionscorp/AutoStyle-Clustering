public class IntList {
	...
	private ListNode insert(ListNode p, ListNode head) {
		...
		else if (newp.myItem < head.myItem) {
			newp.myNext = head;
			head.myPrev = newp;
			return newp;
		}
		
		while (temp.myNext != null && newp.myItem > temp.myItem) {
			temp = temp.myNext;
		}
		...
		if (temp.myNext == null) {
			...
			newp.myPrev = temp.myPrev;
			temp.myPrev.myNext = newp; //nullpointer here
			newp.myNext = temp;
			temp.myPrev = newp;
			return head;
		}
...
//			smallElements.addToEnd(myHead.myItem);
...
//		}
		...
		if (smallElements.mySize == 1 && largeElements.mySize == 1) {
			IntList rtnList = new IntList();
			rtnList.addToEnd(smallElements.myHead.myItem);
			rtnList.addToEnd(divider);
			rtnList.addToEnd(largeElements.myHead.myItem);
			return rtnList;
		} else if (smallElements.mySize == 1) {
			IntList rtnList = new IntList();
			rtnList.addToEnd(smallElements.myHead.myItem);
			rtnList.addToEnd(divider);
			rtnList.append(largeElements.quicksort());
			return rtnList;
		} else if (largeElements.mySize == 1) {
			IntList rtnList = new IntList();
			rtnList.append(smallElements.quicksort());
			rtnList.addToEnd(divider);
			rtnList.addToEnd(largeElements.myHead.myItem);
			return rtnList;
		}
	...
	public IntList mergeSort() {
		...
		for (int i = 0; i < mySize; i ++) {
			...
			return merge(otherHalf.myHead, oneHalf.mergeSort().myHead);
		}
		else {
