public class IntList {
	...
	private ListNode insert(ListNode p, ListNode head) {
		...
		if (head == null) {
			...
			ListNode toInsert = new ListNode(p.myItem);
			while (compare.myNext != null && toInsert.myItem > compare.myItem) {
				compare = compare.myNext;
			}
			if (compare.myPrev == null) {
				toInsert.myNext = compare;
				compare.myPrev = toInsert;
				head = toInsert;
			} else if (compare.myNext == null && compare.myItem < toInsert.myItem) {
				compare.myNext = toInsert;
				toInsert.myPrev = compare;
			} else {
				compare.myPrev.myNext = toInsert;
				toInsert.myPrev = compare.myPrev;
				compare.myPrev = toInsert;
				toInsert.myNext = compare;
			}
			return head;
	...
	public IntList quicksort() {
		...
		for (int i = 1; i < this.mySize; i++) {
			if (node.myItem < divider) {
				smallElements.addToEnd(node.myItem);
			} else {
				largeElements.addToEnd(node.myItem);
			}
			node = node.myNext;
		}
	...
	public IntList mergeSort() {
		...
		for (int i = 0; i < this.mySize / 2; i++) {
			oneHalf.addToEnd(n.myItem);
			n = n.myNext;
		} for (int i = 0; n != null; i++) {
			otherHalf.addToEnd(n.myItem);
			if (n.myNext == null) {
				break;
			} n = n.myNext;
		} 
		
		if (oneHalf.myHead.myItem > otherHalf.myHead.myItem) {
			return merge(otherHalf.mergeSort().myHead, oneHalf.mergeSort().myHead);
		} else {
