public class IntList {
	...
	private ListNode insert(ListNode p, ListNode head) {
		if (head == null) {
			...
			if (p.myItem <= head.myItem) {
				ListNode myNode = new ListNode(p.myItem,null,head);
				head.myPrev = myNode;
				return myNode;
			}
			else if (p.myItem > head.myItem) {
				ListNode myCopy = head;
				while (p.myItem > myCopy.myItem) {
					if (myCopy.myNext != null) {
						myCopy = myCopy.myNext;
					}
					else {
						ListNode myNode = new ListNode(p.myItem, myCopy, null);
						myNode.myPrev.myNext = myNode;
						return head;
					}
				}
				ListNode myNode = new ListNode(p.myItem, myCopy.myPrev, myCopy);
				myCopy.myPrev = myNode;
				myNode.myPrev.myNext = myNode;
				return head;
			}
		}
	...
	public IntList quicksort() {
		...
		IntList myMerge = merge(smallElements.quicksort().myHead, new ListNode(divider));
		return merge(myMerge.myHead,largeElements.quicksort().myHead);
	}
	...
	public IntList mergeSort() {
		...
		// TODO your code here!
		int counter1 = 0;
		int counter2 = mySize/2;
		
		while (counter1 < mySize/2) {
			oneHalf.addToEnd(myHead.myItem);
			myHead = myHead.myNext;
			counter1++;
		}
		while (counter2 < mySize) {
