public class IntList {
	...
	private ListNode insert(ListNode p, ListNode head) {
		...
		if (head == null) {
			...
			ListNode temp2 = null;
			for (ListNode n = head; n != null; n = n.myNext) {
				if (temp.myItem < n.myItem ) {
					if (n.myPrev == null) {
						n.myPrev = temp;
						temp.myNext = n;
						head = temp;
					} else {
						n.myPrev.myNext = temp;
						temp.myPrev = n.myPrev;
						temp.myNext = n;
						n.myPrev = temp;
					}
					break;
	...
	public IntList quicksort() {
		...
		//IntList dividerList = new IntList();
		int divider = myHead.myItem;
		//dividerList.addToEnd(divider);
		IntList dividerList = new IntList(new ListNode(divider));
		...
		IntList sortedSmallList = smallElements.quicksort();
		IntList sortedLargeList = largeElements.quicksort();

		sortedSmallList.append(dividerList);
		sortedSmallList.append(sortedLargeList);
	...
	public IntList mergeSort() {
		...
		IntList oneHalfSorted = oneHalf.mergeSort();
		IntList otherHalfSorted = otherHalf.mergeSort();
		
		return merge(oneHalfSorted.myHead, otherHalfSorted.myHead);
	}
	...
	public static void main(String[] args) {
		...
		values = new IntList();
		System.out.print("Before quicksort 0: ");
		for (int k = 0; k < 5; k++) {
			values.addToFront(randomInt());
		}
		System.out.println(values);
		sortedValues = values.quicksort();
		System.out.print("After quicksort 0: ");
		System.out.println(sortedValues);
