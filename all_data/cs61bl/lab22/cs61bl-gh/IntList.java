public class IntList {
	...
	private ListNode insert(ListNode p, ListNode head) {
		...
		if (head == null) {
			...
			ListNode rv = new ListNode(p.myItem, null, head);
			head.myPrev = rv;
			return rv;
		} else {
			ListNode rv = new ListNode(head.myItem, null,
					insert(p, head.myNext));
			rv.myNext.myPrev = rv;
			return rv;
		}
	}
	...
	public IntList quicksort() {
		...
		// TODO your code here!
		ListNode curr = myHead.myNext;
		for (int i = 1; i < mySize; i++) {
			if (curr.myItem < divider) {
				smallElements.addToEnd(curr.myItem);
			} else {
				largeElements.addToEnd(curr.myItem);
			}
			curr = curr.myNext;
		}
		IntList leftSide = smallElements.quicksort();
		IntList rightSide = largeElements.quicksort();
		leftSide.addToEnd(divider);
