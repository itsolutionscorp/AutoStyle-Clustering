...
//			System.out.println(newNode.myItem +" <? " +current.myItem);
//			System.out.println(newNode.myItem<=current.myItem);
			if (newNode.myItem<=current.myItem){
				if (current==head){
					newNode.myNext = current;
					current.myPrev = newNode;
					return newNode;
				}
//				System.out.println(current.myItem);
//				System.out.println(current.myPrev.myItem);
//				System.out.println(current.myPrev.myNext.myItem +"electric boogaloo");
				newNode.myPrev = current.myPrev;
				current.myPrev.myNext = newNode;
				current.myPrev = newNode;
				newNode.myNext = current;				
				return head;
			} 
	...
	public IntList quicksort() {
		...
		for (ListNode current=myHead; current!=null; current=current.myNext ){
			...
			} else if (current.myItem>divider){
