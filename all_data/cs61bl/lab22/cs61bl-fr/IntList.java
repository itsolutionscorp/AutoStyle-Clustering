public class IntList {
	...
	private ListNode insert(ListNode p, ListNode head) {
		ListNode p2 = new ListNode(p.myItem, null, null);
		if (head == null) {
			return p2;
		}
		ListNode currentNode = head;
		while (currentNode.myItem < p2.myItem) {
			if (currentNode.myNext == null) {
				p2.myNext = null;
				p2.myPrev = currentNode;
				currentNode.myNext = p2;
				return head;
			}
			currentNode = currentNode.myNext;
		}
		p2.myNext = currentNode;
		p2.myPrev = currentNode.myPrev;
		if (currentNode.myPrev == null) {
			currentNode.myPrev = p2;
			return p2;
		}
		currentNode.myPrev.myNext = p2;
		currentNode.myPrev = p2;
		return head;
	}
	...
	public IntList quicksort() {
		...
		for (ListNode node = myHead.myNext; node != null; node = node.myNext) {
		...
		IntList sorted = smallElements.quicksort();
		sorted.append(new IntList(myHead));
		sorted.append(largeElements.quicksort());
	...
	public IntList mergeSort() {
		...
		for (ListNode node = myHead; node != null; node = node.myNext) {
			if (index < mySize/2) {
				oneHalf.addToEnd(node.myItem);
			}
			else {
				otherHalf.addToEnd(node.myItem);
			}
			index++;
		}
