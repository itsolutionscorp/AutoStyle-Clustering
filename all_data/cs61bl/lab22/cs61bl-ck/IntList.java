public class IntList {
	...
	private ListNode insert(ListNode p, ListNode sofar) {
		...
		if (sofar == null) {
			ListNode newNode = new ListNode(p.myItem, null, null);
			sofar = newNode;
			return sofar;
		} else if (sofar.myItem > p.myItem) {
			ListNode newNode = new ListNode(p.myItem, null, sofar);
			sofar.myPrev = newNode;
			return newNode;
		}
		ListNode pointer = sofar;
		while (sofar.myNext != null) {
			ListNode newNode = new ListNode(p.myItem, null, null);
			if (sofar.myItem < newNode.myItem && sofar.myNext.myItem > newNode.myItem) {
				ListNode temp = sofar.myNext;
				sofar.myNext = newNode;
				newNode.myPrev = sofar;
				newNode.myNext = temp;
				temp.myPrev = newNode;
				return pointer;
			}
		...
		ListNode newNode = new ListNode(p.myItem, sofar, null);
		sofar.myNext = newNode;
		return pointer;	
	}
