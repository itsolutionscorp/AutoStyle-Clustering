public class IntList {
	...
	private ListNode insert(ListNode p, ListNode head) {
		...
		if (head == null) {
			ListNode l = new ListNode(p.myItem);
			return l;
		}
		else if (head.myNext == null && p.myItem > head.myItem) {
			ListNode l = new ListNode(p.myItem);
			head.myNext = l;
			l.myPrev = head;
			return head;
		}
		else if (head.myPrev == null && p.myItem < head.myItem) {
			ListNode l = new ListNode(p.myItem);
			l.myNext = head;
			head.myPrev = l;
			return l;
		}
		else if (p.myItem > curr.myItem && p.myItem <= curr.myNext.myItem) {
			ListNode l = new ListNode(p.myItem);
			curr.myNext.myPrev = l;
			l.myNext = curr.myNext;
			curr.myNext = l;
			l.myPrev = curr;
			return curr;
				
		}else {
			curr.myNext = insert(p, curr.myNext);
			return curr;
		}
	...
	public IntList quicksort() {
		...
		IntList smsorted = smallElements.quicksort();
		IntList lgsorted = largeElements.quicksort();
		smsorted.addToEnd(divider);
		for (ListNode p = lgsorted.myHead; p != null; p = p.myNext) {
			smsorted.addToEnd(p.myItem);
		}
		return smsorted;
	...
	public IntList mergeSort() {
		...
		// TODO your code here!
		int split = this.mySize/2;
		ListNode curr = this.myHead;
		for (int i = 0; i < split; i++) {
			oneHalf.addToEnd(curr.myItem);
			curr = curr.myNext;			
		}
		for (int i = split; i < this.mySize; i++) {
			otherHalf.addToEnd(curr.myItem);
			curr = curr.myNext;
		}
		IntList one = oneHalf.mergeSort();
		IntList other = otherHalf.mergeSort();
		return merge (one.myHead, other.myHead);
	}
