...
public class IntList {
	ArrayList<IntList> elements = new ArrayList<IntList>(); 
	ListNode myHead;
	ListNode myTail;
	int mySize;
	...
	private ListNode insert(ListNode p, ListNode head) {
		//Traverse through list until p's item is less than or == to a link's item\
		if (head == null) {
			return p; 
		}
	    	...
	    	ListNode a = head; 
		    while (a.myItem < p.myItem) {
			a = a.myNext; 
		    }
		ListNode temp = a.myPrev; 
		temp.myNext = p; 
		p.myNext = a; 
		a.myPrev = p;
		p.myPrev = temp; 
	...
	public IntList quicksort() {
		...
		// TODO your code here!
		ListNode p = myHead; 
		ListNode q = largeElements.myHead; 
		ListNode r = smallElements.myHead; 
		while (p != null) {
			p = p.myNext; 
			if (p.myPrev.myItem > divider) {
				q.myItem = p.myPrev.myItem; 
				q = q.myNext; 
			}
			else {
				r.myItem = p.myPrev.myItem; 
				r = r.myNext; 
			}
	...
	public IntList mergeSort() {
		...
		temp.myPrev.myNext = null; 
		temp.myPrev = null; 
		otherHalf.myHead = temp; 
