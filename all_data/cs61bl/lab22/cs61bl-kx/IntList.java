public class IntList {
	...
	private ListNode insert(ListNode p, ListNode head) {
		...
		ListNode n = p;
		for (n = p; n.myPrev != null; n = n.myPrev) {
			if (n.myItem < n.myPrev.myItem) {
				int temp = n.myPrev.myItem;
				n.myPrev.myItem = n.myItem;
				n.myItem = temp;
			} else {
				return head;
			}
	...
	public IntList quicksort() {
		...
		IntList first = smallElements.quicksort();
		IntList second = largeElements.quicksort();
		IntList list = new IntList();
		list.append(first);
		while (count != 0) {
			list.addToEnd(divider);
			count--;
		}
		list.append(second);
		return list;
	...
	public IntList mergeSort() {
		...
		IntList first = oneHalf.mergeSort();
		IntList second = otherHalf.mergeSort();
		return merge(first.myHead, second.myHead);
	}
