public class IntList {
	...
	private ListNode insert(ListNode p, ListNode head) {
		...
		if (p.myItem <= head.myItem) {
			ListNode rtn = new ListNode(p.myItem, null, head);
		...
		for (front = head, back = head.myNext; back != null && p.myItem > back.myItem; front = front.myNext, back = back.myNext);
		ListNode insertedP = new ListNode(p.myItem, front, back);
		front.myNext = insertedP;
		if (back != null) {
			back.myPrev = insertedP;
		}
		return head;
	}
			
//Need to finish this insert method
	/**
	...
	public IntList quicksort() {
		...
		while (i < this.mySize) {
			...
			if (this.myHead.myItem < divider) {
				smallElements.addToFront(this.myHead.myItem);
			} else {
				largeElements.addToFront(this.myHead.myItem);
			}
	...
	public IntList mergeSort() {
		...
		while (i < this.mySize) {
			i++;
			if (i % 2 == 0) {
				oneHalf.addToEnd(this.myHead.myItem);

			} else {
				otherHalf.addToEnd(this.myHead.myItem);
			}
			myHead = myHead.myNext;
