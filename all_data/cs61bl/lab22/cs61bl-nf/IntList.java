public class IntList {
    ...
    private ListNode insert(ListNode p, ListNode head) {
        if (head == null) {
            ...
            ListNode newHead = new ListNode(p.myItem);
            newHead.myNext = head;
            head.myPrev = newHead;
            return newHead;
        } else {
            ...
            while (tail.myNext != null && p.myItem > tail.myNext.myItem) {
                tail = tail.myNext;
            }

            if (tail.myNext != null)
                tail.myNext.myPrev = newNode;
            newNode.myNext = tail.myNext;
            tail.myNext = newNode;
            newNode.myPrev = tail;
            return head;
        }
    ...
    public IntList quicksort() {
        ...
        ListNode head = myHead.myNext;
        for (head = myHead.myNext; head != null; head = head.myNext) {
    ...
    public IntList mergeSort() {
        ...
        for (int i = 0; i < mySize / 2; i++) {
            oneHalf.addToEnd(next.myItem);
            next = next.myNext;
        }
        while (next != null) {
            otherHalf.addToEnd(next.myItem);
            next = next.myNext;
        }
