public class IntList {
	...
	private ListNode insert(ListNode p, ListNode head) {
		...
		ListNode temp = head;
			...
			for(ListNode i = head; i != null; i = i.myNext){
				temp = i;
				cur = i;
				if(p.myItem < i.myItem){
				
					break;
				}
			}
			if( head == cur && p.myNext != null){
				p.myPrev.myNext = p.myNext;
				p.myNext.myPrev = p.myPrev;
				p.myNext = cur;
				cur.myPrev = p;
			...
			if( head == cur.myPrev && p.myNext !=null){
				p.myPrev.myNext = p.myNext;
				p.myNext.myPrev = p.myPrev;
				cur.myPrev.myNext = p;
...
//			
	...
	public IntList quicksort() {
		...
		while(cur != null){
			...
			smallElements.addToEnd(cur.myItem);
			cur = cur.myNext;
			}else{
				largeElements.addToEnd(cur.myItem);
				cur = cur.myNext;
			}
		}
		// TODO your code here!
		IntList small = smallElements.quicksort();
		IntList large = largeElements.quicksort();
		small.addToEnd(divider);
		small.append(large);
		return small;
	}
	...
	public IntList mergeSort() {
		...
		ListNode cur = myHead;
		for(int i= 0 ; i < mid; i++){
			oneHalf.addToEnd(cur.myItem);
			cur = cur.myNext;
		}
		for(int i= mid ; i < mySize; i++){
			otherHalf.addToEnd(cur.myItem);
			cur = cur.myNext;
		}
