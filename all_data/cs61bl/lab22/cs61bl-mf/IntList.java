public class IntList {
	...
	private class ListNode {
		...
		@Override
		public String toString() {
			// return "(" + myPrev.myItem + ", " + myItem + ", " + myNext.myItem
			// + ")";
			return "" + myItem;
		}
	}
	...
	private ListNode insert(ListNode p, ListNode head) {
		...
		insertAtEnd(new ListNode(p.myItem), head);
		ListNode tail = head;
		while (tail.myNext != null)
			tail = tail.myNext;
		while (tail != head) {
			if (tail.myItem < tail.myPrev.myItem) {
				swap(tail.myPrev, tail, head);
				if (tail.myNext == head) {
					head = tail;
				}
			} else
	...
	private void swap(ListNode one, ListNode two, ListNode head) {
		ListNode previous = one.myPrev;

		ListNode next = two.myNext;
		if (previous != null) {
			previous.myNext = two;
		}
		two.myPrev = previous;
	...
	private void insertAtEnd(ListNode toInsert, ListNode head) {
		ListNode tail = head;
		while (tail.myNext != null)
			tail = tail.myNext;

		tail.myNext = toInsert;
		toInsert.myPrev = tail;
