public class IntList {
	...
	private ListNode insert(ListNode p, ListNode head) {
		...
		while (curr.myNext != null && ln.myItem > curr.myItem) {
			curr = curr.myNext;
		}
		if (curr.myNext == null
				&& ln.myItem > curr.myItem) {
			curr.myNext = ln;
			ln.myPrev = curr;
			return head;
		} else if (curr.myPrev == null) {
			curr.myPrev = ln;
			ln.myNext = curr;
			return ln;
		} else {
			ListNode temp = curr.myPrev;
			temp.myNext = ln;
			curr.myPrev = ln;
			ln.myPrev = temp;
			ln.myNext = curr;
	...
	public IntList mergeSort() {
		...
		IntList merged = merge(oneHalf.myHead, otherHalf.myHead);
		return merged;
	}
