public class IntList {
	...
	private ListNode insert(ListNode p, ListNode head) {
		...
		if(p == null){
			...
			if(ToInsert.myItem < head.myItem){
				head.myPrev = ToInsert;
				ToInsert.myNext = head;

				return ToInsert;
			}
			for(ListNode q = head; q != null; q = q.myNext){
				curr = q;
				if(ToInsert.myItem < q.myItem){
					ListNode temp = q.myPrev;
					q.myPrev = ToInsert;
					ToInsert.myNext = q;
					temp.myNext = ToInsert;
					ToInsert.myPrev = temp;
					return head;
				}
			}
			curr.myNext = ToInsert;
			ToInsert.myPrev = curr;
		}
		return head;
	}
	...
	public IntList quicksort() {
		...
		IntList qwe = smallElements.quicksort();
		IntList rt = largeElements.quicksort();
		qwe.addToEnd(divider);
		qwe.append(rt);
		return qwe;
	}
	...
	public IntList mergeSort() {
		...
		// TODO your code here!
		int subsize = this.mySize/2;
		for(int i = 0; i < subsize; i++){
			oneHalf.addToEnd(temp.myItem);
			temp = temp.myNext;
		}
		for(int i = subsize; i < mySize; i++){
...
//		oneHalf.mergeSort();
//		otherHalf.mergeSort();
//		oneHalf.insertionSort();
//		otherHalf.insertionSort();
