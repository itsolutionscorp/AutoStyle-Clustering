public class IntList {
	...
	private ListNode insert(ListNode p, ListNode head) {
		...
		ListNode insertible =  new ListNode(p.myItem);
		if (head == null){
			head  = insertible;
			return head;
		}
		if (insertible.myItem < head.myItem){
			head.myPrev = insertible;
			insertible.myNext = head;
			head = insertible;
			return head;
		}
		if (insertible.myItem > head.myItem){
			for (ListNode a = head; a != null; a = a.myNext){
				if(a.myItem > insertible.myItem){
					insertible.myPrev = a.myPrev;
					insertible.myNext = a;
					a.myPrev.myNext = insertible;
					a.myPrev = insertible;
					break;
				}
				if (a.myNext == null){
	...
	public IntList quicksort() {
		...
		for (ListNode a = myHead; a != null; a = a.myNext){
			if (a.myItem == divider){
				largeElements.addToFront(a.myItem);
			}
			if(a.myItem < divider){
				smallElements.addToEnd(a.myItem);			}
			if (a.myItem > divider){
				largeElements.addToFront(a.myItem);
			}
		}
		smallElements.selectionSort();
		largeElements.selectionSort();
	...
	public IntList mergeSort() {
		...
		// TODO your code here
			//ListNode temp = new ListNode (myHead.myItem);
		int counter = 0;
		for (ListNode a = myHead; a!= null; a = a.myNext){
			if (counter  < mySize / 2){
				oneHalf.addToFront(a.myItem);
				counter++;
			}else{
				otherHalf.addToFront(a.myItem);
				counter++;
			}
		}
