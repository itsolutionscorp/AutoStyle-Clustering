public class IntList {
	...
	ListNode myHead; // myFirst
	ListNode myTail; // myLast
	int mySize;
	...
	private ListNode insert(ListNode p, ListNode head) {
		if (head == null) {
			// the first node
			head = new ListNode(p.myItem);
		} else {
			// find the correct position to insert
			int item = p.myItem;
			// check if it should go to the very front
			// e.g. 78 99 100, then insert 10 which should go before 78
			if (item < head.myItem) {
				ListNode newFront = new ListNode(item, null, head);
				head.myPrev = newFront;
				return newFront;
			}
			for (ListNode curr = head; curr != null; curr = curr.myNext) {
				if (curr.myNext == null) {
					// 1st case to stop: next is end
					curr.myNext = new ListNode(item, curr, null);
					break;
				} else if (curr.myItem <= item && curr.myNext.myItem >= item) {
					// 2nd case to stop: next is not end and next is the correct position
					ListNode next_save = curr.myNext;
					curr.myNext = new ListNode(item, curr, next_save);
					next_save.myPrev = curr.myNext;
					break;
				}
			}
		}
	...
	public IntList quicksort() {
		...
		for (ListNode curr = myHead.myNext; curr != null; curr = curr.myNext) {
			if (curr.myItem < divider) {
				smallElements.addToFront(curr.myItem);
			} else {
				largeElements.addToFront(curr.myItem);
			}
		}
		IntList div = new IntList(new ListNode(divider));
		IntList sq = smallElements.quicksort();
		IntList lq = largeElements.quicksort();
		div.append(lq);
		sq.append(div);
		return sq;
	}
	...
	public IntList mergeSort() {
		...
		for (int i = 0; i < mySize / 2; i++) {
			oneHalf.addToEnd(curr.myItem);
			curr = curr.myNext;
		}
		for (int i = 0; i < mySize / 2; i++) {
			otherHalf.addToEnd(curr.myItem);
			curr = curr.myNext;
		}
		if (mySize % 2 != 0) {
			// odd size
			// need one more adding
			otherHalf.addToEnd(curr.myItem);
		}
