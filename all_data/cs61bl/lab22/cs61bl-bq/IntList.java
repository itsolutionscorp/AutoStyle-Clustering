public class IntList {
	...
	private ListNode insert(ListNode p, ListNode head) {
		ListNode insertN = new ListNode(p.myItem);
		if (head == null) {
			return insertN;
		}
		if (head.myItem >= insertN.myItem) {
			insertN.myNext = head;
			head.myPrev = insertN;
			return insertN;
		}
		ListNode front = head;
		while (head != null) {
			ListNode previous = head.myPrev;
			if (head.myItem >= insertN.myItem && (previous.myItem <= insertN.myItem)) {
				previous.myNext = insertN;
				insertN.myNext = head;
				insertN.myPrev = previous;
				head.myPrev = insertN;
				return front;
			}
			if (head.myNext == null) {
	...
	public IntList quicksort() {
		...
		ListNode currentNode = myHead.myNext;
		while (currentNode != null) {
			int insertThis = currentNode.myItem;
			if (currentNode.myItem < divider) {
				smallElements.addToEnd(insertThis);
			} else {
				largeElements.addToEnd(insertThis);
			}
			currentNode = currentNode.myNext;
		}
		IntList list1 = smallElements.quicksort();
		list1.addToEnd(divider);
		IntList list2 = largeElements.quicksort();
		list1.append(list2);
		return list1;
	}
	...
	public IntList mergeSort() {
		...
		for (int i = 0; i < this.mySize; i++) {
			int insertThis = currentNode.myItem;
			if (i < (this.mySize / 2)) {
				oneHalf.addToEnd(insertThis);
				currentNode = currentNode.myNext;
				
			} else {
				otherHalf.addToEnd(insertThis);
				currentNode = currentNode.myNext;
			}
		}
		IntList list1 = oneHalf.mergeSort();
		IntList list2 = otherHalf.mergeSort();
		return merge(list1.myHead, list2.myHead);
	}
