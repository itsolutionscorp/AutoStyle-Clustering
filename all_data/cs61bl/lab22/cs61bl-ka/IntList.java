public class IntList {
	...
	private ListNode insert(ListNode p, ListNode head) {
		...
		if (head == null){
			...
			if (p.myItem <= head.myItem){
				ListNode adding = new ListNode(p.myItem);
				adding.myNext = head;
				adding.myPrev = head.myPrev;
				head.myPrev = adding;
				return adding;
	...
	public IntList quicksort() {
		...
		// TODO your code here!
		int howManyDivider = 0;
		for (int i=0; i < mySize;i++){
			if (this.myHead.myItem < divider){
				smallElements.addToEnd(this.myHead.myItem);
				this.myHead = this.myHead.myNext;
			} else if (this.myHead.myItem == divider){
				howManyDivider++;
				this.myHead = this.myHead.myNext;
			} else {
				largeElements.addToEnd(this.myHead.myItem);
				this.myHead = this.myHead.myNext;
			}
		}

		IntList toReturn = smallElements.quicksort();
		for (int i = 0; i < howManyDivider; i++){
	...
	public IntList mergeSort() {
		...
		for (int i=(mySize/2);i<mySize;i++){
			...
			this.myHead = this.myHead.myNext;//null?
		}
