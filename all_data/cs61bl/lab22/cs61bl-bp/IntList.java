public class IntList {
	...
	private ListNode insert(ListNode p, ListNode head) {
		...
		int ins = p.myItem;
		if(current == null) {
			return new ListNode(ins);
		} else if(ins < head.myItem) {
			ListNode result = new ListNode(ins,null,head);
			head.myPrev = result;
			return result;
		} else {
			while(current.myNext != null && current.myNext.myItem < ins) {
				current = current.myNext;
			}
			ListNode result = new ListNode(ins,current,current.myNext);
			current.myNext = result;
			if(result.myNext!= null) {
				result.myNext.myPrev = result;
			}
			return head;
		}
	}
	...
	public IntList mergeSort() {
		...
		return merge(oneHalf.mergeSort(),otherHalf.mergeSort());
	}
	...
	private static IntList merge(IntList i1, IntList i2) {
		return merge(i1.myHead,i2.myHead);
	}
