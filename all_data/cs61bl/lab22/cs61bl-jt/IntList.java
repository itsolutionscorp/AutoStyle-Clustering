public class IntList {
	...
	private ListNode insert(ListNode p, ListNode head) {
		...
		if (p.myPrev == null) {
			...
			if (p.myItem < head.myItem) {
				head = new ListNode(p.myItem, null, head);
				head.myNext.myPrev = head;
			} else {
				if (head.myNext != null)
					head = new ListNode(head.myItem, null, insert(p,
							head.myNext));
				else
	...
	public IntList quicksort() {
		...
		return merge(smallElements.myHead, merge(new ListNode(divider), largeElements.myHead).myHead);
	}
