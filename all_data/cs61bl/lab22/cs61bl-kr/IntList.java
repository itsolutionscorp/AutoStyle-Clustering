public class IntList {
	...
	private ListNode insert(ListNode p, ListNode head) {
		...
		for (ListNode current = p ;current != myHead && current.myItem < current.myPrev.myItem; current = current.myPrev) {
			int temp = current.myItem;
			current.myItem = current.myPrev.myItem;	
			current.myPrev.myItem = temp;
		}
	...
	public IntList quicksort() {
		...
		IntList sameElements = new IntList();
		...
		for (ListNode p = myHead; p != null; p = p.myNext) {
			if(p.myItem < divider) {
				...
				sameElements.addToEnd(p.myItem);
			}
		} 
		smallElements = smallElements.quicksort();
		largeElements = largeElements.quicksort();

		return merge(merge(smallElements.myHead,sameElements.myHead).myHead,largeElements.myHead);
	}
	...
	public IntList mergeSort() {
		...
		int counter = 0;
		for (ListNode p = myHead; p != null; p = p.myNext, counter++) {
			if(counter < mySize/2) {
				oneHalf.addToEnd(p.myItem);
			}
			else if(counter >= mySize/2) {
