public class IntList {
	...
	private ListNode insert(ListNode p, ListNode head) {
		...
		while (curNode != null) {
			if (p.myItem < curNode.myItem) {
				ListNode newNode = new ListNode(p.myItem);
				if (curNode.myPrev == null) {
					curNode.myPrev = newNode;
					newNode.myNext = curNode;
					head = newNode;
				} else {
					ListNode temp = curNode.myPrev;
					curNode.myPrev.myNext = newNode;
					curNode.myPrev = newNode;
					newNode.myNext = curNode;
					newNode.myPrev = temp;
				}
	...
	public IntList mergeSort() {
		...
		for (int i=0;i<halfPoint;i++) {
			oneHalf.addToEnd(curNode.myItem);
			curNode = curNode.myNext;
		}
		for (int i=halfPoint; i<mySize; i++) {
			otherHalf.addToEnd(curNode.myItem);
			curNode = curNode.myNext;
		}
