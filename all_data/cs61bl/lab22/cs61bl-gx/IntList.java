public class IntList {
	...
	private ListNode insert(ListNode p, ListNode head) {
		...
		if (head == null) {
			...
			ListNode last = null;
			for (ListNode myNode = head; myNode != null && myNode.myItem < item;) {
				last = myNode;
				myNode = myNode.myNext;
			}
			
			if (last != null) {
				newNode.myNext = last.myNext;
				newNode.myPrev = last;
				last.myNext = newNode;
				if (!(newNode.myNext == null)) {
					newNode.myNext.myPrev = newNode;
				}
			} else {
				newNode.myNext = head;
				myHead = newNode;
	...
	public IntList mergeSort() {
		...
		// TODO your code here!
		int middlePoint = mySize/2;
		for (int i = 0; i < middlePoint; i++) {
			oneHalf.addToEnd(myHead.myItem);
			myHead = myHead.myNext;
		}
		for (int i = middlePoint; i < (mySize); i++) {
