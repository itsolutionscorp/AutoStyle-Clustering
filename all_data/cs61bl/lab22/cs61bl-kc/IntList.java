public class IntList {
	...
	private ListNode insert(ListNode p, ListNode head) {
		...
		for (ListNode a = head; a != null; a = a.myNext) {
			if (a.myNext == null) {
				// if a.myNext is null, must mean p has to be at the end 
				a.myNext = p;
				p.myPrev = a;
				p.myNext = null;
				return head;
			}
			if (p.myItem <= a.myNext.myItem) {
				// Inserting p after a and before a.myNext
				ListNode origNext = a.myNext; 
				a.myNext = p;
				p.myPrev = a;
				p.myNext = origNext;
				origNext.myPrev = p;
	...
	public IntList quicksort() {
		...
		// TODO your code here!
		ListNode next = myHead.myNext;
		while (next != null) {
			if (next.myItem > divider) {
				largeElements.addToEnd(next.myItem);
			} else if (next.myItem < divider){
				smallElements.addToFront(next.myItem);
			} else if (next.myItem == divider){
				smallElements.addToEnd(next.myItem);
			}
			next = next.myNext;
		}
	...
	public IntList mergeSort() {
		...
		ListNode h = myHead;
		while (h != null) {
			oneHalf.addToEnd(h.myItem);
			h = h.myNext;
			if (h != null) {
				otherHalf.addToEnd(h.myItem);
				h = h.myNext;
			}
		}
