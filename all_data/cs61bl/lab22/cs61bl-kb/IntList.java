	...
	private ListNode insert(ListNode p, ListNode head) {
		...
		if (head != null) {
			...
			if (p.myItem < temp.myItem) {
				p = new ListNode(p.myItem, null, temp);
				temp.myPrev = p;
				head = p;
				return head;
			} else {
				while (temp != null) {
					if (p.myItem < temp.myItem) {
						p = new ListNode(p.myItem, temp.myPrev, temp);
						temp.myPrev.myNext = p;
						temp.myPrev = p;
						return head;
					}
					temp = temp.myNext;
...
//		
//		ListNode newHead = new ListNode(head.myItem);
//		ListNode pointer = head.myNext;
//		while (pointer != null) {
//			ListNode q = newHead;
//			ListNode next = pointer.myNext;
//			if (p.myItem < q.myItem) {
//				ListNode oldH = newHead;
//				newHead = pointer;
//				newHead.myNext = oldH
//			}
//		}
