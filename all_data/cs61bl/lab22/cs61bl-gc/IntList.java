public class IntList {
	...
	public IntList quicksort() {
		...
		ListNode head = this.myHead.myNext;
		for (int i = 1; i < this.mySize; i++) {
			if (head.myItem < divider) {
				smallElements.addToEnd(head.myItem);
			} else {
				largeElements.addToEnd(head.myItem);
			}
			head = head.myNext;
		}
		...
		smallElements = merge(smallElements.myHead, largeElements.myHead);
	...
	public IntList mergeSort() {
		...
		ListNode head = this.myHead;
		for (int i = 0; i < this.mySize; i++) {
			if  (i < half) {
				oneHalf.addToEnd(head.myItem);
			} else {
				otherHalf.addToEnd(head.myItem);
			}
			head = head.myNext;
		}
