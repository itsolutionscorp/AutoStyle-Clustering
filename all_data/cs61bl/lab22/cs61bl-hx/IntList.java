public class IntList {
	...
	private ListNode insert(ListNode p, ListNode head) {
		...
		if (head == null){
			return new ListNode(p.myItem, head, null);
		}		
		
		if (p.myItem < head.myItem){
			return new ListNode(p.myItem, head.myPrev, head);
		}
		
		return new ListNode(head.myItem, head.myPrev, insert(p, head.myNext));
	}
	...
	public IntList quicksort() {
		...
		int size = mySize;
		while (size > 1){
			if (myHead.myItem<divider){
				smallElements.addToEnd(myHead.myItem);
			} else {
				largeElements.addToEnd(myHead.myItem);
			}
			myHead = myHead.myNext;
			size = size-1;	
		}
	...
	public IntList mergeSort() {
		...
		int size = mySize;
		//while (Math.abs(oneHalf.mySize - otherHalf.mySize)){
		while (size > 1){
			oneHalf.addToEnd(myHead.myItem);
			otherHalf.addToFront(myTail.myItem);
			myHead = myHead.myNext;
			myTail = myTail.myPrev;
			size = size-2;			
		}
...
//	private IntList helper (oneHalf left, otherHalf right){
//		if (left.mySize==1&&right.mySize==1){
//			return merge(left.myHead, right.myHead);
//		}
//		return merge(helper(left))
//	}
//	private ListNode half (ListNode list){
//		for{int i 
//	}
	/**
