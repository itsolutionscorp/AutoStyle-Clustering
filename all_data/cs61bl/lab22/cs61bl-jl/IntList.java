public class IntList {
	...
	private ListNode insert(ListNode p, ListNode head) {
		...
		while (p.myPrev != null && p.myItem < p.myPrev.myItem) {
			if (p.myItem < p.myPrev.myItem) {
				ListNode temp1 = p.myPrev;
				ListNode temp2 = p.myNext;
				if (p.myPrev.myPrev == null) {
					head = p;
				} else {
	...
	public IntList quicksort() {
		...
		IntList finale1 = smallElements.quicksort();
		IntList finale2 = largeElements.quicksort();
		finale1.append(middle);
		finale1.append(finale2);
		return finale1; 
	}
