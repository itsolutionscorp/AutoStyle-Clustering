public class IntList {
	...
	private ListNode insert(ListNode p, ListNode head) {
		...
		ListNode to_insert = new ListNode(p.myItem);
		ListNode pointer = head;
		if (head == null) {
			return to_insert;
		}
		while(pointer != null) {
			if (to_insert.myItem < pointer.myItem) {
				to_insert.myNext = pointer;
				to_insert.myPrev = pointer.myPrev;
				if (pointer.myPrev != null) {
					pointer.myPrev.myNext = to_insert;
				}
				pointer.myPrev = to_insert;
				break;
			} else if (pointer.myNext == null) {
				pointer.myNext = to_insert;
				to_insert.myPrev = pointer;
				break;
			}
			pointer = pointer.myNext;
		}
		ListNode pointer2 = to_insert;
		while(pointer2 != null) {
			if (pointer2.myPrev == null) {
				break;
			}
			pointer2 = pointer2.myPrev;
		}
		return pointer2;
	}
	...
	public IntList quicksort() {
		...
		while(pointer != null) {
			...
			} else if (pointer.myItem > divider){
	...
	public IntList mergeSort() {
		...
		for(int i = 0; i < (mySize / 2); i++) {
			if (i + 1 == (mySize / 2)) {
				otherHalf.myHead = pointer.myNext;
				otherHalf.mySize = i + 1;
				oneHalf.mySize = i + 1;
				pointer.myNext = null;
			}
			pointer = pointer.myNext;
