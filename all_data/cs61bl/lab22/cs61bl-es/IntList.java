public class IntList {
	...
	private ListNode insert(ListNode p, ListNode head) {
		...
		ListNode current = p;
		while (current.myPrev != null) {
			current = p.myPrev;
			if (current == head && p.myItem < current.myItem) {
				if (p.myNext != null) {
					p.myNext.myPrev = head;
				}
				current.myNext = p.myNext;
				p.myPrev = null;
				p.myNext = head;
				head.myPrev = p;
				return p;
			}
			else if (p.myItem < current.myItem) {
				if (p.myNext != null) {
					p.myNext.myPrev = current;
				}
				current.myNext = p.myNext;
				current.myPrev.myNext = p;
				p.myPrev = current.myPrev;
				p.myNext = current;
				current.myPrev = p;
			}
			else {
	...
	public IntList quicksort() {
		...
		ListNode current = myHead.myNext;
		while (current != null) {
			if (current.myItem < divider) {
	...
	public IntList mergeSort() {
		...
		oneHalf.myTail = current.myPrev;
		oneHalf.mySize = half;
		
		IntList otherHalf = new IntList();
		otherHalf.myHead = current;
		
		current.myPrev.myNext = null;
		current.myPrev = null;
		
		while (current.myNext != null) {
			current = current.myNext;
		}
		otherHalf.myTail = current;
		otherHalf.mySize = mySize - half;
