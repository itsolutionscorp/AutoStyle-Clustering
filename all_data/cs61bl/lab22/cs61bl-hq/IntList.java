public class IntList {
	...
	private ListNode insert(ListNode p, ListNode head) {
		...
		ListNode point = head;
		if (p.myItem >= point.myItem) {
			while (point.myNext != null) {
				if (point.myNext.myItem > p.myItem) {
					ListNode in = new ListNode(p.myItem, point, point.myNext);
					point.myNext.myPrev = in;
					point.myNext = in;
					return head;
				} else {
					while (point.myNext != null && p.myItem > point.myNext.myItem) {
						point = point.myNext;
					}
					ListNode in = new ListNode(p.myItem, point, point.myNext);
					if (point.myNext != null) {
						point.myNext.myPrev = in;
					}
					point.myNext = in;
					return head;
			...
			ListNode in = new ListNode(p.myItem, point, null);
			point.myNext = in;
			return head;
		} else {
			ListNode in = new ListNode(p.myItem, null, point);
			point.myPrev = in;
			return in;
		}
	}
	...
	public IntList quicksort() {
		...
		IntList pivot = new IntList(new ListNode(divider));
		IntList front = smallElements.quicksort();
		IntList back = largeElements.quicksort();
		pivot.append(back);
		front.append(pivot);
		return front;
	}
