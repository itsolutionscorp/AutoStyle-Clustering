public class IntList {
	...
	private ListNode insert(ListNode p, ListNode head) {
		...
		p= new ListNode(p.myItem,null,null);
		if(head ==null){
			head =p;
		}else{
			...
			if(p.myItem<=head.myItem){
				...
				while(temp.myNext!=null){
					...
					if(p.myItem>=temp.myItem&&p.myItem<=temp.myNext.myItem)
					{	p.myNext=temp.myNext;
	...
	public IntList mergeSort() {
	    ...
	    ListNode secondhalf = myHead.myNext;
	    myHead.myNext=null;
	    while(secondhalf!=null){
	    	
	    	otherHalf.addToEnd(secondhalf.myItem);
	    	secondhalf=secondhalf.myNext;
	    }
	    while(temp!=null){
