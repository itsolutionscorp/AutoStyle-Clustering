public class IntList {
	...
	private class ListNode {
		...
		public String toString() {
			String s = "" + myItem + " ";
			for (ListNode p = myNext; p != null; p = p.myNext) {
				s = s + p.myItem + " ";
			}
			return s;
		}
	...
	public IntList insertionSort() {
		ListNode soFar = null;
		for (ListNode p = myHead; p != null; p = p.myNext) {
			soFar = insert(new ListNode(p.myItem), soFar);
		}
		return new IntList(soFar);
	}
	
	private void swap(ListNode l1, ListNode l2) {
		ListNode l0 = l1.myPrev;
		ListNode l3 = l2.myNext;
		l1.myPrev = l2;
		l1.myNext = l3;
		l2.myPrev = l0;
		l2.myNext = l1;
	...
	private ListNode insert(ListNode p, ListNode head) {
		...
		p.myNext = head;
		head.myPrev = p;
		while (p.myNext != null && p.myItem > p.myNext.myItem) {
			swap(p, p.myNext);
		}
		while (p.myPrev != null) {
			p = p.myPrev;
	...
	public IntList quicksort() {
		...
		IntList s1 = smallElements.quicksort();
		IntList s2 = largeElements.quicksort();
		s1.addToEnd(divider);


		if (!s1.isEmpty()){
			s1.myTail.myNext = s2.myHead;
		}
		if(!s2.isEmpty()) {
			s2.myHead.myPrev = s1.myTail;
		}
	...
	public IntList mergeSort() {
		...
		IntList s1 = oneHalf.mergeSort();
		IntList s2 = otherHalf.mergeSort();

		return merge(s1.myHead, s2.myHead);
