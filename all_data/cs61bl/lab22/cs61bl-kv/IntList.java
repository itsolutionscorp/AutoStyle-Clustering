public class IntList {
	...
	private ListNode insert(ListNode p, ListNode head) {
		...
		head.myNext = this.insert(p, head.myNext);
		return head;
	}
	...
	public IntList quicksort() {
		...
		// TODO your code here!
		ListNode blah = this.myHead.myNext;
		while (blah != null) {
			if (blah.myItem < divider) {
				smallElements.addToEnd(blah.myItem);
			} else {
				largeElements.addToEnd(blah.myItem);
			}
			blah = blah.myNext;
			
		}
		IntList a = smallElements.quicksort();
		a.addToEnd(divider);
		a.append(largeElements.quicksort());
		return a;
	}
	...
	public IntList mergeSort() {
		...
		int k = 0;
		ListNode m = myHead;
		while (k < this.mySize/2) {
			k++;
			oneHalf.addToEnd(m.myItem);
			m = m.myNext;
		}
		while (k < this.mySize) {
			k++;
			otherHalf.addToEnd(m.myItem);
			m = m.myNext;
		}
//		oneHalf.mergeSort();
//		otherHalf.mergeSort();
