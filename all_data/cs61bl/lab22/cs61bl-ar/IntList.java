public class IntList {
	...
	private ListNode insert(ListNode p, ListNode head) {
		ListNode createdNode = new ListNode(p.myItem);
		ListNode myCurr = head;
		if(myCurr == null){
			head = createdNode;
			return head;
		}
		if(head.myItem>createdNode.myItem){
			 createdNode.myNext = head;
			 return createdNode;
		}
		while(myCurr!=null){
			if(myCurr.myNext!=null&&createdNode.myItem>=myCurr.myItem&&createdNode.myItem<=myCurr.myNext.myItem){
				createdNode.myNext = myCurr.myNext;
				myCurr.myNext = createdNode;
				return head;
			}else if(myCurr.myNext==null&&createdNode.myItem>=myCurr.myItem){
				myCurr.myNext = createdNode;
				return head;
			}
   ...
   public boolean liesNext(ListNode n1,ListNode n2){
	   if(n1==null||n2==null){
		   return false;
	   }
	   if(n1 == n2){
	...
	public IntList quicksort() {
		...
		for(ListNode myCurr=myHead.myNext;myCurr!=null;myCurr=myCurr.myNext){
			if (myCurr.myItem >= divider){
				largeElements.addToEnd(myCurr.myItem);
			}else{
				smallElements.addToEnd(myCurr.myItem);
			}
		}
