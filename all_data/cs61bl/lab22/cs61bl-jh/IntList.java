public class IntList {
	...
	private ListNode insert(ListNode p, ListNode head) {
	    ...
	    ListNode pCopy = new ListNode(p.myItem);
	    for (ListNode pCPointer = p; pCPointer != null; pCPointer = pCPointer.myNext) {
	        pCopy.myNext = p.myNext;
	        
	    }
	    if(head == null) {
	        System.out.println("p  "+new ListNode(p.myItem));
	        return new ListNode(p.myItem);
	    }
	    if (pCopy.myItem <= head.myItem) {
	        head.myPrev = pCopy;
	        pCopy.myNext = head;
	        System.out.println("PCopy  "+pCopy);
	        return pCopy;
	    } 
	    if (pCopy.myItem > head.myItem && head.myNext == null) {
	        ListNode temp = new ListNode(pCopy.myItem);
	        head.myNext = temp;
	        temp.myPrev = head;
	    ...
	    ListNode pointer = head.myNext;
	    ListNode prev = pointer.myPrev;
	    while (pointer != null) {
	        prev = pointer.myPrev;
	        if (pCopy.myItem <= pointer.myItem) {
	            prev = pointer.myPrev;
	            prev.myNext = pCopy;
	            pCopy.myPrev = prev;
	            pCopy.myNext = pointer;
	            pointer.myPrev = pCopy;
	            System.out.println("head  " + head);
	            return head;
	        } else {
	    ...
	    ListNode l = new ListNode(pCopy.myItem);
	    prev.myNext.myNext = l;
	    l.myPrev = prev.myNext;
	...
	public IntList quicksort() {
		...
		for (ListNode p = myHead; p != null; p = p.myNext) {
		    ...
		    } else if (p.myItem > divider){
		        largeElements.addToEnd(p.myItem);
		    }
		}
		smallElements = smallElements.quicksort();
		largeElements = largeElements.quicksort();
		smallElements = merge(smallElements.myHead, new ListNode(divider));
		return merge(smallElements.myHead, largeElements.myHead);
	}
