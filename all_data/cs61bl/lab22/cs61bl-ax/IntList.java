public class IntList {
	...
	private ListNode insert(ListNode p, ListNode head) {
		if (head == null) {
			...
			head.myPrev = new ListNode(p.myItem, null, head);
			return head.myPrev;
		} else if (head.myNext == null) {
			head.myNext = new ListNode(p.myItem, head, null);
			return head;
		} else {
			ListNode n = head;
			while (n != null) {
				if (p.myItem < n.myItem) {
					ListNode toInsert = new ListNode(p.myItem, n.myPrev, n);
					n.myPrev.myNext = toInsert;
					n.myPrev = toInsert;
					return head;
				}
				if (n.myNext == null) {
					n.myNext = new ListNode(p.myItem, n, null);
					return head;
				}
				n = n.myNext;
			}
	...
	public IntList quicksort() {
		...
		// TODO your code here!
		for (ListNode n = myHead.myNext; n != null; n = n.myNext) {
			if (n.myItem < divider) {
				smallElements.addToEnd(n.myItem);
			} else {
				largeElements.addToEnd(n.myItem);
			}
		}
		IntList lst = smallElements.quicksort();
		lst.addToEnd(divider);
		lst.append(largeElements.quicksort());
