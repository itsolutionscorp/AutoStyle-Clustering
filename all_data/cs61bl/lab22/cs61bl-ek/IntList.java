public class IntList {
	 ...
	 */
	private ListNode insert(ListNode p, ListNode head) { //newest starting point, new beginning
		if (head == null) {
			return p;
		}
		for (ListNode q = p; q != head && q.myItem < q.myPrev.myItem; q = q.myPrev) {
			if (q.myPrev == myHead && q == myTail) {
				q.myNext = q.myPrev;
				q.myPrev.myPrev = q;
				q.myPrev.myNext = null;
				q.myPrev = null;
				myHead = q;
				head = q;
				myTail = q.myNext;
				q = q.myNext;
			} else if (q.myPrev == myHead) {
				q.myNext.myPrev = q.myPrev;
				q.myPrev.myNext = q.myNext;
				q.myPrev.myPrev = q;
				q.myNext = q.myPrev;
				q.myPrev = null;
				myHead = q;
				...
				q.myNext = q.myPrev;
				q.myPrev.myPrev.myNext = q;
				q.myPrev = q.myPrev.myPrev;
				q.myNext.myNext = null;
				q.myNext.myPrev = q;
				myTail = q.myNext;
				q = q.myNext;
			} else {
	...
	public IntList quicksort() {
		...
		for (int k = 0; k < mySize-1; k++) {
			if (p.myItem < divider) {
				smallElements.addToFront(p.myItem);
			} else {
				largeElements.addToFront(p.myItem);
			}
			p = p.myNext;
		}
	...
	public IntList mergeSort() {
		...
		for (int k = 0; k < mySize; k++) {
			if (k < split) {
				oneHalf.addToFront(p.myItem);
			} else {
				otherHalf.addToFront(p.myItem);
			}
			p = p.myNext;
		}
