public class IntList {
    ...
    private ListNode insert(ListNode p, ListNode head) {
        ...
        if(head == null){
            ListNode node = new ListNode(p.myItem, null, null);
            head = node;

        }else if(head.myItem > p.myItem ){
            ListNode node = new ListNode(p.myItem, null, head);
            head = node;
        }
            ...
            ListNode runner = head.myNext;

            while(runner != null && runner.myItem < p.myItem){
                current = runner;
                runner = runner.myNext;
            }

            ListNode node = new ListNode(p.myItem, current, runner);
            current.myNext = node;
