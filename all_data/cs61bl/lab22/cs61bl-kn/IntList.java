public class IntList {
	...
	private ListNode insert(ListNode p, ListNode head) {
		...
		if (head == null) {
		...
		else if (p.myItem < head.myItem) {
			ListNode a = new ListNode(p.myItem);
			ListNode b = a;
			for (ListNode q = head; q != null; q = q.myNext) {
				b.myNext = new ListNode (q.myItem, b, null);
				b = b.myNext;
			}
			return a;
		}
		else {
			ListNode a = new ListNode(head.myItem);
			ListNode b = a;
			for (ListNode q = head.myNext; q != null; q = q.myNext){
				if (p.myItem < q.myItem){
					b.myNext = new ListNode(p.myItem, b, null);
					b = b.myNext;
					for (ListNode r = q; r != null; r = r.myNext ) {
						b.myNext = new ListNode(r.myItem, b, null);
						b = b.myNext;
					}
					return a;
				}
			...
			b.myNext = new ListNode (p.myItem);
			return a;
		}
	}
	...
	public IntList quicksort() {
		...
		// TODO your code here!
		ListNode p;
		for (p = myHead.myNext; p != null; p = p.myNext) {
			if (p.myItem < divider) {
		...
		IntList newList = smallElements.quicksort();
		newList.addToEnd(divider);
		newList.append(largeElements.quicksort());
		return newList;
	}
