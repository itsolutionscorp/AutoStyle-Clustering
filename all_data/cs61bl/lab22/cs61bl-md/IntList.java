public class IntList {
	...
	private ListNode insert(ListNode p, ListNode head) {
		...
		if (head.myNext==null && head.myItem<p.myItem){
			head.myNext = newp;
			newp.myPrev = head;
			return head;
		}
		if (head.myItem>p.myItem){
			newp.myNext = head;
			newp.myPrev = head.myPrev;
			head.myPrev = newp;
			return newp;
		}

		ListNode current = head.myNext;
		while(current!=null){
			if (current.myItem>newp.myItem){
				newp.myNext = current;
				newp.myPrev = current.myPrev;
				current.myPrev.myNext = newp;
				current.myPrev = newp;
				break;
			} 
	...
	public IntList quicksort() {
		...
		for (ListNode p = myHead.myNext; p != null; p = p.myNext) {
			...
			else largeElements.addToEnd(p.myItem);
		}
	...
	public IntList mergeSort() {
		...
		// TODO your code here!
		ListNode halfNode = myHead;
		int halfPoint = (mySize/2); 
		for(int i=0; i<halfPoint-1;i++){
			halfNode = halfNode.myNext;
		}
		ListNode halfNodeNext = halfNode.myNext;

		
		halfNode.myNext = null;
		oneHalf.myHead = myHead;
		oneHalf.myTail = halfNode;
		oneHalf.mySize = halfPoint;

		halfNodeNext.myPrev = null;
		otherHalf.myHead = halfNodeNext;
		otherHalf.myTail = myTail;
		otherHalf.mySize = mySize-halfPoint;


		resultList = merge(oneHalf.mergeSort().myHead,otherHalf.mergeSort().myHead);
		return resultList;
	}
