public class IntList {
	...
	private ListNode insert(ListNode p, ListNode head) {	
		if(head == null) return new ListNode(p.item());
		ListNode insert = new ListNode(p.item());
		insert.myNext = head;
		head.myPrev = insert;		
		ListNode curr = head;
		ListNode front = insert;
		
		while(curr != null && insert.item() > curr.item()){
			swapItem(insert, curr);	
			insert = curr;
			curr = curr.myNext;
		}
		return front;
	...
	public void swapItem(ListNode p, ListNode curr){
		int temp = p.myItem;
		p.myItem = curr.myItem;
		curr.myItem = temp;					
	}
	...
	public IntList quicksort() {
		...
		myHead = myHead.myNext;
		while(myHead != null){
			if(myHead.item() < divider){
				smallElements.addToEnd(myHead.item());
			}
			else{
				largeElements.addToEnd(myHead.item());
			}
			remove(myHead);
		}		
	...
	public IntList mergeSort() {
		...
		int half = mySize/2; int end = mySize;
		for(int i = 0; i < half; i++){
			oneHalf.addToEnd(myHead.item());
			remove(myHead);
		}
		for(int i = half; i < end; i++){
			otherHalf.addToEnd(myHead.item());
			remove(myHead);
		}
		ListNode left = oneHalf.mergeSort().myHead;
		ListNode right = otherHalf.mergeSort().myHead;
