public class IntList {
	...
	private ListNode insert(ListNode p, ListNode head) {
		...
		while (head.myNext != null && head.myItem <= p.myItem) {
			head = head.myNext;
		}
		if (head.myItem <= p.myItem) {
			ListNode temp = head.myNext;
			head.myNext = new ListNode(p.myItem);
			head.myNext.myNext = temp;
			head.myNext.myPrev = head;
			if (temp != null) {
				temp.myPrev = head.myNext;
			}
		} else {
			ListNode temp = head.myPrev;
			head.myPrev = new ListNode(p.myItem);
			head.myPrev.myPrev = temp;
			head.myPrev.myNext = head;
			if (temp != null) {
				temp.myNext = head.myPrev;
			}
		}
	...
	public IntList quicksort() {
		...
		// TODO your code here!
		for (ListNode l = myHead.myNext; l != null; l = l.myNext) {
			if (l.myItem < divider) {
				smallElements.addToFront(l.myItem);
			}
			else {
				largeElements.addToFront(l.myItem);
			}
		}
		IntList ret = smallElements.quicksort();
		ret.addToEnd(divider);
		ret.append(largeElements.quicksort());
		return ret;
	}
	...
	public IntList mergeSort() {
		...
		int start = 0;
		while (start < middle) {
			oneHalf.addToFront(temp.myItem);
			temp = temp.myNext;
			start++;
		}
		while (start < mySize) {
			otherHalf.addToFront(temp.myItem);
			temp = temp.myNext;
			start++;
		}
