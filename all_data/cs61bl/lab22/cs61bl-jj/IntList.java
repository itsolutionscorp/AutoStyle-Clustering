public class IntList {
	...
	private ListNode insert(ListNode p, ListNode head) {
		...
		for (insertIn = head; insertIn.myNext != null; insertIn = insertIn.myNext) {
			if (insertIn.myNext.myItem > pCopy.myItem) {
				break;
			}
		}
		
		pCopy.myNext = insertIn.myNext;
		pCopy.myPrev = insertIn;
		if (pCopy.myNext != null)
			pCopy.myNext.myPrev = pCopy;
		insertIn.myNext = pCopy;
	...
	public IntList quicksort() {
		...
		IntList small = new IntList();
		IntList large = new IntList();
		int pivot = myHead.myItem;
		// TODO your code here!
		for(ListNode cur =myHead.myNext; cur != null; cur = cur.myNext){
			if(cur.myItem < pivot){
				small.addToEnd(cur.myItem);
			} else{
				large.addToEnd(cur.myItem);
			}
	
		}
		IntList Return = new IntList();
		Return.append(small.quicksort());
		Return.addToEnd(pivot);
		Return.append(large.quicksort());
	...
	public IntList mergeSort() {
		...
		for(int i = 0; i < half;i++) {
			Left.addToEnd(current.myItem);
			current = current.myNext;
		}
		for (;current != null; current = current.myNext) {
			Right.addToEnd(current.myItem); 
		}
		
		return IntList.merge(Left.mergeSort().myHead, Right.mergeSort().myHead);
	}
