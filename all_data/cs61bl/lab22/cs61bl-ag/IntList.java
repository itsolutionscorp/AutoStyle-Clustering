public class IntList {
	...
	private ListNode insert(ListNode p, ListNode head) {
		ListNode takua = new ListNode(p.myItem);
		if(head == null) {
			return takua;
		}
		for (ListNode l = head; l!=null; l = l.myNext) {
			if(l.myItem > takua.myItem) {
				if (l.myPrev != null) {
					l.myPrev.myNext = takua;
					takua.myPrev = l.myPrev;
					takua.myNext = l;
					l.myPrev = takua;
					return head;
				}
	...
	public IntList quicksort() {
		...
		for(ListNode l = myHead.myNext; l != null; l = l.myNext) {
			if(l.myItem <= divider) {
				smallElements.addToEnd(l.myItem);
			}
			else {
				largeElements.addToEnd(l.myItem);
			}
		}
		IntList toReturn = new IntList();
		toReturn.append(smallElements.quicksort());
		toReturn.addToEnd(divider);
	...
	public IntList mergeSort() {
		...
		for(int i = 0; i < half; i++) {
			oneHalf.addToEnd(l.myItem);
			l = l.myNext;
		}
		IntList otherHalf = new IntList();
		for(int i = half; i < mySize; i++) {
			otherHalf.addToEnd(l.myItem);
			l = l.myNext;
		}
		IntList mergeOne = oneHalf.mergeSort();
		IntList mergeTwo = otherHalf.mergeSort();
		IntList toReturn = merge(mergeOne.myHead, mergeTwo.myHead);
		return toReturn;
	}
