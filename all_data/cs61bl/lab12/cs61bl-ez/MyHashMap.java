...
public class MyHashMap<K, V> implements Iterable<K> {
	...
	public boolean containsKey(K key) {
		ArrayList<KVPair> temp = hashie.get(key.hashCode()%capacity);
	...
	public boolean containsValue(V value) {
		...
		for(int i = 0; i<size; i++) {
			...
			for(int j = 0; j< size2; j++) {
				if(hashie.get(i).get(j).getValue().equals(value) {
					return true;
				}
			}
		}
	...
	private void expand(int newCapacity) {
		...
		ArrayList<ArrayList<KVPair>> temp = new ArrayList<ArrayList<KVPair>>();
		int size = hashie.size();
		for(int i = 0; i<size; i++){
			int size2 = hashie.get(i).size();
			for(int j = 0; j<size2; j++) {
	...
	private class HashMapIterator implements Iterator<K> {
		...
		public K next() {
			if(numSeen==hashie.get(outerIndex).size()) {
				outerIndex++;
				prevIndex = 0;
				numSeen = 0;
				return next();
