...
public class ArrayHeap<T> {
	...
	public boolean contains(int i) {
		return containsHelper(1, i);
	}
	
	public boolean containsHelper(int index, int node) {
		if (getNode(index) == null) {
			return false;
		} else if (getNode(index).item().equals(node)) {
			return true;
		} else {
			return (containsHelper(getRightOf(index), node) || containsHelper(getLeftOf(index), node));
		}
	}
	...
	//returns Node with matching item
	public Node findNode(T item) {
		int matchingNode = findMatchingNode(item, 1); 
		return getNode(matchingNode);
	}
