...
public class Graph implements Iterable<Integer> {
    ...
    public ArrayList<Integer> shortestPath(int startVertex, int endVertex) {
        ...
        while (!fringe.isEmpty()) {
            ArrayHeap<Integer>.Node vertex = fringe.removeMin();
            Integer vertexItem = (Integer) vertex.item();
            Double vertexPriority = vertex.priority();
            visited.add(vertexItem);
            for (Edge neighbor : myAdjLists[vertexItem]) {
                if (fringe.findPriority(neighbor.myTo) != null) {
                    Double dis = Math.min(fringe.findPriority(neighbor.myTo), vertexPriority + (Integer) neighbor.myEdgeInfo);
                    fringe.changePriority(neighbor.myTo, dis);
                    if (dis.equals(vertexPriority + (Integer) neighbor.myEdgeInfo)) {
                        path.put(neighbor.myTo, vertexItem); 
                    }
                }
            }
        }
        Stack<Integer> result = new Stack<Integer>();
        Integer pre = path.get(endVertex);
        result.add(endVertex);
        result.add(pre);
        while (!pre.equals(startVertex)) {
            pre = path.get(pre);
            result.add(pre);
        }
    ...
    public static void main(String[] args) {
        ...
        // // System.out.println(g1.pathExists(2, 0));
        // while (iter.hasNext()) {
