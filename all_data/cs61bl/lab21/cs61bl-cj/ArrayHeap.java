...
public class ArrayHeap<T> {
	...
	public void insert(T item, double priority) {
		while (lastIndex + 1 >= contents.size()) {
			contents.add(null);
		}
		lastIndex++;
		contents.set(lastIndex, new Node(item, priority));
		bubbleUp(lastIndex);
	}
	...
	public T removeMin() {
		...
		Node last = contents.get(lastIndex);
		setNode(1, last);
		contents.remove(lastIndex);
		//while (getNode(lastIndex) == null) {
		lastIndex--;
		bubbleDown(1);
		return min.item();
	}
