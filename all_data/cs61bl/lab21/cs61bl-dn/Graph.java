...
public class Graph implements Iterable<Integer>{
    ...
    public void addUndirectedEdge(int v1, int v2, Object edgeInfo) {
    	Edge e = new Edge(v1, v2, edgeInfo);
    	myAdjLists[v1].add(e);
    	Edge f = new Edge(v2, v1, edgeInfo);
    	myAdjLists[v2].add(f);
    }
    ...
    private class DFSIterator implements Iterator<Integer> {
        ...
        public Integer next() {
            //your code here
        	int rtn = fringe.pop();
        	while (visited.contains(rtn) && !fringe.isEmpty()) {
        		rtn = fringe.pop();
        	}
    ...
    public ArrayList<Integer> path(int startVertex, int stopVertex) {
        ...
        for (int i = 0; i < temp; i++) {
        	if (isAdjacent(visited.get(i), curr)) {
        		if (visited.get(i) == startVertex) {
        			break;
        		}
    ...
    public ArrayList<Integer> shortestPath (int startVertex, int endVertex){
        //your code here...
    	HashMap<Integer, Integer> nodeToCost = new HashMap<>();
    	HashMap<Integer, Integer> Previous = new HashMap<>();
    	HashSet<Integer> finalized = new HashSet<>();
    	
    	class NodeComparator implements Comparator<Integer> {
        	@Override
        	public int compare(Integer i, Integer j) {
        		return (nodeToCost.get(i) - nodeToCost.get(j));
        	}
        }
    	...
    	for (int i = 0; i < myVertexCount; i ++) {
    		if (i == startVertex) {
    			...
    			nodeToCost.put(i, Integer.MAX_VALUE);
    			fringe.offer(i);
    			Previous.put(i, null);
    		}
    	}
    	...
    	while (!fringe.isEmpty()) {
    		...
    		for (Edge e: myAdjLists[i]) {
    			if (!finalized.contains(e.to()) && 
    					((Integer) e.info() + nodeToCost.get(i)) < nodeToCost.get(e.to())) {
    				nodeToCost.put(e.to(), ((Integer) e.info() + nodeToCost.get(i)));
    				Previous.put(e.to(), i);
    				fringe.remove(e.to());
    				fringe.add(e.to());
    			}
