...
public class ArrayHeap<T> {
	...
	public void changePriority(T item, int priority) {
		...
		for (int index = 0; index < contents.size(); index++) {
			if (getNode(index) != null && getNode(index).item().equals(item)) {
	...
	public void changeItem(T toReplace, T newItem) {
		for (int index = 0; index < contents.size(); index++) {
			if (getNode(index) != null && getNode(index).item().equals(toReplace)) {
				getNode(index).myItem = newItem;
			}
		}
	}
	
	/**
	 * Get priority given an item. If that item does not exist, returns null.
	 */
	public Integer getPriority(T item) {
		for (Node n : contents) {
			if (n != null && n.item().equals(item)) {
				return (Integer) n.priority();
			}
		}
		return null;
	}
