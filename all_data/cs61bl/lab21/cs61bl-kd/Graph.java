...
public class Graph implements Iterable<Integer>{
    ...
    public List neighbors(int vertex) {
    	...
    	for (Edge e : this.myAdjLists[vertex]) {
    		myList.add(e.to());
    	}
        return myList;
    }
    ...
    private class TopologicalIterator implements Iterator<Integer> {
        ...
        public Integer next() {
        	if (hasNext()) {
            	...
            	for (int i : ((LinkedList<Integer>)neighbors(current))) {
    ...
    public ArrayList<Integer> shortestPath (int startVertex, int endVertex){
    	...
    	Edge[] parents = new Edge[myVertexCount];
    	...
    	while (fringe.contents.size() > 1 && fringe.peek().item() != endVertex) {
    		ArrayHeap<Integer>.Node minimum = fringe.removeMin();
    		int num = minimum.item();
    		double prior = minimum.priority();
    		finalized.add(num);
    		for (Edge e : this.myAdjLists[num]) {
    			if (!finalized.contains(e.to())) {
    				double totprior = (double) (prior + (int) e.info());
    				if (fringe.changePriority(e.to(), totprior)) {
    					parents[e.to()] = e;
    				}
				}
			}
		}
		if (fringe.peek().item() != endVertex) {
			return null;
		} 
		else {
    ...
    public static void main(String[] args) {
        ...
        Graph djik = new Graph(5);
        djik.addEdge(0, 1, 10);
        djik.addEdge(0, 2, 30);
        djik.addEdge(0, 4, 100);
        djik.addEdge(1, 2, 50);
        djik.addEdge(2, 4, 10);
        djik.addEdge(3, 2, 20);
        djik.addEdge(3, 4, 60);
        System.out.println(djik.shortestPath(0, 4));
    }
