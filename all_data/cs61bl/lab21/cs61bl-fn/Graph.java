...
public class Graph implements Iterable<Integer> {
	...
	public ArrayList<Integer> shortestPath(int startVertex, int endVertex) {
		if (pathExists(startVertex, endVertex)) {
			...
			int[] distances = new int[myVertexCount];
			...
			while (!vertices.isEmpty()) {
				int current = shortest(vertices, distances);
				vertices.remove(vertices.indexOf(current));
				for (Edge currentEdge : myAdjLists[current]) {
					int neighborVertex = currentEdge.to();
					if (distances[neighborVertex] > distances[current]
							+ (int) currentEdge.myEdgeInfo) {
						distances[neighborVertex] = distances[current]
								+ (int) currentEdge.myEdgeInfo;
						steps.put(neighborVertex, current);
					}
				}
			}
			Stack<Integer> reversePath = new Stack<>();
			Integer currentStep = endVertex;
			while (currentStep != null) {
				Integer previousStep = steps.get(currentStep);
				if (previousStep != null) {
					reversePath.push(previousStep);
	...
	private int shortest(List<Integer> vertices, int[] distances) {
		int currentMin = Integer.MAX_VALUE;
		int vertex = 0;
		for (int i : vertices) {
			if (!(currentMin < distances[i])) {
