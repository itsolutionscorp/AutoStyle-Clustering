...
//		}
	...
	public ArrayList<Integer> shortestPath (int startVertex, int endVertex){
		...
		Integer[] edgeInfos = new Integer[myVertexCount];
		ArrayList<Integer> rtnList = new ArrayList<Integer>();
		
		for (int i = 0; i < myVertexCount; i++) {
			edgeInfos[i] = Integer.MAX_VALUE;
			prev[i] = -1;
		}
		
		edgeInfos[startVertex] = 0;
		prev[startVertex] = startVertex;
		for (int i = 0; i < myVertexCount; i++) {
			fringe.insert(i, edgeInfos[i]);
		}
		
		while (!fringe.isEmpty()) {
			Integer temp = fringe.removeMin().item();
			if (temp == endVertex) {
				break;
			}
			List<Integer> nextTo = neighbors(temp);
			for (int i = 0; i < nextTo.size(); i++) {
				Integer info = null;
				LinkedList<Edge> edges = myAdjLists[temp];
				for (int j = 0; j < edges.size(); j++) {
					if (edges.get(j).to() == nextTo.get(i)) {
						info = (Integer) edges.get(j).info();
					}
				}
				if (info == null) {
					info = 0;
				}
				if (edgeInfos[nextTo.get(i)] > edgeInfos[prev[temp]] + info) {
					prev[nextTo.get(i)] = temp;
					edgeInfos[nextTo.get(i)] = edgeInfos[prev[i]] + info;
					fringe.changePriority(nextTo.get(i), edgeInfos[nextTo.get(i)]);
				}
			}
		}
		Integer tracker = endVertex;
		while (tracker != startVertex) {
			rtnList.add(0, tracker);
...
//		System.out.println(lst);
		...
		g8.addEdge(6, 4);
		g8.addEdge(5, 7);
		System.out.println(g8.shortestPath(0, 4));
	}
