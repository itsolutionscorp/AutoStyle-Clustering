...
public class Graph implements Iterable<Integer>{
    ...
    private class TopologicalIterator implements Iterator<Integer> {
        ...
        private void fringeUpdate() {
        	for (int i = 0; i < currentInDegree.length; i++) {
        		if (currentInDegree[i].equals(0) && !visited.contains(i)) {
        			fringe.push(i);
        			visited.add(i);
        		}
        	}
    ...
    public ArrayList<Integer> shortestPath (int startVertex, int endVertex){
        ...
        Integer[] minDistances = new Integer[myAdjLists.length];
        Integer[] prevVertex = new Integer[myAdjLists.length];
        ...
        while (fringe.peek() != null) {
        	int minVertex = fringe.peek().item();
        	minDistances[minVertex] = (int)fringe.removeMin().priority();
        	
        	for (Edge edge : myAdjLists[minVertex]) {
    			int newDistance = minDistances[minVertex] + (Integer)edge.myEdgeInfo;
        		if (fringe.contains(edge.myTo) && 
        				newDistance < minDistances[edge.myTo]) {
        			fringe.changePriority(edge.myTo, newDistance);
        			minDistances[edge.myTo] = newDistance;
        			prevVertex[edge.myTo] = minVertex;
        			
        		}
        	}
        	if (minVertex == endVertex) {
        		Integer currentVertex = minVertex;
