...
public class Graph implements Iterable<Integer>{
	...
	public ArrayList<Integer> shortestPath (int startVertex, int endVertex){
	    ...
	    while (i.hasNext()) {
	    		Integer x = i.next();
	    		Vertex v = new Vertex(x, Integer.MAX_VALUE, null);
	    		map.put(x, v);
	    		distances.offer(v);
	    }
	    findModify(distances, map, startVertex, 0, null);
	    while (!distances.isEmpty()) {
	    		Vertex u = distances.poll();
	    		for (Edge e : myAdjLists[u.value]) {
				if (map.get(e.to()).distanceTo > u.distanceTo + (Integer) e.myEdgeInfo) {
	    				findModify(distances, map, map.get(e.to()).value, u.distanceTo + (Integer) e.myEdgeInfo, u.value);
	    			}
	    		}
	    }
	    Vertex end = map.get(endVertex);
	    Stack<Integer> swag = new Stack<Integer>();
	    while (end != null) {
	    		swag.push(end.value);
	    		end = map.get(end.prev);
	    }
	    ArrayList<Integer> wooo = new ArrayList<Integer>();
	    while (!swag.isEmpty()) {
	    		wooo.add(swag.pop());
	    }
	    return wooo;
	...
	public void findModify (PriorityQueue<Vertex> v, HashMap<Integer, Vertex> map, Integer objectValue, Integer newDistance, Integer newPrev) {
		Vertex obj = map.get(objectValue);
		v.remove(obj);
		obj.distanceTo = newDistance;
		obj.prev = newPrev;
		v.offer(obj);
	}
	...
	private static class Vertex implements Comparable<Vertex>{
		...
		public Vertex(Integer v, Integer d, Integer p) {
			value = v;
			distanceTo = d;
			prev = p;
		}
	...
	public static void main(String[] args) {
		...
		g3.addEdge(2, 4, 5);
		g3.addEdge(4, 3, 1);
		ArrayList<Integer> x = g3.shortestPath(1, 4);
		for (Integer a: x) {
