...
public class Graph implements Iterable<Integer> {
	...
	public boolean isAdjacent(int from, int to) {
		return myAdjLists[from].contains(new Edge(from, to, ""));
	}
	...
	public ArrayList<Integer> shortestPath(int startVertex, int endVertex) {
		...
		int currentVertex = heap.removeMin().item();
		while(currentVertex != endVertex) {
			visit.add(currentVertex);
			for(Edge e: myAdjLists[currentVertex]) {
				if(!visit.contains(e.to())) {
					double newDist = (Integer)e.myEdgeInfo + distance[currentVertex]; 
					if(newDist < distance[e.to()]) {
						heap.update(heap.index(e.to()), newDist);
						distance[e.to()] = newDist;
						parents.put(e.to(), e.from());
					}
				}
			}
			currentVertex = heap.removeMin().item();
		}
		return pathHelper(startVertex,endVertex,parents);
	}
	...
	public ArrayList<Integer> path(int startVertex, int stopVertex) {
		...
		while (!fringe.isEmpty()) {
			...
			if (currentVertex == stopVertex) {
				return pathHelper(startVertex, stopVertex, path);
			}
			for (Edge e : myAdjLists[currentVertex]) {
				if (!visited.contains(e.to())) {
	...
	public ArrayList<Integer> pathHelper(int startVertex, int stopVertex,
			HashMap<Integer, Integer> path) {
		Stack<Integer> newPath = new Stack<Integer>();
		ArrayList<Integer> result = new ArrayList<Integer>();
		int currentVertex = stopVertex;
		newPath.push(currentVertex);
		while (currentVertex != startVertex) {
			newPath.push(path.get(currentVertex));
			currentVertex = path.get(currentVertex);
		}
		while (!newPath.isEmpty()) {
			result.add(newPath.pop());
		}
		return result;
	}
...
//		}
		...
		g3.addEdge(3,4,60);
		g3.addEdge(2,4,10);
		System.out.println(g3.shortestPath(0,4));
	}
