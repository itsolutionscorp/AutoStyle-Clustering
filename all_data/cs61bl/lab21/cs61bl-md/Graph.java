...
public class Graph implements Iterable<Integer>{
    ...
    public ArrayList<Integer> shortestPath (int startVertex, int endVertex){
        ...
        int[][] GraphAlg = new int[myVertexCount][2];
        for (int i=0;i<myVertexCount;i++){
        	if(i==startVertex){
            	GraphAlg[i][0] = 0;
            	GraphAlg[i][1] = startVertex;
            	continue;
        	}
        	GraphAlg[i][0] = Integer.MAX_VALUE;
        	GraphAlg[i][1] = -1;
        }
        fringe.insert(startVertex, 0);
        while(fringe.peek() != null){
        	int current = fringe.removeMin().item();
        	for (Edge e : myAdjLists[current]) {
    	        if (!visited.contains(e.to()))
    	        	fringe.insert(e.to(), Math.min(GraphAlg[e.to()][0], 
    	        			GraphAlg[current][0] + (Integer)e.info()));
            }
        	for (Edge e : myAdjLists[current]) {
        		if(GraphAlg[e.to()][1]==-1){
        	        GraphAlg[e.to()][0] = (Integer) e.info() + GraphAlg[current][0];
        	        GraphAlg[e.to()][1] = current;
        	        
        		} else if (GraphAlg[e.to()][0] > GraphAlg[current][0] + (Integer) e.info()){
    				GraphAlg[e.to()][0] = GraphAlg[current][0] + (Integer)e.info();
       				GraphAlg[e.to()][1]= current;
//       				System.out.println("current " + current);
//       				System.out.println("GraphAlg changed : " +e.to() + "dis"+ GraphAlg[e.to()][0]);
        		}
            }
            visited.add(current);
...
//        for (int i=0; i < myVertexCount; i++){
//	        System.out.print(GraphAlg[i][0] + " ");
//	        System.out.println(GraphAlg[i][1]);
//        }
        
        int currInd = endVertex;
        resultS.push(currInd);
        while (currInd != startVertex){
            currInd = GraphAlg[currInd][1];
            if (currInd == -1) {
        ...
        System.out.println("shortestpath: " + result);
    ...
    public static void main(String[] args) {
        ...
        System.out.println();
        System.out.println();
        System.out.println("Shortest Path 1");
        ...
        System.out.println("Shortest Path 2");
        ...
        g7.addEdge(3, 2, 20);
        g7.addEdge(3, 4, 60);
        System.out.println(g7.shortestPath(0, 4));
