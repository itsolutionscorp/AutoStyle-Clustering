...
public class BinaryTree<T> implements Iterable<T> {
    ...
    public BinaryTree(ArrayList<T> a, ArrayList<T> b) {
    	myRoot = binaryTreeHelper(myRoot, a, b);
    	
    }
    
    private TreeNode binaryTreeHelper(TreeNode n, List<T> a, List<T> b) {
    	if (a.size() == 0) {
    		return null;
    	}
    	n = new TreeNode(a.get(0));
    	if (a.size() == 1) {
    		return n;
    	}
    	int indexB = 0;
    	while (indexB < b.size()) {
    		if (b.get(indexB).equals(a.get(0))) {
    			break;
    		}
    		indexB++;
    	}
    	int indexA = 0;
    	while (indexA < a.size()) {
    		if (a.get(indexA).equals(b.get(indexB-1))) {
    			break;
    		}
    		indexA++;
    	}
    	n.myLeft = binaryTreeHelper(n.myLeft, a.subList(1, indexA+1), b.subList(0, indexB));
    	n.myRight = binaryTreeHelper(n.myRight, a.subList(indexA+1, a.size()), b.subList(indexB+1, b.size()));
    	return n;
    }
