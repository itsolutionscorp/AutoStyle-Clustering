...
public class BinaryTree<T> implements Iterable<T> {
    ...
    protected class TreeNode {
        ...
        public TreeNode(List<T> preorder, List<T> inorder) {
        	if (preorder.size() != inorder.size()) {
				...
				myItem = preorder.get(0);
				int splitpoint  = inorder.indexOf(myItem);
				if (splitpoint > 0) {
					myLeft = new TreeNode(preorder.subList(1, splitpoint+1)
							, inorder.subList(0, splitpoint));
				}
				if (splitpoint < preorder.size()-1) {
					myRight = new TreeNode(preorder.subList(splitpoint+1, preorder.size())
							, inorder.subList(splitpoint +1, inorder.size()));
				}
        	}
