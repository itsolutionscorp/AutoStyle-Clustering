...
public class BinaryTree<T> implements Iterable<T> {
	...
	private TreeNode makeTree(ArrayList<T> preOrder, ArrayList<T> inOrder) {
		if (inOrder.size() == 1)
			return new TreeNode(inOrder.get(0));
		T first = preOrder.get(0);
		int firstIndex = inOrder.indexOf(first);
			
		ArrayList<T> nextOrder = (ArrayList<T>) preOrder.subList(1, preOrder.size());
		ArrayList<T> leftOrder = (ArrayList<T>) inOrder.subList(0, firstIndex);
		ArrayList<T> rightOrder = (ArrayList<T>) inOrder.subList(firstIndex + 1, inOrder.size());
		return new TreeNode(first, makeTree(nextOrder, leftOrder), makeTree(nextOrder, rightOrder));	
	}
	...
	public static BinaryTree<String> fillSampleTree2() {
		BinaryTree<String> t = new BinaryTree<String>();
		t.myRoot = t.new TreeNode("a", t.new TreeNode("b", t.new TreeNode("c",
				t.new TreeNode("d"), t.new TreeNode("e")), null),
				t.new TreeNode("f"));
		return t;
	}
