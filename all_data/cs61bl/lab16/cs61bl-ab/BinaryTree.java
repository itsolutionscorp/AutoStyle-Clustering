...
public class BinaryTree<T> implements Iterable<T> {
    ...
    public BinaryTree(ArrayList<T> pre, ArrayList<T> inorder){
    	myRoot = constructorHelper(pre, inorder);
    }
    private TreeNode constructorHelper(ArrayList<T> pre, ArrayList<T> inorder){
    	if(inorder.size()==0){
    		return null;
    	}
    	T root = pre.get(0);
    	int index=0;
    	ArrayList<T> leftPre=new ArrayList();
    	ArrayList<T> rightPre=new ArrayList();
    	ArrayList<T> leftIn=new ArrayList();
    	ArrayList<T> rightIn=new ArrayList();
    	...
    	for(T t: pre){
    		if(leftIn.contains(t)){
    			leftPre.add(t);
    		} else if (rightIn.contains(t)){
    			rightPre.add(t);
    		}
    	}
    	return new TreeNode(root, constructorHelper(leftPre, leftIn), constructorHelper(rightPre, rightIn));
    }
    ...
    private void prettyhelper(TreeNode t,String indent){
    	if(t==null){
    		return ;
    	}
    	prettyhelper(t.myRight, indent+"    ");
    	System.out.println(indent+t.myItem);
    	prettyhelper(t.myLeft, indent+"    ");
    }
