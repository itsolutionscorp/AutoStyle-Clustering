	...
	public TreeNode createTree(ArrayList<T> preorder, ArrayList<T> inorder) {
		if (preorder != null && inorder != null) {
			if (preorder.size() > 0) {
				...
				ArrayList<T> newInorderL;
				ArrayList<T> newInorderR;
				ArrayList<T> newPreorderL;
				if (preorder.size() > 1) {
					newPreorderL = new ArrayList<T>(preorder.subList(1,
							preorder.size()));
				} else {
					newPreorderL = null;
				}
				if (indexOfRoot > 0) {
					newInorderL = new ArrayList<T>(inorder.subList(0,
							indexOfRoot));
					newInorderR = new ArrayList<T>(inorder.subList(
							indexOfRoot + 1, inorder.size()));
				} else {
					newInorderL = null;
					newInorderR = null;
				}
				preorder.remove(0);
				if (newInorderL != null) {
					for (T item : newInorderL) {
						preorder.remove(item);
					}
				}
				if (newInorderR != null) {
					for (T item : newInorderR) {
						newPreorderL.remove(item);
					}
				}
				TreeNode t = new TreeNode(root, createTree(newPreorderL,
						newInorderL), createTree(preorder, newInorderR));
				return t;
			}
		}
		return null;
