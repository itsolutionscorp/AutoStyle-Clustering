...
public class BinaryTree<T> implements Iterable<T> {
	...
	public BinaryTree(ArrayList<T> preorder, ArrayList<T> inorder) {
		BinaryTreeConstructor(myRoot, preorder, inorder);
	}

	public void BinaryTreeConstructor(TreeNode root, ArrayList<T> preorder,
			ArrayList<T> inorder) {
		myRoot.myItem = preorder.get(0);
		if (preorder.isEmpty() || inorder.isEmpty()) {
			return;
		} else {
			int myIndex = inorder.indexOf(myRoot.myItem);
			BinaryTreeConstructor(root.myLeft, buildListAfter(preorder, 1),
					buildListBefore(inorder, myIndex));
			BinaryTreeConstructor(root.myRight, buildListAfter(preorder, 2),
					buildListAfter(inorder, myIndex));
		}
	}

	public ArrayList<T> buildListBefore(ArrayList<T> myList, int index) {
	...
	public ArrayList<T> buildListAfter(ArrayList<T> myList, int index) {
		ArrayList<T> answer = new ArrayList<T>();
