...
public class BinaryTree<T> implements Iterable<T> {
	...
	public BinaryTree(ArrayList<T> preorder, ArrayList<T> inorder) {
		if (preorder.isEmpty()) {
			...
			myRoot = BinaryTreeHelper(new LinkedList<T>(preorder), inorder);
		}
	}

	private TreeNode BinaryTreeHelper(LinkedList<T> preorderQ,
			List<T> inorder) {
		if (inorder.size() == 1) {
			return new TreeNode(preorderQ.remove());
		}

		int rootIndex = 0;
		T root = preorderQ.remove();
		for (; rootIndex < inorder.size(); rootIndex++) {
			T inorderE = inorder.get(rootIndex);
			if (root.equals(inorderE))
				break;
		}
		...
		if (rootIndex == 0)
			...
			r.myLeft = BinaryTreeHelper(preorderQ,
					inorder.subList(0, rootIndex));
		if (rootIndex == inorder.size() - 1)
			r.myRight = null;
		else
			r.myRight = BinaryTreeHelper(
					preorderQ,
