    ...
    public BinaryTree(ArrayList preorder, ArrayList inorder){
    	if(preorder != null){
    		for(int i = 0; i < preorder.size(); i++){
    			myRoot = add(myRoot, i, preorder, inorder);
    		}
    	}
    }
    
    private TreeNode add(TreeNode t, int position, ArrayList preorder, ArrayList inorder){
    	if(t == null){
    		return new TreeNode( (T) preorder.get(position));
    	}
    	if(inorder.indexOf(preorder.get(position)) < inorder.indexOf(t.myItem)){
    		t.myLeft = add(t.myLeft, position, preorder, inorder);
    	} else{
    		t.myRight = add(t.myRight, position, preorder, inorder);
    	}
    	return t;
    }
...
//        print(r, "sample tree 2");
        ...
        inorder.add("F");
        inorder.add("C");        
        BinaryTree run = new BinaryTree(preorder, inorder);
        print(run, "Additional Exercise");
    }
