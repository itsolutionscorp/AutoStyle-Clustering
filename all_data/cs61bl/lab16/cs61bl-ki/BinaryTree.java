...
public class BinaryTree<T> implements Iterable<T> {
	...
	public BinaryTree(ArrayList<T> preOrder, ArrayList<T> inOrder) {
		for (int index = 0; index < preOrder.size(); index++){
			myRoot = treeHelper(myRoot, index, preOrder, inOrder);
		}
	}

	public TreeNode treeHelper(TreeNode t, int index, ArrayList<T> preOrder, ArrayList<T> inOrder) {
		if (t==null){
			return new TreeNode(preOrder.get(index));
		}
		if (inOrder.indexOf(preOrder.get(index))< inOrder.indexOf(t.myItem)){
			t.myLeft = treeHelper(t.myLeft, index, preOrder,inOrder);
		} else {
			t.myRight = treeHelper(t.myRight, index, preOrder,inOrder);
		}
		return t;
	}
	...
	public static void main(String[] args) {
		...
		in.add("F");
		in.add("C");
		BinaryTree<String> treee = new BinaryTree<String>(pre,in);
		print(treee, "test!");
	}
