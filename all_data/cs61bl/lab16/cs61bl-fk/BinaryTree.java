...
public class BinaryTree<T> implements Iterable<T> {
    ...
    public BinaryTree(ArrayList<T> preList, ArrayList<T> inList) {

		if (preList.isEmpty()) {
			myRoot = null;
		} else {
			myRoot = constHelper(preList, inList);
		}
	}

    private TreeNode constHelper(ArrayList<T> preList, ArrayList<T> inList) {
    	if (!preList.isEmpty()) {
	    	...
	    	TreeNode left, right;
	    	T mainObj = preList.get(0);
	    	
	    	int i = 0;
	    	while (!inList.get(i).equals(mainObj)) {
	    		leftpre.add(preList.get(i + 1));
	    		leftin.add(inList.get(i));
	    		i++;
	    	}
	    	
	    	left = constHelper(leftpre, leftin);
	    	
	    	while (i + 1 < preList.size()) {
	    		rightpre.add(preList.get(i + 1));
	    		rightin.add(inList.get(i + 1));
	    		i++;
	    	}
	    	
	    	right = constHelper(rightpre, rightin);
	    	return new TreeNode(mainObj, left, right);
		} else {
			return null;
		}
	}
