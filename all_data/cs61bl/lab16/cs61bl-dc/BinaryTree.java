...
public class BinaryTree<T> implements Iterable<T>
{
	...
	private TreeNode BinaryTreeHelper(ArrayList<T> preorder, ArrayList<T> inorder)
	{
		if (preorder.isEmpty())
			return null;
		...
		ArrayList<T> leftInorder = new ArrayList<T>(inorder.subList(0, index));
		ArrayList<T> rightInorder = new ArrayList<T>(inorder.subList(index + 1, inorder.size()));
		ArrayList<T> leftPreorder = new ArrayList<T>();
		ArrayList<T> rightPreorder = new ArrayList<T>();
		for (T item : preorder)
		{
			if (leftInorder.contains(item))
				leftPreorder.add(item);
			else if(rightInorder.contains(item))
				rightPreorder.add(item);
		}
		t.myLeft = BinaryTreeHelper(leftPreorder, leftInorder);
		t.myRight = BinaryTreeHelper(rightPreorder, rightInorder);
		return t;
	}
