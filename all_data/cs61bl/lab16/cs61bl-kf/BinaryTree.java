...
public class BinaryTree<T> implements Iterable<T> {
    ...
    public BinaryTree (ArrayList<T> preorder, ArrayList<T> inorder) {
    	 if (preorder.size() == 0) {
    		 ...
    		 myRoot = arrayConstructor(preorder, 0, preorder.size() - 1, inorder, 0, inorder.size() - 1);
    	 }
    }
    
    public TreeNode arrayConstructor(ArrayList<T> preorder, int preBegin, int preFinish, ArrayList<T> inorder, int inBegin, int inFinish) {
    	if (inBegin > inFinish) {
    		return null;
    	}
    	int parentIndex = 0;
    	for (int i = inBegin; i <= inFinish; i++) {
    		if (inorder.get(i) == preorder.get(preBegin)) {
    			parentIndex = i;     
    			break;
    		}  	
    	}
    	int diff = parentIndex - inBegin;
		TreeNode parent = new TreeNode(preorder.get(preBegin));
		parent.myLeft = arrayConstructor(preorder, preBegin + 1, preBegin + diff, inorder, inBegin, parentIndex - 1);
		parent.myRight = arrayConstructor(preorder, preBegin + diff + 1, preFinish, inorder, parentIndex + 1, inFinish);
		return parent;        	 	
   }
    ...
    public static void main(String[] args) {
        ...
        BinaryTree<String> arrayTest = new BinaryTree<String>(preorder, inorder);
