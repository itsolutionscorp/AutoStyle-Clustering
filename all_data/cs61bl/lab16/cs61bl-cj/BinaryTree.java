...
public class BinaryTree<T> implements Iterable<T> {
    ...
    public BinaryTree(ArrayList<T> preorder, ArrayList<T> inorder) {
    	if (preorder.size() != inorder.size()) {
    		throw new IllegalArgumentException("Please enter two arrays with same size");
    	}
    	myRoot = arrayToTree(preorder, inorder);
    }

    private TreeNode arrayToTree(List<T> preorder, List<T> inorder) {
		...
		t.myLeft = arrayToTree(preorder.subList(1, index + 1), inorder.subList(0, index));
		t.myRight = arrayToTree(preorder.subList(index + 1, preorder.size()), inorder.subList(index+1, inorder.size()));
