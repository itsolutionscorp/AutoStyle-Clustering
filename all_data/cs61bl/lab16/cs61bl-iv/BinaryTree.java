...
public class BinaryTree<T> implements Iterable<T> {
    ...
    public BinaryTree(ArrayList<T> preorder, ArrayList<T> inorder) {
    	myRoot = BinaryTreeBuilder(preorder,inorder);
    }
    
    private TreeNode BinaryTreeBuilder(ArrayList<T> preorder, ArrayList<T> inorder) {
    	if (preorder.isEmpty() || inorder.isEmpty()) {
    		return null;
    	}
    	else {
    		T item = preorder.remove(0);
        	int inorderIndex = inorder.indexOf(item);
        	ArrayList<T> inorder1 = new ArrayList<T>(inorder.subList(0, inorderIndex));
        	ArrayList<T> inorder2 = new ArrayList<T>(inorder.subList(inorderIndex+1, inorder.size()));
        	return new TreeNode(item, BinaryTreeBuilder(preorder,inorder1), BinaryTreeBuilder(preorder,inorder2));
    	}
