...
public class DistributionSorts {
	...
	public static void MSDRadixSortFromDigitInBounds(int[] arr, int digit,
		...
		for(int i = start; i < end; i++) {
			result[boundary[(int) ((arr[i]/(Math.pow(10, digit)))%10)]] = arr[i];
			boundary[(int) ((arr[i]/(Math.pow(10, digit)))%10)]++;			
		}
		for(int j = start; j < end; j++){
			arr[j] = result[j - start];
		}
		result = null;
		for(int p = 0; p < boundary.length -1 ; p++){
			if(digit != 0){
				MSDRadixSortFromDigitInBounds(arr, digit-1,	boundary[p], boundary[p+1]);
			}
		}
	}
	...
	private static int[] countingSortByDigitInBounds(int[] arr, int digit,
		...
		for(int i = start; i < end; i++){	
			counts[(int) ((arr[i]/(Math.pow(10, digit)))%10)]++;			
		}
		int sum = 0;
		for(int j = 0; j< 10; j++){
			boundary[j] = sum;
	...
	public static void main(String[] args) {
		...
		//System.out.println("mostDigitsIn: " + mostDigitsIn(arr3));
		MSDRadixSort(arr3);
