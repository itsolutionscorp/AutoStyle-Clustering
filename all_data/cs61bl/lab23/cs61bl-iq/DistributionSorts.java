...
public class DistributionSorts {
	...
	public static void countingSort(int[] arr) {
		...
		int[] started = new int[10];
		int thisStart = 0;
		int counter = 0;
		for (int integer : count) {
			started[counter] = thisStart;
			counter++;
			thisStart += integer;
		}
		...
		for (int integer : arr) {
			sorted[started[integer]] = integer;
			started[integer] = started[integer] + 1;
		}
		
		for (int counter2 = 0; counter2 < arr.length; counter2++) {
			arr[counter2] = sorted[counter2];
		}
	}
