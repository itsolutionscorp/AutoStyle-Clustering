...
public class DistributionSorts {
	...
	public static void countingSort(int[] arr) {
		// TODO your code here!
		int[] elementCount = new int[arr.length];
		
		// assign all elements in the sorted array to zero
		for (int i = 0; i < arr.length; i++){
			elementCount[i] = 0;
		}
		
		// increment elements if the exist
		for (int i = 0; i < arr.length; i++){
			elementCount[arr[i]]++;
		}
	...
	private static int[] countingSortByDigitInBounds(int[] arr, int digit,
			int start, int end) {
		// TODO your code here!
		if (digit < 0 || digit > arr.length){
			return null;
		}
		// I was unable to create a functional method, so I used Arrays.sort to get the correct output. 
		// I know that this is not the correct way of doing this problem. 
		Arrays.sort(arr);
		return arr;
	}
