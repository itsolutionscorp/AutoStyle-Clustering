...
public class DistributionSorts {
	...
	public static void MSDRadixSortFromDigitInBounds(int[] arr, int digit,
		...
		if (digit==-1 || start == end || end-start==1) {
			return;
		}
	...
	private static int[] countingSortByDigitInBounds(int[] arr, int digit,
		...
		for (int i=start; i<end; i++) {
			int num = arr[i];
			int importantDigit = (int) (num % Math.pow(10, digit+1)) / (int) Math.pow(10, digit) ;
			counts[importantDigit] += 1;
		}
		int[] sorted = new int[arr.length];
		int[] toBeReturn = new int[10];
		toBeReturn[0] = counts[0];
		for (int i=1; i<counts.length; i++) {
			counts[i] = counts[i-1] + counts[i];
			toBeReturn[i] = counts[i];
		}
		for (int j=end-1; j>=start; j--) {
			int num = arr[j];
			int realNum = (int) (num % Math.pow(10, digit+1)) / (int) Math.pow(10, digit);
			counts[realNum] = counts[realNum] -1;
			int index = counts[realNum]+start;
			sorted[index] = num;
		}
		for (int i=start; i<end; i++) {
			arr[i] = sorted[i];
