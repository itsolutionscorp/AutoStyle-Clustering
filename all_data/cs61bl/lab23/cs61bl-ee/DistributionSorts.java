...
public class DistributionSorts {
	...
	public static void MSDRadixSortFromDigitInBounds(int[] arr, int digit,
		...
		for(int i = 0; i < bound.length; i++){
			if(bound[i] == 1){
				newStart++;}
			if(bound[i] > 1){
				MSDRadixSortFromDigitInBounds(arr,digit - 1, newStart, newStart+bound[i]);
				newStart = newStart+bound[i];
			}
		}
	...
	private static int[] countingSortByDigitInBounds(int[] arr, int digit,
		...
		for(int j = start; j < end; j++){
			counts[(arr[j]/divider)%mod]++;
		}
		
		starts[0] = start;
		int eachAmount = start;
		for(int i = 1; i < starts.length ; i++){
			eachAmount += counts[i - 1];
			starts[i] =  eachAmount;
		}
		for(int i = start; i < end; i++){
			sorted[starts[(arr[i]/divider)%mod]] = arr[i];			
			starts[(arr[i]/divider)%mod]++;
		}
		for(int i = start; i < end; i++){
			arr[i] = sorted[i];
		}
