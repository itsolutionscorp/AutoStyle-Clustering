...
public class DistributionSorts {
	...
	public static void countingSort(int[] arr) {
		...
		start[0] = 0;
		int counter = counts[0];
		for (int i = 1 ; i < 10 ; i++){ // set the starting array
			start[i] = counter;
			counter += counts[i];
		}
	...
	public static void MSDRadixSortFromDigitInBounds(int[] arr, int digit,
		...
		int[] startIndex = countingSortByDigitInBounds(arr, 0, 0, arr.length);
		
		for (int currDigit = 0; currDigit < mostDigitsIn(arr); currDigit++) {
			for (int i = 0; i < startIndex.length; i++) {
				if (i == 0) {
					MSDRadixSortFromDigitInBounds(arr, currDigit, 0, startIndex[i+1]);
				}
				if (i == startIndex.length-1) {
					MSDRadixSortFromDigitInBounds(arr, currDigit, startIndex[i], arr.length);
				}
				MSDRadixSortFromDigitInBounds(arr, currDigit, startIndex[i], startIndex[i+1]);
			}
	...
	private static int[] countingSortByDigitInBounds(int[] arr, int digit,
		...
		for (int i = start; i < end; i++) {
			int myDigit = (int) ((arr[i] / Math.pow(10, digit))%10);
			counts[myDigit]++;
		}
		...
		int[] nonModifiedStartIndex = new int[10];
		for (int i = 0; i < startIndex.length; i++) {
			nonModifiedStartIndex[i] = startIndex[i];
		}
		
		int[] sorted = new int[arr.length];
		for (int i = start; i < end; i++) {
			sorted[startIndex[arr[i]]] = arr[i];
			startIndex[arr[i]]++;
		}
