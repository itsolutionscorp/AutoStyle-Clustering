...
public class DistributionSorts {
	...
	public static void MSDRadixSortFromDigitInBounds(int[] arr, int digit,
		...
		if (digit == 0) {
			...
			int[] temp = countingSortByDigitInBounds(arr, digit, start, end);
			for (int i = 0; i < temp.length-1; i++) {
				MSDRadixSortFromDigitInBounds(arr, digit - 1, temp[i],
						temp[i + 1]);
			}
			MSDRadixSortFromDigitInBounds(arr, digit-1, temp[temp.length-1], end);
		}
	}
	...
	private static int[] countingSortByDigitInBounds(int[] arr, int digit,
		...
		for (int i = start; i<end; i++) {
			counts[arr[i]/new Double(Math.pow(10, digit)).intValue()] = counts[arr[i]/new Double(Math.pow(10, digit)).intValue()] + 1;
		}
		for (int i = 1; i < starts.length; i++) {
			starts[i] = starts[i - 1] + counts[i - 1];
		}
		...
		for (int i = start; i<end; i++) {
			...
			while (counts[arr[i]/new Double(Math.pow(10, digit)).intValue()] > 0) {
				newarr[starts[arr[i]/new Double(Math.pow(10, digit)).intValue()] + counts[i] -1] = arr[i];
				counts[arr[i]/new Double(Math.pow(10, digit)).intValue()] = counts[arr[i]/new Double(Math.pow(10, digit)).intValue()] - 1;
			}
		...
		//has to modify the original array based on newarr
