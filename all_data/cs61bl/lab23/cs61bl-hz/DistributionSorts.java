...
public class DistributionSorts {
	...
	public static void countingSort(int[] arr) {
		...
		for (int j = 0; j < arr.length; j++) {
			if (counts[numeros] == 0 && numeros == 9) {
				break;
			} else if (counts[numeros] == 0) {
				numeros++;
				j--;
			} else {
				arr[j] = numeros;
	...
	public static void MSDRadixSortFromDigitInBounds(int[] arr, int digit,
		...
		int[] n = countingSortByDigitInBounds( arr, digit,
				 start, end);
		int index = 0;
...
//			System.out.println(arr[i]);
		...
		if (digit > 0) {
			for (int j = start; j < end; j++) {
				if (s < (arr[j]/((int)Math.pow(10, digit)))%10) {
					MSDRadixSortFromDigitInBounds( arr,  digit - 1,
							 last,  j);
					last = j;
					 s = (arr[j]/((int)Math.pow(10, digit)))%10;
				}
			}
			MSDRadixSortFromDigitInBounds( arr,  digit - 1,
					 last,  end);
	...
	private static int[] countingSortByDigitInBounds(int[] arr, int digit,
		...
		int[] n = new int[10];
		for (int i = start; i < end; i++) {
			n[(arr[i]/((int)Math.pow(10, digit)))%10]++;
		}
		...
		for (int k = 0; k < 10; k++) {
			...
			for (int m = start; m < end; m++) {
					if ((arr[m]/((int)Math.pow(10, digit)))%10 == k) {
						b[index] = arr[m];
						index++;
						n[k]--;
					}
