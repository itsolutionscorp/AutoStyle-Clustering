...
public class AmoebaFamily implements Iterable<AmoebaFamily.Amoeba>{
	...
	public void replaceName(String currentName, String newName) {
		// Your goal is to make this as similar as possible to addChild
		if (myRoot != null) {
			myRoot.replaceNames(currentName, newName);
		}
	}
	...
	public class AmoebaIterator implements Iterator<Amoeba> {
		...
		private Queue<Amoeba> frontier;

		public AmoebaIterator() {
			frontier = new LinkedList<Amoeba>();
			if (myRoot != null) {
				frontier.add(myRoot);
			}
		}
		...
		public Amoeba next() {
			Amoeba toReturn = frontier.poll();
			for (Amoeba a : toReturn.myChildren) {
				frontier.add(a);
			}
			return toReturn;
		}
...
//        	}
        ...
        public void replaceNames(String currentName, String newName) {
        	if (myName.equals(currentName)) {
        		myName = newName;
        	}
        	else {
        		for (Amoeba a : myChildren) {
        			a.replaceNames(currentName, newName);
        		}
        	}
        }
        ...
        public int indentHelper() {
        	if (myParent == null) {
        		return 0;
        	}
        	return 1 + myParent.indentHelper();
        }
        
        public void print() {
        	String indent = "    ";
        	int toIndent = this.indentHelper();
        	String toReturn = "";
        ...
        public String longestName() {
        	int length = longestNameLength();
        	if (myName.length() == length) {
        		return myName;
        	}
        	for (Amoeba a : myChildren) {
        		if (a.longestName().length() == length) {
        			return a.longestName();
        		}
        	}
        	return "";
        }
        ...
        public int height() {
        	if (myChildren.isEmpty()) {
        		...
        		int[] possibleHeights = new int[myChildren.size()];
        		int i = 0;
        		for (Amoeba a : myChildren) {
        			possibleHeights[i] = a.height();
        			i++;
        		}
        		Arrays.sort(possibleHeights);
        		return 1 + possibleHeights[myChildren.size() - 1];
        	}
