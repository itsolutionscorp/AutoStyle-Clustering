...
public class AmoebaFamily implements Iterable<AmoebaFamily.Amoeba>{
	...
	public void makeNamesLowercase() {
		...
		if (myRoot.myChildren != null) {
			for (Amoeba x: myRoot.myChildren){
				x.lowercase();
			}
		}
	}
	...
	public void replaceName(String currentName, String newName) {
		...
		} else if(myRoot.myChildren != null){
			for (Amoeba x: myRoot.myChildren){
				x.replaceName(currentName, newName);
			}
		}
	}
...
//		System.out.println(family.height());
	...
	public class AmoebaIterator implements Iterator<Amoeba> {
		...
		LinkedList<Amoeba> names = new LinkedList<Amoeba>();
		...
		public Amoeba next() {
			Amoeba temp = names.remove();
			if(temp.myChildren != null){
				for(int i = temp.myChildren.size()-1; i>=0; i--){
					names.add(temp.myChildren.get(i));
				}
			}
			return temp;
		}
	...
	public static class Amoeba {
        ...
        public int longestNameLength() {
            ...
            //System.out.println(maxLengthSeen);
            for (Amoeba a : myChildren) {
        ...
        public String longestName(){
            ...
            for (Amoeba a : myChildren) {
                maxLengthSeen = Math.max(maxLengthSeen, a.longestNameLength());
                if(maxLengthSeen == a.longestNameLength()){
