...
public class AmoebaFamily implements Iterable<AmoebaFamily.Amoeba>{
	...
	public void replaceName(String currentName, String newName) {
		if (myRoot != null) {
			myRoot.replaceMyName(currentName, newName);
		}
	}
	...
	public class AmoebaIterator implements Iterator<Amoeba> {
		...
		private Queue<Amoeba> fringe = new ArrayDeque<Amoeba>();
		...
		public Amoeba next() {
			Amoeba returnValue = (Amoeba) fringe.remove();
				if (returnValue.myChildren.size() > 0) {
					for (Amoeba c : returnValue.myChildren) {
						fringe.add(c);
					}
				}
				return returnValue;
	...
	public static class Amoeba {
        ...
        public void makeNameLowercase() {
			...
			for (Amoeba child: myChildren) {
				child.makeNameLowercase();
			}
        }

        public void replaceMyName(String currentName, String newName) {
        	if (myName.equals(currentName)) {
    			myName = newName;
    		} else {
    			for (Amoeba child: myChildren) {
    				child.replaceMyName(currentName, newName);
    			}
    		}

        }

        public void printMyFlat() {
        ...
        public void printMyTabbed(int tabnum) {
        	int tabs = tabnum;
        	String tab = "";
        	while (tabs > 0) {
        		tab +=  "    ";
        	...
        	System.out.println(tab + myName);
        	for (Amoeba child: myChildren) {
				child.printMyTabbed(tabnum + 1);
			}
        }
        ...
        public String longestName() {
            String name = myName;
            for (Amoeba a : myChildren) {
                name = (name.length() > a.longestName().length()) ? name:a.longestName() ;
            }
            return name;
        }

        public int count(int soFar) {
        	soFar++;
        	for (Amoeba a : myChildren) {
        		soFar += a.count(0);
            }
