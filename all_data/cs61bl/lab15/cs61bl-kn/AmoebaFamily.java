...
public class AmoebaFamily implements Iterable<AmoebaFamily.Amoeba>{
	...
	public class AmoebaIterator implements Iterator<Amoeba> {
		...
		//private Stack<Amoeba> myStack = new Stack();
		...
		public Amoeba next() {
			...
			Amoeba e = myStack.pop();
			for (int a = e.myChildren.size(); a > 0; a--) {
				myStack.push(e.myChildren.get(a-1));
			}
			return e;*/
			if (myQueue.isEmpty()) {
				throw new NoSuchElementException("tree ran out of elements");
			}
			Amoeba e = (Amoeba) myQueue.poll();
			for (int a = 0; a < e.myChildren.size(); a++) {
				myQueue.add(e.myChildren.get(a));
			}
			return e;
		}
	...
	public static class Amoeba {
        ...
        public String longestName() {
        	...
        	for (Amoeba a: myChildren) {
        		if (maxNameSeen.length() < a.longestName().length()) {
        ...
        public Amoeba busiest() {
        	Amoeba p = this;
        	for (Amoeba a: myChildren) {
        		if (p.busiestHelper() < a.busiest().busiestHelper()) {
        			p = a;
        		}
        	}
        	return p;
        ...
        private int height() {
        	int bestSoFar = 0;
        	for (Amoeba a : myChildren) {
        		bestSoFar = Math.max(bestSoFar, a.height());
        		}
            return (bestSoFar+1);
        }
    }
