...
//
//		public Amoeba next() {
//			Amoeba blob = fringe.pop();
//			for (int index = blob.myChildren.size() - 1; index >= 0; index--) {
//				fringe.push(blob.myChildren.get(index));
//			}
//			return blob;
//		}
//		
		private class SoQueue<A> {
			private LinkedList<A> myQueue;
			public SoQueue() {
				myQueue = new LinkedList<A>();
			}
		...
		private SoQueue<Amoeba> fringe = new SoQueue<Amoeba>();
		...
		public Amoeba next() {
			Amoeba blob = fringe.remove();
			for (Amoeba a : blob.myChildren) {
				fringe.add(a);
			}
			return blob;
		}
	...
	public static class Amoeba {
        ...
        public String longestName() {
        	String maxStringSeen = myName;
            for (Amoeba a : myChildren) {
            	String check = a.longestName();
            	if (check.length() > maxStringSeen.length()) {
            		maxStringSeen = check;
            	}
            } 
