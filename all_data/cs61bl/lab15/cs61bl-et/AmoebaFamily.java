...
public class AmoebaFamily implements Iterable<AmoebaFamily.Amoeba>{
	...
	public class AmoebaIterator implements Iterator<Amoeba> {
		...
		//		    	int i;
		//		        for (i=node.myChildren.size()-1;i>-1;i--){
		...
		public AmoebaIterator() {
			if (myRoot != null) {
				int i;
				for (i = 0; i < myRoot.myChildren.size();i++){
					fringe.add(myRoot.myChildren.get(i));
				}
			}
		}
		...
		public Amoeba next() {
			...
			Amoeba node = (Amoeba) fringe.remove(0);
			if (node.myChildren != null) {
				int i;
				for (i=0;i<node.myChildren.size();i++){
					fringe.add(node.myChildren.get(i));
				}
			}
	...
	public static class Amoeba {
		...
		public void print(){
			System.out.println(myName);
			for (Amoeba a : myChildren){
				System.out.print(printHelper(a));
				a.print();
			}
		}
		public String printHelper(Amoeba haha){
			if (haha.myParent != null){
				return "    " + printHelper(haha.myParent);
			}else{
				return "";
			}
		}
		...
		public String longestName() {
			...
			int haha = this.longestNameLength();
			for (Amoeba a : myChildren){
				if (haha == a.longestName().length()){
