...
public class AmoebaFamily implements Iterable<AmoebaFamily.Amoeba>{
	...
	public void makeNamesLowercase() {
		...
		if (myRoot != null) {
			myRoot.makeNamesLowercase(myRoot.myName);
		}
	}
	...
	public static void main(String[] args) {
		...
		family.print();
		Iterator myIter = family.iterator();
...
//				int count = node.myChildren.size();
//			    while (count > 0) {
//			        fringe.push(node.myChildren.get(count - 1));
//			        count--;
//			    }
//			}
//			return node;
...
//		}
		...
		public Amoeba next() {
			Amoeba node = (Amoeba) fringe.remove();
			if (myRoot.myChildren != null) {
				int count = 0;
			    while (count < node.myChildren.size()) {
			        fringe.add(node.myChildren.get(count));
			        count++;
			    }
			}
			return node;
	...
	public static class Amoeba {
        ...
        public void makeNamesLowercase(String name) {
			myName = myName.toLowerCase();
			for (Amoeba a : myChildren) {
				a.makeNamesLowercase(a.myName);
			}
		}
        ...
        public String longestName() {
        	String longName = myName;
        	for (Amoeba a : myChildren) {
        		if (longName.length() < a.longestName().length()) {
        			longName = a.longestName();
        		}
        	}
        	return longName;
        }
        
        public int size(int familySize) {
        	if (myName != null) {
        		familySize += 1;
        	} for (Amoeba a : myChildren) {
        		familySize = a.size(familySize);
        	}
        	return familySize;
        }
        
        private int height(int original, int bestSoFar) {
        	if (myChildren.isEmpty()) {
            	return bestSoFar;
            } else {
                for (Amoeba a : myChildren) {
                    System.out.println("original: " + original+ " bestSoFar: " + bestSoFar);
                    original = Math.max(a.height(original, bestSoFar+1), original);
                }
                return original;
            }
        }
