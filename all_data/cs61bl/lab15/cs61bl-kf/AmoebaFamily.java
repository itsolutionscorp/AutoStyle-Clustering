...
//		family.replaceName("marge", "doodle");
//		family.printFlat();
		family.print();
...
//		System.out.println(family.height());
		System.out.println();
		Iterator<Amoeba> test = family.iterator();
		while (test.hasNext()) {
...
//			for (int i = next.myChildren.size() - 1; i >= 0; i--) {
//				fringe.push(next.myChildren.get(i));
//			}
//			return next;
		
		private ArrayList<Amoeba> fringe;
		private int index;
		
		public AmoebaIterator() {
			fringe = new ArrayList<Amoeba>();
			fringe.add(myRoot);
			index = 0;
		}

		public boolean hasNext() {
			return index < fringe.size();
		}

		public Amoeba next() {
			Amoeba next = fringe.get(index);
			index++;
			for (Amoeba a : next.myChildren) {
				fringe.add(a);
			}
			return next;
		}
	...
	public static class Amoeba {
		...
		public String longestName() {
			...
			for (Amoeba a : myChildren) {
				if (a.longestName().length() > longestSoFar.length()) {
		...
		private int height() {
			if (myChildren.isEmpty()) {
				...
				int maxChild = 0;
				for (Amoeba a : myChildren) {
					maxChild = Math.max(maxChild, a.height());
				}
				return 1 + maxChild;
			}
		}
