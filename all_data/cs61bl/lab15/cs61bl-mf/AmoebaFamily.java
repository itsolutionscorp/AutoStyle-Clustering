...
public class AmoebaFamily implements Iterable<AmoebaFamily.Amoeba> {
	...
	public static void main(String[] args) {
		...
		family.print();
		Iterator<Amoeba> iter = family.new AmoebaIterator();
		while (iter.hasNext())
	...
	public class AmoebaIterator implements Iterator<Amoeba> {
		...
		// code taken from lab15 page on
		// cs61bl.github.io/materials/lab/lab15/lab15.html
		private LinkedBlockingQueue<Amoeba> fringe = new LinkedBlockingQueue<Amoeba>();
		...
		public Amoeba next() {
			if (!hasNext()) {
				throw new NoSuchElementException(
						"AmoebaFamily ran out of Amoebas");
			}
			Amoeba amoeba;
			try {
				amoeba = (Amoeba) fringe.take();
				for (Amoeba child : amoeba.myChildren) {
					fringe.add(child);
				}
				return amoeba;
	...
	public static class Amoeba {
		...
		public void print(int indentLevel) {
			for (int indent = indentLevel; indent > 0; indent--) {
				System.out.print("    ");
			}
			System.out.println(myName);
			if (myChildren.isEmpty()) {
		...
		public String longestName() {
			...
			if (myChildren.isEmpty()) {
				...
				for (Amoeba a : myChildren) {
					temp = a.longestName();
					if (temp.length() > longestNameSeen.length()) {
						longestNameSeen = temp;
					}
				}
			}
