...
//		}
//		for(Amoeba node : myRoot.myChildren){
//			System.out.println(node.myName);
//		}
		if(myRoot != null){
			myRoot.print();
		}
...
//						  for(int x = node.myChildren.size()-1; x>=0; x--){
//							  fringe.push(node.myChildren.get(x));
//						  }
//			    	 }
//			  
//			    return node;
...
//	} // end of AmoebaIterator nested class
	...
	public class AmoebaIterator implements Iterator<Amoeba> {
		...
		public Amoeba next() {
			 if (!hasNext()) {
			    ...
			    for(int x = 0; x<=node.myChildren.size()-1; x++){
					  fringe.add(node.myChildren.get(x));
				  }
			    return node;
		}
	...
	public static class Amoeba {
        ...
        public void makeNamesLowerCase(){
    		...
    		for(Amoeba node : myChildren){
    			node.makeNamesLowerCase(); 
        ...
        public void replaceName(String currentName, String newName){
        	if(myName.equals(currentName)){
    			...
    			for(Amoeba node :myChildren){
    			node.replaceName(currentName, newName);
    			}
        }
        public void printFlat() {
        ...
        public void print(){
        	System.out.println(myName);
        		for(Amoeba node : myChildren){
        			for(int i = 0; i < indentHelper(node);i++){
        				System.out.print("    ");
        			}
        			node.print();
        		}
        ...
        public int indentHelper(Amoeba node){
        	int i = 0;
        ...
        public String longestName() {
        	...
        	for (Amoeba node : myChildren) {
        		if (node.longestName().length() > maxSeen.length()) {
        			maxSeen = node.longestName();
        		}
        	}
        	return maxSeen;
        }
