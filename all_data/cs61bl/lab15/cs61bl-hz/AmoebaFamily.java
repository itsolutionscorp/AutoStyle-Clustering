...
//		System.out.println("Here's the family:");
		family.print();
//		family.replaceName("mom/dad", "dad/dad");
		Iterator<Amoeba> e = family.iterator();
		while (e.hasNext()) {
			System.out.println(e.next().myName);
		}
...
//		System.out.println(family.height());
	...
	public class AmoebaIterator implements Iterator<Amoeba> {
		...
		int count;
//		Stack<Amoeba> stack = new Stack<Amoeba>();
...
//			for (int i = node.myChildren.size() - 1; i >= 0; i--) { for stack implement
			for (int i = 0; i < node.myChildren.size(); i++) {
				if (node.myChildren.get(i) != null) {
//					stack.push(node.myChildren.get(i));
					queue.add(node.myChildren.get(i));
				}
			}
			return node;
		}
	...
	public static class Amoeba {
        ...
        public String longestName() {
        	if (myName.length() == longestNameLength()) {
        		return myName;
        	} else {
        		for (Amoeba a : myChildren) {
        			if (a.myName.length() == longestNameLength()) {
        				return a.myName;
        			}
        		}
        		return null;
