...
public class AmoebaFamily implements Iterable<AmoebaFamily.Amoeba>{
	...
	public class AmoebaIterator implements Iterator<Amoeba> {
		...
		private LinkedList<Amoeba> amoebaList = new LinkedList<Amoeba>();

		public AmoebaIterator() {
			if (myRoot != null) {
				amoebaList.push (myRoot);
	    }
		}

		public boolean hasNext() {
			return !amoebaList.isEmpty();
		}

		public Amoeba next() {
			Amoeba temp = (Amoeba)amoebaList.remove();
	...
	public static class Amoeba {
		...
		public String longestName() {
			String temp = myName;
			for (Amoeba a : myChildren) {
				if (a.longestName().length()>temp.length()){
					temp = a.myName;
				}
            }
			return temp;
