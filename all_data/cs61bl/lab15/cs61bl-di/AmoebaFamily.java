...
public class AmoebaFamily implements Iterable<AmoebaFamily.Amoeba> {
	...
	/*public class AmoebaIterator implements Iterator<Amoeba> {
		...
		public Amoeba next() {
		    if (!hasNext()) {
		        throw new NoSuchElementException("amoeba ran out of children");
		    }
		    Amoeba node = (Amoeba) fringe.pop();
		    for (int k = node.myChildren.size()-1; k >= 0; k-- ){
		    	if (node.myChildren.get(k) != null) {
			        fringe.push(node.myChildren.get(k));
			    }
		...
		public class AmoebaIterator implements Iterator<Amoeba> {
			...
			public Amoeba next() {
			    ...
			    for (int k = 0; k < node.myChildren.size(); k++ ){
			    	if (node.myChildren.get(k) != null) {
				        fringe.add(node.myChildren.get(k));
				    }
	...
	public static class Amoeba {
		...
		public void print(String prevIndent) {
			for (Amoeba b : myChildren) {
				myIndent = prevIndent + "    ";
				System.out.println(myIndent + b.myName);
				b.print(myIndent);
			}
		}
		...
		public String longestName() {
			String namer = "";
			int desiredInt = this.longestNameLength();
			if (myName.length() == desiredInt) {
				namer = myName;
				return namer;
			}
			for (Amoeba e : myChildren) {
				e.longestName();
			}
			return namer;
		}
