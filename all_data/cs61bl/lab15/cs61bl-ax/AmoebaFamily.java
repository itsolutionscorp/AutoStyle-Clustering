...
public class AmoebaFamily implements Iterable<AmoebaFamily.Amoeba>{
	...
	public void print() {
		if (myRoot != null) {
			System.out.println(myRoot.print(1));
		}
	}
	...
	public class AmoebaIterator implements Iterator<Amoeba> {
		...
		public Amoeba next() {
			...
			Stack<Amoeba> children = new Stack<Amoeba>();
			Amoeba a = fringe.pop();
			for (Amoeba c : a.myChildren) {
				children.push(c);
			}
			while (!children.isEmpty()) {
				Amoeba c = children.pop();
				fringe.push(c);
			}
			return a;
		}
	...
	*/
	...
	public static class Amoeba {
		...
		public String longestName(int longestLength) {
			if (myName.length() == longestLength) {
				return myName;
			}
			for (Amoeba a : myChildren) {
				if (a.longestName(longestLength).length() == longestLength)
					return a.longestName(longestLength);
			}
			return null;
		}
