...
public class AmoebaFamily implements Iterable<AmoebaFamily.Amoeba>{
	...
	public static void main(String[] args) {
		...
		while (family_it.hasNext()) {
			System.out.println(family_it.next());
		}
	}
	...
	public class AmoebaIterator implements Iterator<Amoeba> {
		...
		public Amoeba next() {
			if (!hasNext()) {
				throw new NoSuchElementException("No more amoebas in the family!");
			}
			...
			/* ArrayList<Amoeba> children = (ArrayList<Amoeba>) a.myChildren.clone();
			Collections.reverse(children);
			for (Amoeba child : children) {
				fringe.push(child);
			}
			Stack new_fringe = new Stack();
			while (!fringe.isEmpty()) {
				new_fringe.push(fringe.pop());
			}
			fringe = new_fringe; */
			return a;
		}
