...
public class AmoebaFamily implements Iterable<AmoebaFamily.Amoeba>{
	...
	public static void main(String[] args) {
		...
		AmoebaIterator iter = (AmoebaIterator) family.iterator();
		while(iter.hasNext()){
			System.out.println(iter.next());
		}
		//System.out.println("");
		//family.makeNamesLowercase();
		//family.replaceName("wilma", "OMG");
		//family.printFlat();
	...
	public class AmoebaIterator implements Iterator<Amoeba> {
		...
		//public Amoeba next() {
		    ...
		    //if (a.myChildren != null) {
		    	//for(int i = a.myChildren.size()-1; i > -1; i--)
		        //fringe.push(a.myChildren.get(i));
		    //}
		    //return a;
		//}
		
		public Amoeba next(){ // the queue version
			if(!hasNext()){
				throw new NoSuchElementException("no more Amoebas");
			}
			Amoeba a = (Amoeba) fringe.poll();
			if(a.myChildren != null){
				for(Amoeba x: a.myChildren){
					fringe.add(x);
				}
			}
			return a;
	...
	public static class Amoeba {
		...
		public void replaceName(String currentName, String newName){
			//System.out.println(myName+" "+currentName+" "+newName);
		...
		public void print(int genCount){
			printHelper(genCount);
			genCount++;
			for(Amoeba a: myChildren){
				a.print(genCount);
			}
		...
		public void printHelper(int generation){
			String tab = "";
		...
		public String longestName(){
			...
			for (Amoeba a : myChildren) {
				longName = maxString(longName, a.longestName());
			}
			return longName;
		} 
		...
		private int height(int bestSoFar) {
			if (myChildren.isEmpty()) {
				...
				for (Amoeba a : myChildren) {
					bestSoFar = Math.max(a.height(bestSoFar)+1, bestSoFar);
				}
				return bestSoFar;
			}
		}
