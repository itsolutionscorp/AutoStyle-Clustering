...
public class AmoebaFamily implements Iterable<AmoebaFamily.Amoeba>{
	...
	public static void main(String[] args) {
		...
		family.replaceName("amos mccoy", "newName");
	...
	public class AmoebaIterator implements Iterator<Amoeba> {
		...
		public Amoeba next() {
			if (!hasNext()) {
				throw new UnsupportedOperationException("tree ran out of elements");
			}
			Amoeba child = (Amoeba)fringe.pop();
			if (child.myChildren!=null) {
				for (int i = child.myChildren.size()-1; i>=0;i--) {
					fringe.push(child.myChildren.get(i));
				}
			}
			return child;
		}
		*/
		...
		public Amoeba next() {
			...
			Amoeba child = (Amoeba)fringe.poll();
			if (child.myChildren!=null) {
				for (int i = 0; i<child.myChildren.size();i++) {
					fringe.add(child.myChildren.get(i));
				}
			}
			return child;
		}
	...
	public static class Amoeba {
        ...
        public void print() {
        	System.out.println(printHelper()+myName);
        	for (Amoeba a: myChildren) {
        		a.print();
        	}
        }
        ...
        public String longestName() {
        	...
        	for (Amoeba a: myChildren) {
        		if (length<a.longestName().length()) {
