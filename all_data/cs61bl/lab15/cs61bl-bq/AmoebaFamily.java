...
public class AmoebaFamily implements Iterable<AmoebaFamily.Amoeba> {
	...
	public static void main(String[] args) {
		...
		System.out.println(family.longestName() + " is the longest name");
		System.out.println(family.size() + " is the size of the family");
		System.out.println(family.height() + " is the height of the family");
		family.makeNamesLowercase();
		family.print();
		Iterator iter = family.iterator();
		while (iter.hasNext()) {
			Amoeba kid = (Amoeba) iter.next();
			System.out.println(kid.myName);
		}
	}
	...
	public class AmoebaIterator implements Iterator<Amoeba> {
		...
		public Amoeba next() {
			if (!hasNext()) {
				throw new NoSuchElementException("your tree ran out of elements");
			}
			Amoeba node = (Amoeba) fringe.poll();
			for (Amoeba a : node.myChildren) {
				((LinkedList) fringe).add(a);
			}
			return (Amoeba) node;
		}
...
//			ArrayList<Amoeba> theKids = node.myChildren;
//			for (int i = theKids.size(); i > 0; i --) {
//				fringe.push(theKids.get(i - 1));
//			}
//			return node;
//		}
//		
