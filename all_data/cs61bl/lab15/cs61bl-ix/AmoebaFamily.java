...
public class AmoebaFamily implements Iterable<AmoebaFamily.Amoeba> {
	...
	public void replaceName(String currentName, String newName) {
		// Your goal is to make this as similar as possible to addChild
		if (myRoot != null) {
			myRoot.nameReplace(currentName, newName);
		}
	}
	...
	public static void main(String[] args) {
		...
		family.print();
		Iterator amoebaIter = family.iterator();
		while(amoebaIter.hasNext()){
			System.out.println(amoebaIter.next());
		}
	}
...
//		}
		...
		public Amoeba next() {
			Amoeba node = (Amoeba) fringe.removeFirst();
			for(Amoeba i: node.myChildren){
				fringe.add(i);
			}
			return node;
		}
	...
	public static class Amoeba {
		...
		public void nameReplace(String currentName, String newName) {
			if (this.myName.equals(currentName)) {
				this.myName = newName;
			} else {
				for (Amoeba a : myChildren) {
					a.nameReplace(currentName, newName);
				}
			}
		}

		public void flatprint() {
		...
		public void printhelper(int level) {
			System.out.println(indent(level) + this.myName);
			for (Amoeba a : myChildren) {
				a.printhelper(level + 1);
			}
		}
		...
		public String longestName() {
			...
			for (Amoeba a : myChildren) {
				int temp = Math.max(longest.length(), a.longestName().length());
				if (temp == a.longestName().length()) {
