...
public class AmoebaFamily implements Iterable<AmoebaFamily.Amoeba>{
	...
	//	private Stack<Amoeba> fringe = new Stack<Amoeba>(); 
		...
		private ArrayList<Amoeba> seen = new ArrayList<Amoeba>(); 
		...
		//	return !fringe.empty(); 
			...
			public Amoeba next() {
				if(hasNext() == false) {
					throw new NoSuchElementException("We are out of family");
				}
...
//				}
//				if (!seen.isEmpty()) {
//				for(int i = seen.size(); i > 0; i--) {
//					fringe.push(seen.get(i-1));
//					}
//				}
//					seen.clear();
				return node;
	...
	public static class Amoeba {
        ...
        public String longestName() {
        	String s = myName;
        	for (Amoeba a: myChildren) {
        		if (s.length() < a.longestNameLength()) {
        			s = a.longestName();
        		}
        	}
        	return s;
        ...
        String s = "";
        public void print() {
        	System.out.print(printhelper(indent) + myName);
        	System.out.println();
			for (Amoeba a: myChildren) {
				indent ++;
				a.printhelper(indent);
				a.print();
			}
			indent --;
