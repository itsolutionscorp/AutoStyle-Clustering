...
public class AmoebaFamily implements Iterable<AmoebaFamily.Amoeba>{
	...
	public static void main(String[] args) {
		...
		/*family.print();
		Iterator<Amoeba> DrDoom = family.iterator(); 
		while(DrDoom.hasNext()) {
			System.out.println(DrDoom.next());
		}*/
		Iterator<Amoeba> DrDoom = family.iterator(); 
		while(DrDoom.hasNext()) {
			System.out.println(DrDoom.next());
	...
	public class AmoebaIterator implements Iterator<Amoeba> {
		...
		private Queue<Amoeba> Q;
		
		//private Stack Spiderman = new Stack();
		
		public AmoebaIterator() {
			Q = new LinkedList();
			if (myRoot != null) Q.add(myRoot);
			/*if (myRoot != null) {
				Spiderman.push(myRoot);
			}*/
		}

		public boolean hasNext() {
			return !Q.isEmpty();
			// return !Spiderman.empty();
		}

		public Amoeba next() {	
			Amoeba theThing = (Amoeba) Q.poll();
			for (Amoeba a: theThing.myChildren) {
				Q.add(a);
		    }
			return theThing;
			/*Amoeba CaptainUnderPants = (Amoeba) Spiderman.pop();
			for (Amoeba a: CaptainUnderPants.myChildren) {
				Spiderman.push(a);
		    }
		    return CaptainUnderPants;*/
		}
	...
	public static class Amoeba {
        ...
        public void makeNamesLowercase() {
        	if (myName != null) myName = myName.toLowerCase();
        	for (Amoeba a : myChildren) {
        ...
        public void replaceName(String currentName, String newName) {
        	if (myName.equals(currentName)) {
        		...
        		for (Amoeba a : myChildren) {
                    a.addChild(currentName, newName);
                }
        	}
        }
        public void printFlat(){
        ...
        public void print() {
        	...
        	System.out.println(result + myName + this.height());
        	for (Amoeba a : myChildren) {
                a.print();
            }
        }
        ...
        public int depth() {
        	int depth = depthHelper(myParent, 0 );
            return depth;
        }
        
        private int depthHelper (Amoeba a, int depth) {
        	if (a == null) return depth;
        	else return depthHelper(a.myParent, depth + 1);
        }
        ...
        public String longestName() {
        	...
        	for (Amoeba a : myChildren) {
        		if (Math.max(maxNameSeen.length(), a.myName.length()) == a.myName.length()) {
        			maxNameSeen = a.myName;
        		}
            }
        	return maxNameSeen;
        }
