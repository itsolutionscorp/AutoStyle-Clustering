...
//		private Stack<Amoeba> fringe = new Stack<Amoeba>();
		...
		public Amoeba next() {
//			Amoeba top = fringe.pop();
//			System.out.println(top.myName);
//			if (top.myChildren.size() > 0) {
//				for (int k = (top.myChildren.size() - 1); k >= 0; k--) {
//					fringe.push(top.myChildren.get(k));
//				}
//			}
//			return top;
			Amoeba front = fringe.remove();
			if (front.myChildren.size() > 0) {
				for (Amoeba child : front.myChildren) {
					fringe.add(child);
				}
			}
			return front;
	...
	public static class Amoeba {
        ...
        public String longestName() {
        	...
        	for (Amoeba each : myChildren) {
        		if (longest.length() < each.longestName().length()) {
        			longest = each.myName;
        		}
        	}
        	return longest;
