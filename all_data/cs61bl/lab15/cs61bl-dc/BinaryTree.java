...
public class BinaryTree
{
	...
	public static class TreeNode
	{
		...
		private int height()
		{
			...
			int right = 0;
			if (myLeft != null)
				left = myLeft.height() + 1;
			if (myRight != null)
				right = myRight.height() + 1;
		...
		private boolean isCompletelyBalanced()
		{
			...
			if (myRight != null)
			{
				...
				rightBalanced = myRight.isCompletelyBalanced();
			}
			if (myLeft != null)
			{
				leftHeight = myLeft.height();
				leftBalanced = myLeft.isCompletelyBalanced();
			}
			return rightHeight == leftHeight && rightBalanced && leftBalanced;
		}
		...
		private void optimize()
		{
			...
			if (myItem.equals("+") || myItem.equals("*"))
			{
				try
				{
					...
					if (myItem.equals("+"))
						myItem = String.valueOf(left + right);
					else
						myItem = String.valueOf(left * right);
					myLeft = null;
					myRight = null;
	...
	private TreeNode exprTreeHelper(String expr)
	{
		if (expr.charAt(0) != '(')
		{
			int indexOfOpp = expr.indexOf('+');
			if (indexOfOpp == -1)
			{
				indexOfOpp = expr.indexOf('*');
			}
			if (indexOfOpp == -1)
				return new TreeNode(expr);
			return new TreeNode(expr.substring(indexOfOpp, indexOfOpp + 1), new TreeNode(expr.substring(0, indexOfOpp)), new TreeNode(expr.substring(indexOfOpp + 1)));
		} else
		{
