...
public class JugSolver {
	...
	private HashSet<JugContents> myset;
	...
	public boolean tryPouring (JugContents jugsObject) {
		...
		Iterator<JugContents> it = myset.iterator();
		while(it.hasNext()){
			JugContents cop= it.next();
			if ( cop.jugs[0]==jugsObject.jugs[0] && cop.jugs[1]==jugsObject.jugs[1]
					&& cop.jugs[1]==jugsObject.jugs[1] ){
				return false;
			}
		}
		myset.add(jugsObject);
