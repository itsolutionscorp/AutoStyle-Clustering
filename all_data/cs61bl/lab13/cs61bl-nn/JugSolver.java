...
public class JugSolver {
	...
	private HashSet<JugContents> memories = new HashSet<JugContents>();
	...
	public boolean tryPouring(JugContents jugsObject) {
		...
		if (jugsObject.jugs[0] == desiredAmt || jugsObject.jugs[1] == desiredAmt || jugsObject.jugs[2] == desiredAmt) {
			System.out.println(jugsObject.jugs[0]+" "+jugsObject.jugs[1]+" "+jugsObject.jugs[2]);
			return true;
		}
		...
		for (int i = 0; i < 3; i++) {
			for (int j = 0; j < 3; j++) {
				JugContents result = pour(jugsObject, i, j);
				if (memories.contains(result) == true){
				} else {
					memories.add(result);
					if (i != j && tryPouring(result)) {
	...
	public JugContents pour(JugContents current, int from, int to) {
		...
		debugPrint("Pouring " + amtPoured + " from jug " + from + " to jug " + to);
