...
public class JugSolver {
	...
	public boolean tryPouring (JugContents jugsObject) {
		...
		else if (myAmts.contains(jugsObject.contents())) {
			return false;
		} else {
			myAmts.add(jugsObject.contents());
			for (int i = 0; i < 3; i++) {
				for (int j = 0; j < 3; j++) {
	...
	public static void main (String[] arg) {
		...
		manyPoursNeeded.tryPouring(initial);
	}
