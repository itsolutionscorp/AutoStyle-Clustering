...
public class JugSolver {
	...
	private HashSet<JugContents>  jugresultst;
	private HashSet<JugContents> jugresultsf;
	...
	public JugSolver (int jug0size, int jug1size, int jug2size, int desired) {
		...
		capacity[2] = jug2size;
		desiredAmt = desired;
		jugresultst = new HashSet<JugContents>();
		jugresultsf = new HashSet<JugContents>();
	}
	...
	public boolean tryPouring (JugContents jugsObject) {
		...
		if (jugsObject.jugs[0] == desiredAmt || jugsObject.jugs[1] == desiredAmt
			...
			jugresultst.add(jugsObject);
			return true;
		}
		
		// Your code at this line, and possibly below
		if(jugresultst.contains(jugsObject)){
			return true;
		}
		else if(jugresultsf.contains(jugsObject)){
			return false;
		}
		for (int i = 0; i < 3; i++) {
			for (int j = 0; j < 3; j++) {
