...
public class JugSolver {
	...
	private HashSet<JugContents> myTrue;
	private HashSet<JugContents> myFalse;
	...
	public JugSolver (int jug0size, int jug1size, int jug2size, int desired) {
		...
		capacity[2] = jug2size;
		desiredAmt = desired;
		myTrue = new HashSet<JugContents>();
		myFalse = new HashSet<JugContents>();
	}
	...
	public boolean tryPouring (JugContents jugsObject) {
		...
		else if (myTrue.contains(jugsObject)) {
			return true;
		} 
		else if (myFalse.contains(jugsObject)) {
			return false;
		}
		else {
			myFalse.add(jugsObject);
			for (int i = 0; i < 3; i++) {
				for (int j = 0; j < 3; j++) {
					if (i != j && tryPouring (pour(jugsObject, i, j))) {
						System.out.println ("Pouring from jug " + i + " to jug " + j);
						myTrue.add(jugsObject);
						return true;
					}
				}
			}
