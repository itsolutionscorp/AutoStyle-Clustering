...
//    }
    ...
    public int hashCode() {
    	...
    	for (int i = 0; i < 3; i++){
    		for (int j = 0; j < 3; j++){
    			char character = myBoard[i][j];
    			switch(character) {
    			case 'O':
    				boardString  = boardString*10 + 2;
    				break;
    			case 'X':
    				boardString  = boardString*10 + 1;
    				break;
    			case BLANK:
    				boardString  = boardString*10 + 0;
    				break;
    			}
    		}
    	}
    	return base3ToBase10(boardString);
    }
    
    public static int base3ToBase10(int toConvert) {
    	if (toConvert < 10)
    		return toConvert;
    	int first_digit = toConvert % 10;
    	return first_digit + 3*(base3ToBase10(toConvert/10));
    }
