...
public class JugSolver {
	...
	private HashSet<JugContents> right;
	private HashSet<JugContents> wrong;
	
	public JugSolver (int jug0size, int jug1size, int jug2size, int desired) {
		right = new HashSet<JugContents>();
		wrong = new HashSet<JugContents>();
		capacity = new int [3];
		capacity[0] = jug0size;
		capacity[1] = jug1size;
		capacity[2] = jug2size;
	...
	public boolean tryPouring (JugContents jugsObject) {
		...
		if (right.contains(jugsObject)) {
			return true;
		}
		if (wrong.contains(jugsObject)) {
			return false;
		}
		wrong.add(jugsObject);
		for (int i = 0; i < 3; i++) {
			for (int j = 0; j < 3; j++) {
				if (i != j && tryPouring (pour(jugsObject, i, j))) {
					System.out.println ("Pouring from jug " + i + " to jug " + j);
					right.add(jugsObject);
					return true;
				}
			}
		}
