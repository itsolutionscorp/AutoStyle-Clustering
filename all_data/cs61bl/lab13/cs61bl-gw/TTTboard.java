...
public class TTTboard {
    ...
    private int hashString(){
    	String convertS = new String();
    	for ( int r = 0; r < 3; r++){
    		for (int c = 0; c < 3; c++) {
    			convertS = convertS + myBoard[r][c];
    		}
    	}
    	return convertS.hashCode();
    }
    
    private int hashInt(){
    	char tempRead; // temp char to read TTT 
    	int res = 0;
    	int base = 3;
    	int temp; //convert TTT char to int
    	for (int r = 0; r < 3; r++){
    		for (int c = 0; c < 3; c++) {
    			tempRead = myBoard[r][c];
    			if (tempRead == ' ')
    				temp = 0;
    			else if (tempRead == 'X')
    				temp = 1;
