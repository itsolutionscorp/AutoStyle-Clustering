...
public class JugSolver {
	...
	private HashMap<Integer, int[]> jugMoves;
	...
	public JugSolver (int jug0size, int jug1size, int jug2size, int desired) {
		...
		capacity[2] = jug2size;
		desiredAmt = desired;
		jugMoves = new HashMap<Integer, int[]>();
	}
	...
	public boolean tryPouring (JugContents jugsObject) {
		...
		int position = jugsObject.hashCode();
		if (jugsObject.equals(jugMoves.get(position))) {
			return false;
		}
		int[] arr = new int[]{jugsObject.jugs[0],jugsObject.jugs[1],jugsObject.jugs[2]};
		jugMoves.put(position, arr);
