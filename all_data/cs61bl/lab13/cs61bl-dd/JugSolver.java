...
public class JugSolver {
	...
	public boolean tryPouring (JugContents jugsObject) {
		...
		for (int i = 0; i < 3; i++) {
			for (int j = 0; j < 3; j++) {
				//check whether the object has been checked before
				if(config.contains(jugsObject.mark(i, j))){
					continue;
				}
	...
	public JugContents pour (JugContents current, int from, int to) {
		...
		//add checked object to the HashSet
		config.add(current.mark(from, to));
		return afterPour;
	}
