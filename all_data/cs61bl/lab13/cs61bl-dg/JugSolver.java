...
public class JugSolver {
	...
	HashSet<JugContents> previous = new HashSet<JugContents>();
	...
	public boolean tryPouring (JugContents contents) {
		debugPrint (contents.toString());
		if (contents.jugs[0] == desiredAmt || contents.jugs[1] == desiredAmt
		        || contents.jugs[2] == desiredAmt) {
			return true;
		}
		if  (!(previous.add(contents))) {
			return false;
		}
		for (int i = 0; i < 3; i++) {
			for (int j = 0; j < 3; j++) {
				if (i != j && tryPouring (pour(contents, i, j))) {
