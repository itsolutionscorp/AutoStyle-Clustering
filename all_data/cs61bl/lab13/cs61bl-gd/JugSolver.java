...
public class JugSolver {
	...
	//private HashSet<JugContents> result;
	...
	public JugSolver (int jug0size, int jug1size, int jug2size, int desired) {
		...
		capacity[2] = jug2size;
		desiredAmt = desired;
		//result = new HashSet<JugContents>();
		pour = new HashSet<Integer>();
	}
	...
	public boolean tryPouring (JugContents jugsObject) {
		...
		//if(result.contains(jugsObject)){
		//	return false;
		//}
		
		for (int i = 0; i < 3; i++) {
			for (int j = 0; j < 3; j++) {
				if(!pour.contains(position(jugsObject,i,j))){
	...
	public JugContents pour (JugContents current, int from, int to) {
		...
		pour.add((current.hashCode()*100+from*10+to));
		return afterPour;
	}
	...
	public int position(JugContents jug,int from, int to){
		return jug.hashCode()*100+from*10+to;
	}
