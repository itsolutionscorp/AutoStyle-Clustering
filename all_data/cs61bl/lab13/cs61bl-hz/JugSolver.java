...
public class JugSolver {
	...
	public boolean tryPouring (JugContents jugsObject) {
		...
		if (solutions.contains(jugsObject.toString().hashCode())) {
			return true;
		} if (bad.contains(jugsObject.toString().hashCode())) {
			return false;
		}
		for (int i = 0; i < 3; i++) {
			for (int j = 0; j < 3; j++) {
				if (i != j && tryPouring (pour(jugsObject, i, j))) {
					...
					solutions.add(jugsObject.toString().hashCode());
					return true;
				}
				bad.add(jugsObject.toString().hashCode());
			}
		}
		return false;
	}
