...
public class JugSolver {
	...
	private HashMap<Integer, JugContents> map;
	...
	public JugSolver (int jug0size, int jug1size, int jug2size, int desired) {
		...
		capacity[1] = jug1size;
		capacity[2] = jug2size;
		map = new HashMap<Integer, JugContents>();
		desiredAmt = desired;
	}
	...
	public boolean tryPouring (JugContents jugsObject) {
		...
		if (map.containsKey(jugsObject.hashCode())) {
			return true;
		}
		map.put(jugsObject.hashCode(), jugsObject);
		for (int i = 0; i < 3; i++) {
			for (int j = 0; j < 3; j++) {
