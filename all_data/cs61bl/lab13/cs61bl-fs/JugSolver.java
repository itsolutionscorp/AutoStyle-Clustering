...
public class JugSolver {
	...
	private HashSet<JugContents> seenBefore;
	...
	public JugSolver (int jug0size, int jug1size, int jug2size, int desired) {
		...
		capacity[2] = jug2size;
		desiredAmt = desired;
		seenBefore = new HashSet<JugContents>();
	}
	...
	public boolean tryPouring (JugContents jugsObject) {
		...
		if (seenBefore.contains(jugsObject)) {
			return false;
		}
		
		for (int i = 0; i < 3; i++) {
			for (int j = 0; j < 3; j++) {
				JugContents tried = pour(jugsObject, i, j);
				if (i != j && tryPouring (tried)) {
					System.out.println ("Pouring from jug " + i + " to jug " + j);
					return true;
				}
				else {
					seenBefore.add(tried);
				}
			}
		}
		return false;
