...
public class JugSolver {
	...
	public boolean tryPouring (JugContents jugsObject) {
		...
		for (int i = 0; i < 3; i++) {
			for (int j = 0; j < 3; j++) {
				if (i != j){
					if(jugsObject.jugs[i]!=0 &&
						capacity[j]-jugsObject.jugs[j]!=0&&
						!seen.contains(pour(jugsObject, i, j))&&
						tryPouring (pour(jugsObject, i, j))) {
