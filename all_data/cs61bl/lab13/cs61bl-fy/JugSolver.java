...
public class JugSolver {
	...
	private HashSet<JugContents> configs;
	public JugSolver (int jug0size, int jug1size, int jug2size, int desired) {
		...
		capacity[2] = jug2size;
		desiredAmt = desired;
		configs = new HashSet<JugContents>(); 
	}
	...
	public boolean tryPouring (JugContents jugsObject) {
		...
		configs.add(jugsObject);
		
		for (int i = 0; i < 3; i++) {
			for (int j = 0; j < 3; j++) {
				if (i != j && !configs.contains(pour(jugsObject, i, j)) && tryPouring (pour(jugsObject, i, j))) {
					System.out.println ("Pouring from jug " + i + " to jug " + j);
					//the one time pour works here
					return true;
				}
			}
		}
	...
	public JugContents pour (JugContents current, int from, int to) {
		...
		//returns the result of pouring as much to another
		return afterPour;
	}
