...
public class TTTboard {
    ...
    public int hashCodeInt() {
        ...
        for (int r = 2; r >= 0; r-=1){
        	for (int c = 0; c < 3; c+=1){
        		...
        		if (myBoard[r][c] == BLANK){
        			values.add(0);
        		}
        	}
        }
    	for (int i = 0; i < 9; i += 1){
    		haji += values.get(i) * (3^(8-i));
    	}
    	return haji;
    }
    ...
    public static void main (String[] args) {
        ...
        */ 
        ...
        Timer stringTimer = new Timer();
        stringTimer.start();
        for(int i = 0; i<100;i++){
        	int stringHash = board.hashCodeString();
        }
        stringTimer.stop();
        double stringTime = stringTimer.elapsed();
