...
public class JugSolver {
	...
	HashMap<String, Boolean> flasher = new HashMap<String,Boolean>();
	...
	public boolean tryPouring (JugContents jugsObject) {
		...
		System.out.println(flasher.get(jugsObject.toString()));
		
		if(flasher.get(jugsObject.toString())!=null){
			return flasher.get(jugsObject.toString());
		}
		
		flasher.put(jugsObject.toString(),false);
		
		System.out.println(jugsObject.toString().hashCode());

		for (int i = 0; i < 3; i++) {
			for (int j = 0; j < 3; j++) {
				if (i != j && tryPouring (pour(jugsObject, i, j))) {
					System.out.println ("Pouring from jug " + i + " to jug " + j);
					flasher.put(jugsObject.toString(),true);
					return true;
				}
			}
		}
