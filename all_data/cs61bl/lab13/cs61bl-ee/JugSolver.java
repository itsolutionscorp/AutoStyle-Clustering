...
public class JugSolver {
	...
	HashSet<JugContents> ContentSet = new HashSet<JugContents>();
	...
	public boolean tryPouring (JugContents jugsObject) {
		...
		for (int i = 0; i < 3; i++) {
			for (int j = 0; j < 3; j++) {
				if(i !=j){
//				JugContents temp =  pour(jugsObject, i, j);
//				if(ContentSet.contains(temp)){
//					continue;
//				}
				JugContents temp =  pour(jugsObject, i, j);
				String tempStr = temp.toString();
				if(test.contains(tempStr)){
					continue;
				}
//				ContentSet.add(temp);
				test.add(tempStr);
				System.out.println(temp.toString());
				if (i != j && tryPouring (temp)) {
