...
public class TTTboard {
	...
	public int hashCode() {
		int result = 0;
		for (int i = 0; i < 9; i++) {
			char c = myBoard[i / 3][i % 3];
			switch(c) {
			case 'X':
				result += Math.pow(1, 8 - i);
				break;
			case 'O':
				result += Math.pow(2, 8 - i);
				break;
			case ' ':
				result += Math.pow(0, 8 - i);
				break;
			}
		}
		return result;
...
//		return toString().hashCode();
	...
	public void makeMove(boolean player1isMoving, int row, int col) {
		if (cell(row, col) != BLANK) {
			System.err.println("Can't move to " + row + "," + col + "; move already made.");
	...
	public boolean wins(int row, int col) {
		...
		case 0:
			switch (col) {
			case 0:
				return allMatch(0, 1, 2) || allMatch(0, 3, 6) || allMatch(0, 4, 8);
			case 1:
				return allMatch(0, 1, 2) || allMatch(1, 4, 7);
			case 2:
				return allMatch(0, 1, 2) || allMatch(2, 5, 8) || allMatch(2, 4, 6);
			}
		case 1:
			switch (col) {
			case 0:
			...
			case 1:
				return allMatch(3, 4, 5) || allMatch(1, 4, 7) || allMatch(0, 4, 8) || allMatch(2, 4, 6);
			case 2:
		...
		case 2:
			switch (col) {
			case 0:
				return allMatch(6, 7, 8) || allMatch(0, 3, 6) || allMatch(2, 4, 6);
			case 1:
				return allMatch(6, 7, 8) || allMatch(1, 4, 7);
			case 2:
				return allMatch(6, 7, 8) || allMatch(2, 5, 8) || allMatch(0, 4, 8);
			}
		}
		return false;
	}
	...
	private boolean allMatch(int a, int b, int c) {
		...
		int rowc = c / 3;
		int colc = c % 3;
		return (cell(rowa, cola) == cell(rowb, colb)) && (cell(rowb, colb) == cell(rowc, colc));
	}
