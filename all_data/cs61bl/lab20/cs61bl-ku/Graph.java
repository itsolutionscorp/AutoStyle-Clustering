...
public class Graph implements Iterable<Integer>{
    ...
    public void addUndirectedEdge(int v1, int v2, Object edgeInfo) {
    	...
    	Edge e = new Edge(v1, v2, edgeInfo);
    	myAdjLists[v1].add(e);
    	Edge reverse = new Edge(v2, v1, edgeInfo);
    	myAdjLists[v2].add(reverse);
    }
    ...
    private class DFSIterator implements Iterator<Integer> {
        ...
        public Integer next() {
            if (hasNext()) {
            	Integer num = fringe.pop();
            	visited.add(num);
            	LinkedList<Edge> edges = myAdjLists[num];
            	for (Edge e: edges) {
    ...
    public ArrayList<Integer> path(int startVertex, int stopVertex) {
    	...
    	if (!pathExists(startVertex, stopVertex)) {
			...
			Iterator<Integer> iter = new DFSIterator(startVertex);
			int end = iter.next();
			while (iter.hasNext() && end != stopVertex) {
				end = iter.next();
				result.add(end);
			} if (end != stopVertex) {
    ...
    private class TopologicalIterator implements Iterator<Integer> {
        ...
        public Integer next() {
            if (hasNext()) {
            	Integer num = fringe.pop();
            	LinkedList<Edge> edges = neighbors(num);
            	for (Edge e: edges) {
