...
public class Graph implements Iterable<Integer>{
    ...
    public void addEdge(int v1, int v2, Object edgeInfo) {
    	...
    	if(myAdjLists[v1].isEmpty()){
    		myAdjLists[v1].add(temp);}else{
    			if(myAdjLists[v1].contains(temp)){
    				return;
    			}else{myAdjLists[v1].add(temp);}
    		}
    ...
    private class DFSIterator implements Iterator<Integer> {
        ...
        public Integer next() {
        	...
        	List currentAdj = neighbors(current);
        	for ( Object n: currentAdj){
        		if(!visited.contains(n))  {
        			fringe.push((Integer) n);
        			visited.add((Integer) n);
    ...
    public ArrayList<Integer> path(int startVertex, int stopVertex) {
    	...
    	while(!fringe.empty()){
    		...
    		visited.add(current);
    		for(Object a : neighbors(current)){
    			if(!visited.contains((Integer)a)){
    				steps.put((Integer) a, current);
    				fringe.push((Integer) a);
    			}
    		}
    		
    	}
    	Stack<Integer> reversePath = new Stack<>();
    	Integer target = stopVertex;
    	reversePath.push(target);
    	while(target!=null){
    		
    		Integer previousStep= steps.get(target);
    ...
    private class TopologicalIterator implements Iterator<Integer> {
        ...
        public TopologicalIterator() {
            ...
            for(Integer a : all){
            	if(neighbors(a).isEmpty()){
            		sink =a;
            		
            	}
            }
            
            if(sink==null||sources==null){
            	throw new IllegalStateException();
            }
            fringe.push(sources);
            
        }
        public boolean isSources(Integer a){
        	Integer degree=0;
        	for(Integer v:visited){
        		if(neighbors(v).contains(a)){
        			degree++;
        		}
        	}
        	if(inDegree(a)<=degree){
        		return true;
        	}
        	return false;
        }
    ...
    public static void main(String[] args) {
        ...
        test.addEdge(8, 10);
        test.addEdge(9, 10);
        //result = test.topologicalSort();
        System.out.println("Start of 11 elements");
        result = test.topologicalSort();
