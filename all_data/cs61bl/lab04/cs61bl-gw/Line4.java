...
public class Line4 {
    ...
    void printLength() {
        double length;
        length = Math.sqrt ( Math.pow(points[0].getX()-points[1].getX(), 2)+Math.pow(points[0].getY()-points[1].getY(), 2)) ;
        System.out.println ("Line length is " + length);
    }
    
    void printAngle() {
        double angleInDegrees = Math.atan2 ( Math.abs(points[0].getY()-points[1].getY()) , Math.abs(points[0].getX()-points[1].getX()) ) * 180.0 / Math.PI;
