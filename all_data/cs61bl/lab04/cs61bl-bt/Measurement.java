public class Measurement {
	...
	public Measurement minus(Measurement m2) {
		int newFeet = this.myFeet;
		int newInches = this.myInches;
		if (m2.myInches <= this.myInches){
			newInches -= m2.myInches;
			newFeet -= m2.myFeet;
		}
		else{
			newFeet--;
			newInches = newInches + 12 - m2.myInches;
			newFeet -= m2.myFeet;
		}
	...
	public Measurement multiple(int multipleAmount) {
		int newFeet = this.myFeet;
		int newInches = this.myInches;
		newInches += (newFeet * 12);
		newFeet = 0;
		newInches *= multipleAmount;
		return new Measurement(newFeet, newInches); // provided to allow the file to compile
	}
