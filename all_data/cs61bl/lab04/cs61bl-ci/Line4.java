...
public class Line4 {
    ...
    void printLength() {
        double length;
        length = Math.sqrt ( Math.pow(this.points[0].getX() - this.points[1].getX(),2) +
        					 Math.pow(this.points[0].getY() - this.points[1].getY(), 2)) ;
        System.out.println ("Line length is " + length);
    }
    
    void printAngle() {
        double angleInDegrees = Math.atan2 (this.points[1].getY() - this.points[0].getY(),
        		this.points[1].getX() - this.points[0].getX() ) * 180.0 / Math.PI;
