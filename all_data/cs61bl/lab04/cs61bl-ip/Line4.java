...
public class Line4 {
    ...
    void printLength() {
        double length;
        length = Math.sqrt (Math.pow((points[1].x - points[0].x), 2) 
        		+ Math.pow((points[1].y - points[0].y), 2)) ;
        System.out.println ("Line length is " + length);
    }
    
    void printAngle() {
        double angleInDegrees = Math.atan2 (points[1].y - points[0].y, 
    ...
    public static void main(String[] args) {
        ...
        test.points = new Point[]{new Point(5, 10), new Point(45, 40)};
        test.printLength();
        test.printAngle();
    }
