public class Line3 {
    ...
    void printLength() {
        double length;
        length = Math.sqrt (Math.pow((coords[2] - coords[0]), 2) 
        		+ Math.pow((coords[3] - coords[1]), 2)) ;
        System.out.println ("Line length is " + length);
    }
    
    void printAngle() {
        double angleInDegrees = Math.atan2 (coords[3] - coords[1], 
        		coords[2] - coords[0]) * 180.0 / Math.PI;
    ...
    public static void main(String[] args) {
		 ...
		 * object. Initialize myLine's coords[0] and coords[1] (elements 0 and 1 of coords) to
		 * the point (5, 10), and initialize myLine's coords[2] and coords[3] (elements 2 and
        ...
        Line3 test = new Line3();
        test.coords = new int[]{5, 10, 45, 40};
        test.printLength();
        test.printAngle();
    }
