...
public class GitletTest {
	...
	/**
	 * Tests proper behavior of staging files, committing, and checking out.
	 * Involves init, add, commit, and checkout.
	 */
    @Test
    public void testBasicAddCommit() {
        String fileA = TESTING_DIR + "a.txt";
        gitlet("init");
        createFile(fileA, "This is a.");
        gitlet("add", fileA);
        gitlet("commit", "this is a.txt");
        writeFile(fileA, "modified a");
        gitlet("add", fileA);
        writeFile(fileA, "This is a.");
        gitlet("commit", "updated a");
        writeFile(fileA, "This is a.");
        gitlet("checkout", fileA);
        System.out.println(getText(fileA));
        assertEquals("modified a", getText(fileA));
    }


	@Test
    public void testBasicRm() {
        String fileA = TESTING_DIR + "a.txt";
        gitlet("init");
        createFile(fileA, "This is a.");
        gitlet("add", fileA);
        ...
        System.out.println(getText(fileA));
        assertEquals("This is a.", getText(fileA));
    }
	...
	/**
	 * Tests merging of branches according to the spec.
	 * Involves init, add, commit, branch, and checkout.
	 */
	@Test
	    public void testMerge() {
	        String aDir = TESTING_DIR + "a.txt";
	        String bDir = TESTING_DIR + "b.txt";
	        String cDir = TESTING_DIR + "c.txt";
	        String dDir = TESTING_DIR + "d.txt";
	        ...
	        gitlet("add", aDir);
	        gitlet("add", bDir);
	        gitlet("commit", "Added a and b");
	        gitlet("add", dDir);
	        gitlet("commit", "Added d");
	        gitlet("branch", "secondline");
	 
	        writeFile(aDir, "a2");
	        gitlet("add", aDir);
	        gitlet("commit", "Changed a");       
	        gitlet("checkout", "secondline");
	        assertEquals("a1", getText(aDir));
	 
	        writeFile(aDir, "a2");
	        gitlet("add", aDir);
	        gitlet("commit", "a1 changed to a2");
	        writeFile(cDir, "c2");
	        gitlet("add", cDir);
	        gitlet("commit", "Added c2");
	        writeFile(dDir, "d2");
	        gitlet("add", dDir);
	        gitlet("commit", "Changed d"); 
	 
	        gitlet("checkout", "master");
	        assertEquals("a2", getText(aDir));
	        assertEquals("d1", getText(dDir));
	        writeFile(cDir, "omg");
	        gitlet("merge", "secondline");
	        assertEquals("a2", getText(aDir));
	        assertEquals("a2", getText(TESTING_DIR + "a.txt.conflicted"));
	        assertEquals("d2", getText(dDir));
	        assertEquals("omg", getText(cDir));
	        assertEquals("b1", getText(bDir));
	    }
