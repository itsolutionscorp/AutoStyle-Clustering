...
public class InsertionSortTest {
    ...
    public void isOK2() {
    	int[] ood = {5, 9, 11, 4, 13};
        InsertionSort.isOK(ood, 2);
        
        exception.expect(IllegalStateException.class);
        InsertionSort.isOK(ood, 4);
    ...
    @Test
    public void insert2() {
    	int [] io = {1, 2, 3, 4, 5, 6, 7}; // all in order
    	
    	for (int i = 0; i < io.length - 1; i++) {
            InsertionSort.insert(io, i);
        }
    }
    ...
    public void insertionSort2() {
    	int[] duplicate = {0, 1, 2, 2, 0, 1, 2, 2};
        int[] result = InsertionSort.insertionSort(duplicate);
        int[] check = {0, 0, 1, 1, 2, 2, 2, 2};
        assertEquals(Arrays.toString(result), Arrays.toString(check));
