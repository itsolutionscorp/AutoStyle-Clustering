public class InsertionSort {
    ...
    public static void insert (int list[], int k) {
        ...
        for (int n = k; n > indexK ; n--) {
        	list[n] = list[n-1];
        }
        list[indexK] = valueK;

    }
    /*Also fill in the body for the insert method, which takes the kth element 
    of a array and inserts it correctly into the array so that the first k + 1 elements are sorted correctly.*/
    ...
    public static void isOK (int[] list, int k) {
    	...
    	if (k < 0 || k > list.length-1) {
    		throw new IllegalStateException("k must be between 0 and "+(list.length-1)+"!");
    	}
    	
        for (int m = 0; m < k; m++) {
        	if (list[m] > list[m+1]) {
        		throw new IllegalStateException("Elements at index "+m+" and "+(m+1)+" are in decreasing order!");
        	}
        }
