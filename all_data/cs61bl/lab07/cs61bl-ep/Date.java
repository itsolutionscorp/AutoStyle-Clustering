    ...
    public void isOK() {
        ...
        if (myMonth==4 || myMonth==6 || myMonth==9 || myMonth==11) {
        	if (myDateInMonth < 1 || myDateInMonth > 30) {
        		throw new IllegalStateException("DateInMonth must be between 1 and 30 when Month is "+myMonth);
        	}
        } else if (myMonth==2) {
        	if (leapDay) {
        		if (myDateInMonth < 1 || myDateInMonth > 29) {
        			throw new IllegalStateException("DateInMonth must be between 1 and 29 in February of a leap year!");
        		}
        	} else {
        		if (myDateInMonth < 1 || myDateInMonth > 28) {
        			throw new IllegalStateException("DateInMonth must be between 1 and 28 in February of a non-leap year!");
        		}
        	}
        } else {
        	if (myDateInMonth < 1 || myDateInMonth > 31) {
        		throw new IllegalStateException("DateInMonth must be between 1 and 31 when Month is "+myMonth);
        	}
        }
    }
}
