public class Date {
    ...
    public void isOK() {
        ...
        if (myYear < 1900 || myYear > 2100) {
        	throw new IllegalStateException("The input year is out of range.");
        }
        if (myMonth > 12 || myMonth < 1) {
        	throw new IllegalStateException("The input month is out of range.");
        }
        if (leapDay) {
        	daysInMonth[1] = 29;
        }
        if (myDateInMonth > daysInMonth[myMonth-1] || myDateInMonth < 1) {
        	throw new IllegalStateException("The input day is out of range.");
        }
    }
}
