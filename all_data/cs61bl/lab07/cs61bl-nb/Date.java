public class Date {
    ...
    public void isOK() {
        ...
        int[] nonLeapmonthLengths = {31, 28, 31, 30, 31, 30, 31,
            31, 30, 31, 30, 31};
        int[] LeapmonthLengths = {31, 29, 31, 30, 31, 30, 31,
           31, 30, 31, 30, 31};
       
     
		
		if(leapDay){
			if(myDateInMonth <1 || myDateInMonth > LeapmonthLengths[myMonth-1]){
				throw new IllegalStateException("Day not in range.");
			}
		} else {
			if(myDateInMonth <1 || myDateInMonth > nonLeapmonthLengths[myMonth-1]){
				throw new IllegalStateException("Day not in range.");
	        }
	    }
