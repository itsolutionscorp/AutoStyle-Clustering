...
public class NumberGuesser {
	...
	public static void main(String[] args) throws Exception {
		...
		String answer;
		System.out
				.println("Please think of an integer between 0 and 20 (inclusive).");
		int low, high, guess;
		low = 0; // #1
		high = 20; // #2
		guess = (low + high) / 2; // #3
		...
		while (!answer.equals("y")) {
			...
			if (!NumberGuesser.isOK(guess, low, high, previousGuesses)) {
				System.out.println("problem with guesses: " + "guess = "
						+ guess + ", low = " + low + ", high = " + high);
			}
			System.out.println("Is youn" + "r number " + guess
					+ "? (Type y or n.)");
			answer = keyboard.readLine();
		}
	}

	public static boolean isOK(int guess, int low, int high,
			ArrayList<Integer> previousGuesses) {
		try {
			if (high == 20 || low == 0) {
				throw new IllegalStateException(
						"one of high or low has not changed");
			}
