public class Date {
    
    private int myMonth;        // months range from 1 (January) through 12 (December)
    private int myDateInMonth;  // dates-in-month range from 1 through the number of days in the month
    private int myYear;         // years are between 1900 and 2100 (arbitrary decision)

    public Date (int month, int dateInMonth, int year) {
        myMonth = month;
        myDateInMonth = dateInMonth;
        myYear = year;
    }
    
    
    
    // Determine if the date information is internally consistent.
    public void isOK() {
        boolean leapDay;
        if (myYear % 4 != 0) {
            leapDay = false;
        } else if (myYear % 100 != 0) {
            leapDay = true;
        } else if (myYear % 400 != 0) {
            leapDay = false;
        } else {
            leapDay = true;
        }

        if (myYear < 1900 || myYear > 2100)
        	throw new IllegalStateException("Year out of range");
        
        if (myMonth < 0 || myMonth > 12)
        	throw new IllegalStateException("Month out of range");
        
        if (myDateInMonth < 0 || myDateInMonth > 31)
        	throw new IllegalStateException("Date in month out of range");
        
        if (leapDay && myMonth == 2 && myDateInMonth > 29)
        	throw new IllegalStateException("Date cannot be greater than 29");
        
        if (myMonth == 4 || myMonth == 6 || myMonth == 9 || 
        		myMonth == 11) {
        	if (myDateInMonth > 30)
        		throw new IllegalStateException("Date cannot be greater than 30");
        }
        if (!leapDay && myMonth == 2 && myDateInMonth > 28)
        	throw new IllegalStateException("Date cannot be greater than 28");
    }
}
