public class Date {
    ...
    public void isOK() {
        ...
        if (myYear < 1900 || myYear > 2100) {
        	throw new IllegalStateException("Not in scope of years");
        }
        if (myMonth < 1 || myMonth > 12) {
        	throw new IllegalStateException("Not in scope of months");
        }
        if(leapDay) {
        	monthLengths = new int[] {31, 29, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31};
        }
        if (myDateInMonth > monthLengths[myMonth - 1] || myDateInMonth < 1) {
        	throw new IllegalStateException("not in scope of days whoops");
        }
    }
}
