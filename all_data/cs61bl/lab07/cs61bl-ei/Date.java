public class Date {
    ...
    public void isOK() {
        ...
        if(myYear>2100||myYear<1900){
        	throw new IllegalStateException("Put year between 1900-2100");
        }
        if(myMonth>12||myMonth<1){
        	throw new IllegalStateException("Put month between 1-12");
        }
        if(myDateInMonth>monthLengths[myMonth-1] || myDateInMonth <1){
        	if (myDateInMonth==29 && leapDay &&myMonth == 2){
        		//dont do anything
        	} else {
        		throw new IllegalStateException("Invalid day in month");
        	}
        }
    }
}
