public class InsertionSort {
    ...
    public static void isOK (int[] list, int k) {
    	...
    	if (k < 0 || k > list.length - 1) {
    		throw new IllegalStateException("k is negative or k exceeds maximum list index");
    	}
    	for (int i = 0; i < k; i++) {
    		if (list[i] > list[i+1]) {
    			throw new IllegalStateException("elements of list are not sorted in increasing order");
    		}
    	}
    }
