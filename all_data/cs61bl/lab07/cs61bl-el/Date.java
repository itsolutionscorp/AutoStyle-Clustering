public class Date {

	private int myMonth; // months range from 1 (January) through 12 (December)
	private int myDateInMonth; // dates-in-month range from 1 through the number
								// of days in the month
	private int myYear; // years are between 1900 and 2100 (arbitrary decision)

	public Date(int month, int dateInMonth, int year) {
		myMonth = month;
		myDateInMonth = dateInMonth;
		myYear = year;
	}

	// Determine if the date information is internally consistent.
	public void isOK() {
		boolean leapDay;
		if (myYear % 4 != 0) {
			leapDay = false;
		} else if (myYear % 100 != 0) {
			leapDay = true;
		} else if (myYear % 400 != 0) {
			leapDay = false;
		} else {
			leapDay = true;
		}

		// YOUR CODE HERE
		if (myDateInMonth < 0) {
			throw new IllegalStateException("Can't have negative days");
		}

		if (!(myMonth >= 1 && myMonth <= 12)) {
			throw new IllegalStateException("Month is not in range");
		}
		if (!(myYear >= 1900 && myYear <= 2100)) {
			throw new IllegalStateException("Year is not in range of 1900-2100");
		}
		if (leapDay == true && myMonth == 2 && myDateInMonth > 29) {
			throw new IllegalStateException(
					"Leap Year. No date in Feb after the 29th");
		}
		if (leapDay == false && myMonth == 2 && myDateInMonth > 28) {
			throw new IllegalStateException(
					"Not Leap Year. No date in Feb after 28th");
		}
		if ((myMonth == 1 || myMonth == 3 || myMonth == 5 || myMonth == 7
				|| myMonth == 8 || myMonth == 10 || myMonth == 12)
				&& myDateInMonth > 31) {
			throw new IllegalStateException("Max 31 days in this month.");
		}
		if ((myMonth == 4 || myMonth == 6 || myMonth == 9 || myMonth == 11)
				&& myDateInMonth > 30) {
			throw new IllegalStateException("Max 30 days in this month.");
		}

	}
}
