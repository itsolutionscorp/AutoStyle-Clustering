...
public class Line2 {
    ...
    Point p1, p2;
    /*
     * Recall that the length of the line segment connecting points (x1, y1) and 
     *(x2, y2) is the square root of (x2–x1)2 + (y2–y1)2, and the angle is the 
     *arctangent2 of (y2–y1, x2–x1).
     */
    void printLength() {
        double length;
        length = Math.sqrt ( Math.pow(p1.getX() - p2.getX(), 2) + 
        		Math.pow(p1.getY() - p2.getY(), 2) ) ;
        System.out.println ("Line length is " + length);
    }
    
    void printAngle() {
        double angleInDegrees = Math.atan2 ( p2.getY() - p1.getY() , p2.getX() - p1.getX() ) 
        		* 180.0 / Math.PI;
