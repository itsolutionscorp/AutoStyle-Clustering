...
public class Line2 {
    ...
    void printLength() {
        double length;
        length = Math.sqrt ( (this.p2.x - this.p1.x) * (this.p2.x - this.p1.x) + (this.p2.y - this.p1.y) * (this.p2.y - this.p1.y) ) ;
        System.out.println ("Line length is " + length);
    }
    
    void printAngle() {
        double angleInDegrees = Math.atan2 ( this.p2.y - this.p1.y , this.p2.x - this.p1.x ) * 180.0 / Math.PI;
