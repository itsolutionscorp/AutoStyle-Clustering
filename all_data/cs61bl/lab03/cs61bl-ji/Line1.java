public class Line1 {
    ...
    void printLength() {
        double length;
        length = Math.sqrt ((this.x2 - this.x1)*(this.x2 - this.x1) + (this.y2 - this.y1) * (this.y2 - this.y1)) ;
        System.out.println ("Line length is " + length);
    }
    
    void printAngle() {
        double angleInDegrees = Math.atan2 ( this.y2 - this.y1 , this.x2 - this.x1 ) * 180.0 / Math.PI;
