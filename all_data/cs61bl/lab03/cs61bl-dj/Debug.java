public class Debug {
	...
	public static void main(String[] args) {
		...
		check("1", ""); // should be true when s2 is empty
		check("abc23", "abc"); //should be false when s1 has two more characters
		check("ab3c", "abc"); //should be true if s1 has one more char in middle
		check("4ac", "ac"); //should be true if s1 has one more char in beginning
		check("aaa", "aaa"); //should be false if s1 and s2 are identical order and char
		check("", ""); //false
		check("", "1"); //true
