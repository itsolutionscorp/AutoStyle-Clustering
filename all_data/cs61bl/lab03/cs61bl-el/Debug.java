public class Debug {
	...
	public boolean contains1MoreThan(String s) {
		if (myString.length() == 0) {
			return false; //if myString is empty
		} else if (s.length() == 0) {
			return myString.length() == 1; //returns true if myString is only one character.
	...
	public static void main(String[] args) {
		...
		check("2abc", "abc");//true
		check("abcdefz", "abcdef");//true
		check("ccccc", "cccc"); //true
		check("ccc  ", "ccc"); //false
		check("ccccc", "cc"); //false
		check("cccc", "cccc"); //false
		check("", ""); //false
		check("2a", 2);
