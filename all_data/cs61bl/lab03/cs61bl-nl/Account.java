/**
 * This class represents a bank account whose current balance is a nonnegative
 * amount in US dollars.
 */
public class Account {
	
	public int balance() {
		return this.myBalance;
	}

	private int myBalance; 	// instance variables
	
	private Account parentAccount;

	public Account(int balance) {
		this.myBalance = balance;
		this.parentAccount = null;
	}
	
	public Account(int balance, Account parentAccount) {
		this.myBalance = balance;
		this.parentAccount = parentAccount;
	}

	public void deposit(int amount) {
		if (amount < 0) {
			System.out.println("Cannot deposit negative amount.");
		} else {
			this.myBalance += amount;
		}
	}

	/**
	 * Subtract the given amount from the account if possible. If the amount
	 * would leave a negative balance, print an error message and leave the
	 * balance unchanged.
	 */
	public boolean withdraw(int amount) {
		if (amount < 0) {
			System.out.println("Cannot withdraw negative amount.");
			return false;
		} else if (this.myBalance < amount) {
			if (this.parentAccount == null || this.parentAccount.balance() < amount - this.myBalance) { 
			System.out.println("Insufficient funds");
			return false;
			} else {
				this.parentAccount.withdraw(amount-this.myBalance);
				System.out.println("Overdraft Protection, parentAccount charged " + (amount-this.myBalance));
				this.myBalance = 0;
				return true;
			}
		} else {
			this.myBalance -= amount;
			return true;
		}
	}
	
	public void merge(Account anotherAcct) {
		this.myBalance += anotherAcct.balance();
		anotherAcct.withdraw(anotherAcct.balance());
	}

}
