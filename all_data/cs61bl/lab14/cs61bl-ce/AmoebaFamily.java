...
public class AmoebaFamily implements Iterable<AmoebaFamily.Amoeba>{
	...
	//Returns height of Amoeba object (non-buggy)
	public int height() {
		if (myRoot != null) {
	...
	public void replaceName(String currentName, String newName) {
		// Your goal is to make this as similar as possible to addChild
		if(myRoot!= null) {
			AmoebaFamily.replaceNameHelper(myRoot, currentName, newName);
			}
		}
	
	public static void replaceNameHelper(Amoeba currentAmoeba, String currentName, String newName) {
		if (currentAmoeba.myName == currentName) {
			currentAmoeba.myName = newName;
		} else {
			Iterator<Amoeba> iter = currentAmoeba.myChildren.iterator();
			while (iter.hasNext()) {
				AmoebaFamily.replaceNameHelper(iter.next(), currentName, newName);
			}
		}
	...
	public String longestName() {
		...
		// longestNameLength
		if (myRoot !=null) {
			return AmoebaFamily.longestNameHelper(myRoot);
		}
		return myRoot.myName;
	}
	
	public static String longestNameHelper(Amoeba currentAmoeba) {
		String maxLengthName = currentAmoeba.myName;
		Iterator<Amoeba> iter = currentAmoeba.myChildren.iterator();
		while (iter.hasNext()) {
			String childMaxLengthName = AmoebaFamily.longestNameHelper(iter.next());
	...
	//Returns number of amoebas in the family
	public int size() {
		if (myRoot != null) {
	...
	public class AmoebaIterator implements Iterator<Amoeba> {
		...
		public Amoeba next() {
			...
			if (current.myChildren != null) {
				int length = current.myChildren.size();
				for (int i=0; i<length; i++) {
					fringe.add(current.myChildren.get(i));
				}
			}
			return current;
//			if (myRoot == null) {
...
//				fringe.push(node.myChildren.get(0));
//			}
//			return node;
