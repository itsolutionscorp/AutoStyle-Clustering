...
public class ArrayHeap<T> {
	...
	public Node removeMin() {
		...
		//bubble down
		int index = 1;
		while (getNode(index).priority() > getNode(getLeftOf(index)).priority() || getNode(index).priority() > getNode(getRightOf(index)).priority()) {
			bubbleDown(index);
			index = contents.indexOf(first);
		}
		return null;
	}
	...
	public void changePriority(T item, double priority) {
		...
		for (int i = 0; i < contents.size(); i++) {
			if (contents.get(i) != null && item.equals(contents.get(i))) {
				index = i;
				break;
			}
		}
		...
		if (getNode(index).priority() > getNode(getLeftOf(index)).priority()
			...
			while (getNode(index).priority() > getNode(getLeftOf(index)).priority()
					|| getNode(index).priority() > getNode(getRightOf(index)).priority()) {
				bubbleDown(index);
			}
		}
