...
public class ArrayBST {
	...
	public boolean contains(int value) {
		int rootIndex = 1;
		while (getNode(rootIndex) != null) {
			if (getNode(rootIndex).myValue == value) {
				return true;
			} else if (getNode(rootIndex).myValue > value) {
				rootIndex = getLeftOf(rootIndex);
			} else {
				rootIndex = getRightOf(rootIndex);
			}
		}
		return false;
	}
