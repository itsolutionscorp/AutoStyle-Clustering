...
public class ArrayBST {
	...
	public boolean contains (int value) {
	      //TODO rewrite to run in time proportional to the height of the tree 
		  //Do something to iterate through positions that contain nodes
		  
		  
//		  Their code that runs in time proportional to the length of the array that contains the tree   			  
//		  
		int i = 1;
		while (getNode(i) != null) {
			if (getNode(i).myValue == value) {
				return true;
			}
			else if (getNode(i).myValue < value) {
				i = getRightOf(i);
			}
			else if (getNode(i).myValue > value) {
				i = getLeftOf(i);
