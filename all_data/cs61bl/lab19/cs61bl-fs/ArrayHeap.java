...
public class ArrayHeap<T> {
	...
	public Node removeMin() {
		...
		Node result = contents.remove(contents.size()-1);
		bubbleDown(1);
		return result;
	}
	...
	private void bubbleUp(int index) {
		if (index < 2) {
			return;
		} else if (contents.get(index).priority() < contents.get(getParentOf(index)).priority()) {
	...
	private void bubbleDown(int index) {
		if (index + 1 >= contents.size()) {
			...
			} else if (contents.get(getLeftOf(index)).priority() > contents.get(getRightOf(index)).priority()) {
