...
public class ArrayHeap<T> {
	...
	private void bubbleDown(int index) {
		if (contents.size() >= getLeftOf(index) + 1) {
			while (min(index, getLeftOf(index)) == index) {
				swap(index, getLeftOf(index));
			}
		}
		if (contents.size() >= getRightOf(index) + 1) {
			while (min(index, getRightOf(index)) == index) {
