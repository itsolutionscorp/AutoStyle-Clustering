...
public class ArrayBST {
	...
	public boolean contains (int value) {
		...
		else return containsHelper (1, value);
	}
	
	private boolean containsHelper (int index, int value) {
		if (index > contents.size()) return false;
		if (contents.get(index) == null) return false;
		else if (contents.get(index).myValue == value) return true;
		else return this.containsHelper(getLeftOf(index), value) || this.containsHelper(getRightOf(index), value);
	}
