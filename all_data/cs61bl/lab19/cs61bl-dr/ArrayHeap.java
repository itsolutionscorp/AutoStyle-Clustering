...
public class ArrayHeap<T> {
	 ...
	 * 		Item of a Node.
	 * @return
	 * 		The Node with this item.
	 */
	public Node findNode(T item){
		return findNodeHelper(item, 1);
	}
	
	private Node findNodeHelper(T item, int index){
		Node node = getNode(index);
		if(node != null){
			if(node.item().equals(item)) return node;
			else{
				Node left = findNodeHelper(item, getLeftOf(index));
				Node right = findNodeHelper(item, getRightOf(index));
	...
	private void bubbleUp(int index) {
		...
		if(parent != null && parent.priority() > getNode(index).priority()){
	...
	private void bubbleDown(int index) {	
		...
		if(next != null && next.priority() < getNode(index).priority()){
	...
	public static void main(String[] args) {
		...
		while(heap.peek() != null) System.out.println(heap.removeMin());
