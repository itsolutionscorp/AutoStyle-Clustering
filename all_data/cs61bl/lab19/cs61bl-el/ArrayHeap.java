...
public class ArrayHeap<T> {
	...
	boolean first = contents.add(null);
	int last = 0;
	...
	public void insert(T item, double priority) {
		...
		last++;
		setNode(last, toAdd); // insert at next available spot in the bottom row
		bubbleUp(last);
	}
	...
	private void bubbleUp(int index) {
		...
		if (getParentOf(index) > 0
				&& getParentOf(index) != min(index, getParentOf(index))) {
	...
	private void bubbleDown(int index) {
		...
		if (getNode(getLeftOf(index)) == null
			...
			return; // no need to bubble down
				...
				(min(getLeftOf(index), getRightOf(index))))) {
