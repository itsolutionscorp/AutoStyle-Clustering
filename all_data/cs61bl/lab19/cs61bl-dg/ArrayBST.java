...
public class ArrayBST {
    ...
    public boolean contains (int value) {
        return nodeContains(1, value);
    }

    public  boolean nodeContains (int index, int value) {
        if (index >= contents.size() || contents.get(index) == null) {
            return false;
        }
        if (value > contents.get(index).myValue) {
            return nodeContains(getRightOf(index), value);
        }
        if (value < contents.get(index).myValue) {
            return nodeContains(getLeftOf(index), value);
        }
        return true;
    }
