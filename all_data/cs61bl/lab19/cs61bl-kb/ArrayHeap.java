...
public class ArrayHeap<T> {
	...
	public void insert(T item, double priority) {
		...
		if (contents.isEmpty()) {
			contents.add(0, null);
			contents.add(1, toBeAdded);
			return;
		}
		contents.add(toBeAdded);
		if (min(contents.indexOf(toBeAdded), getParentOf(contents.indexOf(toBeAdded))) == 
			contents.indexOf(toBeAdded)) {
			bubbleUp(contents.indexOf(toBeAdded));
		}
		if (min(contents.indexOf(toBeAdded), getParentOf(contents.indexOf(toBeAdded))) ==
				getParentOf(contents.indexOf(toBeAdded))) {
			bubbleDown(contents.indexOf(toBeAdded));
		}
		return;
	}
...
//				(min(index, getRightOf(index)) == getRightOf(index))) {
			...
			if (min(index, getLeftOf(index)) == getLeftOf(index) && 
					min(index, getRightOf(index)) == getRightOf(index)) {
				int newIndex = contents.indexOf(min(getLeftOf(index), getRightOf(index)));
...
//				if (contents.size() < getLeftOf(index)) {
//					return;
//				}
//				else {
...
//				if (contents.size() < getRightOf(index)) {
//					return;
//				}
//				else {
...
//				}
	...
	public class Node {
		...
		@Override
		public String toString() {
			return getItem().toString() + ", " + priority();
		}
	}
