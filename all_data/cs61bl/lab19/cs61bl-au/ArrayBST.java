...
public class ArrayBST {
    ...
    public boolean contains (int value) {
        return this.containsHelper(1, value);
    }
    
    public boolean containsHelper (int index, int value) {
    	System.out.println(index + "===");
    	if (getNode(index) == null) return false;
    	if (getNode(index).value() == value) return true;
    	if (getNode(index).value() < value) return containsHelper(getRightOf(index), value);
    	else return containsHelper(getLeftOf(index), value);
    }
    ...
    public static void main(String[] args) {
		...
		System.out.println(bst.getNode(7).myValue);
