...
public class ArrayBST {
	...
	public boolean contains (int value) {
	    for (int i=1; i< contents.size(); i++) {
	        if (getNode(i) != null && getNode(i).myValue == value) {
	            return true;
	        }else if( getNode(getLeftOf(i)).myValue < value){
	        	i = 2*i; // look at the right child (minus one for loop)
	        }else if( getNode(getRightOf(i)).myValue > value){
	        	i = 2*i-1; // look at the left child (minus one for loop)
	        }
	    }
	    return false;
	}
