...
//		int size = contents.size() - 1;
	...
	public void changePriority(T item, double priority) {
		// TODO Complete this method!
		for (int k = 1; k < this.contents.size(); k++) {
			Node n = this.contents.get(k);
			if (n.myItem.equals(item)) {
				boolean up = false;
				if (priority < n.myPriority) {
					up = true;
				}
	...
	private void bubbleUp(int index) {
		...
		if (parent == 0
				|| getNode(parent).priority() < getNode(index).priority()) {
			return;
		}
	...
	private void bubbleDown(int index) {
		...
		if (getNode(min) == null
				|| getNode(index).priority() < getNode(min).priority()) {
			return;
		}
		this.swap(index, min);
		bubbleDown(min);
