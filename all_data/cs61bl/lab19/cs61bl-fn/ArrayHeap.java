...
public class ArrayHeap<T> {
	...
	public void changePriority(T item, double priority) {
		...
		for (int i = 1; i < contents.size(); i++) {
			...
			if (treeNode != null && treeNode.item().equals(item)) {
				...
				Node parentNode = getNode(getParentOf(i));
				if (parentNode != null
						&& parentNode.priority() > getNode(i).priority()) {
					bubbleUp(i);
				} else {
					bubbleDown(i);
				}
	...
	private int getLeftOf(int i) {
		int temp = 2 * i;
		return (temp >= contents.size()) ? 0 : temp;
	}
	...
	private void bubbleUp(int index) {
		Node parentNode = getNode(getParentOf(index));
		if (parentNode != null
				&& parentNode.priority() > getNode(index).priority()) {
	...
	private void bubbleDown(int index) {
		...
		if (!(leftNode == null && rightNode == null)) {
			...
			if (getNode(toSwap).priority() < getNode(index).priority()) {
				swap(index, toSwap);
				bubbleDown(toSwap);
			}
		}
