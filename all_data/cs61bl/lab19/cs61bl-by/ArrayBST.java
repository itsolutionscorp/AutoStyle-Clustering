...
public class ArrayBST {
    ...
    public boolean contains (int value) {
        int j = contents.size();
        return containsHelp(value, 1, j);
    }
    
    public boolean containsHelp(int value, int index, int size){
    	System.out.println("a run");
    	if (index >= size){
    		return false;
    	}else if (contents.get(index)== null){
    		return false;
    	} else if (contents.get(index).value() == value){
    		return true;
    	} else {
    		if (contents.get(index).value() < value){
    			return containsHelp(value, getRightOf(index), size);
    		} else {
    			return containsHelp(value, getLeftOf(index), size);
    		}
    		//return containsHelp(value, index*2, size) || containsHelp(value, index*2 +1, size);
    	}
    }
