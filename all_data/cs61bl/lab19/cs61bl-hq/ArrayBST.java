...
public class ArrayBST {
	...
	public boolean contains(int value) {
		if (contents.size() == 1) {
			return (value == getNode(1).value());
		} 
		return containsHelper(1, value);
	}
	
	private boolean containsHelper(int count, int value) {
		if (getNode(count) == null) {
			return false;
		} if (value == getNode(count).value()) {
			return true;
		} else if (value < getNode(count).value()) {
			return containsHelper(getLeftOf(count), value);
		} else {
			return containsHelper(getRightOf(count), value);
		}
	}
