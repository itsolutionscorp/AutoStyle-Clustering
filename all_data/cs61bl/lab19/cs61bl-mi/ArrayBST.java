...
public class ArrayBST {
	...
	public boolean containsHelper(int item, int pos) {
		if (pos >= contents.size()) {
			return false;
		}
		int currItem = 0;
		if (getNode(pos) != null) {
			currItem = getNode(pos).value();
		}
		else {
			return false;
		}
		...
		else if (item < currItem){
			return containsHelper(item, 2*pos);
		}
		else {
			return containsHelper(item, 2*pos + 1);
		}
	}
