...
public class ArrayHeap<T> {
	...
	public void insert(T item, double priority) {
		for (int index = 1;index< contents.size();index++){
			if (getNode(index).item().equals(item)&&getNode(index).priority()==priority){
				return;
			}
		}
	...
	private void bubbleUp(int index) {
		// TODO Complete this method!
		while(getParentOf(index)!=0&&min(getParentOf(index),index) == index){
	...
	private void bubbleDown(int index) {
		// TODO Complete this method!
	    if (getLeftOf(index) >= contents.size()) return;
	    if (getRightOf(index) >= contents.size()){
	    	if (min(index,getLeftOf(index)) == index) return;
	    	else swap(index,getLeftOf(index));
	    }
		int minChild = min(getLeftOf(index),getRightOf(index));
		if (min(index,minChild) == index) return;
		else{
			swap(index,minChild);
			bubbleDown(minChild);
		}
