...
public class ArrayBST {
	...
	public boolean contains (int value){
		// 1 represents the index of the node
		return containsHelper(getNode(1),1,value);
	}
	
	public boolean containsHelper(Node node,int index,int value){
		if (node == null){
			return false;
		}if (node.value() == value) return true;
		 if (node.value() > value) 
			 return containsHelper(getNode(getLeftOf(index)),getLeftOf(index),value);
		 return containsHelper(getNode(getRightOf(index)),getRightOf(index),value);
	}
