...
//	}
	...
	public boolean contains (int value) {
		...
		//System.out.println(getNode(getLeftOf(i)).myValue);
		if (getNode(k) != null && getNode(k).value() == value) {
			return true;
		}
		else if(getNode(getLeftOf(k)) != null && getNode(getLeftOf(k)).value() >= value) {
			k = getLeftOf(k);
			return contains(value);
		}
		else if(getNode(getLeftOf(k)) != null && getNode(getRightOf(k)).value() >= value) {
			k = getRightOf(k);
			return contains(value);
		}
		return false;
