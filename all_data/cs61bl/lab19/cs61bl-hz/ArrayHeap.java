...
public class ArrayHeap<T> {
	...
	private void bubbleUp(int index) {
		// TODO Complete this method!
		int i = index;
		while (i > 1 && getNode(i).priority() < getNode(getParentOf(i)).priority() ) {
	...
	private void bubbleDown(int index) {
		...
		if (getNode(left) == null && getNode(right) == null) {
			return;
		} else if (getNode(parent).priority() < getNode(left).priority() && getNode(parent).priority() < getNode(right).priority()) {
			return;
		} else {
			swap(parent, min);
			bubbleDown(min);
