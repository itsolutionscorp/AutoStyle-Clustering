...
public class ArrayBST {
	...
	public boolean contains (int value) {
		int currentIndex = 1;
		int current = this.contents.get(1).value();
		if (current == value) {
			return true;
		}else if (current < value) {
			return this.containsHelper(value, getRightOf(currentIndex));
		}else 
			return this.containsHelper(value, getLeftOf(currentIndex));
	}

	public boolean containsHelper(int value, int current) {
		if (contents.size() < current) {
			return false;
		}if (contents.get(current) == null) {
			return false;
		}
		int valueCheck = contents.get(current).value();
		if (value == valueCheck) {
			return true;
		}if (value < valueCheck) {
			return containsHelper(value, getLeftOf(current));
		}else 
			return containsHelper(value, getRightOf(current));
	}
