...
public class ArrayHeap<T> {
	...
	public Node removeMin() {
		// TODO Complete this method!
		Node returnNode = contents.get(1);
	...
	public void changePriority(T item, double priority) {
		...
		for ( int i = 0 ; i<contents.size() ; i++){
			if(getNode(i).myItem.equals(item)){
				...
				if(priority < getNode(getParentOf(i)).myPriority){
					bubbleUp(i);
				}
				else if (priority > getNode(getLeftOf(i)).myPriority || priority > getNode(getRightOf(i)).myPriority){
					bubbleDown(i);
				}
				break;
			}
	...
	private void bubbleUp(int index) {
		// TODO Complete this method!
		if (index > 1){
			if(getNode(index).myPriority < getNode(getParentOf(index)).priority()){
	...
	private void bubbleDown(int index) {
		...
		if(getNode(getLeftOf(index)) != null && getNode(getRightOf(index)) != null){
			if(getNode(index).priority() > getNode(getLeftOf(index)).priority()
				...
				minChild = minPriority(getLeftOf(index), getRightOf(index));
				swap(index, minChild);
				bubbleDown(minChild);
			}
		}
	...
	private int minPriority(int index1, int index2){
	...
	public static void main(String[] args) {
		...
		for( int i = 0 ; i < 10 ; i ++){
			System.out.print(heap.removeMin().priority() + "   ");
		}
