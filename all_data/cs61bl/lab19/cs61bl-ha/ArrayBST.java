...
public class ArrayBST {
	...
	private boolean containsHelper(int index, int value) {
		if (getNode(index) != null){
			if (getNode(index).value() == value){
				return true;
			} else if (value > getNode(index).value()){
				return containsHelper(getRightOf(index), value);
			} else {
				return containsHelper(getLeftOf(index), value);
			}
		} else {
			return false;
		}
	...
	public static void main(String[] args) {
		...
		System.out.println(bst.contains(9)); //should be true
		System.out.println(bst.contains(12)); //should be false
