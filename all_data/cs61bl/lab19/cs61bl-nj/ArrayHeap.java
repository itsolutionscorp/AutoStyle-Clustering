...
public class ArrayHeap<T> {
	...
	public Node removeMin() {
		...
		setNode(1,contents.remove(contents.size()-2));
		bubbleDown(1);
		return result;
	}
	...
	public void changePriority(T item, double priority) {
		...
		for (int i = 1; i<contents.size()-1;i++) {
			if (contents.get(i)!=null) {
				...
				if (a.myItem.equals(item)) {
					double oldPriority = a.myPriority;
	...
	private void bubbleUp(int index) {
		...
		while (getNode(parent)!=null&&getNode(parent).priority()>getNode(index).priority()){
			swap(parent,index);
			index = parent;
	...
	private void bubbleDown(int index) {
		...
		while ((getNode(left)!=null&&getNode(left).priority()<getNode(index).priority())||(getNode(right)!=null&&getNode(right).priority()<getNode(index).priority())) {
