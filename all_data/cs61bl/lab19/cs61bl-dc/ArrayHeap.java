...
public class ArrayHeap<T> {
	...
	private void changePriority(int index, T item, double priority){
		...
		if(curr == null){
			return;
		} else if(curr.myItem.equals(item)){
			curr.myPriority = priority;
			if(priority < getNode(getParentOf(index)).priority())
				bubbleUp(index);
			else
				bubbleDown(index);
		} else {
			changePriority(getLeftOf(index), item, priority);
	...
	private void bubbleUp(int index) {
		...
		if(parent != null && parent.myPriority > current.myPriority){
	...
	private void bubbleDown(int index) {
		...
		if(getNode(getLeftOf(index)) != null || getNode(getRightOf(index)) != null){
			...
			Node child = getNode(min);
			if(current.myPriority > child.myPriority){
				swap(index, min);
				bubbleDown(min);
			}
		}
