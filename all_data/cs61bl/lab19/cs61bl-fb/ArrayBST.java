...
public class ArrayBST {
	...
	public boolean contains (int value) {
		return containsHelper(value, intToLookAt);
	}
	public boolean containsHelper(int value, int lookIndex){
		if(lookIndex + 1 >= contents.size()){
			return false;
		}
		
		
		if (getNode(lookIndex).value() == value) {
			return true;
		}
		else if(getNode(lookIndex).value() > value){
			return containsHelper(value, getLeftOf(lookIndex));
		
		}else{
			return containsHelper(value, getRightOf(lookIndex));
