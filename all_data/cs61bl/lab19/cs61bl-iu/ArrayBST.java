...
public class ArrayBST {
	...
	public boolean contains (int value) {
	    int nextIndex = 1;
		while (nextIndex < contents.size() && getNode(nextIndex) != null) {
			Node treeNode = getNode(nextIndex);
			if (treeNode.myValue == value) {
				return true;
	        } else if (treeNode.myValue > value){
	        	nextIndex = getLeftOf(nextIndex);
	        } else{
	        	nextIndex = getRightOf(nextIndex);
	        }
	    }
	    return false;
	}
