...
public class Sequence implements Iterable {
	...
	public void delete(int deletePos) {
		DListNode x = this.myHead;
		if(deletePos == 0){
			this.myHead = x.myNext;
			myHead.myPrev = x.myPrev;
			mySize--;
		}
		else {
			for (int i = 0; i < this.mySize; i++) {
				if (i == deletePos) {
					DListNode temp = x;
					x.myPrev.myNext = x.myNext;
					x.myNext.myPrev = temp.myPrev;
				} else
					x = x.myNext;
			}
			mySize--;
	...
	private class SequenceIterator implements Iterator {
		...
		public void remove(){
			DListNode recent = me.myPrev;
			recent.myPrev.myNext = me;
			me.myPrev = recent.myPrev;
			mySize--;
		}
