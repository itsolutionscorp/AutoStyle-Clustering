...
public class Sequence implements Iterable {
    ...
    public void delete(int deletePos) {
        if (deletePos < 0 || deletePos >=mySize) {
        	...
        	if (mySize == 1) {
        		...
        		DListNode curNode = myHead;
            	for (int i=0; i<deletePos; i++ ) {
            		curNode = curNode.myNext;
            	}
            	curNode.myPrev.myNext = curNode.myNext;
            	curNode.myNext.myPrev = curNode.myPrev;
            	if (deletePos == 0) {
            		myHead = curNode.myNext;
            	}
            	mySize--;
        	}
        }
    ...
    private class SequenceIterator implements Iterator {
        ...
        public void remove(){
            if (curNode.myItem == null) { 
            	//sentinel node; do nothing
            } else {
            	int pos = 0;
            	for (DListNode d=myHead; d!=curNode; d=d.myNext) {
            		pos++;
            	}
            	delete(pos);
            }
