...
public class Sequence implements Iterable {
	...
	public void delete(int deletePos) {
		...
		DListNode newPrev = myHead;
		DListNode newNext = myHead;
		if(deletePos == 0){
			myHead = myHead.myNext;
			temp.myPrev.myNext = myHead;
		}else{
			for(int i = 0; i < deletePos; i++){
				temp = temp.myNext;
				newPrev = newPrev.myNext;
				newNext = newNext.myNext;
			}
			newPrev.myPrev.myNext = temp.myNext;
			newNext.myNext.myPrev = temp.myPrev;
		}
		mySize--;
	}
