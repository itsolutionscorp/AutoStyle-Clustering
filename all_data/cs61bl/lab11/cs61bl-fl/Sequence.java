...
/** 
 * OVERVIEW:
 * This Sequence class implements a circular doubly linked list in which
...
public class Sequence implements Iterable {
    ...
    /* Substituted with version below.
    ...
    */
    ...
    public void delete (int deletePos) {
    	...
    	if (deletePos < 0 || deletePos >= mySize) {
    		//Also handles case of deleting from an empty list.
    		throw new IllegalArgumentException("Position must be within the list bounds.");
    	}
    	DListNode toDelete = myHead;
    	for (int position = 0; position != deletePos; position++) {
    		toDelete = toDelete.myNext;
    	}
    	toDelete.myPrev.myNext = toDelete.myNext;
    	toDelete.myNext.myPrev = toDelete.myPrev;
    	mySize --;
    	if (deletePos == 0) {
    		myHead = toDelete.myNext;
    		if (mySize == 0) {
    			myHead = null;
    		}
    	}
    ...
    private class SequenceIterator implements Iterator {
        ...
        public boolean hasNext() {
            ...
            return !hasStarted || current != start;
        }
        ...
        public void remove(){
        	...
        	if (!hasStarted) {
        		throw new RuntimeException(
        				"Can't delete value if no new next() values have been returned.");
        	}
        	hasStarted = false;
        	index --;
        	delete(index);
