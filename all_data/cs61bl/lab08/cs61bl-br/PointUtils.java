...
public class PointUtils {
	...
	public static Point highestPoint(List<Point> points) {
		...
		Point theHighest = pointIterator.next();
		
		while (pointIterator.hasNext()) {
			Point thisPoint = pointIterator.next();
			if (thisPoint.getY() > theHighest.getY()) {
				theHighest = thisPoint;
			}
		}
		return theHighest;
	}
	...
	public static Point centroid(List<Point> points) {
		...
		while (pointIterator.hasNext()) {
			Point thisPoint = pointIterator.next();
			sumX = (int) (sumX + thisPoint.getX());
			sumY = (int) (sumY + thisPoint.getY());
		}
		double avgX = (sumX / points.size());
		double avgY = (sumY / points.size());
		Point theCentroid = new Point();
		theCentroid.setLocation(avgX, avgY);
		return theCentroid;
	}
