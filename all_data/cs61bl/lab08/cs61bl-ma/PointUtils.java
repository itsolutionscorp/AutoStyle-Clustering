...
public class PointUtils {
	...
	public static Point highestPoint(List<Point> points) {
		...
		Point largest = pointIterator.next();
		while (pointIterator.hasNext()) {
			Point current = pointIterator.next(); 
			if (current.y > largest.y) {
				largest = current; 
			}
		}
		return largest;
	...
	public static Point centroid(List<Point> points) {
		...
		//doesn't return accurate centroid due to Point class being unable to take doubles...
		Point centroid = new Point();
		centroid.setLocation(xsum/points.size(), ysum/points.size()); 
		return centroid;
	}
