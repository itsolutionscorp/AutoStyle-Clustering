...
public class PointUtils {
	...
	public static Point highestPoint(List<Point> points) {
		...
		Point max_point = pointIterator.next();
		int max_value = max_point.y;
		
		// TODO use the iterator to complete this method!
		while(pointIterator.hasNext()){
			Point currentPoint = pointIterator.next();
			if(currentPoint.y > max_value){
				max_point = currentPoint;
				max_value = currentPoint.y;
			}
		}
		return max_point;
	}
	...
	public static Point centroid(List<Point> points) {
		...
		while(pointIterator.hasNext()){
			Point currentPoint = pointIterator.next();
			x_total += currentPoint.x;
			y_total +=currentPoint.y;
			counter++;
		}
		return new Point(x_total/counter, y_total/counter);
