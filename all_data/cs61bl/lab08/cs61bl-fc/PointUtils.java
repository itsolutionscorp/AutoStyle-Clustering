...
public class PointUtils {
	...
	public static Point highestPoint(List<Point> points) {
		Point p = new Point();
		Point largestSoFar = new Point();
//		largestSoFar.x=1;
		largestSoFar.y=(int) Double.NEGATIVE_INFINITY;
		/* This is the equivalent of the initialization step mentioned in lab. */
		Iterator<Point> pointIterator = points.iterator();
		
		while (pointIterator.hasNext()) {
			p = pointIterator.next();
			if (p.getY()>largestSoFar.getY()) {				
				largestSoFar.x = (int) p.getX();
				largestSoFar.y = (int) p.getY();
			}
		}
		return largestSoFar;
	}
	...
	public static Point centroid(List<Point> points) {
		...
		while (pointIterator.hasNext()) {
			p = pointIterator.next();
			returnValue.x += (int) (p.getX());
			returnValue.y += (int) (p.getY());
		}
		returnValue.x = returnValue.x/points.size();
		returnValue.y = returnValue.y/points.size();
		return returnValue;
	}
