...
public class PointUtils {
	...
	public static Point highestPoint(List<Point> points) {
		...
		int highestY = 0;
		Point higestYpoint = null;
		Point tmpPoint;
		// TODO use the iterator to complete this method!
		while (pointIterator.hasNext()){
			tmpPoint =  pointIterator.next();
			int value = (int) tmpPoint.getY();
			if(highestY < value){
				highestY = value;
				higestYpoint = tmpPoint;
			}
	...
	public static Point centroid(List<Point> points) {
		...
		int count = 0;
		while (pointIterator.hasNext()){
		    Point tmpPoint = pointIterator.next();
			sumX +=  tmpPoint.getX();
			sumY +=  tmpPoint.getY();
			count += 1;
		}
		
		Point resultPoint = new Point(sumX/count,sumY/count);
		return resultPoint;
	}
