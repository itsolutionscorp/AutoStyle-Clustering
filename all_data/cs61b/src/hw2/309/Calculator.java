...
public class Calculator {
    ...
    public int add(int x, int y) {
        ...
        int result = 0;
        while (mask != 0) {
            xbit = x & mask; // x-bit: 0 if mask = 0, 1 if mask = 1
            ybit = y & mask;
            if (xbit == 0 && ybit == 0 && abit == 0) {
                abit = 0;

            } else if (xbit != 0 && ybit != 0 && abit != 0) {
                result = result | mask;
                abit = 1;
            } else if ((xbit != 0 && ybit == 0 && abit == 0)
                    || (xbit == 0 && ybit != 0 && abit == 0)
                    || (xbit == 0 && ybit == 0 && abit != 0)) {
                result = result | mask;
                abit = 0;
            } else // two bits are nonzero
            {
                abit = 1;
            }
            mask = mask << 1;
    ...
    public int multiply(int x, int y) {
        // YOUR CODE HERE
        int mask = 0x80000000; // largest int
        int result = 0;
        int ybit;
        while (mask != 0) {
            ybit = y & mask;
    ...
    public void saveEquation(String equation, int result) {
        // YOUR CODE HERE
        EquationList equ = new EquationList(equation, result, null);
        if (eqList == null) {
            eqList = equ;
        } else {
            equ.next = eqList;
    ...
    public void printAllHistory() {
        // YOUR CODE HERE
        EquationList pEqu = eqList;
        while (pEqu != null) {
            System.out.println(pEqu.equation + " = " + pEqu.result);
            pEqu = pEqu.next;
        }
    }
