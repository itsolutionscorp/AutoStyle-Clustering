...
public class SortedComparableList {
    ...
    public void insert(Comparable c) {
      ...
      if (c != null) {
        if (c.compareTo(head) < 0) {
          ...
          while (tracker.tail != null && c.compareTo(tracker.tail.head) > 0) {
            tracker = tracker.tail;
          }
          tracker.tail = new SortedComparableList(c, tracker);
        } 
      } */
    }
    ...
    public void extend(SortedComparableList that) {
      SortedComparableList tracker = that;
      while (tracker != null) {
        insert(tracker.head);
        tracker = tracker.tail;
      }
    ...
    public static SortedComparableList sublist(SortedComparableList L, int start, int len) {
        if (len == 0) {
          ...
          SortedComparableList tracker = subTail(L, start);
          return new SortedComparableList(tracker.head, sublist(tracker.tail, 0, len - 1));
        }
    }
    ...
    public void squish() {
        if (tail != null) {
          ...
          while (next != null) {
            if (tracker.head.equals(next.head)) {
              tracker.tail = next.tail;
              next = tracker.tail;
            } else {
              tracker = tracker.tail;
              next = next.tail;
