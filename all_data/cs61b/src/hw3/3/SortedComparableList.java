...
public class SortedComparableList {
    ...
    public static SortedComparableList sublist(SortedComparableList L, int start, int len) {
        SortedComparableList newList = SortedComparableList.subTail(L, start);
        if (newList == null || len == 0) {
            return null;
        } else {
            int trueLen = 1;
            SortedComparableList temp = new SortedComparableList(newList.head, null);
            newList = newList.tail;
            while (trueLen != len) {
                temp.tail = new SortedComparableList(newList.head, null);
                newList = newList.tail;
            }
            return temp;
        }
    ...
    public void squish() {
        ...
        while (current != null) {
            if (current.head.compareTo(previous.head) == 0) {
