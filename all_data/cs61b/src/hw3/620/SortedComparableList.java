...
public class SortedComparableList {
    ...
    public void insert(Comparable c) {
        ...
        if (c.compareTo(temp.head) < 0) temp = new SortedComparableList(c, temp);
        if (temp.tail == null) {
            temp.tail = new SortedComparableList(c, null);
        } else {
            while (temp.tail != null) {
                if (c.compareTo(temp.tail.head) > 0) temp = temp.tail;
                else break;
            }
    ...
    public static SortedComparableList sublist(SortedComparableList L, int start, int len) {
        ...
        while (i < len) { 
            if (scl == null) {
                scl = new SortedComparableList(temp.head, null);
            } else {
                scl.insert(temp.head);
            }
            temp = temp.tail;
            i += 1;
        }
    ...
    public void squish() {
        ...
        while (temp.tail != null) {
            ...
            Comparable curr = temp.head;
            int i = 0;
            while (temp2.head == curr) {
                i += 1;
                temp2 = temp2.tail;
            }
