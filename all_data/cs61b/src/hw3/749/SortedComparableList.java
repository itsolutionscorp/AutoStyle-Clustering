...
public class SortedComparableList {
    ...
    public void insert(Comparable c) {
      ...
      SortedComparableList previous = null;

      while((pointer != null) && (pointer.head.compareTo(c) < 0)){
        previous = pointer;
        pointer = pointer.tail;
      }

      if(previous != null){
        previous.tail = new SortedComparableList(c, pointer);
      } else {
        this.tail = this;
        this.head = c;
      }
    ...
    public static SortedComparableList sublist(SortedComparableList L, int start, int len) {
      ...
      SortedComparableList endResult = result;

      while(endResult != null && len > 1){
        endResult = endResult.tail;
        len = len - 1;
      }
    ...
    public void squish() {
      ...
      SortedComparableList prevNode = this;
      SortedComparableList pointer = this.tail;
      while(pointer != null) {
        if(pointer.head.compareTo(prev) != 0) {
         prev = pointer.head;
         prevNode = prevNode.tail;
         pointer = pointer.tail;
        } else {
          pointer = pointer.tail;
          prevNode.tail = pointer;
        }
      }
    }
