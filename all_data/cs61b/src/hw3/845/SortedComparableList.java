...
public class SortedComparableList {
    ...
    public void insert(Comparable c) {
    	//test for a null
    	if(c!=null){
    		if(c.compareTo(head)<0){ //c is less than everything else and gets placed at the head
    			tail = new SortedComparableList(head,tail);
    			head = c;
    		}
    		else{//go through list to find spot
    			SortedComparableList temp = this;
    			while(temp.tail!= null && c.compareTo(temp.head)>0){//.compareTo(value) < 
    				temp = temp.tail;
    			}
    ...
    public void extend(SortedComparableList that) {
        SortedComparableList toInsert = that;
        while(toInsert!=null){
