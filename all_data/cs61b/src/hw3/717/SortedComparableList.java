...
public class SortedComparableList {
    ...
    public void insert(Comparable c) {
        // REPLACE THIS LINE WITH YOUR SOLUTION
        if (c==null) return;
        SortedComparableList node = this;
        if (c.compareTo(node.head) < 0){ 
        	tail = new SortedComparableList(head, tail);
        	head = c;
        	return;
        }
        while (node.tail!=null && c.compareTo(node.tail.head) > 0) node = node.tail;
        node.tail = new SortedComparableList(c, node.tail);
    }
    ...
    public static void expungeTail(SortedComparableList L, int len) {
        // REPLACE THIS LINE WITH YOUR SOLUTION
        SortedComparableList node = L;
        if (node==null) return;
        int i = 0;
        while (node.tail!=null && i < len){ 	
        	node = node.tail;
        	i++;
        }
        node.tail = null;
    ...
    public void squish() {
        ...
        while (node.tail!=null){
            while (node.tail !=null && node.tail.head.equals(node.head)) node.tail = node.tail.tail;
            node = node.tail;
        }
    }
    ...
    public void twin() {
        // REPLACE THIS LINE WITH YOUR SOLUTION
        SortedComparableList node = this;
        SortedComparableList tempTail;
        while (node!=null){
            tempTail = node.tail;
            node.tail = new SortedComparableList(node.head, tempTail);
            node = node.tail.tail;
        }
    }
