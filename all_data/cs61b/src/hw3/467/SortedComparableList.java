...
public class SortedComparableList {
    ...
    public void insert(Comparable c) {
        ...
        if (c != null) {
            ...
            // would this work???
            //     this = SortedComparableList(c, this);
            // }
                //tail = this;
                ...
                while (temp != null && c.compareTo(temp.head) > 0) {
                    temp = temp.tail;
                }
                temp = new SortedComparableList(c, temp);
            }
        }
    }
    ...
    public static SortedComparableList sublist(SortedComparableList L, int start, int len) {
        ...
        SortedComparableList mew = subTail(L, start);
        if (len == 1) {
            mew = new SortedComparableList(mew.head, null);
        }
        else {
            SortedComparableList temp = new SortedComparableList(mew.head, mew.tail);
            while (len > 1) {
                temp = temp.tail;
                len -= 1;
            }
            //will this alter the original elements? 
            //Or did I create a copy when I made mew?
            temp.tail = null;
            mew = new SortedComparableList(mew.head, mew.tail);
        }
        return mew;
    }
    ...
    public void twin() {
        // REPLACE THIS LINE WITH YOUR SOLUTION
        if (head != null) {
            SortedComparableList t = this;
            while (t != null) {
                t.tail = new SortedComparableList(t.head, t.tail);               
                t = t.tail.tail;
            }
        }
    }
