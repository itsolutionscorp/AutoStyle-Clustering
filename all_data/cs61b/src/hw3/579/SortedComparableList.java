...
public class SortedComparableList {
        ...
        if (head==null){
        	...
        	if (head.compareTo(c)<=0){
        		if (tail==null){
        			...
        			while(tmp.tail!=null && tmp.head.compareTo(c)<=0){
        				tmp=tmp.tail;
        			}
        			tmp.tail=new SortedComparableList(c,tmp.tail);
        		}
    	...
    	if(head==null){
    		...
    		this.tail=that.tail;
	     	...
	     	this.tail=that;
	       	...
	       	SortedComparableList tmp=that;
	       	while(tmp!=null){
	       		this.insert(tmp.head);
	       		tmp=tmp.tail;
	       	}
	    }
    }
    ...
    public static SortedComparableList subTail(SortedComparableList L, int start) {
        SortedComparableList tmp=new SortedComparableList(L.head,L.tail);
        ...
        SortedComparableList tmp2=new SortedComparableList();
        while(tmp!=null){
        	tmp2.insert(tmp.head);
        	tmp=tmp.tail;
        }
        return tmp2;
    }
     ...
     **/
    public void squish() {
        Comparable prev=this.head;
        SortedComparableList prevlist=this;
        SortedComparableList tmp=this.tail;
        while(tmp!=null){
        	if (tmp.head.equals(prev)){
        		prevlist.tail=tmp.tail;
        		tmp=tmp.tail;
        	}
        	else{
        		prev=tmp.head;
    ...
    public void twin() {
        SortedComparableList tmp=this;
        while (tmp!=null){
        	tmp.tail=new SortedComparableList(tmp.head,tmp.tail);
        	tmp=tmp.tail.tail;
        }
    }
