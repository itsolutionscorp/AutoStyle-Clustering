...
public class SortedComparableList {
    ...
    public Comparable get(int i) {
        if (head == null || i == 0) {
            ...
            SortedComparableList curr = this;  //always make a copy
            while (curr.tail != null && i != 0) { //go through until we get desired index
                //it's curr.tail and not curr b/c it's possible i is out of range?
                curr = curr.tail;
                i--;  //decrementing i!!! not increasing!!
            }
            return curr.head;  //when i = 0
        }
    }

    /** Adds every item in THAT to this list. */
    public void extend(SortedComparableList that) {
        SortedComparableList curr = that;  //make copy of what we are inserting
        while (curr != null) {
            insert(curr.head); //insert each element separately.
            curr = curr.tail;
        }
    }
    ...
    public static SortedComparableList subTail(SortedComparableList L, int start) {
        if (L == null || start == 0) {
            ...
            return subTail(L.tail, start-1); //decreasing both to hit base case.
        } //but we are not saying L = L.tail, so not modifying original
    }
    ...
    public static SortedComparableList sublist(SortedComparableList L, int start, int len) {
        ...
        else if (start > 0) { //keep going through until we have the whole subtail. len stays same
            return sublist(L.tail, start - 1, len);
        }
        return new SortedComparableList(L.head, sublist(L.tail, 0, len - 1)); //when start = 0
    }
    ...
    public void squish() {
        if (tail != null) {
            SortedComparableList prev = this; //copy everything
            SortedComparableList curr = tail; //copy tail
            while (curr != null) {
                if (prev.head.equals(curr.head)) { //if we have duplicate
                    prev.tail = curr.tail; //we cut off the head of curr since we already have in prev
                    curr = prev.tail;
                }
                else {
                    prev = prev.tail;
    ...
    public void twin() {
        ...
        while (prev != null) { //why not prev.tail != null? b/c we need to get every single item
            prev.tail = new SortedComparableList(prev.head, prev.tail);
            prev = prev.tail.tail; //skipping the 2 duplicates so that the next head is to be duplicated
        }
    }
