...
public class SortedComparableList {
    ...
    public void insert(Comparable c) {
      if (c.compareTo(head) <= 0){ // This means that c should come before or equal to the current head
        tail = new SortedComparableList(head, tail); // Push back head one spot in the list
        head = c; // Put c where head used to be
      } else if (tail == null){ // We're inserting into a list of length 1 and c comes after the head
        tail = new SortedComparableList(c, null); // Put c at the end of the list
      } else { // We're not at the end and c comes after where we're at
        tail.insert(c); // Call insert on the rest of the list
      }
    }
    ...
    public static SortedComparableList subTail(SortedComparableList L, int start) {
      if (start != 0){
        ...
        SortedComparableList outList = new SortedComparableList(L.head, null);
        outList.extend(L.tail);
        return outList;
      }
    }
    ...
    public static SortedComparableList sublist(SortedComparableList L, int start, int len) {
      SortedComparableList tailList = SortedComparableList.subTail(L, start);
      SortedComparableList.expungeTail(tailList, len - 1);
      return tailList;
    }
    ...
    public static void expungeTail(SortedComparableList L, int len) {
      if (len == 0){
        ...
        SortedComparableList.expungeTail(L.tail, len - 1);
      }
    }
    ...
    public void squish() {
      ...
      while (next != null){
        currHead = curr.head;
        nextHead = next.head;
        if (currHead.compareTo(nextHead) == 0){
          curr.tail = next.tail;
        } else {
          curr = next;
        }
