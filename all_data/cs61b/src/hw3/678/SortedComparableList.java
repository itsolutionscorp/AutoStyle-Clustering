...
public class SortedComparableList {
    ...
    public Comparable get(int i) {
        ...
        // }
        // SortedComparableList curr = this;
        // for (int index = 0, index < i; index += 1){
        //     curr = curr.tail;
        // }
        // return curr.head;
        if (i == 0 || head == null){
    ...
    public static SortedComparableList subTail(SortedComparableList L, int start) {
        ...
        SortedComparableList return_l = L;
        for (int i = 0; i < start; i+=1){
            return_l = return_l.tail;
        }
        return return_l;
    }
    ...
    public static SortedComparableList sublist(SortedComparableList L, int start, int len) {
      if (len == 0){
        ...
        SortedComparableList sub = SortedComparableList.subTail(L, start);
        SortedComparableList sub2 = sub;
        while (len > 1 && sub2 != null){
          sub2 = sub2.tail;
          len -=1;
        }
    ...
    public static void expungeTail(SortedComparableList L, int len) {
      ...
      if (L == null){
        ...
        expungeTail(L.tail, len - 1); //will this compile?**
      }
    }
    ...
    public void squish() {
        SortedComparableList curr = this;
        Comparable last = this.head;
        SortedComparableList replacement = new SortedComparableList(head, null);
        curr = curr.tail;
        while (curr != null){
            if (curr.head.compareTo(last) > 0){
              last = curr.head;
              replacement.insert(last);
            }
            curr = curr.tail;
        }
    ...
    public void twin() {
      SortedComparableList curr = this;
      SortedComparableList replacement = new SortedComparableList(head,  new SortedComparableList(head, null));
      curr = curr.tail;
      while (curr != null){
            replacement.insert(curr.head);
            replacement.insert(curr.head);
            curr = curr.tail;
        }
      head = replacement.head;
