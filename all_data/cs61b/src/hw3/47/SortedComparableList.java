...
public class SortedComparableList {
    ...
    public void insert(Comparable c) {
      if (c != null) {
        ...
        if (head.compareTo(c) == 1) {
          tail = new SortedComparableList(head, tail);
          head = c; 
          return;
        }
        while (current.tail != null && current.tail.head.compareTo(c) != 1) {
    ...
    public void extend(SortedComparableList that) {
      SortedComparableList current_that = that; 
      while (current_that != null) {
        insert(current_that.head);
        current_that = current_that.tail;
      }
    ...
    public void squish() { 
      if (tail != null) {
        ...
        while (current != null) {
          if (prev.head.compareTo(current.head) == 0) {
