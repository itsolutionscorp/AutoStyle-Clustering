...
public class SortedComparableList {
    ...
    public void insert(Comparable c) {
		...
		SortedComparableList step = this;
		while (step.tail != null && c.compareTo(step.tail.head) > 0)
			step = step.tail;
		step.tail = new SortedComparableList(c, step.tail);
    }
    ...
    public void extend(SortedComparableList that) {
        // REPLACE THIS LINE WITH YOUR SOLUTION
		SortedComparableList step = that;
		while (step != null) {
			insert(step.head);
			step = step.tail;
		}
    ...
    public void squish() {
		...
		if (tail == null)
			return;
		SortedComparableList last = this;
		SortedComparableList step = tail;
		while (step != null) {
			if (last.head.equals(step.head)) {
				last.tail = step.tail;
				step = step.tail;
			}
			else {
    ...
    public void twin() {
		...
		SortedComparableList last = this;
		while (last != null) {
			last.tail = new SortedComparableList(last.head, last.tail);
			last = last.tail.tail;
		}
    }
