...
public class SortedComparableList {
    ...
    public void insert(Comparable c) {
    	if (c !=null){
    		...
    		// If c is smaller than the head, so inserting to the front
    		if (c.compareTo(head) < 0) {
    			...
    			// While c is greater than s
    			while (s.tail != null 
    					&& c.compareTo(s.tail.head) > 0){
    				s = s.tail;
    			}
    ...
    public void extend(SortedComparableList that) {
		SortedComparableList toAdd = that;
		while (toAdd != null){
			insert(toAdd.head);
			toAdd = toAdd.tail;
		}
    ...
    public static SortedComparableList subTail(SortedComparableList L, int start) {
    	// Can't use a pointer in static method, use recursion
		 if (L == null || start == 0){
			 return L;
		 }
