...
public class SortedComparableList {
	...
	SortedComparableList ref = this;
	while (ref.tail != null) {
            if (c.compareTo(ref.tail.head) <= 0) {
	        ref.tail = new SortedComparableList(c, ref.tail);
		return;
	    }
	    ref = ref.tail;
	}
	ref.tail = new SortedComparableList(c, null);
    }
	...
	SortedComparableList ref = subTail(newList, start);
	SortedComparableList cutTail = ref;
	while (len != 1) {
	    cutTail = cutTail.tail;
	    len--;
	} 
	cutTail.tail = null;
	return ref;       
	...
	SortedComparableList ref = this;
	while (ref.tail != null) {
	    if (ref.head.compareTo(ref.tail.head) == 0) {
		ref.tail = ref.tail.tail;
		continue;
	    }
	    ref = ref.tail;
	}        
	...
	SortedComparableList ref = this;
	while (ref.tail != null) {
	    ref.tail = new SortedComparableList(ref.head, ref.tail);
	    ref = ref.tail.tail;
	}    
	ref.tail = new SortedComparableList(ref.head, null);    
    }
