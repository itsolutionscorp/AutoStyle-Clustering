...
public class SortedComparableList {
    ...
    public void insert(Comparable c) {
        ...
        if(c != null){
            ...
            else{ // If c is greater than head
                // create a moving pointer
                SortedComparableList ptr = this;
                // Must check if c is greater than the tail's head
                while(ptr.tail != null){
                    // If still greater, move ptr 
                    if(c.compareTo(ptr.tail.head) > 0){
                        ptr = ptr.tail;
                    }
                    // now c is less than a head
    ...
    public static SortedComparableList subTail(SortedComparableList L, int start) {
        // SortedComparableList ptr = this; // cant do this
        if(L == null || start == 0){
            return L; //empty
        }
        else{
