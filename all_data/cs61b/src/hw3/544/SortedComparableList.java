...
public class SortedComparableList {
    ...
    public void insert(Comparable c) {
        // REPLACE THIS LINE WITH YOUR SOLUTION
      SortedComparableList parse = this;
      if (c.compareTo(parse.head) < 0){
        tail = new SortedComparableList(parse.head, parse.tail);
        head = c;
        }
      else {
          while (parse.tail != null && c.compareTo(parse.tail.head) >= 0 ){
            parse = parse.tail;
          }

          parse.tail = new SortedComparableList(c, parse.tail);
            }
    }
    ...
    public static SortedComparableList sublist(SortedComparableList L, int start, int len) {
        // REPLACE THIS LINE WITH YOUR SOLUTION
        SortedComparableList sublen = subTail(L, start);
        SortedComparableList parsing = sublen;
        while (len > 1){
          parsing = parsing.tail;
          len -= 1;
        }
        parsing.tail = null;
        return sublen;
    ...
    public static void expungeTail(SortedComparableList L, int len) {
        // REPLACE THIS LINE WITH YOUR SOLUTION
      SortedComparableList expunging = L;
        while (len > 0){
          expunging = expunging.tail;
          len -= 1;
          }
        expunging.tail = null;
    ...
    public void squish() {
        // REPLACE THIS LINE WITH YOUR SOLUTION
      SortedComparableList squishy = this;
      while (squishy.tail != null){
        if (squishy.head.compareTo(squishy.tail.head) == 0){
          squishy.tail = squishy.tail.tail;
          }
        else {
          squishy = squishy.tail;
          }
        }
    }
    ...
    public void twin() {
        // REPLACE THIS LINE WITH YOUR SOLUTION
      SortedComparableList twinning = this;
      while(twinning != null){
        twinning.tail = new SortedComparableList(twinning.head, twinning.tail);
        twinning = twinning.tail.tail;
        }
      }
