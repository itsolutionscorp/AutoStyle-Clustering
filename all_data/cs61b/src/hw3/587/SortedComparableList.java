...
public class SortedComparableList {
    ...
    public Comparable get(int i) {
        int index = 0;
        SortedComparableList pointer = this;
        while(pointer.tail != null && index < i) {
          pointer = pointer.tail;
          index += 1;
        }
        return pointer.head;
    ...
    public static SortedComparableList sublist(SortedComparableList L, int start, int len) {
        ...
        if(start > 0) return sublist(L.tail, start-1, len);
    ...
    public void squish() {
        if (tail != null) {
            SortedComparableList pointer = this;
            SortedComparableList pointerTail = tail;
            while (pointerTail != null) {
                if (pointer.head.equals(pointerTail.head)) {
                    pointer.tail = pointerTail.tail;
                    pointerTail = pointer.tail;
                }
                else {
                    pointer = pointer.tail;
                    pointerTail = pointer.tail;
