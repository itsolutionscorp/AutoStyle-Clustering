...
+			ptr.tail = new SortedComparableList(c, ptr.tail);
	...
	public static SortedComparableList sublist(SortedComparableList L, int start, int len) {
		...
		SortedComparableList newList = subTail(L, start);
	...
	public void squish() {
		...
		SortedComparableList ptr = tail;
		while (ptr != null) {
			if (prev.head.equals(ptr.head)) {
				prev.tail = ptr.tail;
				ptr = prev.tail;
			} else {
				prev = prev.tail;
