...
public class SortedComparableList {
    ...
    public void insert(Comparable c) {
        if(head.compareTo(c) > 0 || head.compareTo(c) == 0){
            Comparable origHead = head;
            head = c;
            tail = new SortedComparableList(origHead, tail);
        } else if(tail == null){
    ...
    public static SortedComparableList subTail(SortedComparableList L, int start) {
        ...
        return new SortedComparableList(sub.head, sub.tail);
    }
    ...
    public static SortedComparableList sublist(SortedComparableList L, int start, int len) {
        SortedComparableList toCut = subTail(L, start);
        SortedComparableList toAdd = new SortedComparableList(toCut.head, null);
        while(len > 1 && toCut.tail!= null){
            toCut = toCut.tail;
            toAdd.insert(toCut.head);
            len--;
        }
        return toAdd;
    ...
    public static void expungeTail(SortedComparableList L, int len) {
        SortedComparableList l = SortedComparableList.sublist(L, 0, len+1);
        L.tail = l.tail;
    }
    ...
    public void twin() {
        if(tail == null){
            SortedComparableList origTail = tail;
            tail = new SortedComparableList(head, origTail);
            return;
        } else {
            SortedComparableList origTail = tail;
            tail = new SortedComparableList(head, origTail);
    ...
    public void main (String[] args){
        SortedComparableList test = new SortedComparableList(5, null);
        test.insert(3);
        test.insert(10);
    }
}