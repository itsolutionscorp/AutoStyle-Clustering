...
public class SortedComparableList {
    ...
    public void insert(Comparable c) {
        SortedComparableList hold = this;
    ...
    public void extend(SortedComparableList that) {
        SortedComparableList point = that;
        while (point.tail != null) {
          this.insert(point.head);
          point = point.tail;
        }
    }
    ...
    public static SortedComparableList sublist(SortedComparableList L, int start, int len) {
      SortedComparableList subtail = SortedComparableList.subTail(L, start);
      if (len == 0) {
        return new SortedComparableList();
      } else {
        SortedComparableList tail = SortedComparableList.sublist(subtail.tail, 0, len - 1);
        return new SortedComparableList(subtail.head, tail);
      }
    }
    ...
    public static void expungeTail(SortedComparableList L, int len) {
        if (len == 0) {
          L.tail = new SortedComparableList();
        } else {
    ...
    public void squish() {
      ...
      SortedComparableList check = tail;
      int start = 0;
      while (check.head.compareTo(curr) == 0) {
        check = check.tail;
        start = start + 1;
      }
      SortedComparableList subtail = SortedComparableList.subTail(this, start);
      subtail.squish();
      this.head = curr;
      this.tail = subtail;
    }
