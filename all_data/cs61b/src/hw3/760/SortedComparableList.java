...
public class SortedComparableList {
    ...
    public void insert(Comparable c) {
		...
		if (c.compareTo(head) < 0) {
			...
			SortedComparableList curr = this; // make a copy because cannot move actual head or tail
	...
	while (tracker.head != null && that.tail != null) {
		insert(that.head); // already in file, so no need to use obj notation
                that = that.tail;
	}   
 }
    ...
    public static SortedComparableList sublist(SortedComparableList L, int start, int len) {
       // REPLACE THIS LINE WITH YOUR SOLUTION
       return subtail(subtail(L, start), len);
}
