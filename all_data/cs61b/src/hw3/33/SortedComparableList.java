...
public class SortedComparableList {
    ...
    //Could alternatively write insert for each item in that
    public void extend(SortedComparableList that) {
        SortedComparableList pointer = this;
        SortedComparableList front;
        while (pointer.head.compareTo(that.head) > 0) {
          insert(that.head);
          pointer = pointer.tail;
        }
        while (pointer.tail != null && that != null) {
          if (pointer.tail.head.compareTo(that.head) > 0) {
            pointer.tail = new SortedComparableList(that.head, pointer.tail);
            that = that.tail;
          }
          pointer = pointer.tail;
        }
    ...
    public static SortedComparableList subTail(SortedComparableList L, int start) {
        if (start > 0) {return subTail(L.tail, start - 1);}
        return L;
    }
    ...
    public static SortedComparableList sublist(SortedComparableList L, int start, int len) {
      ...
      SortedComparableList front = subTail(L, start);
      SortedComparableList pointer = front;
      while (len > 1) {
        pointer = pointer.tail;
        len -= 1;
      }
