...
public class SortedComparableList {
    ...
    public void insert(Comparable c) {
        if (c != null) {
            if (c.compareTo(head) < 0) {
                ...
                while (curr.tail != null && curr.tail.head.compareTo(c) < 0) {
                    curr = curr.tail;
                }
    ...
    public static SortedComparableList sublist(SortedComparableList L, int start, int len) {
        SortedComparableList it = L;
        for (int i = 0; i < start; ++i) {
            it = L.tail;
        }
        
        // too complicated
        SortedComparableList current = new SortedComparableList(it.head, null);
        for (int i = 0; it != null && i < len; ++i) {
            SortedComparableList temp = new SortedComparableList(it.head, null);
            current.tail = temp;
            current = temp;
        }
        return current;
