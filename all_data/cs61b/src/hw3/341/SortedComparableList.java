...
public class SortedComparableList {
    ...
    public void insert(Comparable c) {
        ...
        if (c != null) {
          if (c.compareTo(head) < 0) {
            //insert c to the beginning of the list
            tail = new SortedComparableList(head, tail);
            head = c;
          }
          else { //if c is > head, insert it into current position, recurse on tail?
            SortedComparableList temp = this;
            while (temp.tail != null && c.compareTo(temp.tail.head) > 0) { //if end of list hasn't been reached, compare it to the head of the rest
              temp = temp.tail;
            }
            temp.tail = new SortedComparableList(c, temp.tail);//insert into correct position
          }
        }
    }
    ...
    public Comparable get(int i) {
      if (i == 0 || head == null) {
        ...
        SortedComparableList temp = this; //create a new list to destroy
        while (temp.tail != null && i != 0) { 
          temp = temp.tail;
          i -= 1; //increment i and increment along the list
        }
        return temp.head;
      }
        //return null
      ...
      if (tail != null) {
        ...
        while (curr != null) {
          if (prev.head == curr.head) {
            prev.tail = curr.tail; //jumping over that term
            curr = prev.tail; //replace
          }
          else {
            prev = prev.tail; //continue incrementing
            curr = prev.tail;
          }
        }
      }
