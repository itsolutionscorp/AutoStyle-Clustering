    ...
    public static SortedComparableList sublist(SortedComparableList L, int start, int len) {
    	if (start == 0) {
    		if (len == 0)
    			...
    			return new SortedComparableList(L.head, SortedComparableList.sublist(L.tail, start, len - 1));
    	} else if (L != null)
    		return SortedComparableList.sublist(SortedComparableList.subTail(L.tail, start - 1), 0, len);
        return null;
    }
