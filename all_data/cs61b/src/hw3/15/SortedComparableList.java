...
public class SortedComparableList {
    ...
    public void insert(Comparable c) {
        // REPLACE THIS LINE WITH YOUR SOLUTION
	    if (c != null) {
	        // inserting to front of list
	        if (c.compareTo(head) < 0) {
	        	tail = new SortedComparableList(head, tail);
	        	head = c;
	        }
	        else {
                // inserting to middle or end of list
    ...
    /** Adds every item in THAT to this list. The added elements should be placed in their correct location based on
     *  ascending order. */
    public void extend(SortedComparableList that) {
        // REPLACE THIS LINE WITH YOUR SOLUTION
        // note to self: resulting list must still be sorted
     ...
     *  beginning with item START (where the first item is at position 0).
     *
