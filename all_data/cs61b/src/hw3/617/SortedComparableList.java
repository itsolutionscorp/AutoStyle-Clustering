...
public class SortedComparableList {
    ...
    public void insert(Comparable c) {
       ...
       if (head.compareTo(c)>0){
           ...
           SortedComparableList listInsert = this;
       while (listInsert.tail!=null&&listInsert.tail.head.compareTo(c)<0){
           listInsert=listInsert.tail;
       }
        listInsert.tail = new SortedComparableList(c,listInsert.tail);}
       }
    }
    ...
    public Comparable get(int i) {
        int x = 0;
        SortedComparableList listGet= this;
        while (x<i){
            listGet=listGet.tail;
            x++;
        }
        return listGet.head;
    }
    ...
    public void extend(SortedComparableList that) {
        SortedComparableList listExtend = that;

        while (listExtend!=null){
            insert(listExtend.head);
            listExtend=listExtend.tail;
        }
    ...
    public static SortedComparableList sublist(SortedComparableList L, int start, int len) {
        SortedComparableList scl=subTail(L,start);
        SortedComparableList returnedSubList = new SortedComparableList();
        SortedComparableList pointer = returnedSubList;
        int i =0;
        while (i<len-1){
            pointer.head=scl.head;
            scl=scl.tail;
            pointer.tail=new SortedComparableList();
     ...
     **/
    public void twin() {
        SortedComparableList scl = this;
        while (scl!=null){
            scl.tail=new SortedComparableList(scl.head,scl.tail);
            scl=scl.tail.tail;
        }
    }
