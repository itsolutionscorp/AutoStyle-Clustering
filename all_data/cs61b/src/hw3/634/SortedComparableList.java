...
public class SortedComparableList {
    ...
    public void insert(Comparable c) {
        if (c != null) {
            ...
            if (c.compareTo(head) < 0) {
                ...
                SortedComparableList compare = this;
                while (compare.tail != null && c.compareTo(compare.tail.head) > 0) {
                    compare = compare.tail;
                }
                compare.tail = new SortedComparableList(c, compare.tail);
            }
        }
        // REPLACE THIS LINE WITH YOUR SOLUTION
    }
    ...
    public void extend(SortedComparableList that) {
        SortedComparableList compare = that;
        while (compare != null) {
            insert(compare.head);
            compare = compare.tail;
        }
    ...
    public void squish() {
        if (tail != null) {
            ...
            SortedComparableList compare = tail;
            while (compare != null) {
                if (prev.head.equals(compare.head)) {
                    prev.tail = compare.tail;
                    compare = prev.tail;
                }
                else {
                    prev = prev.tail;
