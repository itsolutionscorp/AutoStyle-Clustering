...
public class SortedComparableList {
    ...
    private void insertHelper(Comparable c, SortedComparableList scl) {
        if (c.compareTo(scl.head)<=0) {
          scl.tail = new SortedComparableList(scl.head, scl.tail);
          scl.head = c;
        }
        else if (scl.tail==null) {
          scl.tail = new SortedComparableList(c, null);
        }
        
        else insertHelper(c, scl.tail);
      }
    ...
    public static SortedComparableList subTail(SortedComparableList L, int start) {
        if (start==0) {
          return new SortedComparableList(L.head, L.tail);
        }
        else {
    ...
    public static SortedComparableList sublist(SortedComparableList L, int start, int len) {
        ...
        SortedComparableList temp = subTail(L, start);
        SortedComparableList pointer = temp;
        while (len>=1) {
          if (len==1) {
            pointer.tail = null;
            len -=1;
          }
          else if (pointer.tail==null && len>1) {
            len = 0;
          }
          else {
