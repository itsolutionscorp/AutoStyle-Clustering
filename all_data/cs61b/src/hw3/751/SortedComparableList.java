...
public class SortedComparableList {
    ...
    public void insert(Comparable c) {
        ...
        while (c != null) {
            if (c.compareto(head < 0)){
                ...
                SortedComparableList cycle = this;
                while (cycle.tail != null && c.compareTo(cycle.tail.head) > 0) {
                    cycle = cycle.tail;
                }
                cycle.tail = new SortedComparableList(c, cycle.tail);
            }
        }
    }

    /** Returns the i-th int in this list.
     *  The first element, which si in location 0, is the 0th element.
    ...
    public void extend(SortedComparableList that) {
        SortedComparableList cycle = that;
        while (cycle != null){
            this.insert(cycle.head)
            cycle = cycle.tail
        }
    }
    ...
    public static SortedComparableList sublist(SortedComparableList L, int start, int len) {
        if (len == 0){
            ...
            return new SortedComparableList(head, sublist(L.tail, 0, len - 1));
        }
    }
    ...
    public void squish() {
        ...
        while (previous != null && previous.tail != null){
            if (previous.head == previous.tail.head) {
                if (previous.tail.tail != null) {
                    previous.tail = previous.tail.tail;
                    previous.tail.tail = previous.tail;
                } else {
                    previous.tail = null;
                }
            }
