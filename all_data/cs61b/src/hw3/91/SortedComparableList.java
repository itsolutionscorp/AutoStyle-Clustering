...
public class SortedComparableList {
    ...
    public void insert(Comparable c) {
        ...
        if (c != null) {
            if (c.compareTo(head) < 0 || c.compareTo(head) == 0) {
                ...
                SortedComparableList list = this;

                while (list.tail != null && c.compareTo(list.tail.head) > 0) {
                    list = list.tail;
                }



                list.tail = new SortedComparableList(c, list.tail);
            }
        }
    }
    ...
    public Comparable get(int i) {
        if (i == 0) {
          return head;
            ...
            int count = 1;
            SortedComparableList list = this.tail;
            while (count != i && list.tail != null) {
                list = list.tail;
                count += 1;
            }
            return list.head;
    ...
    public void extend(SortedComparableList that) {
        // REPLACE THIS LINE WITH YOUR SOLUTION
        SortedComparableList list = that;
        while (list != null) {
            insert(list.head);
            list = list.tail;
        }
    ...
    public static SortedComparableList sublist(SortedComparableList L, int start, int len) {
        ...
        if (L == null) {
            ...
            SortedComparableList list = L;
            SortedComparableList newList = null;
            for (int i = 0; i < start; i += 1) {
                list = list.tail;
            }

            newList = new SortedComparableList(list.head, null);
            list = list.tail;

            for (int i = 0; i < len - 1; i += 1) {
                newList.insert(list.head);
                list = list.tail;
            }
    ...
    public void squish() {
        ...
        while (list.tail != null) {
            if (c.compareTo(list.tail.head) == 0) {
                if (list.tail.tail != null) {
                    list.tail = list.tail.tail;
                } else {
                    list.tail = null;
                }
            } else if (c.compareTo(list.tail.head) != 0) {
                c = list.tail.head;
                list = list.tail;
            }
        }
    ...
    public void twin() {
        ...
        while (list != null) {
            list.tail = new SortedComparableList(list.head, list.tail);
            list = list.tail.tail;
        }
    }
