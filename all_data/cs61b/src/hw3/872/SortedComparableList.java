...
public class SortedComparableList {
    ...
    public static SortedComparableList sublist(SortedComparableList L, int start, int len) {
        ...
        while (i < len && temp != null) {
            i+=1;
            temp = temp.tail;
            result.tail = new SortedComparableList(temp.head, null);
            result = result.tail;
        }
        return result;
    ...
    public void squish() {
        ...
        SortedComparableList temp = tail;
        while (temp != null) {
            if (temp.head.compareTo(previous.head) == 0) {
                previous.tail = temp.tail;
                temp = previous.tail;
            } else {
                previous = temp;
                temp = temp.tail
