...
def main():
  if len(sys.argv) != 3:
    print >> sys.stderr, "Usage: spark_dist_matrix.py <input_dir> <function name>"
    exit(-1)
  ...
  indexes = range(0,len(os.listdir(input_dir)))
  ...
  # Construct RDD
  treeTuplesRDD = sc.parallelize(treeTuples, 64) # Set to 8 * cores
...
def construct_trees(input_dir, indexes, method):
  ...
  for i in indexes:
    ...
    trees.append(tree.abstract_syntax_tree(input_dir + "/" + str(i) + "/Calculator.java.ast", method))
  return trees
