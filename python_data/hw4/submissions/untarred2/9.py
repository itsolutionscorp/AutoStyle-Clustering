...
def make_word_master(goal_word):
    ...
    def word_master(guess_word):
        if (is_valid_guess(guess_word)):
            if (len(get_list(guess_word)) != len(get_list(goal_word))):
...
def subsets(lst, n):
    ...
    [2, 3, 4]
    """
    def add_subsets(choices, list_result, n, combi = []):
        if (len(combi) == n):
            combi.sort()
            if(not(combi in list_result)):
                list_result += [combi]
        else:
            for elt in choices:
                combi_copy = combi[:] + [elt]
                new_choices = choices[:]
                new_choices.remove(elt)
                add_subsets(new_choices, list_result, n, combi_copy)
    all_subsets = []
    add_subsets(lst, all_subsets, n)
    return all_subsets
...
def compatible(guess, score, letter_list):
    ...
    if (len(guess_list) == score):
        for l in letter_list:
            if (not l in guess_list):
                return False
        return True
    else:
        common_letters = 0
...
def make_deductions(possible_subsets, letters):
    ...
    ['d', 'f']
    """
    def sort(present, not_present, subsets, letter):
        for s in subsets:
            if (letter in s):
                if (letter in not_present):
                    not_present.remove(letter)
    ...
    for letter in letters:
       sort(present, not_present, possible_subsets, letter)
