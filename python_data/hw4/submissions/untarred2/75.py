...
def subsets(lst, n):
    ...
    [2, 3, 4]
    """
    def add_every(lst, elem):
        if lst == []:
            return []
        result = add_every(lst[1:], elem)
        return [[elem] + lst[0]] + result
    if n == 0:
        return [[]]
    elif lst == []:
        return []
        ...
        recursive_result = subsets(rest, n)
        new = add_every(subsets(rest, n - 1), first)
        return recursive_result + new
...
def make_deductions(possible_subsets, letters):
    ...
    for l in letters:
        ...
        for s in possible_subsets:
            in_all_subsets = in_all_subsets and (l in s)
            in_no_subsets = in_no_subsets or (l in s)
        if in_all_subsets:
            present += [l]
        if in_no_subsets == False:
            not_present += [l]
