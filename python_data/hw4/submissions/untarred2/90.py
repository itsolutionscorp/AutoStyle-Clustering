...
def make_word_master(goal_word):
    ...
    "*** YOUR CODE HERE ***"
    def judge(guess):
        if get_string(goal_word) is get_string(guess):
            return win_message
        if not is_valid_guess(get_string(guess)):
            return not_a_word
        if len(get_list(guess)) is not len(get_list(goal_word)):
...
def subsets(lst, n):
    ...
    """
    "*** YOUR CODE HERE ***" 
    if len(lst) < n or n is 0:
        return []
    else:
        # lst = []
        # for i in range(0, n-1):
        #     lst += lst[i]
        # return lst + subsets(lst[1:], n)
...
def filter_subsets(word, score, possible_subsets):
    ...
    for elem in possible_subsets:
        if compatible(make_word_from_list(elem), score, get_list(word)):
            lst += [elem]
    return lst
