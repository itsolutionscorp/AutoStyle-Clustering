...
def subsets(lst, n):
    ...
    "*** YOUR CODE HERE ***"
    result = []
    def helper(start, end, depth=0):
        if (depth == n or not end) and len(start) == n:
            result.append(start)
            return
        for i in range(min(n, len(end))):
            helper(start + [end[i]], end[i+1:], depth+1)
    helper([], lst)
    return result
...
def filter_subsets(word, score, possible_subsets):
    ...
    while i < len(possible_subsets):
        if compatible(word, score, subsets[i]) == True:
            good_subsets += [subsets[i]]
            i += 1
        else:
            i += 1
    return good_subsets
...
def make_deductions(possible_subsets, letters):
    ...
    "*** YOUR CODE HERE ***"
    for x in letters:
        if all(x in s for s in possible_subsets):
            present += [x]
        if not any(x in s for s in possible_subsets):
            not_present += [x]
