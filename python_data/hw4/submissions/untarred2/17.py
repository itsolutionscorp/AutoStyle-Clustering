...
def make_word_from_list(lst):
    ...
    if len(lst)== 1:
        ...
        ADT = lst[0]+make_word_from_list(lst[1:])
    return ADT
...
def subsets(lst, n):
    ...
    """
    "*** YOUR CODE HERE ***"
    def helper(lst, n, get):
        if n == 0:
            return [get]
        elif lst == []:
            return []
        return helper(lst[1:], n-1, get + [lst[0]]) + helper(lst[1:], n, get)
    return helper(lst, n, [])
...
def filter_subsets(word, score, possible_subsets):
    ...
    for subset in possible_subsets:
        if score ==num_common_letters(make_word_from_list(subset), word):
            set+=[subset]
    return set
