...
def subsets(lst, n):
    ...
    def accumulate(i,lst):
        if lst == []:
            return []
        else:
            return [[i] + lst[0]] + accumulate(i, lst[1:])
    ...
    if len(lst) < n:
        ...
        no_first_list = subsets(lst[1:],n)
        yes_first_list = accumulate(lst[0],subsets(lst[1:],n-1))

        return no_first_list + yes_first_list
...
def make_deductions(possible_subsets, letters):
    ...
    for element in letters:
        ...
        if result == len(possible_subsets):
            present += [element]
