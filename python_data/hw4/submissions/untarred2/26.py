...
def subsets(lst, n):
    ...
    def powerset(lst):
        if lst ==[] :
            ...
            recurssion = powerset(lst[1:])
            new_elem = add_to_all(recurssion, lst[0])
            return new_elem + recurssion

    def add_to_all(lst, elem) :
        if lst == [] :
            return []
        recurssion = add_to_all(lst[1:], elem)  
        return [[elem]+lst[0]] + recurssion 
        
    result =[elem for elem in powerset(lst) if len(elem) == n]    
...
def filter_subsets(word, score, possible_subsets):
    ...
    for i in range(len(possible_subsets)) :
        if compatible(possible_subsets[i],score, get_list(word))==True :
...
def make_deductions(possible_subsets, letters):
    ...
    """
    total_subsets = []
    present = possible_subsets[0]
    for i in range(len(possible_subsets)) :
        present = [elem for elem in present if elem in possible_subsets[i]]
        total_subsets += [elem for elem in possible_subsets[i] if elem not in total_subsets]

    not_present = [elem for elem in letters if elem not in total_subsets]
