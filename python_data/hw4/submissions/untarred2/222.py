...
def make_word_master(goal_word):
    ...
    """
    def h(guess):
    	goal_word_str = get_string(goal_word)
    	guess_str = get_string(guess)
    	if is_valid_guess(guess) == False:
    		return not_a_word
    	if len(goal_word_str) != len(guess_str):
    		return bad_num_letters
    	elif goal_word_str == guess_str:
    		return win_message
    	else:
...
def subsets(lst, n):
    ...
    if n > len(lst):
    	...
    	recursive_list = subsets(lst[1:], n-1)
    	result = [[lst[0]] + x for x in recursive_list] 
...
def compatible(guess, score, letter_list):
    ...
    False
    """
    goal_str = get_string(make_word_from_list(letter_list))
    guess_str = get_string(guess) 
    if make_word_master(goal_str)(guess_str) == score:
        return True
    else:
        return False 
...
def filter_subsets(word, score, possible_subsets):
    ...
    [['a', 'b', 'e', 'l', 's'], ['b', 'l', 'e', 's', 't']]
    """
    return [subset for subset in possible_subsets if \
        make_word_master(make_word_from_list(subset))(word) == score]
    ...
    for ltr in letters:
        ...
        for subset in possible_subsets:
            for char in subset:
                if char == ltr:
                    correct += 1
        if correct == len(possible_subsets):
            present += ltr 
        elif correct == 0:
            not_present += ltr
