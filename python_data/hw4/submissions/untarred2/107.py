...
def make_word_master(goal_word):
    ...
    """
    "*** YOUR CODE HERE ***"
    return lambda x: make_word_master_helper(goal_word, x)

def make_word_master_helper(goal_word, guess_word):
    if not is_valid_guess(guess_word):
        return not_a_word
    elif len(goal_word) != len(guess_word):
        return bad_num_letters
    elif guess_word == goal_word:
        return win_message
    else:
...
def subsets(lst, n):
    ...
    """
    "*** YOUR CODE HERE ***"
    sublst = subsets_helper(lst, n, [])
    retlst =[]
    for i in range(len(sublst)):
        if sublst[i][2] > sublst[i][1] and sublst[i][1] > sublst[i][0]:
            retlst.append(sublst[i])
    return retlst

def subsets_helper(lst, n , sublst):
    for i in lst:
        for j in lst:
            for k in lst:
                sublst.append([i, j, k])
...
def compatible(guess, score, letter_list):
    ...
    """
    "*** YOUR CODE HERE ***"
    guess_str, guess_score = get_string(guess), 0
    for i in range(len(letter_list)):
        if letter_list[i] in guess_str:
            guess_score += 1
    return guess_score == score
