...
def subsets(lst, n):
    ...
    if len(lst)==0:
        ...
        for first_index in range(len(lst)):
            if first_index<len(lst)-n+1:
                recursive_result=subsets(lst[first_index+1:],n-1)
                for result in recursive_result:
                    new_list.append([lst[first_index]]+result)
        return new_list
...
def compatible(guess, score, letter_list):
    ...
    if len(get_list(guess)) != len(letter_list):
        return False
    elif num_common_letters(guess,mwfl(letter_list))==score:
        return True
    else:
        return False
