# CS 61A Spring 2014
# Name: David Au
# Login: cs61a-bm@cory.eecs.berkeley.edu
...
def subsets(lst, n):
    ...
    if n == 0:
        ...
        return []
    subset = subsets(lst[1:], n -1)
    var = []
    for s in subset:
        item_to_use= lst[0]
        var = var + [[item_to_use] + s]
    else:
        return subsets(lst[1:], n) + var 
...
def make_deductions(possible_subsets, letters):
    ...
    for u in letters:
        tars = []
        for t in possible_subsets:
            if u in t:
                tars = tars + [u]
        if len(tars) == len(possible_subsets):
            present += [u]
        if tars == []:
            not_present += [u]
