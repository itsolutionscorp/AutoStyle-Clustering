...
def subsets(lst, n):
    ...
    if n == 0:
        ...
        return insert_elem(first, subsets(rest,n-1)) + subsets(rest, n) 
...
def compatible(guess, score, letter_list):
    ...
    for letter in get_list(guess):
        if letter not in letters_already_checked:
            letters_already_checked += [letter]
    if num_common_letters(make_word_from_list(letters_already_checked),letter_list) == score:
        return True
    return False
...
def make_deductions(possible_subsets, letters):
    ...
    for letter in letters:
        ...
        for subset in possible_subsets:
            if compatible(letter, 1, subset):
                letter_in_no_subsets = False
            else:
                letter_in_all_subsets = False
        if letter_in_all_subsets:
            present += [letter]
        if letter_in_no_subsets:
            not_present += [letter]
