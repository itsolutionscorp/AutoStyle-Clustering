...
def make_word_master(goal_word):
    ...
    def word_master(guess):
        ...
        common = num_common_letters(goal_word, guess)
        if get_string(goal_word) == get_string(guess):
            return win_message
        else:
            return common
    return word_master
...
def subsets(lst, n):
    ...
    def insert(elm, lst):
        if lst == []:
            ...
            return [[elm] + lst[0]] + insert(elm, lst[1:])
    ...
    def all_subsets(lst):
        if lst == []:
            ...
            current_elm = lst[0]
            rest = lst[1:]
            all_subsets_without_first = all_subsets(rest)
            return all_subsets_without_first + insert(current_elm, all_subsets_without_first)


    subsets_list = all_subsets(lst)
    subsets_list = [item for item in subsets_list if len(item) == n]
    return subsets_list
...
def compatible(guess, score, letter_list):
    ...
    common = num_common_letters(goal, guess)
    return common == score
...
def filter_subsets(word, score, possible_subsets):
    ...
    """
    "*** YOUR CODE HERE ***"
    return [item for item in possible_subsets if compatible(item, score, word)]
...
def make_deductions(possible_subsets, letters):
    ...
    for i in range(0, len(letters)):
        ...
        for subset in possible_subsets:
            if letters[i] in subset:
                counts[i] += 1
    for i in range(0, len(counts)):
        if counts[i] >= len(possible_subsets):
            present += letters[i]
        if counts[i] == 0:
