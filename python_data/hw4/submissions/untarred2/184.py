...
def subs(lst, n):
    if lst==[]:
        ...
        without_first = subs(rest, n)
        return without_first + ins(first, without_first)

def ins(elem, lst):
    if lst == []:
        return []
    else:
        return [[elem] + lst[0]] + ins(elem, lst[1:])
...
def make_deductions(possible_subsets, letters):
    ...
    present = []
    not_present = []
    actual_count = len(possible_subsets)
    for l in letters:
        count = 0
