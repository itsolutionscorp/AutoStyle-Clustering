...
def make_word_from_string(s):
    ...
    """
    "*** YOUR CODE HERE ***"
    word=link('word', empty)
    for string in s:
        word=extend_link(word, link(string, empty))
    return word
        
#s='hello'
#print(make_word_from_string(s))
...
#lst=['a','b','c','d']
#print(make_word_from_string(lst))
...
def get_string(word):
    ...
    i=1
    string=''
    while i <len_link(word):
        c=getitem_link(word,i)
        i=i+1
        string=string+c
    return string 

word=['word', ['h', ['e', ['l', ['l', ['o', 'empty']]]]]]
#print(get_string(word))
