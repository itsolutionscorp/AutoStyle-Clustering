...
def make_word_master(goal_word):
    ...
    True
    """
    def master_word_function(guess):
        guess_list = get_list(guess)
        goal_list = get_list(goal_word)
...
def subsets(lst, n):
    ...
    def add_to_all(lst_of_lsts, elem):
        result = []
        for sublsts in lst_of_lsts:
            new = [[elem] + sublsts]
            result = result + new
        return result
    ...
    if n == 0:
        ...
        sub_lst = add_to_all(subsets(rest, n-1), first) + subsets(rest, n)
    return sub_lst
...
def filter_subsets(word, score, possible_subsets):
    ...
    for x in possible_subsets:
        if num_common_letters(word,x) == score:
            sub_lst = sub_lst + [x]
    return sub_lst
...
def make_deductions(possible_subsets, letters):
    ...
    ['d', 'f']
    """
    #function works if subset doesnt have multiple copies of the same letter..
    present = []
    not_present = []
    counter = 0 
    for x in letters:
