...
def make_word_from_list(lst):
    ...
    for elem in lst:
        new_string += elem
    return make_word_from_string(new_string)
...
def make_word_master(goal_word):
    ...
    def check_work_master(guess):
        if is_valid_guess(guess) != True:
            ...
            elif guess_string == goal_string:
                return win_message
            else:
                return num_common_letters(goal_word, guess)


    return check_work_master
...
def make_deductions(possible_subsets, letters):
    ...
    not_present = []
    new_list = []
    should_appear = len(possible_subsets)
    appear_times = 0
    for char in letters:
        for elem in possible_subsets:
            if char in elem:
                appear_times += 1
                if appear_times == should_appear:
                    present += [char]
