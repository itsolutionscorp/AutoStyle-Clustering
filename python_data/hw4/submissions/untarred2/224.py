...
def subsets(lst, n):
    ...
    if n == 0:
        ...
        second_section = subsets(lst[1:], n)
        first_section = subsets(lst[1:], n-1)
        new_list = []
        for element in first_section:
            new_list += [[lst[0]] + element]
        return new_list + second_section                   
...
def filter_subsets(word, score, possible_subsets):
    ...
    for element in possible_subsets: 
        if compatible(element, score, word) == True:
            new_list += [element]
    return new_list
    ...
    for element in letters:
        ...
        for subset in possible_subsets:
            if element in subset:    
                present_counter += 1
                if present_counter == len(possible_subsets): 
                    present += [element]                        
            if element not in subset: 
                not_present_counter += 1
                if not_present_counter == len(possible_subsets):
                    not_present += [element]     
