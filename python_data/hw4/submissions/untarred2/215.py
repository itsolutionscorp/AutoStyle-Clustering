...
def subsets(lst, n):
    ...
    if n == 0:
        ...
        first, rest = lst[0], lst[1:] # FIRST IS A NUMBER!
        set1 = [[first] + item for item in subsets(rest, n-1)]
        set2 = subsets(rest, n)
        return set1 + set2
...
def compatible(guess, score, letter_list):
    ...
    """
    "*** YOUR CODE HERE ***"
    guess_word = make_word_from_string(guess)
    letter_list_word = make_word_from_list(letter_list)
    
    return num_common_letters(letter_list_word, guess_word) == score
