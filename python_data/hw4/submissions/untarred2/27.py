...
def make_word_master(goal_word):
    ...
    def word_master(guess_word):
        ...
        if len(guess_word_list) != len(goal_word_list):
            return bad_num_letters
        if guess_word_list == goal_word_list:
            return win_message
        if not is_valid_guess(guess_word):
            return not_a_word

        result = num_common_letters(goal_word, guess_word)
        return result
...
def subsets(lst, n):
    ...
    subset1 = subsets(lst[1:], n-1)
    subset2 = subsets(lst[1:], n)
    result1 = []
    result2 = []
