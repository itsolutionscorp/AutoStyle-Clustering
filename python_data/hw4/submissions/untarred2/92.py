...
def make_word_from_list(lst):
    ...
    """if lst == []:
        ...
        return lst[0] + make_word_from_list[1:]"""
    i = 0
    wrd = ""
    while i < len(lst):
        wrd += lst[i]
...
def num_common_letters(goal_word, guess):
    ...
    0
    """
    """if get_string(goal_word) == "":
        return 0
    if get_list(goal_word) == "":
        return 0
    elif
        (get_string(goal_word))[0] == (get_string(guess))[0]:
            return num_common_letters(goal_word)"""

    """for elem in get_list(goal_word):
        if elem == (get_list(guess))[0]:
            return 1 + num_common_letters(get_list(goal_word), guess[1:])
        else:
            return 0"""
...
def subsets(lst, n):
    ...
    def helper_subset(last, lst, n):
        ...
        for x in lst:
            a = 1
            help = [x]
            new = helper_subset(last + help, lst[a:], n - 1)
        return final
    lst = [n]
    for x in lst[:len(lst)-n+1]:
        lst = [lst] + [(helper_subset([x], lst[(lst.index(x)+1):], n-1))]
    return lst 
