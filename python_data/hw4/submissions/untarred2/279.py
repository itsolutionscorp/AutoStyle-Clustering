...
def compatible(guess, score, letter_list):
    ...
    """
    "*** YOUR CODE HERE ***"
    subSetList = subsets(letter_list,score)
    for i in range(len(subSetList)):
        subSetList[i]=make_word_from_list(subSetList[i])
        if num_common_letters(guess,make_word_from_string(subSetList[i]))==score:
            return True
    return False
...
def filter_subsets(word, score, possible_subsets):
    ...
    for s in possible_subsets:
        if(compatible(s,score,word)==True):
            subSetList += s
    return subSetList
...
def make_deductions(possible_subsets, letters):
    ...
    not_present = []
    "*** YOUR CODE HERE ***"
    letters = make_word_from_string("abcdefghijklmnopqrstuvwxyz")
    for letter in letters:
        appearsInEachSubset=True
        for s in possible_subsets:
            ...
            if letterContained==False:
                appearsInEachSubset=False
        if appearsInEachSubset==True:
            present+=letter
        else:
            not_present+=letter
