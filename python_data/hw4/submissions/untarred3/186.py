...
def make_word_from_list(lst):
    ...
    while(i<len(lst)):
        s+=lst[i]
        i+=1
    return make_word_from_string(s)
...
def make_word_master(goal_word):
    ...
    "*** YOUR CODE HERE ***"
    def master(guess):
        if goal_word('number')!=guess('number'):
...
def subsets(lst, n):
    ...
    def a(n,lst1,lst2):
        if n==0:
            ...
            return a(n-1,lst1[1:],lst2+[lst1[0]])+a(n,lst1[1:],lst2) 
    return a(n,lst,[])           
...
def filter_subsets(word, score, possible_subsets):
    ...
    "*** YOUR CODE HERE ***"
    sub=[]
    for temp in possible_subsets:
        if compatible(make_word_from_list(temp),score,get_list(word))==True:
            sub+=[temp]
    return sub        
...
def make_deductions(possible_subsets, letters):
    ...
    for letter in letters:
        for i in range(len(possible_subsets)):
          if (letter in possible_subsets[i])==False:
            present.remove(letter)
            break
    for letter in letters:
        for i in range(len(possible_subsets)):
          if (letter in possible_subsets[i])==True:
