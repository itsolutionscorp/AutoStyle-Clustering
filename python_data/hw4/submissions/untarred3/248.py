...
def make_word_master(goal_word):
    ...
    True
    """
    trans_goal = get_list(goal_word)
    n = len(trans_goal)

    def word_master(guess):
        trans_guess = get_list(guess)
        if len(trans_guess) != n:
            return bad_num_letters
        if not is_valid_guess(trans_guess):
            return not_a_word
        if trans_guess == trans_goal:
            return win_message
        return num_common_letters(goal_word, guess)
    return word_master

def insert_every(num ,lst): # to be used in func. subsets
    if lst==[]:
        return []
...
def compatible(guess, score, letter_list):
    ...
    "*** YOUR CODE HERE ***"
    trans_guess = get_list(guess)
    trans_goal = get_list(letter_list)
    if num_common_letters(trans_goal, trans_guess) == score:
        return True
    return False
...
def filter_subsets(word, score, possible_subsets):
    ...
    for sets in possible_subsets:
        if compatible(word, score, sets) == True:
            new_sets += [sets]
    return new_sets
