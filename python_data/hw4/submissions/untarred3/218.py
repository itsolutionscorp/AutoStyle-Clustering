...
def subsets(lst, n):
    ...
    if n == 0:
        ...
        return [lst] 
    #return [lst[0] + subsets(lst[1:], n - 1)] 
    #well this makes a linked list!
    with_first = [[lst[0]] + elem for elem in subsets(lst[1:], n - 1)]
...
def compatible(guess, score, letter_list):
    ...
    False
    """
    l = make_word_from_list(letter_list)
    if score == num_common_letters(l, guess):
        return True
    return False 
...
def filter_subsets(word, score, possible_subsets):
    ...
    for elem in possible_subsets:
        if compatible(elem, score, get_list(word)):
            subs.append(elem)
    return subs 
