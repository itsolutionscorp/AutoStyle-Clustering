...
def subsets(lst, n):
    ...
    if n == 0:
    	...
    	sans_first = subsets(rest,n-1)
    	return subsets(rest, n) + insert_every(first, sans_first)
...
def insert_every(element, lst):
	if lst == []:
		...
		first, rest = lst[0], lst[1:]
		return [[element] + first] + insert_every(element, rest)
...
def filter_subsets(word, score, possible_subsets):
    ...
    if num_common_letters(word,first) == score:
    ...
    for char in letters:
    	...
    	for subset in possible_subsets:
    		...
    		if char not in subset:
    			num_false += 1
    	if num_true == num_subsets:
    		present += [char]
    	if num_false == num_subsets:
    		not_present += [char]
