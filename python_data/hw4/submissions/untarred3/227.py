...
def make_word_master(goal_word):
    ...
    def f(guess):
        ...
        if not is_valid_guess(get_string(guess)):
            return not_a_word
...
def subsets(lst, n):
    ...
    if lst==[] and n>0:
        ...
        return  [([first]+i) for i in subsets(rest,n-1)]+(subsets(rest,n))
        # (create_list(first,first(subsets(rest,n-1))))
                # [2, 3]            [1]     [2],[3]
...
def compatible(guess, score, letter_list):
    ...
    """
    "*** YOUR CODE HERE ***"
    return num_common_letters(get_string(letter_list),guess)==score
...
def filter_subsets(word, score, possible_subsets):
    ...
    [['a', 'b', 'e', 'l', 's'], ['b', 'l', 'e', 's', 't']]
    """
    return [i for i in possible_subsets if compatible(get_string(word),score,i)]

# filtered=[]
# not_present = []
# letters=['a', 'b', 'c', 'd', 'e', 'f']
# subs = [['a', 'b', 'c'], ['b', 'a', 'e'], ['e', 'a', 'c']]
...
# for s in subs:
#     for l in letters:
#         let=filter_subsets(get_string(l),1, s)
#         if let!=[]:
#             filtered+=let
# filtered=[filter_subsets(get_string(i),1,['a','b','c']) for i in letters ]
# print(filtered)
# print(not_present)
