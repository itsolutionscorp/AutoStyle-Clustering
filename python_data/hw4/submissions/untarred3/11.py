...
def make_word_from_list(lst):
    ...
    if len(lst)==1:
        ...
        x=lst[0]
        return x + make_word_from_list(lst[1:])
def get_string(word):
...
def make_word_master(goal_word):
    ...
    """
    "*** YOUR CODE HERE ***"
    def messages(guess_word):
        guess_temp = get_list(guess_word)
        goal_temp = get_list(goal_word)
        guess_temp_word = get_string(guess_word)
        goal_temp_word = get_string(goal_word)
        if is_valid_guess(guess_word) == False:
            return not_a_word
        elif (len(guess_temp)  == 0) or (len(guess_temp) != len(goal_temp)):
            return bad_num_letters
        elif guess_temp_word == goal_temp_word:
            return win_message
        else:
