...
def make_word_master(goal_word):
    ...
    """
    "*** YOUR CODE HERE ***"
    def The_Word_Word_Guessing_Game(word):
        ...
        if Goal_Word == Guess:
            ...
            for index in range(len(Guess)):
                if index > 0 and index < len(Guess) - 1:
                    if Guess[index] == Guess[index - 1] and Guess[index] == Guess[index + 1]:
                        return not_a_word
            if len(word) != len(goal_word):
                return bad_num_letters
            else:
                return num_common_letters(goal_word, word)
    return The_Word_Word_Guessing_Game
...
def subsets(lst, n):
    ...
    """
    "*** YOUR CODE HERE ***"
    def add_to_each_element(lst1, num):
        result = []
        for elem in lst1:
            result += [[num] + elem]
        return result
    ...
    if len(lst) < n:
        ...
        result = add_to_each_element(subsets(lst[1:], n - 1), lst[0])
...
def filter_subsets(word, score, possible_subsets):
    ...
    "*** YOUR CODE HERE ***"
    result = []
    for subsetss in possible_subsets:
        if compatible(subsetss, score, word):
            result += [subsetss]
    return result
...
def make_deductions(possible_subsets, letters):
    ...
    present = []
    not_present = []
    appearence_counter = [0] * len(letters)
    def check_index(lst, letter):
    ...
    for elem in possible_subsets:
        for num in elem:
            appearence_counter[check_index(letters, num)] += 1
    for index in range(len(appearence_counter)):
        if appearence_counter[index] == 0:
            not_present += [letters[index]]
        if appearence_counter[index] == len(possible_subsets):
            present += [letters[index]]
