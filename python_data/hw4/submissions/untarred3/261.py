...
def subsets(lst, n):
    ...
    if n == 0:
        ...
        ss = subsets(lst[1:], n - 1)
        for elem in ss:
            elem.insert(0, lst[0])
        return ss + subsets(lst[1:], n)
...
def filter_subsets(word, score, possible_subsets):
    ...
    while i < len(possible_subsets):
        if not compatible(word, score, possible_subsets[i]):
            possible_subsets.remove(possible_subsets[i])
        else:
            i += 1
