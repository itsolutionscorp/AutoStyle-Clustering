...
def subsets(lst, n):
	...
	import itertools
	yy=itertools.combinations(lst, n)	
	return [list(row) for row in yy]	
...
def filter_subsets(word, score, possible_subsets):
	...
	"""
	"*** YOUR CODE HERE ***"
	num=len(possible_subsets)
	k=0
	yy=[ ]
	while k<=num-1:
		xx=make_word_from_list( possible_subsets[k] )
		num_x=num_common_letters(word, xx)
		if num_x == score:
			yy=yy+ [ possible_subsets[k] ]
		k=k+1
	return yy
...
def make_deductions(possible_subsets, letters):
	...
	while k <= num_l-1:
		...
		total=0
		while ii<=num-1:
			if yy in  possible_subsets[ii] :
				total_not=False
				total=total+1
			ii=ii+1
		if total_not == True:
			not_present =not_present + [yy]
		if total == num:
			present = present + [yy]
		k=k+1
