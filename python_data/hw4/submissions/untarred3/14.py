...
def compatible(guess, score, letter_list):
    ...
    return score == num_common_letters(letter_list, guess)
...
def filter_subsets(word, score, possible_subsets):
    ...
    for possibility in possible_subsets:
        if compatible(word, score, possibility):
            new_lst += [possibility]
    return new_lst
...
def make_deductions(possible_subsets, letters):
    ...
    present = []
    not_present = []
    length_subset = len(possible_subsets)
    for letter in letters:
        tally = 0
        for sub_lst in possible_subsets:
            if letter in sub_lst:
                tally += 1
                if tally == length_subset:
                    present += [letter]
        if tally == 0:
            not_present += letter
