...
def make_word_master(goal_word):
    ...
    def check_word(guess):
        ...
        if(guess_string == goal_string):
            return win_message
        elif(len(guess_string) != len(goal_string)):
...
def subsets(lst, n):
    ...
    result = []
    temp = [0] * n
    def generate(start_to_pick, length, result):
        if(length == n):
            result.append(list(temp))
        else:
            for i in range(start_to_pick, len(lst)):
                temp[length] = lst[i]
                generate(i + 1, length + 1, result)
...
def compatible(guess, score, letter_list):
    ...
    num = num_common_letters(goal_word, guess)
    if(num == score):
        return True
    else:
        return False
...
def make_deductions(possible_subsets, letters):
    ...
    for l in letters:
        ...
        if(in_all_list):
            present.append(l)
        elif(not in_the_list):
            not_present.append(l)
