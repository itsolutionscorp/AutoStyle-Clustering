...
def make_word_master(goal_word):
    ...
    def word_master(guess):
        len_goal = len(get_string(goal_word))
        goal = get_string(goal_word)
        guess_word = get_string(guess)
        len_guess = len(get_string(guess))
        letters_correct = num_common_letters(goal_word, guess)
        if is_valid_guess(guess_word) != True:
            return not_a_word
...
def subsets(lst, n):
    ...
    def adding_first(elem, list_of_lists):
        new_list = []
        for each_list in list_of_lists:
            new_list += [[elem] + each_list]
        return new_list
    if n > len(lst):
        return []
    if n == 0:
    ...
    without_first = subsets(rest, n)

    return adding_first(first, subsets(rest, n - 1)) + without_first
...
def filter_subsets(word, score, possible_subsets):
    ...
    """
    new_list = []
    for each_list in possible_subsets:
        if compatible(word, score, each_list):
            new_list += [each_list]
    return new_list
...
def make_deductions(possible_subsets, letters):
    ...
    for each_letter in letters:
        for each_sublist in possible_subsets:
            if each_letter in each_sublist and each_letter not in present:
                present += [each_letter]
            elif each_letter not in not_present:
                not_present += [each_letter]
    return present, not_present
    """
    count = 0
    repeated = len(possible_subsets)
    present = []
    not_present = []
    complete_list = [each for each in possible_subsets]
    sorted_list = complete_list.sort()
    for each in complete_list:
        if each in sorted_list:
            count += 1
        if count == 3:
            present.append(each)
            count = 0
        if count >= 1 and preserve != each:
