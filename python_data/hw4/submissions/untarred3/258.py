...
def make_word_master(goal_word):
    ...
    True
    """
    def make_word_master_return(guess):
        goal_lst = get_list(goal_word)
        guess_lst = get_list(guess)
        i,count = 0,0
        if is_valid_guess(guess) != True:
            return not_a_word
        elif len(guess_lst) != len(goal_lst):
            return bad_num_letters
        while i < len(goal_word):
            if goal_lst[i] == guess_lst[i]:
                count += 1
            i += 1
        if count == len(goal_lst):
            return win_message
        else:
            return num_common_letters(goal_word,guess)
    return make_word_master_return 
...
def subsets(lst, n):
    ...
    if n == 0:
        ...
        return subsets(lst[1::],n) + add(lst[0],subsets(lst[1::],n - 1))
...
def compatible(guess, score, letter_list):
    ...
    False
    """
    word_guess = make_word_from_list(letter_list)
    guesslst,i,j = get_list(guess),0,0
    while i < len(guesslst):
        j = i + 1
        while j < len(guesslst):
            if guesslst[i] == guesslst[j]:
                score -= 1
            j += 1
        i += 1
    if num_common_letters(guess,word_guess) == score:
        return True
    else:
        return False
...
def make_deductions(possible_subsets, letters):
    ...
    not_present = []
    lst = []
    count,i,j,k,p,q,a = 0,0,0,0,0,0,0
    while i < len(letters):
        j = 0
        while j < len(possible_subsets):
            k = 0
            while k < len(possible_subsets[j]):
                if possible_subsets[j][k] == letters[i]:
                    lst += [letters[i]]
                    count += 1
                    break
                k += 1
            j += 1
        if count == len(possible_subsets):
            present += [letters[i]]
        i += 1
    while p < len(letters):
        a = 0
