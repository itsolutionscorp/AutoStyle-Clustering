...
def make_word_master(goal_word):
    ...
    return guess_word_check

def powerset(lst):
    if lst:
        p = powerset(lst[1:])
        return p + [lst[:1] + x for x in p]
    else:
        return [[]]
...
def compatible(guess, score, letter_list):
    ...
    """
    "*** YOUR CODE HERE ***"
    no_repeats = set(guess)                     #Takes repeats out of guess
    if num_common_letters(no_repeats, letter_list) == score:
        return True
    else:
        return False
...
def filter_subsets(word, score, possible_subsets):
    ...
    for el in possible_subsets:
        if compatible(word, score, el) == True:
            filter_list += [el] 
    return filter_list 
...
def make_deductions(possible_subsets, letters):
    ...
    not_present = []
    "*** YOUR CODE HERE ***"
    join_subset = sum(possible_subsets, [])
    count = 0
    for el in letters:
        if join_subset.count(el) == len(possible_subsets):
            present += [el]
        elif join_subset.count(el) == 0:
            not_present += [el] 
