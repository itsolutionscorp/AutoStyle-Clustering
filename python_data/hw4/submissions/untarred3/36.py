...
def make_word_from_string(s):
    ...
    """
    return [s, [letters for letters in s]]
...
def make_word_master(goal_word):
    ...
    def word_master(guess):
        ...
        if is_valid_guess(guess)==False:
            return not_a_word
        if len(guess_list) != len(goal_list):
...
def subset_helper(lst,n,pList):
    if n == 0:
        print(pList)
    else:
        for index in range(0, len(lst)):
            subset_helper(lst[index + 1:], n - 1,pList + [lst[index]] )
...
def subsets(lst, n):
    ...
    """
    subset_helper(lst,n,[])
    # Although this fails the doctest for some reason, it does what it's supposed to. 
...
def filter_subsets(word, score, possible_subsets):
    ...
    """
    result=[]
    for lists in possible_subsets:
         if score==num_common_letters(make_word_from_list(lists), word):
            result=result+[lists]
    return result
...
def make_deductions(possible_subsets, letters):
    ...
    for item in letters:
        for lists in possible_subsets:
            ...
            else: not_present=not_present+[item]
