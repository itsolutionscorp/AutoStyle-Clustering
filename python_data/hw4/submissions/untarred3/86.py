...
def make_word_master(goal_word):
    ...
    def master(x):
        goal=goal_word
        if len(get_string(goal))!=len(get_string(x)):
            return bad_num_letters
        elif get_string(goal)==get_string(x):
            return win_message
        elif is_valid_guess(x)==False:
            return not_a_word
        else:
...
def subsets(lst, n):
    ...
    upper_bin_num=lower_bin_num*pow(2,len(lst)-n)
    for i in range(lower_bin_num,upper_bin_num+1):
        if check(bin(i)):
            temp=bin(i)[2:]
            while len(temp)<len(lst):
                temp='0'+temp
            bin_set+=[temp]

    return output(bin_set)
...
def compatible(guess, score, letter_list):
    ...
    """
    "*** YOUR CODE HERE ***"
    return num_common_letters(guess,make_word_from_list(letter_list))== score
...
def filter_subsets(word, score, possible_subsets):
    ...
    "*** YOUR CODE HERE ***"
    final_subsets=[]
    for i in range(0,len(possible_subsets)):
        if compatible(word, score, possible_subsets[i]):
            final_subsets+=[possible_subsets[i]]
...
def make_deductions(possible_subsets, letters):
    ...
    for i in range(0,len(letters)):
        for x in range(0,len(possible_subsets)):
            if letters[i] in possible_subsets[x]:
                count+=1
