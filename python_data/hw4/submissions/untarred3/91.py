...
def subsets(lst, n):
    ...
    def append_lst(initial, lsts):
        for i in range(len(lsts)): 
            lsts[i] = [initial] + lsts[i] 
        return lsts 

    return append_lst(lst[0], subsets(lst[1:], n)) + subsets(lst[1:], n)
...
def compatible(guess, score, letter_list):
    ...
    False
    """
    return num_common_letters(guess, letter_list) == score 
...
def filter_subsets(word, score, possible_subsets):
    ...
    [['a', 'b', 'e', 'l', 's'], ['b', 'l', 'e', 's', 't']]
    """
    return subsets(possible_subsets, score)
