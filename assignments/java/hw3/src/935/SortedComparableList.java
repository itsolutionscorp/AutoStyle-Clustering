...
public class SortedComparableList {
    ...
    public void insert(Comparable c) {
        ...
        //solution has a null check at the very beginning
        if (c.compareTo(head)<0) {
            tail=new SortedComparableList(head,tail);
           // head = new SortedComparableList(c, tail);
            head=c;
        }
        else {
            //create pointer variable
            SortedComparableList pointer = this;
            while(pointer.tail != null && pointer.tail.head != null) {
                if (c.compareTo(pointer.tail.head) >0) {
    ...
    public Comparable get(int i) {
        ...
        if (i==0) {
            ...
            //create pointer to go through list
            SortedComparableList pointer = this;
    ...
    public static SortedComparableList sublist(SortedComparableList L, int start, int len) {
       // return null; // REPLACE THIS LINE WITH YOUR SOLUTION
        if (len==0) {
          ...
          //  return sublist(L.tail, start + 1, len - 1);
    ...
    public void squish() {
        ...
        if (tail==null) {
            ...
            while(current!=null) {
                // had to reference answers for if else cases
