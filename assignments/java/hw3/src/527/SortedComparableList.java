...
public class SortedComparableList {
    ...
    public static SortedComparableList subTail(SortedComparableList L, int start) {
      ...
      int count = 0; 
      while ((count != start) && (curr.tail != null)) {
        curr = curr.tail; 
        count += 1; 
      }
      return curr; 
    ...
    public static SortedComparableList sublist(SortedComparableList L, int start, int len) {
        // return null; REPLACE THIS LINE WITH YOUR SOLUTION
      SortedComparableList curr = L; 
      Comparable end = curr.get(start + len - 1);  
      SortedComparableList a = new SortedComparableList(end, null); 

      while ((start + len -1) != start) {
        Comparable newHead = curr.get(start + len -2); 
        a = new SortedComparableList(newHead, a); 
        len -= 1; 
      }
