...
public class SortedComparableList {
  ...
  public void insert(Comparable c) {
    ...
    if (c.compareTo(head)<0){
      tail = new SortedComparableList(ptr.head, tail);
      head = c;
      return ;
    }
    while (ptr.tail != null && c.compareTo(ptr.tail.head) > 0){
      ptr = ptr.tail;
    }
    ...
    public static SortedComparableList sublist(SortedComparableList L, int start, int len) {
      ...
      return new SortedComparableList(L.head, sublist(L.tail, 0, len - 1));
        ...
        // while (ptr != null && start != 0){
        //   ptr = ptr.tail;
        //   start--;
        //   end--;
        // }
        // SortedComparableList endptr = ptr;
        // while(endptr != null && end!=0){
        //   endptr = endptr.tail;
        //   end--;
        // }
        // endptr.tail = null;
    ...
    public void squish() {
      SortedComparableList ptr = this;
      SortedComparableList ptrrear = tail;
      ...
      while (ptrrear != null){
        if (ptr.head.equals(ptrrear.head)){
          ptr.tail = ptr.tail.tail;
          ptrrear = ptr.tail;
        }
        else{
          ptr = ptr.tail;
          ptrrear = ptrrear.tail;
        }
      }
