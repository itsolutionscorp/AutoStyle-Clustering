...
public class SortedComparableList {
    ...
    public void insert(Comparable c) {
      if(c != null)
      {
        ...
        {
          SortedComparableList newList = this;
          while(newList.tail != null && c.compareTo(newList.tail.head) > 0)
          {
            newList = newList.tail;
          }
          newList.tail = new SortedComparableList(c, newList.tail);
        }
      }
    }
    ...
    public Comparable get(int i) {
      if(head != null || i > 0)
      {
          SortedComparableList getElem = this;
          while(getElem.tail != null && i > 0)
          {
            getElem = getElem.tail;
            i--;
          }
          return getElem.head;
      }
    ...
    public void extend(SortedComparableList that) {
      if(that != null)
      {
        SortedComparableList that2 = that;
        while(that2 != null)
        {
          insert(that2.head);
          that2 = that2.tail;
    ...
    public void squish() {
      if(tail != null)
      {
        SortedComparableList firstList = this;
        SortedComparableList secondList = tail;
        while (secondList != null)
        {
          if(firstList.head.equals(secondList.head))
          {
            firstList.tail = secondList.tail;
            secondList = firstList.tail;
          }
          else
          {
    ...
    public void twin() {
      ...
      {
        firstList.tail = new SortedComparableList(firstList.head, firstList.tail);
        firstList = firstList.tail.tail;

        // This should work too but it will modify the current list (Destructive way)
        // insert(firstList.head);
        // firstList = firstList.tail;
      }
