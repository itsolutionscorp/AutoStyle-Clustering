...
public class SortedComparableList {
    ...
    public void insert(Comparable c) {
        // REPLACE THIS LINE WITH YOUR SOLUTION
      SortedComparableList M = this;
      while(M.tail != null){
        if(c.compareTo(tail.head) < 0){
          M.tail = new SortedComparableList(c, M.tail);
          return;
        }
        M = M.tail;
      }
      if(M.tail == null){
        M.tail = new SortedComparableList(c, null);
      }
    ...
    public static void expungeTail(SortedComparableList L, int len) {
        SortedComparableList M = L;
        if(len == 0)
          M.tail = null;
        else
          expungeTail(M.tail, len - 1);
    }
    ...
    public void squish() {
      ...
      SortedComparableList M = this;
      while(M.tail != null){
        if(M.head.compareTo(M.tail.head) == 0)
          M.tail = M.tail.tail;
        else
          M = M.tail;
      }
