...
public class SortedComparableList {
      ...
      if (c!= null) {  
        if (this == null) {
            ...
            tail = null;
                ...
                Comparable dummy = head;
                head = c;
                tail = new SortedComparableList(dummy,tail);
        }   
        else {
            SortedComparableList dummy2 = this;
            while (dummy2.head.compareTo(c) < 0 && dummy2.tail != null) {
                dummy2 = dummy2.tail;
            }
            dummy2.tail = new SortedComparableList(c,dummy2.tail);
            }
      }
    } 
    ...
    private Comparable getRecursiveHelper(int i, SortedComparableList thisOne) {
            if (i == 0) {
                return thisOne.head;
            }
            else { 
                return getRecursiveHelper(i-1,thisOne.tail);   
            }
        }
    public Comparable get(int i) {
        return getRecursiveHelper(i,this);
    }

    /** Adds every item in THAT to this list. */
    public void extend(SortedComparableList that) {
        SortedComparableList trackableList = this;
        while (trackableList != null || that != null) {
            if (trackableList.head.compareTo(that.head) < 0) {
                trackableList = trackableList.tail;
            }
            else { // that's head is smaller
                trackableList.tail = new SortedComparableList(that.head, trackableList.tail);
                that = that.tail;
            }
        }
    ...
    public static SortedComparableList subTail(SortedComparableList L, int start) {
        ...
        SortedComparableList newList = new SortedComparableList(L.head,L.tail);
        return newList;*/
        if (start == 0) { // assuming start is always within the indices
            return L;
        }
        else {
    ...
    public static SortedComparableList sublist(SortedComparableList L, int start, int len) {
        ...
        else if (start > 0) { // this statement will keep executing until start == 0 
    ...
    public void squish() {
        while (this != null) {
            if (this.head.compareTo(this.tail.head == 0)) { // if current item and next item are same
                this = this.tail;
            }
