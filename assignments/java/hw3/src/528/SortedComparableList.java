...
public class SortedComparableList {
    ...
    public SortedComparableList(Comparable head0, SortedComparableList tail0) {
      if (tail0 == null || head0.compareTo(tail0.head) < 0) {
        head = head0;
        tail = tail0;
      }
      else {
        head = tail0.head;
        tail = new SortedComparableList(head0, tail0.tail);
      }
    }
    ...
    public void insert(Comparable c) {
        if (c != null) {
          if (c.compareTo(head) < 0) {
            ...
            while(sCL.tail != null && c.compareTo(sCL.tail.head) < 0) {
              sCL = sCL.tail;
            }
            sCL.tail = new SortedComparableList(c, sCL.tail);
          }
        }
    }
    ...
    public static SortedComparableList sublist(SortedComparableList L, int start, int len) {
        ...
        SortedComparableList sCL2 = new SortedComparableList(sCL.get(0), null);
        int i = 1;
        while (i < len) {
          sCL2.insert(sCL.get(i));
          i++;
        }
        return sCL2;
    ...
    public void squish() {
      if (tail != null) {
        ...
        SortedComparableList sCL = this.tail;
        while (sCL != null) {
          if (prev.head.equals(sCL.head)) {
            prev.tail = sCL.tail;
          }
          else {
            prev = prev.tail;
