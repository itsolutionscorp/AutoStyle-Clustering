...
public class SortedComparableList {
    ...
    public void insert(Comparable c) {
       if (c != null) {
        ...
        while (c.compareTo(pointer.tail.head) > 0 && pointer.tail != null) {
    ...
    public static SortedComparableList subTail(SortedComparableList L, int start) {
     ...
     if (L == null || start == 0) {
      return L;
        ...
        return subTail(pointer.tail, start - 1);
      }
     }   
    ...
    public static SortedComparableList sublist(SortedComparableList L, int start, int len) {
        ...
        if (start > 0) {
          return sublist(pointer.tail, start - 1, len);
        }
        return new SortedComparableList(pointer.head, sublist(pointer.tail, 0, len -1));
    }

    /** Removes items from L at position len+1 and later. */
    public static void expungeTail(SortedComparableList L, int len) {
        if (L.tail != null) {
          SortedComparableList previous = L;
          SortedComparableList current = L.tail;
          while (current != null) {
            if (previous.head.equals(current.head)) {
    ...
    public void squish() {
      if (tail != null) {
        ...
        SortedComparableList current = tail;
        while (current != null) {
          if (previous.head.equals(current.head)) {
            previous.tail = current.tail;
            current = previous.tail;
    ...
    public void twin() {
      SortedComparableList previous = this;
      while (previous != null) {
        previous.tail = new SortedComparableList(previous.head, previous.tail);
