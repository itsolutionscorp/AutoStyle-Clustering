...
public class SortedComparableList {
    ...
    public void insert(Comparable c) {
        // REPLACE THIS LINE WITH YOUR SOLUTION
      if (this == null || c.compareTo(this.head) < 0) {
        tail = new SortedComparableList(head, tail);
        head = c;
      }
      else {
        SortedComparableList templist = this;
        while (templist.tail != null && c.compareTo(templist.tail.head) > 0) {
          templist = templist.tail;
        }
        templist.tail = new SortedComparableList(c, templist.tail);
      }
    }
    ...
    public Comparable get(int i) {
        SortedComparableList templist = this;
        for (int j = 0; j < i && templist != null; j += 1) {
          templist = templist.tail;
        } 
    ...
    public static SortedComparableList subTail(SortedComparableList L, int start) {
        SortedComparableList templist = L;
        for (int j = 0; j < start && templist != null; j += 1) {
          templist = templist.tail;
        }
        return templist;
    }
    ...
    public static SortedComparableList sublist(SortedComparableList L, int start, int len) {
        SortedComparableList templist = new SortedComparableList(L.head, L.tail);
        for (int j = 0; j < start; j += 1) {
          templist = templist.tail;
        } 

        SortedComparableList templist2 = templist;
    ...
    public static void expungeTail(SortedComparableList L, int len) {
        // REPLACE THIS LINE WITH YOUR SOLUTION
      SortedComparableList templist = L;
      SortedComparableList mover = templist;
    ...
    public void twin() {
      ...
      SortedComparableList templist = this;
      while (templist != null) {
        templist.tail = new SortedComparableList(templist.head, templist.tail);
        templist = templist.tail.tail;
      }
    }
