...
public class SortedComparableList {
    ...
    public void insert(Comparable c) {
      if (c != null) {
        if (c.compareTo(head) < 0) {
          ...
          while (p.tail != null && c.compareTo(p.tail.head) > 0) {
            p = p.tail;
          }
    ...
    public static SortedComparableList sublist(SortedComparableList L, int start, int len) {
      SortedComparableList smaller = subTail(L, start);
      if (len == 0) {
        return null;
      }
      SortedComparableList p = smaller;
      while (len > 1) {
        p = p.tail;
        len -= 1;
      }
    ...
    public void squish() {
      if (this != null) {
        SortedComparableList p1 = this;
        SortedComparableList p2 = this.tail;
        while (p2 != null) {
          if (p1.head == p2.head) {
            p1.tail = p2.tail;
            p2 = p1.tail;
