...
public class SortedComparableList {
    ...
    public SortedComparableList(){
        // how the fuck am I supposed to distinguish between an empty list
        // and a list whose only element is 0?
        head = 0;     
        tail = null;
    }
    ...
    public static SortedComparableList subTail(SortedComparableList L, int start) {
        if(start == 0)  {
            if (L == null)
                return null;
            SortedComparableList output = new SortedComparableList(L.head, null);
            output.extend(L.tail);
            return output;
        }
        else
    ...
    public static SortedComparableList sublist(SortedComparableList L, int start, int len) {
        SortedComparableList s0 = L.subTail(L, start), tracker = s0;
        for(int i = 1; i < len; i++)
     ...
     **/
    public void twin() {
        this.extend(this.subTail(this, 0));
    }
