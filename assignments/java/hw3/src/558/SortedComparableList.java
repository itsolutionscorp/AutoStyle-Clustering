...
public class SortedComparableList {
    ...
    public void insert(Comparable c) {
        if (c!=null) {
            if (c.compareTo(head) < 0) { //c < head, so c goes before
    ...
    public static SortedComparableList sublist(SortedComparableList L, int start, int len)  {
        ...
        SortedComparableList sub = subTail(L, start); // returns list from start to end of list
        int counter = 1;
    ...
    public void squish() {
        ...
        while (curr != null) {
            if (prev.head.equals(curr.head)) {
                ...
                prev = prev.tail;
                curr = prev.tail; // curr always one ahead of prev
            }
        }
    }
