...
public class SortedComparableList {
    ...
    public void insert(Comparable c) {
        // immediately done, insert at beginning
        ...
        while (ptr.tail != null && !inserted){
            // insert in between current and next
            if (c.compareTo(ptr.head) > 0 && c.compareTo(ptr.tail.head) <= 0){
                ptr.tail = new SortedComparableList(c, ptr.tail);
                inserted = true;
            }
            ptr = ptr.tail;
        }
        if (!inserted){
            ptr.tail = new SortedComparableList(c, null);
        }
    }
    ...
    private static SortedComparableList copy(SortedComparableList original){
        if (original.tail == null){
            return new SortedComparableList(original.head, null);
        }
        else{
            return new SortedComparableList(original.head, copy(original.tail));
        }
    }
    ...
    public static SortedComparableList sublist(SortedComparableList L, int start, int len) {
        SortedComparableList uncut = subTail(L, start);
        SortedComparableList ptr = uncut;
        int i = 1;
        while (i < len){
            ptr = ptr.tail;
            i++;    
    ...
    public void squish() {
        ...
        // SortedComparableList ptr = this;
        // while (ptr.tail != null){
        //     Comparable curr = head;
        //     // removes everything after that is curr;
        //     while (ptr.tail.head.equals(curr)){
     ...
     **/
    public void twin() {
        this.extend(SortedComparableList.copy(this));
    }
