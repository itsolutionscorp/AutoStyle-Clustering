...
public class SortedComparableList {
    ...
    public void insert(Comparable c) {
      if (c != null) {
        if (c.compareTo(head) < 0) {
          ...
          head = c;
          tail = new SortedComparableList(temp, tail);
        }
        else {
    ...
    public void extend(SortedComparableList that) {
        SortedComparableList resorter = that;
        while (resorter != null) {
          insert(resorter.head);
          resorter = resorter.tail;
        }
    }
    ...
    public static SortedComparableList sublist(SortedComparableList L, int start, int len) {
        ...
        SortedComparableList sublister = new SortedComparableList(L.head, sublist(L.tail, 0, len - 1));
        return sublister;
    }
    ...
    public void squish() {
        if (tail != null) {
          SortedComparableList frontchecker = this;
          SortedComparableList backchecker = tail;
          while (backchecker != null) {
            if (frontchecker.head.equals(backchecker.head)) {
              frontchecker.tail = backchecker.tail;
              backchecker = backchecker.tail;
            }
            else {
              frontchecker = frontchecker.tail;
              backchecker = backchecker.tail;
            }
          }
        }
     ...
     **/
    public void twin() {
        SortedComparableList sentinel = this;
        while (sentinel != null) {
          sentinel.tail = new SortedComparableList(sentinel.head, sentinel.tail);
          sentinel = sentinel.tail.tail;
        }
    }
