...
public class SortedComparableList {
    ...
    public static SortedComparableList subTail(SortedComparableList L, int start) {
        if(start == 0 || L == null) return L;
        else return subTail(L.tail, start-1);
    }
    ...
    public static SortedComparableList sublist(SortedComparableList L, int start, int len) {
        ...
        else if(start > 0) return sublist(L.tail, start-1, len);
        else return new SortedComparableList(L.head, sublist(L.tail, 0, len-1));
    }
    ...
    public void squish() {
        if(tail == null) return;
        else if(head.equals(tail.head)){
            tail = tail.tail;
        }
        if(tail != null) tail.squish();
    }
     ...
     **/
    public void twin() {
        SortedComparableList c = this;
        while(c != null){
            SortedComparableList copy = new SortedComparableList(c.head, c.tail);
            c.tail = copy;
            c = c.tail.tail;
        }
    }
