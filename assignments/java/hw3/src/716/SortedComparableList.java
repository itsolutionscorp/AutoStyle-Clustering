...
public class SortedComparableList {
    ...
    public void insert(Comparable c) {
      ...
      if (c.compareTo(head) < 0) { //if c is already the smallest number
        tail = new SortedComparableList(head, tail);
        head = c;
      }
      else {
        SortedComparableList current = this; 
        while (current.tail != null && c.compareTo(current.tail.tail) > 0) { 
    ...
    public static SortedComparableList sublist(SortedComparableList L, int start, int len) {
      if (len == 0) { //why don't you want to check if L is null here??
        return null;
      }
      else if (start > 0) {
