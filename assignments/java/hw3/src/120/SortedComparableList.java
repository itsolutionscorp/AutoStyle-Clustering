...
public class SortedComparableList
{
	...
	while (temp.head.compareTo(c) < 0 && temp.tail != null)
	{
	    temp = temp.tail;
	}
	if (temp.head.compareTo(c) == -1)
	{
    ...
    /**
     * Returns the i-th Comparable in this list. The first element, which is in
	...
	SortedComparableList sublist = subTail(L, start);
	SortedComparableList temp = new SortedComparableList(sublist.head, null);
	SortedComparableList temp2 = temp;
	while (len > 1 && sublist.tail != null)
	{
	    temp2.tail = new SortedComparableList(sublist.tail.head, null);
	    len--;
	    temp2 = temp2.tail;
	...
	while (temp != null)
	{
	    Comparable consecutive = temp.head;
	    while (temp.tail != null)
	    {
		if (consecutive.compareTo(temp.tail.head) == 0)
		{
	...
	String sep;
	sep = "(";
	int cycleLocation = this.detectCycles(this);
	int cnt = 0;
