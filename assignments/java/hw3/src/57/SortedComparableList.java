...
public class SortedComparableList {
    ...
    public void insert(Comparable c) {
      if (c != null) {
        if (c.compareTo(head) <= 0) {
          ...
          while (ref.tail != null && c.compareTo(ref.tail.head) >= 0) {
            ref = ref.tail;
          }
    ...
    public void squish() {
        if (tail == null) return;
        else if (head.compareTo(tail.head) == 0) {
          tail = tail.tail;
          squish();
        }
        else {
     ...
     **/
    public void twin() {
      SortedComparableList place = this;
      while (place != null) {
        place.tail = new SortedComparableList(place.head, place.tail);
        place = place.tail.tail;
      }
    }
