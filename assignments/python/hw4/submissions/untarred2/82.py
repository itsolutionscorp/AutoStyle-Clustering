...
def make_word_master(goal_word):
    ...
    def like_make_word_master(guess):
...
def compatible(guess, score, letter_list):
    ...
    False
    """
    if score == num_common_letters(make_word_from_list(letter_list), guess):
        return True
    return False
...
def filter_subsets(word, score, possible_subsets):
    ...
    for letter in possible_subsets:
        if compatible(letter, score, word) == True:
            new_list += [letter]
    return new_list
...
def make_deductions(possible_subsets, letters):
    ...
    for character in letters:
        counter_for_present = 0
        counter_for_not_present = 0
        for sets in possible_subsets:
            if character in sets:
                counter_for_present += 1
                if counter_for_present == len(possible_subsets):
                    present += [character]
            else:
                counter_for_not_present += 1
                if counter_for_not_present == len(possible_subsets):                    
