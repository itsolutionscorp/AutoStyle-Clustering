...
def get_string(word):
    ...
    "*** YOUR CODE HERE ***"
    result = ''
    for item in range(len(word('test'))):
        result += word('test')[item]
    return result
...
def get_list(word):
    ...
    """
    "*** YOUR CODE HERE ***"
    return [item for item in word('test')]
...
def make_word_master(goal_word):
    ...
    def checker(guess):
        guess_letters = get_list(guess)
        goal_letters = get_list(goal_word)
        if is_valid_goal_word(goal_word) and is_valid_guess(guess):
            if (4<= len(goal_letters) <= 7) and (len(guess_letters) == len(goal_letters)):
                if goal_letters == guess_letters:
                    return win_message
                else:
...
def subsets(lst, n):
    ...
    "*** YOUR CODE HERE ***"
    def helper(lst, n):
        def subsets_helper(elem, lst):
            if lst == []:
                return[]
            else:
                return [[elem] + lst[0]] + subsets_helper(elem, lst[1:])

        if lst == []:
            return [[]]
        else:
            first, rest = lst[0], lst[1:]
            without_first = helper(rest, n)
            return without_first + subsets_helper(first, without_first)
    return [sublist for sublist in helper(lst, n) if len(sublist) == n]
...
def filter_subsets(word, score, possible_subsets):
    ...
    for subset in possible_subsets:
        if num_common_letters(word, make_word_from_list(subset)) == score:
            solutions += [subset]
    return solutions
...
def make_deductions(possible_subsets, letters):
    ...
    for letter in letters:
        for i in range(len(possible_subsets)):
            if letter in possible_subsets[i]:
                letter_count += 1
        if subset_count == letter_count:
            present += letter
        elif letter_count == 0:
            not_present += letter
        letter_count = 0
