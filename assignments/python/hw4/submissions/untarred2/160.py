...
def subsets(lst, n):
    ...
    first = [lst[0]]
    rest = lst[1:]
    subs = subsets (rest, n-1)
    while count < len(subs):
        subs[count] = first + subs[count]
        count += 1
    return subs + subsets(rest, n)
...
def filter_subsets(word, score, possible_subsets):
    ...
    for elem in possible_subsets:
        if num_common_letters(word, elem) == score:
            count += [elem] 
    return count
