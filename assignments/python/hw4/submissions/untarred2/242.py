...
def make_word_master(goal_word):
    ...
    def word_master(guess_word):
        if guess_word==goal_word:
            return win_message
        elif len(get_string(guess_word))!=len(get_string(goal_word)):
...
def subsets(lst, n):
    ...
    def add_all(x,lst):
        if len(lst)==1:
            ...
            return [x+lst[0]]+add_all(x,lst[1:])
    subs=[]
    if n>len(lst):
        return []
    elif n==len(lst):
        ...
        for i in range(n):
            subset=add_all(lst[i],subsets(lst[i:],n-1))
            subs+=[subset]
    return subs
...
def compatible(guess, score, letter_list):
    ...
    False
    """
    word_master=make_word_master(make_word_from_list(letter_list)) 
    if word_master(guess)==score:
        return True
    else:
        return False
...
def filter_subsets(word, score, possible_subsets):
    ...
    for i in range(len(possible_subsets)):
        subset=make_word_from_list(possible_subsets[i])
        if score==num_common_letters(word,subset):
            possible+=[possible_subsets[i]]
    return possible
