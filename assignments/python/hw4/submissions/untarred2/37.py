...
def make_word_master(goal_word):
    ...
    def check_word(guess_word):
        total_letters = len(get_list(goal_word))
        gets_correct = num_common_letters(goal_word, guess_word)
        if (len(get_list(guess_word)) != total_letters):
            return bad_num_letters
        elif is_valid_guess(guess_word) == False:
            return not_a_word
        elif (gets_correct == total_letters and goal_word == guess_word):
            return win_message
        else:
            return gets_correct
    return check_word
...
def subsets(lst, n):
    ...
    all_subs = []
    def get_subsets(lst, n, k, choose_from, answers):
        if (len(lst) == n):
            answers += [lst]
            return
        elif k == len(choose_from):
            return 
        else:
            get_subsets(lst, n, k+1, choose_from, answers)
            lst = lst + [choose_from[k]]
            get_subsets(lst, n, k+1, choose_from, answers)

    get_subsets([], n, 0, lst, all_subs)
    all_subs.sort()
    return all_subs
...
def filter_subsets(word, score, possible_subsets):
    ...
    while index < len(possible_subsets):
        #print(index, ':', possible_subsets[index])
        #if (compatible(word, score, possible_subsets[index]) == False):
            #possible_subsets.pop(index)
        if (compatible(word, score, possible_subsets[index]) == True):
            result += [possible_subsets[index]]
        index += 1
    return result
...
def make_deductions(possible_subsets, letters):
    ...
    present = []
    not_present = []
    subset_count = len(possible_subsets)
    for i in range(len(letters)):
        matching_subsets = filter_subsets( make_word_from_string(letters[i]), 1, possible_subsets)
        if len(matching_subsets) == subset_count:
            present += [letters[i]]
        elif len(matching_subsets) == 0:
