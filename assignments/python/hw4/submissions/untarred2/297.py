...
def subsets(lst, n):
    ...
    if lst == []:
        ...
        for el in [subsets(lst[1:], n)]:
            with_first = [[lst[0]] + el]
            return with_first + [subsets(lst[1:], n-1)]
...
def compatible(guess, score, letter_list):
    ...
    """
    "*** YOUR CODE HERE ***"
    let_lst = make_word_from_list(letter_list)
    if num_common_letters(let_lst, guess) == score:
        return True
    else:
        return False
...
def filter_subsets(word, score, possible_subsets):
    ...
    """
    "*** YOUR CODE HERE ***"
    first, rest = possible_subsets[0], possible_subsets[1:]
    if len(possible_subsets) == 1:
        if compatible(word, score, first):
            return first
    if compatible(word, score, first):
        return [first, filter_subsets(word, score, rest)]
    else:
        return filter_subsets(word, score, rest)
...
def make_deductions(possible_subsets, letters):
    ...
    not_present = []
    first, rest = possible_subsets[0], possible_subsets[1:]
    master_list = first + rest
    count = 0
    for el in master_list[1:]:
        if master_list[0] == el:
            count += 1
