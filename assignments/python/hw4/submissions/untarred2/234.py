...
def make_word_master(goal_word):
    ...
    def guess_word(guess):
        word = get_list(goal_word)
        guess_w = get_list(guess)
        if word == guess_w:
            return win_message
        elif is_valid_guess(guess) == False:
            return not_a_word
        elif len(guess_w) > len(word) or len(guess_w) < len(word):
...
def subsets(lst, n):
    ...
    if n == 0:
        ...
        return [[first]] + subsets(rest, n)
    return subsets(rest, n) + [[first] + subsets(rest, n-1)] #(subsets(rest, n) + [first] + i for i in subsets(rest, n - 1))
