...
def make_word_master(goal_word):
    ...
    True
    """
    def guess_word_master(guess):
        guess_list = get_list(guess)
        goal_word_list = get_list(goal_word)
        if is_valid_guess(guess) == True:
            if len(guess_list) != len(goal_word_list):
                return bad_num_letters
            elif guess_list == goal_word_list:
                return win_message
            else:
...
def subsets(lst, n):
    ...
    if n == 0:
        ...
        return [lst]
    a = subsets(rest,n) 
    b = insert_every(first, subsets(rest, n-1))
    return a + b
