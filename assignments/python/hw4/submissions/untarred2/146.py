...
def get_string(word):
    ...
    become_string = ''
    for letter in word:
        become_string += letter
    return become_string
...
def make_word_master(goal_word):
    ...
    def word_master(guess):
        ...
        if is_valid_guess(guess) != True:
            ...
            return num_common_letters(make_word_from_string(goal), make_word_from_string(guess))
    return word_master
...
def subsets(lst, n):
    ...
    if lst == [] and n == 0:
        ...
        for elem in subsets(lst[1:], n-1):
            second_list += [[lst[0]] + elem]
        return subsets(lst[1:], n) + second_list
