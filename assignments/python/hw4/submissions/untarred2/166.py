...
def make_word_master(goal_word):
    ...
    def wordmaster(word):
        if not is_valid_guess(word):
            ...
            return win_message
        elif not len(word) == len(goal_word):
...
def subsets(lst, n):
    ...
    if lst == []:
        ...
        a = [[lst[0]] + elem for elem in subsets(lst[1:], n - 1)]
        b = [elem for elem in subsets(lst[1:], n)]
        return a + b
...
def compatible(guess, score, letter_list):
    ...
    False
    """
    def returnscore(guess, score, letter_list):
        if len(letter_list) == 1 and letter_list[0] in guess:
            return 1
        elif len(letter_list) == 1:
            return 0
        elif letter_list[0] in guess:
            return 1 + returnscore(guess, score, letter_list[1:])
        else:
            return returnscore(guess, score, letter_list[1:])
    return returnscore(guess, score, letter_list) == score
...
def make_deductions(possible_subsets, letters):
    ...
    present = []
    not_present = []
    def givepresence(letter, list):
        if len(list) == 1 and letter in list[0]:
            return True
        elif not letter in list[0]:
            return False
        else:
            return givepresence(letter, list[1:])
    def giveabsence(letter, list):
        if len(list) == 1 and not (letter in list[0]):
            return True
        elif letter in list[0]:
            return False
        else:
            return giveabsence(letter, list[1:])
    for letter in letters:
        if givepresence(letter, possible_subsets):
            present.append(letter)
        elif giveabsence(letter, possible_subsets):
