...
def subsets(lst, n):
    ...
    if n == 0:
        ...
        for numbers in subsets(lst[1:], n-1):
            tracker += [[lst[0]] + numbers]
        return tracker + subsets(lst[1:], n) 
...
def filter_subsets(word, score, possible_subsets):
    ...
    subsets_true = []
    for tests in possible_subsets:
        if compatible(make_word_from_list(tests), score, get_list(word)):
            subsets_true = subsets_true + [tests]
    return subsets_true
...
def make_deductions(possible_subsets, letters):
    ...
    present = []
    not_present = []
    for tests in letters:
        counter = 0
        for objects in possible_subsets:
            if tests in objects:
                counter = counter + 1
        if counter == len(possible_subsets):
            present = present + [tests]
        elif counter == 0:
            not_present = not_present + [tests]
