...
def make_word_master(goal_word):
    ...
    True
    """
    def game(player_guess):
        correct_Answer = get_string(goal_word)
        attempt = get_string(player_guess)
        if is_valid_guess(attempt) == False:
            return not_a_word
        elif attempt == correct_Answer:
            return win_message
        elif len(correct_Answer) == len(attempt):
            return num_common_letters(correct_Answer,attempt)
        else:
...
def subsets(lst, n):
    ...
    [2, 3, 4]
    """
    '''badass dup_patch stays under the abstraction barrier and removes duplications in the list, yo'''
    def dup_patch(lst):
        result = []
        for elem in lst:
    ...
    def recursion(lst,n):
        ...
        if len(lst) < n:
            ...
            index = n - 1
            while index < len(lst):
                result = result + [lst[:n-1] +[lst[index]]]
                index += 1
            y = [lst[0]] + lst[2:]
            z = lst[1:]
            return result + recursion(y,n) + recursion(z,n)
    return dup_patch(recursion(lst,n))
...
def compatible(guess, score, letter_list):
    ...
    False
    """
    guess0 = get_list(guess)
    mutable_list = letter_list
    index = 0
    possible_score = 0
    while index < len(letter_list):
        if letter_list[index] in guess0:
            possible_score += 1
        index += 1
...
def make_deductions(possible_subsets, letters):
    ...
    for ele in letters:
        ...
        while index < len(possible_subsets):
            if ele in possible_subsets[index]:
                pres_meter += 1
            index += 1
        if pres_meter == len(possible_subsets):
            present = present +[ele]
        elif pres_meter == 0:
            not_present = not_present + [ele]
