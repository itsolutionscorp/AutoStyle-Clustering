...
def make_word_master(goal_word):
    ...
    "*** YOUR CODE HERE ***"
    def helper(guess):
        if str(guess) == str(goal_word):
            return win_message
        elif not is_valid_guess(guess):
            return not_a_word
        elif len(str(guess)) != len(str(goal_word)):
...
def subsets(lst, n):
    ...
    if lst == []:
        return []
    for e in subsets(lst[1:], n-1):
        total += [[lst[0]]+e]
    total += subsets(lst[1:], n)
    return total
...
def filter_subsets(word, score, possible_subsets):
    ...
    "*** YOUR CODE HERE ***"
    lst = []
    if possible_subsets == [] or len(get_list(word)) == 0 or score == 0:
        return []
    for e in possible_subsets:
        if num_common_letters(make_word_from_list(e), word) == score:
            lst += [e]
    return lst
