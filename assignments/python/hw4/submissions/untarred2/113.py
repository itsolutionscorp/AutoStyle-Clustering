...
def make_word_master(goal_word):
    ...
    def foo(guess):
        if is_valid_guess(guess) == False:
            return not_a_word
        elif len(mwfs(goal_word)) != len(mwfs(guess)):
...
def subsets(lst, n):
    ...
    if len(lst) == 0:
        ...
        return []
    short_subsets = subsets(lst[1: ], n - 1)
    new_subsets = []
    for subset in short_subsets:
        subset = [lst[0]] + subset
        new_subsets += [subset] 
    return new_subsets + subsets(lst[1:], n)
...
def compatible(guess, score, letter_list):
    ...
    """
    mwfs = make_word_from_string
    if num_common_letters(mwfs(letter_list), guess) == score:
        return True
    return False
...
def filter_subsets(word, score, possible_subsets):
    ...
    while i < len(possible_subsets):
        if num_common_letters(word, mwfl(possible_subsets[i])) == score:
            matching_set += [possible_subsets[i]]
        i += 1
    return matching_set
...
def make_deductions(possible_subsets, letters):
    ...
    i = 0
    while i < len(possible_subsets):
        total_list += possible_subsets[i]
        i += 1
    i = 0
    while i < len(letters):
        if num_common_letters(mwfl(letters[i]), mwfl(total_list)) == 0:
            not_present += letters[i]
        i += 1
    i = 0
    while i < len(possible_subsets):
        letters = overlap(letters, possible_subsets[i])
        i += 1
    i = 0
    while  i < len(letters):
        present += letters[i]
