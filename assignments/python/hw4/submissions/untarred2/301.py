...
def make_word_from_string(s):
    ...
    while n < len(s):
        lst = lst + [s[n]]
        n += 1
    def selector(a): #picks one or the other
        if a == 'car':
            return s
        elif a == 'cdr':
            return lst
...
def get_string(word):
    ...
    """
    "*** YOUR CODE HERE ***"
    return word('car') # This will be the string component of the 'word' list
...
def get_list(word):
    ...
    """
    "*** YOUR CODE HERE ***"
    return word('cdr') # The list component of the 'word' list
...
def make_word_master(goal_word):
    ...
    def evaluate(guess_word):
        goal, guess = get_string(goal_word), get_string(guess_word)
        if guess == goal:
            return win_message
        elif len(guess) != len(goal):
...
def compatible(guess, score, letter_list):
    ...
    """
    "*** YOUR CODE HERE ***"
    if num_common_letters(guess, make_word_from_list(letter_list)) == score:
        return True
    else:
        return False
...
def filter_subsets(word, score, possible_subsets):
    ...
    """
    "*** YOUR CODE HERE ***"
    return [elem for elem in possible_subsets if compatible(word, score, elem)]
...
def make_deductions(possible_subsets, letters):
    ...
    while element < len(letters):
        subsets_in = [elem for elem in possible_subsets if letters[element] in elem]
        if len(subsets_in) == len(possible_subsets): # letter was present in all subsets
            present = present + [letters[element]]
        elif len(subsets_in) == 0:
            not_present = not_present + [letters[element]] # letter was present in no subsets
        element += 1
