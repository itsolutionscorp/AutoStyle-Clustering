...
def make_word_master(goal_word):
    ...
    def check (guess):
        ...
        if (goalword==guessword):
            return win_message
        if (len(goalword)!=len(guessword)):
            return bad_num_letters
        if (is_valid_guess(guess)!= True):
            return not_a_word
        else:
            total = num_common_letters (goal_word, guess)
            return total
    return check
...
def subsets(lst, n):
    ...
    [2, 3, 4]
    """
    def subset_helper(main_list, list_sofar, n, result):
        if len(list_sofar) == n:
            result += [list_sofar]
            return
        for i in range(min(n, len(main_list))):
            sofar = list_sofar[:]
            sofar += [main_list[i]]
            subset_helper(main_list[i+1:], sofar, n, result)

    result = []
    subset_helper(lst, [], n, result)
    return result
...
def compatible(guess, score, letter_list):
    ...
    False
    """
    letter_words = make_word_from_list (letter_list)
    if score == num_common_letters (letter_words, guess):
        return True
    else:
        return False
