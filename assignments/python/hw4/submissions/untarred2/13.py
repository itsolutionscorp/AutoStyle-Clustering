...
def make_word_from_list(lst):
    ...
    return [reduce(add, lst)] # ''.join(lst) for strings
...
def make_word_master(goal_word):
    ...
    def make_word_guess(x):
        guess = get_list(x)
        if is_valid_guess(x):
            if len(given) == len(guess):
                if num_common_letters(goal_word,x) != len(given):
                    return num_common_letters(goal_word,x)         
                else:
                    return win_message if winner(given, guess) \
                    else num_common_letters(goal_word, x)
            else:
                return bad_num_letters
        else:
            return not_a_word 
    return make_word_guess
...
def subsets(lst, n):
    ...
    if n > len(lst):
        return [] 
    for i in subsets(subs, n-1):
        lst_of_subs.append([lst[0]] + i)
    lst_of_subs += subsets(subs, n)
    return lst_of_subs 
...
def compatible(guess, score, letter_list):
    ...
    """
    "*** YOUR CODE HERE ***"
    checker , eval_word = None, letter_list
    if len(eval_word) == len(letter_list):
        checker = subsets(eval_word, len(eval_word))
        for i in checker:
            if num_common_letters(guess, make_word_from_list(i)) == score:
                return True
            return False
    else:
        return bad_num_letters
...
def make_deductions(possible_subsets, letters):
    ...
    not_present = []
    "*** YOUR CODE HERE ***"
    temp_pres = [set(s) for s in possible_subsets]
    temp_not = set(letters)
    present += list(set.intersection(*temp_pres))

    for i in range(len(possible_subsets)):
        for j in range(len(possible_subsets[i])):
            for k in letters:
                if possible_subsets[i][j] == k:
                    if len(temp_not.intersection(possible_subsets[i][j])) >= 1:
                        temp_not.remove(k)
    return present, list(temp_not)
