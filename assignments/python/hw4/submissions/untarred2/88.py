...
def subsets(lst, n):
    ...
    def combinations(n,r):
        if r == 1: #base case
            a = []
            for elem in n:
                a.append([elem]) #add the list of each element to the return value
            return a
        else:
            c = []
            for elem in combinations(n,r-1): #recursion
                a = []
                b = elem #the element in combination(n,r-1) (it's a list)
                for elem in n: #create new lists
                    if elem not in b: #if the element in n isn't in the list we're operating on
                        a += [(b + [elem])] #you add the element to the list
                c += a #you add the new lists
            for elem in c:
                elem = elem.sort()
            c.sort()
            d = []
...
# def incrementL(orig,new):
#     rV = []
#     for elem in new:
#         rV.append([orig, elem])
#     return rV
# def incrementO(aList,new):
#     rV = []
#     for i in range(len(aList)):
#         rV += incrementL(aList[i],new)
#     return rV
...
def compatible(guess, score, letter_list):
    ...
    """
    "*** YOUR CODE HERE ***"
    goal = make_word_from_list(letter_list)
    if num_common_letters(goal,guess) == score:
        return True
    return False
...
def make_deductions(possible_subsets, letters):
    ...
    for elem in letters:
        ...
        for i in range(len(possible_subsets)):
            if elem in possible_subsets[i]:
                must += 1
            if elem not in possible_subsets[i]:
                cant += 1
        if must == len(possible_subsets):
            present += elem
        if cant == len(possible_subsets):
            not_present += elem
