...
def make_word_from_string(s):
    """Creates an instance of the word from the string s.
    """
    "*** YOUR CODE HERE ***"
    assert str(s) == s
    return s
...
def make_word_from_list(lst):
    """Creates an instance of the word from the list of characters lst.
    """
    "*** YOUR CODE HERE ***"
...
def get_string(word):
    ...
    def string(lst, i):
        if i >= l - 1:
            ...
            return str(lst[i]) + string(lst, i + 1)
    if type(word) == type(''):
        return word
    else:
...
def make_word_master(goal_word):
    ...
    return lambda guess: correct(goal_word, guess)
...
def subsets(lst, n):
    ...
    if n == 0:
        ...
        for comb in subsets(lst[1:], n - 1):
            result += [lst[:1] + comb]
        return result + subsets(lst[1:], n)
...
def compatible(guess, score, letter_list):
    ...
    """
    "*** YOUR CODE HERE ***"
    assert len(guess) == len(letter_list)
    return score == num_common_letters(guess, letter_list)
...
def filter_subsets(word, score, possible_subsets):
    ...
    """
    "*** YOUR CODE HERE ***"
    return [letter for letter in possible_subsets if compatible(word, score, letter)]
...
def make_deductions(possible_subsets, letters):
    ...
    for letter in possible_subsets:
        for elem in letter:
            result += [elem]

    for letter in letters:
            if letter not in result and letter not in not_present:
                not_present += letter
    
    l = len(possible_subsets)

    for letter in result:
        if result.count(letter) == l and letter not in present:
            present += [letter]
