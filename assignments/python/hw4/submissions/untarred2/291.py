...
def make_word_master(goal_word):
    ...
    def feedback(guess_word):
        ...
        elif is_valid_guess(guess_word):
...
def subsets(lst, n):
    ...
    if len(lst) < n:
        ...
        recursive_step = subsets(lst[1:], n)
        new = add_to(recursive_step, lst[0])
        return recursive_step + new
...
def add_to(lst, elm):
    if lst == []:
        ...
        recursive_step = add_to(lst[1:], elm)
        return [lst[0] + [elm]] + recursive_step
...
def compatible(guess, score, letter_list):
    ...
    False
    """
    if not len(letter_list) == len(guess):
        return False
    elif make_word_master(make_word_from_list(letter_list))(guess) >= score:
        return True
    else:
        False
