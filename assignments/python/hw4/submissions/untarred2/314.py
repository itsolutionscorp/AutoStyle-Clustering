...
def make_word_master(goal_word):
    ...
    def master(guess):
        lst_goal, lst_guess = get_list(goal_word), get_list (guess)
        if len(lst_goal) != len(lst_guess):
            return bad_num_letters
        elif not is_valid_guess(guess):
            return not_a_word
        elif guess is goal_word:
            return win_message
...
def make_deductions(possible_subsets, letters):
    ...
    for letter in letters:
        letter_in_all, letter_in_none = True, False
        for subset in possible_subsets:
            if letter not in subset:
                letter_in_all = False
            elif letter in subset:
                letter_in_none = True
        if letter_in_none and letter_in_all:
            present += letter
        elif not letter_in_all and not letter_in_none:
            not_present += letter
