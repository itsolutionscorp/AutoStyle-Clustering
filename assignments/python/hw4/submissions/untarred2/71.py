...
def subsets(lst, n):
    ...
    """
    "*** YOUR CODE HERE ***"
    def subsetsHelper(final, currlst, remaining, k):
        if k == 0:
            final.append(currlst)
            return
        for i in range(len(remaining)+1-k):
            subsetsHelper(final, currlst+[remaining[i]], remaining[i+1:], k-1)
...
def compatible(guess, score, letter_list):
    ...
    string_guess = get_string(guess)
    string_goal_word = get_string(make_word_from_list(letter_list))
    if len(string_guess) != len(string_goal_word):
        return False
    elif num_common_letters(make_word_from_list(letter_list), guess) != score:
        return False
    else:
        return True
