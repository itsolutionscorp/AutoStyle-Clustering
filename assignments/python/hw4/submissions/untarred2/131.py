...
def make_word_master(goal_word):
    ...
    def word_master(guess):
        ...
        if not is_valid_guess(guess):
            return not_a_word
        elif len(goal_list) != len(guess_list):
...
def add_to_all(lst, num):
    """Returns a list of lists where every element is a list in lst with 
    num added to it.
    >>> x = [1,2]
    >>> y = add_to_all(x,0)
    >>> for subset in y:
    ...
    if lst == []:
        ...
        if type(lst[0]) is list:
            return [[num] + lst[0]] + add_to_all(lst[1:], num)
        else:
            return [[num] + [lst[0]]] + add_to_all(lst[1:], num)
...
def filter_subsets(word, score, possible_subsets):
    ...
    for subset in possible_subsets:
        if compatible(word, score, make_word_from_list(subset)):
            result += [subset]
    return result
