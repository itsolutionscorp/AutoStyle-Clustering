...
def num_common_letters(goal_word, guess):
    ...
    #add 1 to total for every elements that repeats, stop at two for each element that has already been counted as a repeat
    total = 0
...
def subsets(lst, n):
    ...
    if n == 0:
        return [[]]
    foo = [subsets(lst[1:], n - 1)]
    return subsets(lst[1:], n) + ((foo[0] + [lst[0]]) + (foo[1:] + [lst[0]]))
...
def filter_subsets(word, score, possible_subsets):
    ...
    for elem in possible_subsets:
        if compatible(guess, score, letter_list) == True:
            return elem
    return []
