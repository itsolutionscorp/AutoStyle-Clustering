...
def add_to_list(elem, lst):
    if lst == []:
        return []
    else:
        return [[elem] + lst[0]] + add_to_list(elem, lst[1:])
...
def subsets(lst, n):
    ...
    if lst == []:
        ...
        return subsets(lst[1:], n) + add_to_list(lst[0], subsets(lst[1:], n-1))
...
def make_deductions(possible_subsets, letters):
    ...
    for x in letters:
        ...
        for i in possible_subsets: 
            if x in i:
                counter += 1
        if len(possible_subsets) == counter:
            present += [x] 
        elif counter == 0:
            not_present += [x]
