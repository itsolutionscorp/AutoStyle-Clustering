...
def compatible(guess, score, letter_list):
    ...
    False
    """
    word = make_word_from_list(letter_list)
...
def make_deductions(possible_subsets, letters):
    ...
    not_present = []
    all_subsets = []
    length_of_all_subsets = len(possible_subsets)
    for subset in possible_subsets:
        all_subsets += subset
    for letter in letters:
        if letter not in all_subsets:
            not_present += [letter] 
        if all_subsets.count(letter) == length_of_all_subsets: 
                present += [letter] 
