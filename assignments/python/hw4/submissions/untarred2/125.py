...
def get_list(word):
    ...
    #return list(word) alternate solution
...
def make_word_master(goal_word):
    ...
    True
    """
    def return_function(guess):
        guess_string = get_string(guess)
        goal_word_string = get_string(goal_word)
        if len(guess_string) != len(goal_word_string):
            return bad_num_letters
        if guess_string == goal_word_string:
            return win_message
...
def subsets(lst, n):
    ...
    if lst == []:
        ...
        for i in range(len(lst)):
            new_list += [lst[:i]] + subsets(lst[i:], n) 
            newer_list += subsets(lst, n-1)
    return newer_list + new_list 
    #Tried a lot of different things but couldn't get it to work. 
...
def compatible(guess, score, letter_list):
    ...
    if score == num_common_letters(letter_list, guess):
        return True
    return False
    ...
    for letter in letters:
        ...
        for i in possible_subsets:
            if letter in i:
                present += [letter]
        if present == [letter] * 3:
            ultimate_present += [letter]
        elif letter not in present:
            ultimate_not_present += [letter]
    return ultimate_present, ultimate_not_present
