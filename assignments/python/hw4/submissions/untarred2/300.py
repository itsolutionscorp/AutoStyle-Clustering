# CS 61A Spring 2014
# Name: edwin.geraldo@berkeley.edu
# Login: cs61a-om
...
def make_word_master(goal_word):
    ...
    "*** YOUR CODE HERE ***"
    i = 0
    def helping_master_word(guess):
        len_str, len_goal = len(get_string(guess)), len(get_string(goal_word))
        if len_str != len_goal:
...
def subsets(lst, n):
    ...
    if not lst or len(lst) < n:
        ...
        temp = subsets(lst[1:], n-1)
        for i in temp:
            i += [lst[0]]
            l += [i]
        return subsets(lst[1:], n) + l
