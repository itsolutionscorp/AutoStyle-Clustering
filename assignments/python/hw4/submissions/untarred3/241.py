...
def subsets(lst, n):
    ...
    if n ==0:
        ...
        rslt = subsets(rest ,n) + [first + x for x in subsets(rest, n-1)]  
        return  rslt
...
def filter_subsets(word, score, possible_subsets):
    ...
    for i in range(len(possible_subsets)):
        if compatible(possible_subsets[i], score, goal):
            rslt += [possible_subsets[i]]
    return rslt 
...
def make_deductions(possible_subsets, letters):
    ...
    not_present = []
    "*** YOUR CODE HERE ***"
    def in_all(x, possible_subsets):
        rslt = True
        for i in range(len(possible_subsets)):
            if x not in possible_subsets[i]:
                rslt = False
        return rslt
    def in_none(x, possible_subsets):
        rslt = True
        for i in range(len(possible_subsets)):
            if x in possible_subsets[i]:
                rslt = False
        return rslt

    for i in range(len(letters)):
        if in_all(letters[i], possible_subsets):
            present += letters[i]
        if in_none(letters[i], possible_subsets):
