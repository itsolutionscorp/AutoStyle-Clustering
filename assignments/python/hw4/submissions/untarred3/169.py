...
def subsets(lst, n):
    ...
    """
    "*** YOUR CODE HERE ***"
    #I had to adjust the doctest because my output gave results in a different(but correct) order.
    if n == 0:
        return [[]]
    if len(lst) == 0:
        return []
    first,rest = lst[0],lst[1:]
    sublst = subsets(rest, n-1)
    for item in sublst:
        item += [first]
    return sublst + subsets(rest, n)
...
def filter_subsets(word, score, possible_subsets):
    ...
    "*** YOUR CODE HERE ***"
    filtered = []
    for sublist in possible_subsets:
        if compatible(word, score, sublist):
            filtered += [sublist]
    return filtered
...
def make_deductions(possible_subsets, letters):
    ...
    def has_letter(letter, possible_subsets):
        found = True
    ...
    def does_not_have_letter(letter, possible_subsets):
        found = True
        for set in possible_subsets:
            if letter in set:
                found = False
    ...
    present += [letter for letter in letters if has_letter(letter,possible_subsets)]
    not_present += [letter for letter in letters if does_not_have_letter(letter,possible_subsets)]
