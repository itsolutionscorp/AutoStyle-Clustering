...
def make_word_from_list(lst):
    ...
    """
    "*** YOUR CODE HERE ***"
    return make_word_from_string(get_string(lst))
...
def num_common_letters(goal_word, guess):
    ...
    """
    "*** YOUR CODE HERE ***"
    assert len(goal_word) == len(guess), 'Goal word and guess word must be of same length'
    assert len(goal_word) == len(set(goal_word)), 'Letters in goal word must be unique'
...
def make_word_master(goal_word):
    ...
    return lambda guess: win_message if win(guess) else (not_a_word if not_word(guess) else (bad_num_letters if bad_num(guess) else (num_common_letters(goal_word, guess))))
