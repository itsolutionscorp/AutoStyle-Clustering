...
def make_word_master(goal_word):
    ...
    def word_master(guess):
        if is_valid_guess(get_string(guess)) == False:
            return not_a_word
        elif len(goal_word) != len(guess):
...
def subsets(lst, n):
    ...
    [2, 3, 4]
    """
    def insert_first(first, uninserted_list):
        return [first + lists for lists in uninserted_list]
    ...
    if n ==0:
        ...
        return insert_first([first],subsets(rest,n-1)) + subsets(rest,n)
...
def compatible(guess, score, letter_list):
    ...
    False
    """
    word_score = make_word_master(letter_list)(get_list(guess))
    if word_score == score:
        return True
    return False
...
def make_deductions(possible_subsets, letters):
    ...
    for i in range(len(possible_subsets)):
        total_subset_list += possible_subsets[i]
    total_list = total_subset_list + letters

    for i in range(len(total_subset_list)):
        if total_subset_list.count(total_subset_list[i]) == len(possible_subsets):
            present += [ total_subset_list[i] ]
        present = list(set(present))

    for i in range(len(total_list)):            
        if total_list.count(total_list[i]) == 1:
            not_present += [ total_list[i] ]
