...
def make_word_master(goal_word):
    ...
    def evaluate_guess(guess):
        length_of_guess = len(get_list(guess))
        length_of_goal_word = len(get_list(goal_word))
        if is_valid_guess(guess) == False:
            return not_a_word
        elif length_of_guess != length_of_goal_word:
...
def subsets(lst, n):
    ...
    if n == 1:
        list_of_subsets = []
        for i in range(len(lst)):
            list_of_subsets += [[lst[i]]]
        return list_of_subsets
    elif n >= 2 and lst != []:
        list_of_subsets = []
        for subset in subsets(lst[1:len(lst)], n-1):
            list_of_subsets += [[lst[0]] + subset]
        list_of_subsets += subsets(lst[1:len(lst)], n)
        return list_of_subsets
    else:
        return []
...
def make_deductions(possible_subsets, letters):
    ...
    total_letters = []
    for subset in possible_subsets:
        total_letters += subset
    for letter in letters:
        if letter not in total_letters:
            not_present += letter
    result1 = []
    result2 = []
    for letter in possible_subsets[0]:
        if letter in letters:
            result1 += letter
    for subset in possible_subsets:
        for letter in subset:
            if letter in result1:
                result2 += letter
        result1 = result2
        result2 = []
    present = result1
