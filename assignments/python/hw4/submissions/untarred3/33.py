...
def make_word_master(goal_word):
    ...
    "*** YOUR CODE HERE ***"
    def func1(x):
        if len(get_string(goal_word)) != len(get_string(x)):
                return bad_num_letters
        if is_valid_guess(x):    
            if get_string(goal_word) == get_string(x):
                return win_message
            else:
...
def subsets(lst, n):
    ...
    if lst == []:
        ...
        return add_to_all(subsets(lst[1:], n - 1), lst[0]) + subsets(lst[1:], n)
...
def filter_subsets(word, score, possible_subsets):
    ...
    for i in possible_subsets:
        if compatible(make_word_from_list(i), score, get_list(word)):
            k += [i]
    return k


def is_in(letter, possible_subsets):
    for subset in possible_subsets:
        if letter not in subset:
            return False
    return True

def is_not_in(letter, possible_subsets):
    for subset in possible_subsets:
        if letter in subset:
            return False
    return True
...
def make_deductions(possible_subsets, letters):
    ...
    "*** YOUR CODE HERE ***"
    for i in letters:
        if is_in(i, possible_subsets):
            present += [i]
        if is_not_in(i, possible_subsets):
            not_present += [i]
