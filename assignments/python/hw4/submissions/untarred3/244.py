...
def make_word_master(goal_word):
    ...
    "*** YOUR CODE HERE ***"
    def helper(s):
        str_guess = get_string(s)
        str_goal = get_string(goal_word)
        if is_valid_guess(s):
            if str_guess == str_goal:
                return win_message
            elif len(str_guess) != len(str_goal):
...
def compatible(guess, score, letter_list):
    ...
    """
    "*** YOUR CODE HERE ***"
    common_letters = num_common_letters(guess, make_word_from_list(letter_list))
    return score == common_letters
