...
def make_word_master(goal_word):
    ...
    def word_master(guess):  
        ...
        if get_list(guess) == get_list(goal_word):
            return win_message
...
def subsets(lst, n):
    ...
    if n == 0:
        ...
        return insert(subsets(lst[1:], n - 1), lst[0]) + subsets(lst[1:], n)
...
def compatible(guess, score, letter_list):
    ...
    if len(guess) == len(letter_list):
        if num_common_letters(make_word_from_list(letter_list), make_word_from_string(guess)) == score:
            return True
        else:
            return False
...
def filter_subsets(word, score, possible_subsets):
    ...
    """
    result = []
    for one_subset in possible_subsets:
        if compatible(make_word_from_list(one_subset),score,get_list(word)):
            result += [one_subset]
    return result
...
def make_deductions(possible_subsets, letters):
    ...
    for elem in letters:
        for one_subset in possible_subsets:
            if elem not in one_subset:
                m += 1
            if elem in one_subset:
                n += 1
            if m == len(possible_subsets):
                not_present += elem
            if n == len(possible_subsets):
                present += elem
        m, n = 0, 0
