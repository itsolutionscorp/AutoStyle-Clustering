...
def compatible(guess, score, letter_list):
    ...
    False
    """
    if num_common_letters(guess, get_string(letter_list)) == score:
        return True
    return False
...
def make_deductions(possible_subsets, letters):
    ...
    first_list = set(possible_subsets[0])
    for subset in possible_subsets:
        first_list = first_list & set(subset)
    present = list(first_list)

    not_in_word = set(letters)
    for subset in possible_subsets:
        not_in_word = not_in_word - set(subset)

    not_present = list(not_in_word)
