...
def make_word_master(goal_word):
    ...
    True
    """
    def take_in_guess(guess_word):
    	if len(goal_word) != len(guess_word):
    		return bad_num_letters
    	if is_valid_guess(guess_word) == False:
    		return not_a_word
    	if get_string(goal_word) == get_string(guess_word):
    		return win_message
    	else:
    		return num_common_letters(goal_word, guess_word)
    return take_in_guess
...
def subsets(lst, n):
    ...
    def get(start_lst, num_left, combined_lst):
    	result_lst = []
    	if num_left == 1:
    		for elem1 in start_lst:
    			result_lst += [combined_lst + [elem1]]
    		return result_lst
    	else:
    		n = 0
    		for elem in start_lst[:1 - num_left:]:
    			n += 1
    			result_lst += get(start_lst[n::], num_left - 1, combined_lst + [elem])
    		return result_lst
    return get(lst, n, [])
...
def compatible(guess, score, letter_list):
    ...
    False
    """
    #print(num_common_letters(guess, letter_list))
...
def filter_subsets(word, score, possible_subsets):
    ...
    """
    result_lst = []
    for filter_subset in possible_subsets:
    	if compatible(word, score, filter_subset) == True:
    		result_lst += [filter_subset]
    return result_lst
...
def make_deductions(possible_subsets, letters):
    ...
    for elem_letter in letters:
    	exist_true = False
    	exist_false = False
    	for deduction_subset in possible_subsets:
    		if (elem_letter in deduction_subset) == True:
    			exist_true = True
    		else:
    			exist_false = True
    	if exist_true == True and exist_false == False:
    		present += elem_letter
    	elif exist_true == False and exist_false == True:
    		not_present += elem_letter
