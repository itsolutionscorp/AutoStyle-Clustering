...
def subsets(lst, n):
    ...
    if n == 0:
        return []
    lst_with_0 = lst[0] + subsets(lst[1:], n-1)
    return lst_with_0 + subsets(lst[1:], n)
...
def filter_subsets(word, score, possible_subsets):
    ...
    """
    correct_subsets = []
    if num_common_letters(possible_subsets, word) == score:
        correct_subsets += possible_subsets
...
def make_deductions(possible_subsets, letters):
    ...
    present = []
    not_present = []
    if letters[0] in possible_subsets:
        present = letters[0] + present
    else:
        not_present = letters[0] + not_present
    letters = letters[1:]
