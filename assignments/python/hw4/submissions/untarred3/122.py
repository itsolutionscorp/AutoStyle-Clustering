...
def make_word_from_string(s):
    ...
    """
    return lambda: [letter for letter in s]
...
def make_word_master(goal_word):
    ...
    def word_master(guess_word):
        if len(get_list(goal_word)) != len(get_list(guess_word)):
...
def subsets(lst, n):
    ...
    for index in range(len(lst) + 1 - n):
        rest = subsets(lst[index + 1:], n - 1)
        combos += [[lst[index]] + subset for subset in rest]
    return combos
...
def play_game(word=""):
    ...
    if not word:
        ...
        word = make_word_from_string(word)
        if not is_valid_goal_word(word):
            return not_a_word
