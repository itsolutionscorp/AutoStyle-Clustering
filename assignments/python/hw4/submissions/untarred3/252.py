...
def make_word_from_list(lst):
    ...
    "*** YOUR CODE HERE ***"
    """
    solution without recursion using special python syntax:
    
    return "".join(lst)
    """
    
    #other solution shown below:
    if len(lst) == 1:
        return str(lst[0])
    return str(lst[0]) + make_word_from_list(lst[1:])
...
def make_word_master(goal_word):
    ...
    def word_master(guess):
        if len(get_string(goal_word)) != len(get_string(guess)):
            return bad_num_letters
        if is_valid_guess(guess):
            if get_string(guess) == get_string(goal_word):
                return win_message
...
def subsets(lst, n):
    ...
    if lst == []:
        return list  
    if n == 1: 
        list += [[elem] for elem in lst]            
    if n > 1:
        for elem in lst:
            if len(lst[(lst.index(elem)):]) < n:
                return(list) 
            list += [[elem] + x for x in subsets(lst[(lst.index(elem))+1:], n-1)]
    return list 
    
    """
	way to simplify code is to use the for loop with an i:
...
def make_deductions(possible_subsets, letters):
    ...
    def create_new_list(possible_subsets):
        if possible_subsets[1:] == []:
            return possible_subsets[0]
        return possible_subsets[0] + create_new_list(possible_subsets[1:])
    new_list = create_new_list(possible_subsets)
    not_present = [elem for elem in letters if new_list.count(elem) == 0]
    present = [elem for elem in letters if new_list.count(elem) == len(possible_subsets)]
    return present, not_present
    """
    
    #solution using recursion shown below:
    def get_present(lst, letters):
        for elem in letters:
            new = [] + [elem for elem in letters if elem in lst[0]] 
            if len(lst)==1:
                present = [] + new
                return present 
            return get_present(lst[1:], new)
    def get_not_present(lst, letters):
        for elem in letters:
            new = [] + [elem for elem in letters if elem not in lst[0]] 
            if len(lst)==1:
                not_present = [] + new
                return not_present 
            return get_not_present(lst[1:], new)
    present = get_present(possible_subsets, letters)
    not_present = get_not_present(possible_subsets, letters)
    return present, not_present
