...
def subsets(lst, n):
    ...
    return a
        ...
        # if len(lst)==n:
        #     return [lst]
        #     return [lst[0],subsets(lst[1:],n)]
        # for i in subsets(lst[1:]):
        #     with_first=lst[0]+subsets(lst[1:],n-1)
        ...
        #     return [[lst[0]]+subsets(lst[1:],n-1)+ subsets(lst[1:],n)]
        # with_first
        

        # else:
        #     for n in range(len(lst)):
        #         lst[0]+subsets(lst[1:],n-1)
...
def compatible(guess, score, letter_list):
    ...
    if len(guess)==len(letter_list):
        if num_common_letters(make_word_from_string(guess),make_word_from_list(letter_list))==score:
            return True
        else:
            return False
...
def filter_subsets(word, score, possible_subsets):
    ...
    for i in possible_subsets:
        ...
        if compatible(i,score,get_list(word)):
            new_list+=[i]
    return new_list
