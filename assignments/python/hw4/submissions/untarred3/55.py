...
def num_common_letters(goal_word, guess):
    ...
    for i in goal_word:
        for k in guess:
            if i == k:
                total += 1
                break #needed to reiterate for loop
    return total
...
def make_word_master(goal_word):
    ...
    def guess_function(word):
        common = num_common_letters(goal_word, word)
        if is_valid_guess(word) == False:
            return not_a_word
        if common == 0:
            return bad_num_letters
        if len(goal_word) != len(word):
            return bad_num_letters
        if common == len(goal_word):
            if word == goal_word:
                return win_message
            else:
                return common
...
def subsets(lst, n):
    ...
    >>> three_subsets = subsets(list(range(5)), 3) #12345
    ...
    """
    "*** YOUR CODE HERE ***"
    #doctests fail claiming three_subsets not defined
    result= []
    k = 0
    while k <= n: 
    	for elem in lst:
...
def filter_subsets(word, score, possible_subsets):
    ...
    for sub in possible_subsets:
        if num_common_letters(word, make_word_from_list(sub)) == score:
