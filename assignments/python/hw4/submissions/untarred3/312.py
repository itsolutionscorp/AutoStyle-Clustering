...
def make_word_master(goal_word):
    ...
    def g(x):
        if is_valid_guess(x):
            if len(get_string(x)) == len(get_string(goal_word)):
                if (get_string(x)) == (get_string(goal_word)):
                    return win_message
            else:
...
def compatible(guess, score, letter_list):
    ...
    if len(guess) == len(letter_list):
        if num_common_letters(get_string(make_word_from_list(letter_list)), (get_string(guess))) == score:
            return True
        else:
            return False
...
def filter_subsets(word, score, possible_subsets):
    ...
    for x in possible_subsets:
        if num_common_letters(get_string(make_word_from_list(x)),get_string(word)) == score:
            result += [x]
    return result
...
def make_deductions(possible_subsets, letters):
    ...
    for let in letters:
        if let not in pool:
            not_present += let
        if occ(pool, let) == len(possible_subsets):
                present += let
    return present, not_present

# make large pool from subsets to compare letters 
