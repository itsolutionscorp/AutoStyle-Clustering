...
def make_word_master(goal_word):
    ...
    True
    """
    def guess_reply(guess_word):
        if(goal_word == guess_word):
            return win_message
        elif(len(get_list(goal_word)) != len(get_list(guess_word))):
...
def subsets(lst, n):
    ...
    def add_to_all(addition, lst):
        new_lst = []
        for each_subset in lst:
            new_lst.append([addition] + each_subset)
        return new_lst
    ...
    if(len(lst) == n):
        ...
        return add_to_all(lst[0], subsets(lst[1:], n - 1)) + subsets(lst[1:], n)


def compatible(guess, score, letter_list):
    """Returns True if it is possible for the word to get the score,
    assuming that the true word only contains letters fromlst
    letter_list.
    ...
    False
    """
    if(num_common_letters(make_word_from_list(letter_list), get_list(guess)) == score):
        return True
    return False
...
def filter_subsets(word, score, possible_subsets):
    ...
    [['a', 'b', 'e', 'l', 's'], ['b', 'l', 'e', 's', 't']]
    """
    output_list_of_subsets =[]
    for each_subset in possible_subsets:
        shared_letters = 0
        for each_letter in each_subset:
            if(each_letter in get_list(word)):
                shared_letters += 1
        if(shared_letters == score):
            output_list_of_subsets.append(each_subset)
    return output_list_of_subsets
...
def make_deductions(possible_subsets, letters):
    ...
    for letter in letters:
        ...
        for subset in possible_subsets:
            if(letter in subset):
                whether_present += 1
        if(whether_present == 0):
            not_present = not_present + [letter]
        elif(whether_present == len(possible_subsets)):
