...
def make_word_master(goal_word):
    ...
    """
    "*** YOUR CODE HERE ***"
    def rohin_the_word_master(word_guess):
        word_lst = get_list(goal_word)
        word_guess_lst = get_list(word_guess)

        if len(word_lst) != len(word_guess_lst):
            return bad_num_letters

        elif word_lst == word_guess_lst:
            return win_message

        elif is_valid_guess(word_guess) == False:
            return not_a_word
        else:
            return num_common_letters(goal_word, word_guess)

    return rohin_the_word_master
...
def subsets(lst, n):
    ...
    """
    "*** YOUR CODE HERE ***"
    big_list =list(list(itertools.combinations(lst,n)))
    final_list = []
       
    for i in range(len(big_list)):
        final_list += list(big_list[i])
...
def compatible(guess, score, letter_list):
    ...
    """
    "*** YOUR CODE HERE ***"
    letter_list_string = get_string(make_word_from_list(letter_list))

    answer = make_word_master(letter_list_string)
...
def filter_subsets(word, score, possible_subsets):
    ...
    "*** YOUR CODE HERE ***"
    filter_list = []
    the_subsets= subsets(possible_subsets,score)
    for i in range(len(the_subsets)):
        

        if compatible(word, score, the_subsets[i]):
            filter_list += [the_subsets[i]]
...
def make_deductions(possible_subsets, letters):
    ...
    for x in range(len(letters)):
        ...
        for el in possible_subsets:
        

            if letters[x] in el and letters[x] not in present:
                present += [letters[x]]

            if (letters[x] not in el) and (letters[x] in present):
                present.remove( letters[x])
                

            if letters[x] not in el and letters[x] not in not_present:
                not_present += [letters[x]]
                break
    for el in not_present:

        for els in possible_subsets:
            
            
            if el in els:
                not_present.remove(el)
                break
    for el in not_present:
