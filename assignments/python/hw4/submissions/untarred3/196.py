...
def make_word_master(goal_word):
    ...
    def word_master_function(guess):
        guess_word_string = get_string(guess)
        if len(guess_word_string) != len(goal_word_string):
            return bad_num_letters
        elif guess_word_string == goal_word_string:
            return win_message
        elif not is_valid_guess(guess):
            return not_a_word
        else:
            return num_common_letters(goal_word, guess)
    return word_master_function
...
def subsets(lst, n):
    ...
    if len(lst) == n or n == 0:
        ...
        lst_subsets = subsets(lst[1:], n-1)
        for i in range(len((lst_subsets))):     
            lst_subsets[i] = [lst[0]] + lst_subsets[i][0:2]
    return lst_subsets + subsets(lst[1:], n)
...
def filter_subsets(word, score, possible_subsets):
    ...
    for i in range(len(possible_subsets)):
        if compatible(word, score, possible_subsets[i]) == True:
            filtered_result += [possible_subsets[i]]
...
def make_deductions(possible_subsets, letters):
    ...
    for i in range(len(letters)):
        presence_count = 0
        for e in range(len(possible_subsets)):
            if letters[i] in possible_subsets[e]:
                presence_count += 1
                if presence_count == len(possible_subsets):
                    present += [letters[i]]
        if presence_count == 0:
