...
def make_word_master(goal_word):
    ...
    True
    """
    goalstr = get_string(goal_word)
    def func(guess):
        guessstr = get_string(guess)
        if is_valid_guess(guess):
            if len(goalstr) == len(guessstr):
                if goalstr==guessstr:
                    return win_message
                else:
...
def compatible(guess, score, letter_list):
    ...
    False
    """
    t = num_common_letters(make_word_from_list(letter_list),guess)
    if t==score:
        return True
    return False
...
def filter_subsets(word, score, possible_subsets):
    ...
    for elem in possible_subsets:
        if not compatible(make_word_from_list(elem),score,word):
            toremove.append(elem)
    for e in toremove:
        possible_subsets.remove(e)
