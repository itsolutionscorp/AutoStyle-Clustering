...
def make_word_master(goal_word):
    ...
    def word_master(guess):
        if len(guess) != len(goal_word):     # Make sure length is right
            return bad_num_letters
        elif is_valid_guess(guess) == False:  # Make sure guess is a word
            return not_a_word   
...
def subsets(lst, n):
    ...
    if len(lst) == 0:
      return []
    lst2 = lst[1:]
    sets = subsets(lst2, n-1)
    for s in sets:
        s[:0] = [lst[0]]
    return sets+ subsets(lst2, n)
...
def filter_subsets(word, score, possible_subsets):
    ...
    "*** YOUR CODE HERE ***"
    x = []
    for pos_subset in (possible_subsets):
        if num_common_letters(pos_subset, word) == score:
            x += [pos_subset]
    return x
...
def make_deductions(possible_subsets, letters):
    ...
    for letter in letters:
        here_everywhere = True
        here_nowhere = True
        for subset in possible_subsets:
            here_in_subset = False
            for element in subset:
                if letter == element:
                    here_in_subset = True
                    here_nowhere = False
            if here_in_subset == False:
                here_everywhere = False
        if here_everywhere == True:
                present += [letter]
        if here_nowhere == True:
