...
def make_word_master(goal_word):
    ...
    """
    "*** YOUR CODE HERE ***"
    def game_master(guess):
        common_letters = num_common_letters(goal_word,guess)
...
def subsets(lst, n):
    ...
    """
    "*** YOUR CODE HERE ***"
    def with_combination(prior, lst, r = n):
        if r==0:
            return [prior]
        elif len(lst)==r:
            return with_combination(prior+[lst[0]],lst[1:],r-1)
        else: 
            return with_combination(prior+[lst[0]],lst[1:],r-1) + with_combination(prior,lst[1:],r)
    return with_combination([],lst)
...
def make_deductions(possible_subsets, letters):
    ...
    for letter in letters:
        letter_count = 0
        for subset in possible_subsets:
            for guess_letter in subset:
                if letter == guess_letter:
                    letter_count += 1
        if letter_count == len(possible_subsets):
            present += [letter]
        elif letter_count==0:
            not_present += [letter]
