...
def make_word_from_string(s):
    ...
    """
    return s   #returns a string, just s
...
def make_word_master(goal_word):
    ...
    def word_master(guess):
        if guess == goal_word:
            return win_message
        elif len(guess) != len(goal_word):
            return bad_num_letters
        elif not is_valid_guess(guess):  #word is in dictionary
            return not_a_word
...
def subsets(lst, n):
    ...
    """
    #tree recursion
    #always returns a list of lists no matter what
    if n == 0:
        return [[]]
    if lst == []:    #empty list
        return lst
    ...
    return subsets(rest, n) + subsets(rest, n - 1)
    # combine subset(rest, n) --> [[2, 3]]
    # with subset(rest, n - 1) --> [[2], [3]] but with +1 
...
def compatible(guess, score, letter_list):
    ...
    False
    """
    goal = make_word_from_list(letter_list)  #true word
    if num_common_letters(goal, guess) == score:  #same number of common letters as score
        return True
    return False
...
def filter_subsets(word, score, possible_subsets):
    ...
    [['a', 'b', 'e', 'l', 's'], ['b', 'l', 'e', 's', 't']]
    """
    #returns a new list containing only the letter combos that are compatible w/ score
    subsets = []
    for elem in possible_subsets:
        if compatible(word, score, elem):  #compatible letter combo
            subsets += [elem]
    return subsets
...
def make_deductions(possible_subsets, letters):
    ...
    ['d', 'f']
    """
    present = possible_subsets[0]  #start with first subset
    not_present = [x for x in letters if x not in present]   #likewise, just reverse
    
    for elem in possible_subsets[1:]:
        present = [x for x in present if x in elem]  
        not_present = [x for x in not_present if x not in elem]
