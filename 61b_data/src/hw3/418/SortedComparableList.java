...
public class SortedComparableList {
    ...
    public static SortedComparableList subTail(SortedComparableList L, int start) {
    	SortedComparableList sub = new SortedComparableList(L.head, L.tail);
    	while (start != 0) {
    		sub = sub.tail;
    		start -= 1;
    	}
    ...
    public static SortedComparableList sublist(SortedComparableList L, int start, int len) {
    	...
    	SortedComparableList subL = subTail(L, start);
    	SortedComparableList point = subL;
    	while (len > 1) {
    		subL = subL.tail;
    		len -= 1;
    	}
    ...
    public void squish() {
	    ...
	    while (tail.tail != null) {	
    		if (!pointer.head.equals(pointer.tail.head)) {
	    		pointer = pointer.tail;
	    	} else {
