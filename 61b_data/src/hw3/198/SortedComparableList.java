...
public class SortedComparableList {
    ...
    public void insert(Comparable c) {
        if (c != null) {
          if (c.compareTo(head) < 0) { // if c is less than head, it should go first
            ...
            // keep iterating through the list till you either reach the end (current.tail = null)
            // or until you find a position that is greater than c.
    ...
    public Comparable get(int i) {
        if (i == 0 || head == null)
          ...
          SortedComparableList current = this;
          while (current.tail != null && i != 0) { // got stuck and took solution
            current = current.tail;
            i -= 1;
          }
          return current.head;
