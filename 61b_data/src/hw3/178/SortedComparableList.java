...
public class SortedComparableList {
    ...
    public void insert(Comparable c) {
      ...
      if (c != null) {
        if (c.compareTo(head) < 0) {
          ...
          /* Insert in the Middle or at the End of List */
          SortedComparableList check = this;
          while (check.tail != null && c.compareTo(check.tail.head) > 0) {
            check = check.tail;
          }
    ...
    public Comparable get(int i) {
        // REPLACE THIS LINE WITH YOUR SOLUTION
      SortedComparableList check = this;
      if (i == 0 || check.head == null) {
        return check.head;
      }

      while (check.tail != null && i > 0) {
        check = check.tail;
        i = i - 1;
      }
    ...
    public void extend(SortedComparableList that) {
        // REPLACE THIS LINE WITH YOUR SOLUTION
      SortedComparableList check = that;
      while (check != null) {
        this.insert(check.head);
        check = check.tail;
      }
    }
    ...
    public static SortedComparableList subTail(SortedComparableList L, int start) {
        if (L == null || start == 0) {
          ...
          SortedComparableList check = L;
          while (check != null && start != 0) {
            check = check.tail;
            start = start - 1;
          }
    ...
    public void squish() {
      ...
      if (this.tail != null) {
        ...
        while (next != null) {
          if (check.head.equals(next.head)) {
            check.tail = next.tail;
            next = next.tail;
          }
          else {
            check = check.tail;
    ...
    public void twin() {
      ...
      SortedComparableList check = this;
      while (check != null) {
        check.tail = new SortedComparableList(check.head, check.tail);
        check = check.tail.tail;
      }
    }
    ...
    /** Returns true if X is a SortedComparableList containing the
