...
public class SortedComparableList {
    ...
    public void insert(Comparable c) {
		if (c != null) {
			int compareResult = head.compareTo(c);
			if (compareResult >= 0) {
    ...
    public static SortedComparableList sublist(SortedComparableList L, int start, int len) {
        SortedComparableList returnList = null;
		for (int i = start + len - 1; i >= start; i--) {
        	returnList = new SortedComparableList(L.get(i), returnList);
        }
    ...
    public void twin() {
        tail = new SortedComparableList(head, tail);
		if (tail.tail != null) { tail.tail.twin(); }
    }
