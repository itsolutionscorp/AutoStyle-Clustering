...
public class SortedComparableList {
    ...
    public void insert(Comparable c) {
    	//find greatest node, insert before
    	//if ran out, insert at the end
    	SortedComparableList p = this;
    	
    	while(p.tail != null){
    		if(p.head.compareTo(c)!=-1){
    ...
    public static SortedComparableList sublist(SortedComparableList L, int start, int len) {
    	for(int i = 0;i<start;i++)
    		L = L.tail;
    	SortedComparableList result = new SortedComparableList(L.head,null);
    	SortedComparableList returnResult = result;
    ...
    public void squish() {
        SortedComparableList p = this;
        while(p.tail != null){
        	if(p.head.compareTo(p.tail.head)==0)
        		p.tail = p.tail.tail;
        	if(p.tail == null)
        		break;
        	p = p.tail;
