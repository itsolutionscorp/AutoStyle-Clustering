...
public class SortedComparableList {
    ...
    public void insert(Comparable c) {
        if (c != null){
          if (c.compareTo(head) < 0){
            ...
            SortedComparableList holder = this;
            while (holder.tail != null && c.compareTo(holder.tail.head) >= 0) {
                holder = holder.tail;
            }
            holder.tail = new SortedComparableList(head, holder.tail);
          }
        }
    }
    ...
    public void extend(SortedComparableList that) {
        SortedComparableList holder = that;
        while (holder != null) {
          insert(holder.head);
          holder = holder.tail;
        }
     ...
     **/
    public void squish() {
        SortedComparableList back = this;
        SortedComparableList now = tail;
        while (now != null) {
          if (back.head.equals(now.head)){
            back.tail = now.tail;
            now = back.tail;
          } else {
            back = back.tail;
