...
public class SortedComparableList {
    ...
    public void insert(Comparable c) {
        SortedComparableList tracker = this.tail;
        SortedComparableList prev = this;
        if (c.compareTo(this.head)<0){
            Comparable d= this.head;
            this.head = c;
            this.tail = new SortedComparableList(d,tracker);
            return;
        }
        while (tracker != null && c.compareTo(tracker.head) > 0) {
            prev=tracker;
            tracker=tracker.tail;
        }
        prev.tail = new SortedComparableList(c,tracker);
    }
    ...
    public static SortedComparableList sublist(SortedComparableList L, int start, int len) {
        SortedComparableList sTail=subTail(L,start);
        SortedComparableList copy= new SortedComparableList();
        SortedComparableList tracker = copy;
        while (len > 0 && sTail!=null)  {
            tracker.head = sTail.head;
            tracker.tail = sTail.tail;
            sTail = sTail.tail;
            tracker = tracker.tail;
            len-=1;
        }
    ...
    public void squish() {
        ...
        while (tracker != null) 
        {
            if(prev.head.compareTo(tracker.head)==0) {
                prev.tail=tracker.tail;
                tracker = tracker.tail;
            }
            else {
                prev = tracker;
     ...
     **/
    public void twin() {
        SortedComparableList tracker = this;
        SortedComparableList prev = this;
        while (tracker!=null) {
            tracker.tail= new SortedComparableList(tracker.head,tracker.tail);
        }
    }
