...
public class SortedComparableList {
    ...
    public static SortedComparableList subTail(SortedComparableList L, int start) {
        if (start == 0)
          ...
          return (L.tail.subTail(L.tail, start - 1));
    }
    ...
    public static SortedComparableList sublist(SortedComparableList L, int start, int len) {
        SortedComparableList firstCut = L.subTail(L, start);
        SortedComparableList secondCut = new SortedComparableList();
        for (int i = 0; i < len; i++)
            secondCut.insert(firstCut.get(i));
        return secondCut.subTail(secondCut, 1);
    }

    /** Removes items from L at position len+1 and later. */
    public static void expungeTail(SortedComparableList L, int len) {
        L.subTail(L, len).tail = null;
    }
     ...
     *  called.
     *
     *  For example, if the input list is [ 0 0 0 0 1 1 3 3 3 4 4 ], the
    ...
    public void squish() {
        if (this.tail == null)
          ...
          while (temp != null && temp.head == head) {
              temp = temp.tail;
          }
          tail = temp;
          if (temp != null)
     ...
     **/
    public void twin() {
        this.extend(deepCopy(this));
    }

    private static SortedComparableList deepCopy(SortedComparableList L) {
      SortedComparableList p = L.tail;
      SortedComparableList result = new SortedComparableList(L.head, null);
      while (p != null) {
        result.insert(p.head);
        p = p.tail;
      }
      return result;
    }
