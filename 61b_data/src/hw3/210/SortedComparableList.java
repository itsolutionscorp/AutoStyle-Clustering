...
public class SortedComparableList {
    ...
    public void insert(Comparable c) {
        if (c != null) {   
            if (c.compareTo(head) < 0){ //insert at front
    ...
    public static SortedComparableList subTail(SortedComparableList L, int start) {
        ...
        while (start > 0 || curr != null) { // I guess we're assuming that start
            curr = curr.tail;               // may or may not be greater than length
            start -= 1;
        }
        return curr;

        /** Also works? (this is from solutions)
        if (L == null || start == 0) {
            return L;
        }
        else {
